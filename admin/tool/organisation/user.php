...
permission::require_can_view_user_org_profile($userid);

$user = core_user::get_user($userid, '*', MUST_EXIST);
$context = \context_system::instance();
require_login(null, false);
$url = new moodle_url('/admin/tool/organisation/user.php', ['id' => $userid]);
if (permission::can_view_index()) {
    \tool_wp\admin_externalpage::setup_page('tool_organisation_structure', '', [], $url);
    \tool_wp\admin_externalpage::setup_subpage(fullname($user));
} else {
    // Must be accessed from the user profile page, in this case it will not have breadcrumb.
