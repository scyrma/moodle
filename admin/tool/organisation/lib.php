...
function tool_organisation_myprofile_navigation(tree $tree, stdClass $user, bool $iscurrentuser, ?stdClass $course) {
    ...
    if (permission::can_view_user_jobs($user)) {
        ...
        foreach ($jobs as $job) {
            ...
            [$managedsjoin, $managedparams] = helper::get_all_direct_managed_users_join($managerjobs);
...
function tool_organisation_potential_users_selector(string $area, int $itemid): ?array {
    if ($area === 'jobassign' || $area === 'manuallyassigned') {
        ...
        // If user autocomplete is called from people page, current user doesn't need to be available for selection.
        if ($area === 'manuallyassigned') {
            $currentuser = database::generate_param_name();
            $where .= " AND u.id <> :{$currentuser}";
            $params[$currentuser] = $itemid;
        }
