 ...
 * @covers     \tool_organisation\organisation
 * @copyright  2018 Moodle Pty Ltd <support@moodle.com>
 * @author     2018 Marina Glancy
 * @license    Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class organisation_test extends advanced_testcase {
     ...
     * @param bool $rebuildtree immediately rebuild the reporting tree
     * @return stdClass
     */
    protected function generate_user(string $username, string $position = null, string $department = null,
        ...
        if ($position && $department) {
            $this->get_generator()->assign_job((object)['userid' => $this->users[$username]->id,
                ...
                'departmentid' => $this->{$department}->id, ], $rebuildtree);
        }
    ...
    public function test_user_with_jobs_create() {
        ...
        $user0 = $this->generate_user('u0', false);
        $user1 = $this->generate_user('u1', 'pa', 'da', false);
        $user2 = $this->generate_user('u2', 'pb', 'db', false);
        $this->generate_user('u2', 'pa', 'da', false);
        $this->get_generator()->execute_adhoc_build_reporting();
        $this->setUser($user0);

        $u0 = organisation::get_user_with_jobs($user0->id);
        $this->assertEquals(0, count($u0->get_jobs()));

        $u1 = organisation::get_user_with_jobs($user1->id);
        $this->assertEquals(1, count($u1->get_jobs()));

        $u2 = organisation::get_user_with_jobs($user2->id);
     ...
     * @param int $time
     * @param bool $childpositionsonly
     */
    public function assert_managed_users(string $managedusers, string $managername, int $permissionbitmask = 0, int $time = 0,
            bool $childpositionsonly = false): void {

        global $DB;
        $managers = organisation::get_users_with_jobs('u.username=:username', ['username' => $managername]);
        $manager = reset($managers);
        if (!$manager) {
            $this->fail('User with username '.$managername.' is not found or does not belong to the current tenant');
        }
        [$sql, $params] = $manager->get_managed_users_select('u', $permissionbitmask, $time,
            $childpositionsonly);
    ...
    public function test_get_managed_users_sql_single_job() {
        ...
        foreach (['pa', 'pa1', 'pa2', 'pa3', 'pb', 'pb1'] as $p) {
            foreach (['da', 'da1', 'da2', 'db'] as $d) {
                $this->generate_user("u_{$p}_{$d}", $p, $d, false);
            }
        }
        $this->get_generator()->execute_adhoc_build_reporting();
        ...
        // except other department leads.Other users are considered managed by those managers through different reporting line.
        // For example, user u_pa1_db is managed by u_pb_da through some of the users (u_pa_da/u_pa_da1/u_pa_da2)
        // because they are global manager in position pa who is parent of pa1(where the user belongs to).
        $this->assert_managed_users('u_pa_da, u_pa_da1, u_pa_da2, u_pa1_da, u_pa1_da1, u_pa1_da2, u_pa2_da, u_pa2_da1, ' .
            'u_pa2_da2, u_pb1_da, u_pb1_da1, u_pb1_da2, u_pa3_da, u_pa3_da1, u_pa3_da2, u_pa1_db, u_pa2_db, u_pa3_db', 'u_pb_da');
        $this->assert_managed_users('u_pa_da1, u_pa1_da, u_pa1_da1, u_pa1_da2, u_pa1_db, u_pa2_da, u_pa2_da1, u_pa2_da2, ' .
            'u_pa2_db, u_pa3_da, u_pa3_da1, u_pa3_da2, u_pa3_db, u_pb1_da1', 'u_pb_da1');
        $this->assert_managed_users('u_pa_da2, u_pa1_da, u_pa1_da1, u_pa1_da2, u_pa1_db, u_pa2_da, u_pa2_da1, u_pa2_da2, ' .
            'u_pa2_db, u_pa3_da, u_pa3_da1, u_pa3_da2, u_pa3_db, u_pb1_da2', 'u_pb_da2');
        $this->assert_managed_users('u_pa_db, u_pa1_da, u_pa1_da1, u_pa1_da2, u_pa1_db, u_pa2_da, u_pa2_da1, u_pa2_da2, ' .
            'u_pa2_db, u_pa3_da, u_pa3_da1, u_pa3_da2, u_pa3_db, u_pb1_db', 'u_pb_db');
        ...
        // Immediate job child positions only (manager of anybody in positions pa1 and pa2 regardless of department).
        $this->assert_managed_users('u_pa1_da, u_pa1_da1, u_pa1_da2, u_pa1_db, u_pa2_da, u_pa2_da1, u_pa2_da2, u_pa2_db',
            'u_pa_da', 0, 0, true);
    }
    ...
    public function test_get_managed_users_sql_multiple() {
        ...
        // except other department leads.
        $this->assert_managed_users('u_pa2_da, u_pa2_da1, u_pa1_da, u_pa1_da1, u_pa_db, u_pa1_db, u_pa2_db, ' .
            'u_pa_db1, u_pa1_db1, u_pa2_db1, u_pa3_db, u_pa3_db1, u_pa3_da, u_pa3_da1', 'u');

        // Retrieve all managed users and check that each of them has one job (and it is relevant).
        $manager = organisation::get_user_with_jobs($u->id);
        [$select, $params] = $manager->get_managed_users_select();
        $managedusers = organisation::get_users_with_jobs($select, $params);
        $this->assertEquals(14, count($managedusers));
        ...
        $allusers = organisation::get_users_with_jobs($likesql, ['username' => 'u_%']);
        $this->assertEquals(20, count($allusers));
        $otherusers = array_diff_key($allusers, $managedusers);
        $this->assertEquals(6, count($otherusers));
        foreach ($otherusers as $user) {
            // Other department leads does not be considered.
    ...
    public function test_get_department_users_select() {
        ...
        $userwithjobs = organisation::get_user_with_jobs($manager->id);
        ...
        // Own department.
        list($select, $params) = helper::get_department_users_select($userwithjobs, false);
        ...
        list($select, $params) = helper::get_department_users_select($userwithjobs, true);
        $users = $DB->get_records_sql("SELECT u.username FROM {user} u WHERE {$select}", $params);
        ...
        list($select, $params) = helper::get_department_users_select($userwithjobs, true, 'u', time() + 10);
        $users = $DB->get_records_sql("SELECT u.username FROM {user} u WHERE {$select}", $params);
        $this->assertEmpty($users);
    }

    /**
     * Test for helper::get_hierarchical_menu
     */
    public function test_get_hierarchical_menu() {
        global $DB;
        $this->generate_structure();
        $d = $this->get_generator()->create_department(['parentid' => $this->dfother->id]);
        $df2 = $this->get_generator()->create_department(['tenantid' => $this->tenant->id, 'name' => $this->df->name]);
        $df3 = $this->get_generator()->create_department(['tenantid' => $this->tenant->id, 'name' => $this->df->name]);
        $dc = $this->get_generator()->create_department(['parentid' => $df2->id]);
        $dd = $this->get_generator()->create_department(['parentid' => $df3->id]);
    ...
    public function test_get_managed_users_menu() {
        ...
        foreach (['pa', 'pa1', 'pa2', 'pb'] as $p) {
            foreach (['da', 'da1', 'db1'] as $d) {
                $users["u_{$p}_{$d}"] = $this->generate_user("u_{$p}_{$d}", $p, $d, false);
            }
        }
        $this->get_generator()->execute_adhoc_build_reporting();
    ...
    public function test_get_all_menu() {
        ...
        $pf2 = $this->get_generator()->create_position(['tenantid' => $this->tenant->id, 'name' => $this->pf->name]);
        $pf3 = $this->get_generator()->create_position(['tenantid' => $this->tenant->id, 'name' => $this->pf->name]);
        $pc = $this->get_generator()->create_position(['parentid' => $pf2->id]);
        $pd = $this->get_generator()->create_position(['parentid' => $pf3->id]);
    ...
    /**
     * Tests users with jobs SQL helper
     */
    public function test_users_with_jobs_sql(): void {
        $this->generate_structure();

        $this->generate_user('user01', 'pa1', 'da1');
        $this->generate_user('user02', 'pa2', 'da2');

        [$where, $params] = \tool_organisation\helper::users_with_jobs_sql('u', null, $this->tenant->id);
        $this->assert_selected_users('user01,user02', $where, $params);
    }
     ...
     */
    public function assert_direct_managed_users(string $managedusers, string $managername, int $permissionbitmask = 0,
                                                int $time = 0, string $message = '') {
        global $DB;
        $managers = organisation::get_users_with_jobs('u.username=:username', ['username' => $managername]);
        $manager = reset($managers);
        if (!$manager) {
        ...
        [$sql, $params] = $manager->get_managed_users_select('u', $permissionbitmask, $time, true);
        $usernames = $DB->get_fieldset_sql("SELECT username FROM {user} u WHERE ".$sql, $params);
        $expected = preg_split('/,\s*/', $managedusers, -1, PREG_SPLIT_NO_EMPTY);
    ...
    public function assert_user_direct_managers(string $directmanagers, string $username, string $message = '') {
        global $DB;
        list($tjoin, $twhere, $tparams) = \tool_tenant\tenancy::get_users_sql('u');
        ...
        $managers = array_values(array_map(function ($m) {
            return $m->username;
        }, organisation::get_user_all_direct_managers($userid)));
    ...
    public function test_get_direct_managed_users_sql_single_job() {
        ...
        $employee1 = $this->get_tenant_generator()->create_user(['tenantid' => $this->tenant->id, 'username' => 'employee1']);
        $employee2 = $this->get_tenant_generator()->create_user(['tenantid' => $this->tenant->id, 'username' => 'employee2']);
        $employee3 = $this->get_tenant_generator()->create_user(['tenantid' => $this->tenant->id, 'username' => 'employee3']);

        // Add some manager as manually assigned to recently added users.
        user_manager::add_assigned_manager($employee1->id, $this->users['u_pa_da']);
        user_manager::add_assigned_manager($employee2->id, $this->users['u_pa_da1']);
        user_manager::add_assigned_manager($employee3->id, $this->users['u_pa_da2']);
        ...
        // Check extra users assigned manually to current previous managers.
    ...
    public function test_get_direct_managed_users_sql_multiple() {
        ...
        // Add 'u' as manually assigned to recently added users.
        user_manager::add_assigned_manager($employee1->id, $u);
        ...
        $this->assert_direct_managed_users('u_pa2_da, u_pa2_da1, u_pa1_da, u_pa1_da1, u_pa_db, u_pa1_db, u_pa2_db, ' .
            'u_pa1_db1, u_pa2_db1, u_pa3_db, employee1', 'u');
    }
     ...
     */
    public static function get_managed_users_and_direct_managers_provider(): array {
        return [
    ...
    /**
     * Test for get_user_all_direct_managers after add manually assigned managers.
     */
    public function test_get_user_all_manually_assigned_managers(): void {
        $this->resetAfterTest();

        // Create some users in same tenant (employees and manager).
        $usera = $this->get_tenant_generator()->create_user(['username' => 'usera']);
        $userb = $this->get_tenant_generator()->create_user(['username' => 'userb']);
        $userc = $this->get_tenant_generator()->create_user(['username' => 'userc']);

        // Add some manually assigned managers.
        user_manager::add_assigned_manager($userb->id, $usera);
        user_manager::add_assigned_manager($userc->id, $userb);

        $this->get_generator()->execute_adhoc_build_reporting();

        $managers = array_values(array_map(function ($mgr) {
            return $mgr->username;
        }, organisation::get_user_all_direct_managers($usera->id)));
        $message = "User $usera->username is expected to have users ".(join(', ', $managers) ?: '-')."
            as manually direct managers";
        $this->assert_user_direct_managers(join(', ', $managers), $usera->username, $message);

        $managers = array_values(array_map(function ($mgr) {
            return $mgr->username;
        }, organisation::get_user_all_direct_managers($userb->id)));
        $message = "User $userb->username is expected to have users ".(join(', ', $managers) ?: '-')."
            as manually direct managers";
        $this->assert_user_direct_managers(join(', ', $managers), $userb->username, $message);

        $managers = array_values(array_map(function ($mgr) {
            return $mgr->username;
        }, organisation::get_user_all_direct_managers($userc->id)));
        $message = "User $userc->username is expected to have users ".(join(', ', $managers) ?: '-')."
            as manually direct managers";
        $this->assert_user_direct_managers(join(', ', $managers), $userc->username, $message);

        // Check manually assigned managers and their subordinates.
        $this->assert_managed_users('userb', 'usera', 0, 0, true);
        $this->assert_managed_users('userb, userc', 'usera');
        $this->assert_managed_users('userc', 'userb', 0, 0, true);
        $this->assert_managed_users('userc', 'userb');
        $this->assert_managed_users('', 'userc', 0, 0, true);
        $this->assert_managed_users('', 'userc');
    }

    /**
     * Test for job_time_and_tenant_select()
     */
    public function test_job_time_and_tenant_select(): void {
        global $DB;
        $this->resetAfterTest();
        self::setAdminUser();
        ...
        $tenant2id = $this->get_tenant_generator()->create_tenant()->id;

        $positionframework1 = $this->get_generator()->create_position(['tenantid' => $defaulttenantid]);
        $position1 = $this->get_generator()->create_position(['parentid' => $positionframework1->id]);
        $positionframework2 = $this->get_generator()->create_position(['tenantid' => $tenant2id]);
        $position2 = $this->get_generator()->create_position(['parentid' => $positionframework2->id]);
        $positionframework3 = $this->get_generator()->create_position(['tenantid' => $sharedtenantid]);
        $position3 = $this->get_generator()->create_position(['parentid' => $positionframework3->id]);

        $departmentframework1 = $this->get_generator()->create_department(['tenantid' => $defaulttenantid]);
        $department1 = $this->get_generator()->create_department(['parentid' => $departmentframework1->id]);
        $departmentframework2 = $this->get_generator()->create_department(['tenantid' => $tenant2id]);
        $department2 = $this->get_generator()->create_department(['parentid' => $departmentframework2->id]);
        $departmentframework3 = $this->get_generator()->create_department(['tenantid' => $sharedtenantid]);
        $department3 = $this->get_generator()->create_department(['parentid' => $departmentframework3->id]);
        ...
        $this->get_tenant_generator()->allocate_user($user2->id, $tenant2id);
        $this->get_generator()->assign_job((object)[
            'userid' => $user2->id,
        ...
        // User assigned to a shared department and shared position.
        ...
        // Assert that returns only the user in Default tenant.
        [$where, $params] = \tool_organisation\helper::job_time_and_tenant_select(0);
        $sql = "SELECT u.id FROM {user} u JOIN {tool_organisation_job} j ON j.userid = u.id WHERE ";
        $result = $DB->get_records_sql($sql . $where, $params);
        $this->assertEqualsCanonicalizing([$user->id, $user3->id], array_keys($result));

        // Assert that returns only the user in tenant other.
        [$where, $params] = \tool_organisation\helper::job_time_and_tenant_select(0, 'j', $tenant2id);
        $result = $DB->get_records_sql($sql . $where, $params);
        $this->assertEquals([$user2->id], array_keys($result));

        // Assert that passing shared space as tenantid returns users from both tenants.
        [$where, $params] = \tool_organisation\helper::job_time_and_tenant_select(0, 'j', $sharedtenantid);
        $result = $DB->get_records_sql($sql . $where, $params);
        $this->assertEqualsCanonicalizing([$user->id, $user2->id, $user3->id], array_keys($result));
    }

    /**
     * Tests users with manager permission SQL helper
     */
    public function test_users_with_managerpermission_sql(): void {
        $this->generate_structure();

        // Add shared space.
        $sharedtenantid = \tool_tenant\sharedspace::enable_shared_space();

        // We create new position with manager and department lead permission in shared space.
        ...
        $positionshared = $this->get_generator()->create_position([
            'parentid' => $positionframeworkshared->id,
            'globalmanager' => 1,
            'departmentmanager' => 1,
        ]);

        // We create new position with manager permission.
        $global = $this->get_generator()->create_position([
            'parentid' => $this->pf->id,
            'globalmanager' => 1,
        ]);

        // We create new position with department lead permission.
        $department = $this->get_generator()->create_position([
            'parentid' => $this->pftenantother->id,
            'departmentmanager' => 1,
        ]);

        // Create users and assign job in different positions/tenants.
        $userallmanager = $this->getDataGenerator()->create_user(['username' => 'userallmanager']);
        $this->get_tenant_generator()->allocate_user($userallmanager->id, $sharedtenantid);
        $this->get_generator()->assign_job((object)[
            'userid' => $userallmanager->id,
            'positionid' => $positionshared->id,
            'departmentid' => $departmentshared->id,
        ]);
        $userglobalmanager = $this->getDataGenerator()->create_user(['username' => 'userglobalmanager']);
        $this->get_tenant_generator()->allocate_user($userglobalmanager->id, $this->tenant->id);
        $this->get_generator()->assign_job((object)[
            'userid' => $userglobalmanager->id,
            'positionid' => $global->id,
            'departmentid' => $this->df->id,
        ]);
        $userdepartmentmanager = $this->getDataGenerator()->create_user(['username' => 'userdepartmentmanager']);
        $this->get_tenant_generator()->allocate_user($userdepartmentmanager->id, $this->tenantother->id);
        $this->get_generator()->assign_job((object)[
            'userid' => $userdepartmentmanager->id,
            'positionid' => $department->id,
            'departmentid' => $this->dftenantother->id,
        ]);

        // Set conditions to retrieve global managers.
        $managerparams = ['globalmanager' => 1, 'departmentmanager' => 0];

        // Get users with global manager permission assigned in job position.
        [$where, $params] = \tool_organisation\helper::users_with_managerpermission_sql('u', $managerparams,
            $this->tenant->id);

        $this->assert_selected_users('userglobalmanager', $where, $params);

        // Set conditions to retrieve department managers.
        $managerparams = ['globalmanager' => 0, 'departmentmanager' => 1];

        // Get users with department lead permission assigned in job position.
        [$where, $params] = \tool_organisation\helper::users_with_managerpermission_sql('u', $managerparams,
            $this->tenantother->id);

        $this->assert_selected_users('userdepartmentmanager', $where, $params);

        // Set conditions to retrieve global/department managers.
        $managerparams = ['globalmanager' => 1, 'departmentmanager' => 1];

        // Get users with global/department managers permission assigned in job position.
        [$where, $params] = \tool_organisation\helper::users_with_managerpermission_sql('u', $managerparams,
            $sharedtenantid);

        $this->assert_selected_users('userallmanager,userglobalmanager,userdepartmentmanager', $where, $params);
    }

    /**
     * Tests direct reports SQL helper
     */
    public function test_users_with_managers_sql(): void {
        global $DB;
        $this->generate_structure();

        // Let's create some extra users and assign them to different positions.
        $this->generate_user('subordinate1', 'pa1', 'da2');
        $this->generate_user('subordinate2', 'pa1', 'da2');
        $this->generate_user('subordinate3', 'pa1', 'da2');
        $extrauser = $this->generate_user('subordinate4', 'pa1', 'da2');

        // Create user and assign job as a global manager.
        $userglobalmanager = $this->getDataGenerator()->create_user(['username' => 'userglobalmanager']);
        $this->get_tenant_generator()->allocate_user($userglobalmanager->id, $this->tenant->id);
        $this->get_generator()->assign_job((object)[
            'userid' => $userglobalmanager->id,
            'positionid' => $this->pa->id,
            'departmentid' => $this->db1->id,
        ]);

        // Create user and assign job as a department lead.
        $userdepartmentmanager = $this->getDataGenerator()->create_user(['username' => 'userdepartmentmanager']);
        $this->get_tenant_generator()->allocate_user($userdepartmentmanager->id, $this->tenant->id);
        $this->get_generator()->assign_job((object)[
            'userid' => $userdepartmentmanager->id,
            'positionid' => $this->pb->id,
            'departmentid' => $this->da->id,
        ]);

        // Create user and assign it as manually manager of $extrauser.
        $usermanuallymanager = $this->getDataGenerator()->create_user(['username' => 'usermanuallymanager']);
        $this->get_tenant_generator()->allocate_user($usermanuallymanager->id, $this->tenant->id);
        user_manager::add_assigned_manager($extrauser->id, $usermanuallymanager);

        // Get all users without manager SQL helper, and exclude admin & guest.
        $managerssql = \tool_organisation\helper::users_with_managers_sql();
        $adminandguestids = [get_admin()->id, guest_user()->id];
        [$notusers, $notusersparams] = $DB->get_in_or_equal($adminandguestids, SQL_PARAMS_NAMED, 'notuser', false);

        // Let's assert that we get all users without managers.
        $where = "{$managerssql} = 0 AND u.id {$notusers}";
        $this->assert_selected_users('userglobalmanager, userdepartmentmanager, usermanuallymanager', $where, $notusersparams);

        // Let's assert that we get all users with managers.
        $where = "{$managerssql} = 1 AND u.id {$notusers}";
        $this->assert_selected_users('subordinate1, subordinate2, subordinate3, subordinate4', $where, $notusersparams);
    }
}
