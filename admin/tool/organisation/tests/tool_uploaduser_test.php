...
class tool_uploaduser_test extends advanced_testcase {
     ...
     * @return array
     */
    public static function create_job_dates_provider(): array {
        return [
     ...
     * @return array
     */
    public static function update_job_dates_provider(): array {
        return [
    ...
    /**
     * As global admin create users in different tenants and immediately assign jobs
     */
    public function test_create_user_with_jobs(): void {
        global $CFG;
        $this->setAdminUser();

        // Create two users in different tenants with the same idnumbers of departments and positions.
        $csv = <<<EOF
username,firstname,lastname,email,tenant,jobdepartment1,jobposition1
jonest,Tom,Jones,jonest@example.com,t1,hrd,manager
reznor,Trent,Reznor,reznor@example.com,t2,hrd,manager
EOF;
        ...
        // Users are allocated to different tenants.
        $user1 = \core_user::get_user_by_username('jonest', '*', null, MUST_EXIST);
        $this->assertEquals($this->tenant->id, tenancy::get_tenant_id($user1->id));
        $user2 = \core_user::get_user_by_username('reznor', '*', null, MUST_EXIST);
        $this->assertEquals($this->othertenant->id, tenancy::get_tenant_id($user2->id));

        // Check users have jobs in their tenants frameworks.
        $userjobs = organisation::get_user_with_jobs($user1->id, null)->get_jobs();
        $this->assertCount(1, $userjobs);
        $userjob = reset($userjobs);
        $this->assertEquals('HR Department', $userjob->get_department()->get('name'));
        $this->assertEquals('Manager', $userjob->get_position()->get('name'));

        $userjobs = organisation::get_user_with_jobs($user2->id, null)->get_jobs();
        $this->assertCount(1, $userjobs);
        $userjob = reset($userjobs);
        $this->assertEquals('HR Department2', $userjob->get_department()->get('name'));
        $this->assertEquals('Manager2', $userjob->get_position()->get('name'));
    }

    /**
     * Data provider for {@see test_delete_jobs}
     *
     * @return array[]
     */
    public static function delete_jobs_provider(): array {
        return [
            ['tenantadmin'],
            ['admin'],
        ];
    ...
    /**
     * Test for job deletion.
     *
     * @param string $user
     *
     * @dataProvider delete_jobs_provider
     */
    public function test_delete_jobs(string $user): void {
        if ($user === 'tenantadmin') {
            self::setUser($this->tenantadmin->id);
        } else if ($user === 'admin') {
            self::setAdminUser();
        ...
        // Create user and allocate job.
        $user = $this->tenantgenerator->create_user(['tenantid' => $this->tenant->id]);
        $this->orggenerator->assign_job((object)['userid' => $user->id,
            'positionid' => $this->pos->id,
            'departmentid' => $this->dep->id, ]);
        // Assert the job assignment was created.
        $userjobs = organisation::get_user_with_jobs($user->id, null)->get_jobs();
        ...
        // Assert the job is not deleted.
        $user->jobdelete1 = 0;
        tool_uploaduser::process_new_user($user, ['jobposition1', 'jobdepartment1', 'jobdelete1'], $this->get_progress_tracker());
        $userjobs = organisation::get_user_with_jobs($user->id, null)->get_jobs();
        $this->assertCount(1, $userjobs);
        // Assert no error.
        $this->assertEmpty($this->get_progress_tracker_error());
        ...
        tool_uploaduser::process_new_user($user, ['jobposition1', 'jobdepartment1', 'jobdelete1'], $this->get_progress_tracker());
        $userjobs = organisation::get_user_with_jobs($user->id, null)->get_jobs();
        $this->assertCount(0, $userjobs);
        // Assert no error.
        $this->assertEmpty($this->get_progress_tracker_error());
    }

    /**
     * Test for job deletion via CSV file
     */
    public function test_delete_jobs_csv(): void {
        $this->setAdminUser();

        // Create two positions in different tenants.
        $pf1 = $this->orggenerator->create_position(['tenantid' => $this->tenant->id]);
        $pos1 = $this->orggenerator->create_position(['name' => 'Manager2', 'idnumber' => 'manager2', 'parentid' => $pf1->id]);
        $pf2 = $this->orggenerator->create_position(['tenantid' => $this->othertenant->id]);
        $pos2 = $this->orggenerator->create_position(['name' => 'Manager3', 'idnumber' => 'manager3', 'parentid' => $pf2->id]);
        // Create two department in different tenants.
        $df1 = $this->orggenerator->create_department(['tenantid' => $this->tenant->id]);
        $dep1 = $this->orggenerator->create_department(['name' => 'HR Department2', 'idnumber' => 'hrd2', 'parentid' => $df1->id]);
        $df2 = $this->orggenerator->create_department(['tenantid' => $this->othertenant->id]);
        $dep2 = $this->orggenerator->create_department(['name' => 'HR Department3', 'idnumber' => 'hrd3', 'parentid' => $df2->id]);

        // Create user.
        $user1 = $this->tenantgenerator->create_user(['tenantid' => $this->tenant->id]);
        // Assign two jobs with same position and department.
        $jobdata = ['userid' => $user1->id, 'positionid' => $this->pos->id, 'departmentid' => $this->dep->id];
        $u1job1 = $this->orggenerator->assign_job($jobdata);
        $u1job2 = $this->orggenerator->assign_job($jobdata);
        // Assign another job with different position and department.
        $u1job3 = $this->orggenerator->assign_job(['userid' => $user1->id, 'positionid' => $pos1->id, 'departmentid' => $dep1->id]);

        // Assert the job assignments was created.
        $userjobs = organisation::get_user_with_jobs($user1->id, null)->get_jobs();
        $this->assertCount(3, $userjobs);

        // Create user.
        $user2 = $this->tenantgenerator->create_user(['tenantid' => $this->othertenant->id]);
        // Assign a job.
        $u2job1 = $this->orggenerator->assign_job((object)['userid' => $user2->id,
            'positionid' => $pos2->id,
            'departmentid' => $dep2->id, ]);
        // Assign job with shared position and department.
        $u2job2 = $this->orggenerator->assign_job((object)['userid' => $user2->id,
            'positionid' => $this->spos->id,
            'departmentid' => $this->sdep->id, ]);
        // Assert the job assignment was created.
        $userjobs = organisation::get_user_with_jobs($user2->id, null)->get_jobs();
        $this->assertCount(2, $userjobs);

        $csv = <<<EOF
username,jobdepartment1,jobposition1,jobdelete1
{$user1->username},{$this->dep->idnumber},{$this->pos->idnumber},1
{$user2->username},{$this->sdep->idnumber},{$this->spos->idnumber},1
{$user2->username},{$dep2->idnumber},{$pos2->idnumber},0
EOF;
        // Process import, make sure there are no errors.
        $output = $this->process_csv_upload($csv, ['--uutype='.UU_USER_UPDATE]);
        $this->assertStringNotContainsString('Error', $output);

        // Assert all jobs with same position and department are deleted.
        $user1jobs = organisation::get_user_with_jobs($user1->id, null)->get_jobs();
        $this->assertCount(1, $user1jobs);
        $user1job = reset($user1jobs);
        $this->assertEquals('HR Department2', $user1job->get_department()->get('name'));
        $this->assertEquals('Manager2', $user1job->get_position()->get('name'));
        $this->assertEquals($u1job3->departmentid, $user1job->get_department()->get('id'));
        $this->assertEquals($u1job3->positionid, $user1job->get_position()->get('id'));

        // Assert job with shared position and department is deleted.
        $user2jobs = organisation::get_user_with_jobs($user2->id, null)->get_jobs();
        $this->assertCount(1, $user2jobs);
        // Assert job is not deleted when jobdelete is 0.
        $user2job = reset($user2jobs);
        $this->assertEquals('HR Department3', $user2job->get_department()->get('name'));
        $this->assertEquals('Manager3', $user2job->get_position()->get('name'));
        $this->assertEquals($u2job1->departmentid, $user2job->get_department()->get('id'));
        $this->assertEquals($u2job1->positionid, $user2job->get_position()->get('id'));
    }
    ...
    protected function create_tenant_organisation_structure(): void {
        ...
        $this->tenant = $this->tenantgenerator->create_tenant(['idnumber' => 't1']);
        $this->othertenant = $this->tenantgenerator->create_tenant(['idnumber' => 't2']);
        ...
        // Create department in another tenant with the same idnumber.
        $dfother = $this->orggenerator->create_department(['tenantid' => $this->othertenant->id]);
        $this->orggenerator->create_department(
            ['name' => 'HR Department2', 'idnumber' => 'hrd', 'parentid' => $dfother->id]);
        ...
        // Create position in another tenant with the same idnumber.
        $pfother = $this->orggenerator->create_position(['tenantid' => $this->othertenant->id]);
        $this->orggenerator->create_position(
            ['name' => 'Manager2', 'idnumber' => 'manager', 'parentid' => $pfother->id]);
    }
