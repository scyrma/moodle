...
class job_manager_test extends advanced_testcase {
    ...
    public function test_jobs_list() {
        ...
        $this->assertInstanceOf(\tool_organisation\jobs_list::class, $report);
        $this->assertEquals(6, reportbuilder_column::count_records(['reportid' => $report->get_id()]));
    ...
    public function test_get_job_tenant_join(): void {
        ...
        $jobtenantjoin = \tool_organisation\local\entities\jobs::get_job_tenant_join('j', $this->tenant->id);
        $sql = "SELECT u.id
                  FROM {user} u
