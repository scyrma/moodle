 ...
 */
class job_manager_test extends advanced_testcase {
    ...
    public function test_jobs_list() {
        ...
        $this->generate_structure();
        $user = $this->get_tenant_generator()->create_user(['tenantid' => $this->tenant->id]);
        ...
        $reportclass = \tool_organisation\reportbuilder\local\systemreports\jobs::class;
        $report = \tool_tenant\system_report_factory::create($reportclass);
    ...
    /**
     * Test job entity get_job_tenant_join method
     *
     * @covers \tool_organisation\local\entities\jobs
     */
    public function test_get_job_tenant_join(): void {
        global $DB;

        $this->generate_structure();

        // Create user in our test tenant, assign them a job.
        $user1 = $this->get_tenant_generator()->create_user(['tenantid' => $this->tenant->id]);
        $this->get_generator()->assign_job((object) [
            'tenantid' => $this->tenant->id,
            'userid' => $user1->id,
            'positionid' => $this->pa1->id,
            'departmentid' => $this->da1->id,
        ]);

        // Create structure in a sub-tenant, note the generator doesn't currently support parentid so we'll hack it.
        [$subtenant, [$subtenantuser]] = $this->get_tenant_generator()->create_tenant_and_users(1, [
            'parentid' => $this->tenant->id,
        ]);

        $subtenant = (new \tool_tenant\tenant(0, $subtenant))
            ->set('parentid', $this->tenant->id)
            ->set('path', "{$this->tenant->path}/{$subtenant->id}")
            ->set('depth', $this->tenant->depth + 1);
        $subtenant->save();
        (new \tool_tenant\manager())->change_sortorder($subtenant->get('id'));

        $basehierarchy = ['tenantid' => $subtenant->get('id')];
        [$position, $department] = $this->get_generator()->create_position_and_department($basehierarchy, $basehierarchy);
        $this->get_generator()->assign_job((object) [
            'tenantid' => $subtenant->get('id'),
            'userid' => $subtenantuser->id,
            'positionid' => $position->id,
            'departmentid' => $department->id,
        ]);

        // Use the job entity helper to generate appropriate tenant join on the jobs table.
        $jobtenantjoin = \tool_organisation\reportbuilder\local\entities\job::get_job_tenant_join('j.tenantid', $this->tenant->id);
        $sql = "SELECT u.id
                  FROM {user} u
                  JOIN {tool_organisation_job} j ON j.userid = u.id AND {$jobtenantjoin}
              ORDER BY u.id";

        $this->assertEquals([$user1->id, $subtenantuser->id], $DB->get_fieldset_sql($sql));
    }
    ...
    public function test_failed_update_job_enddate_before_startdate() : void {
        ...
        // Set startdate above the enddate.
        try {
            $manager->update_job($job->get('id'), (object)['startdate' => strtotime('+2 days')]);
            $this->fail('Exception expected');
        } catch (moodle_exception $e) {
            $this->assertStringContainsString(get_string('errorinvalidenddate', 'tool_organisation'), $e->getMessage());
        }

        // Set enddate below startdate.
        try {
            $manager->update_job($job->get('id'), (object)['enddate' => strtotime('-2 days')]);
            $this->fail('Exception expected');
        } catch (moodle_exception $e) {
            $this->assertStringContainsString(get_string('errorinvalidenddate', 'tool_organisation'), $e->getMessage());
        }
        ...
        try {
            $manager->update_job($job->get('id'),
                (object)['startdate' => strtotime('+2 days'), 'enddate' => strtotime('-2 days')]);
            $this->fail('Exception expected');
        } catch (moodle_exception $e) {
            $this->assertStringContainsString(get_string('errorinvalidenddate', 'tool_organisation'), $e->getMessage());
        }
