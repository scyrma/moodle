@tool @tool_organisation @moodleworkplace @javascript
Feature: Export and import or organisation structure
  As a tenant admin
  I should be able to export and import organisation structure and jobs

  @_file_upload
  Scenario: Importing organisation structure into a different tenant
    Given "2" tenants exist with "4" users and "0" courses in each
    When I log in as "admin"
    And I perform a new import with these options:
      | Step 1 | Upload a file | admin/tool/organisation/tests/fixtures/orgstructure.zip |
      | Step 2 | Select tenant | 1                                                       |
      | Step 2 | Choose tenant | Tenant2                                                 |
    And the following should exist in the "reportbuilder-table" table:
      | Exporter                          | Tenant name | Status  |
      | Organisation structure frameworks | Tenant2     | Success |
    # The default tenant still does not have any positions or departments.
    And I navigate to "Users > Organisation > Organisation structure" in site administration
    And I follow "Departments"
    And I should see "Nothing to display"
    And I follow "Positions"
    And I should see "Nothing to display"
    And I switch to tenant "Tenant2"
    And I follow "Departments"
    And I should see "Department frameworks"
    And I should see "country"
    And I follow "Positions"
    And I should see "Position frameworks"
    And I should see "Main hierarchy"
    And I log out

  @_file_upload
  Scenario: Importing jobs creating new positions
    Given "2" tenants exist with "4" users and "0" courses in each
    And the following "users" exist:
      | username | firstname | lastname | email              |
      | user48   | Anna      | Smith    | user48@example.com |
    And the following users allocations to tenants exist:
      | user   | tenant  |
      | user48 | Tenant1 |
    And the following positions exist in organisation structure:
      | tenant  | name          | parent |
      | Tenant1 | Posframework1 |        |
      | Tenant1 | Posframework2 |        |
    And the following departments exist in organisation structure:
      | tenant  | name         | parent       |
      | Tenant1 | Depframework |              |
      | Tenant1 | Australia    | Depframework |
      | Tenant1 | Spain        | Depframework |
      | Tenant1 | first floor  | Depframework |
    When I log in as "tenantadmin1"
    And I perform a new import with these options:
      | Step 1 | Upload a file          | admin/tool/organisation/tests/fixtures/jobs.zip |
      | Step 4 | Create in framework... | 1                                               |
      | Step 4 | Position framework     | Posframework2                                   |
    And I press "View import" action in the "Organisation structure jobs" report row
    And I click on "[data-action=show-log]" "css_element"
    And I should see "Created new job for 'Anna Smith' - CTO (Australia)"
    And I should see "Position 'CTO' was created"
    And I navigate to "Users > Organisation > Organisation structure" in site administration
    And I follow "Positions"
    And I click on "Expand position framework 'Posframework2'" "button"
    And I should see "CTO"

  @_file_upload
  Scenario: Importing jobs creating new departments
    Given "2" tenants exist with "4" users and "0" courses in each
    And the following "users" exist:
      | username | firstname | lastname | email              |
      | user48   | Anna      | Smith    | user48@example.com |
    And the following users allocations to tenants exist:
      | user   | tenant  |
      | user48 | Tenant1 |
    And the following positions exist in organisation structure:
      | tenant  | name         | parent       |
      | Tenant1 | Posframework |              |
      | Tenant1 | CTO          | Posframework |
      | Tenant1 | Team lead    | CTO          |
      | Tenant1 | team member  | Team lead    |
    And the following departments exist in organisation structure:
      | tenant  | name          | parent |
      | Tenant1 | Depframework1 |        |
      | Tenant1 | Depframework2 |        |
    When I log in as "tenantadmin1"
    And I perform a new import with these options:
      | Step 1 | Upload a file          | admin/tool/organisation/tests/fixtures/jobs.zip |
      | Step 4 | Create in framework... | 1                                               |
      | Step 4 | Department framework   | Depframework2                                   |
    And I press "View import" action in the "Organisation structure jobs" report row
    And I click on "[data-action=show-log]" "css_element"
    And I should see "Created new job for 'Anna Smith' - CTO (Australia)"
    And I should see "Department 'Australia' was created"
    And I navigate to "Users > Organisation > Organisation structure" in site administration
    And I follow "Departments"
    And I click on "Expand department framework 'Depframework2'" "button"
    And I should see "Australia"

  Scenario: Export user does not have capability to export dep/pos but can export jobs
    Given "2" tenants exist with "1" users and "0" courses in each
    Given the following "permission overrides" exist:
      | capability                              | permission  | role               | contextlevel  | reference |
      | tool/organisation:managedepartments     | Prevent     | tool_tenant_admin  | System        |           |
      | tool/organisation:managepositions       | Prevent     | tool_tenant_admin  | System        |           |
    And the following positions exist in organisation structure:
      | tenant  | name         | parent       |
      | Tenant1 | Posframework |              |
      | Tenant1 | CTO          | Posframework |
      | Tenant1 | Team lead    | CTO          |
      | Tenant1 | team member  | Team lead    |
    And the following departments exist in organisation structure:
      | tenant  | name          | parent |
      | Tenant1 | Depframework1 |        |
      | Tenant1 | Depframework2 |        |
    When I log in as "tenantadmin1"
    And I navigate to "Export and import" in workplace launcher
    And I press "Export"
    And I should not see "Organisation structure frameworks"
    # Exporting to Moodle Workplace format.
    And I click on "Organisation structure jobs Moodle Workplace" "radio"
    And I press "Next"
    And the "Department and position frameworks" "checkbox" should be disabled
    And I click on "Select all jobs in any of the selected frameworks..." "radio"
    And I press "Next"
    And I should see "Required"
    And I should see "Step 2"
    And I log out

  @_file_upload
  Scenario: Import user does not have capability to import dep/pos but can import jobs
    Given "2" tenants exist with "1" users and "0" courses in each
    Given the following "permission overrides" exist:
      | capability                              | permission  | role               | contextlevel  | reference |
      | tool/organisation:managedepartments     | Prevent     | tool_tenant_admin  | System        |           |
      | tool/organisation:managepositions       | Prevent     | tool_tenant_admin  | System        |           |
    When I log in as "tenantadmin1"
    And I navigate to "Export and import" in workplace launcher
    And I navigate to "Import" in current page administration
    And I press "Import"
    And I upload "admin/tool/organisation/tests/fixtures/orgstructure.zip" file to "Upload a file" filemanager
    And I press "Next"
    And I should see "Department frameworks (2)"
    And I should see "Position frameworks (2)"
    And I should not see "Next"
    And I should see "We could not find an available importer for this file"
    And I navigate to "Export and import" in workplace launcher
    And I navigate to "Import" in current page administration
    And I press "Import"
    And I upload "admin/tool/organisation/tests/fixtures/jobs.zip" file to "Upload a file" filemanager
    And I press "Next"
    And I should see "Job assignments (4)"
    And I press "Next"
    And I click on "Select all jobs from specific frameworks..." "radio"
    And the "Department and position frameworks" "checkbox" should be disabled
    And I press "Next"
    And I should see "Required"
    And I should see "Step 3"
    And I log out

  Scenario: Export form validation
    Given "2" tenants exist with "1" users and "0" courses in each
    And the following positions exist in organisation structure:
      | tenant  | name         | parent       |
      | Tenant1 | Posframework |              |
      | Tenant1 | CTO          | Posframework |
      | Tenant1 | Team lead    | CTO          |
      | Tenant1 | team member  | Team lead    |
    And the following departments exist in organisation structure:
      | tenant  | name          | parent |
      | Tenant1 | Depframework1 |        |
      | Tenant1 | Depframework2 |        |
    When I log in as "tenantadmin1"
    And I navigate to "Export and import" in workplace launcher
    And I press "Export"
    And I click on "Organisation structure frameworks" "radio"
    And I press "Next"
    And I click on "Select manually..." "radio"
    And I press "Next"
    And I should see "Required"
    And I should see "Step 2"
    And I log out

  @_file_upload
  Scenario: Import form validation
    Given "2" tenants exist with "1" users and "0" courses in each
    When I log in as "tenantadmin1"
    And I navigate to "Export and import" in workplace launcher
    And I navigate to "Import" in current page administration
    And I press "Import"
    And I upload "admin/tool/organisation/tests/fixtures/orgstructure.zip" file to "Upload a file" filemanager
    And I press "Next"
    And I should see "Department frameworks (2)"
    And I should see "Position frameworks (2)"
    And I press "Next"
    And I click on "Select manually..." "radio"
    And I press "Next"
    And I should see "Required"
    And I should see "Step 3"
    And I log out

  @_file_upload
  Scenario: Import departments form validation with csv file upload
    Given "2" tenants exist with "1" users and "0" courses in each
    When I log in as "tenantadmin1"
    And I navigate to "Export and import" in workplace launcher
    And I navigate to "Import" in current page administration
    And I press "Import"
    And I upload "admin/tool/organisation/tests/fixtures/departments.csv" file to "Upload a file" filemanager
    And I press "Next"
    And I click on "Departments importer (CSV)" "radio"
    And I press "Next"
    And the following fields match these values:
      | Create a generic framework  | 1  |
      | Hierarchy of departments... | 1  |
      | Department identifier       | id |
    And I should not see "Department framework idnumber"
    And I set the following fields to these values:
      | Parent       | Not specified |
    And I press "Next"
    And I should see "When hierarchy is selected, the 'Parent' column must have mapping"
    And I set the following fields to these values:
      | Parent       | parent |
    And I press "Next"
    And I should see "Instances (25)"
    And I log out

  @_file_upload
  Scenario: Import positions form validation with csv file upload
    Given "2" tenants exist with "1" users and "0" courses in each
    When I log in as "tenantadmin1"
    And I navigate to "Export and import" in workplace launcher
    And I navigate to "Import" in current page administration
    And I press "Import"
    And I upload "admin/tool/organisation/tests/fixtures/positions.csv" file to "Upload a file" filemanager
    And I press "Next"
    And I click on "Positions importer (CSV)" "radio"
    And I press "Next"
    And the following fields match these values:
      | Create a generic framework  | 1  |
      | Hierarchy of positions...   | 1  |
      | Position identifier         | id |
    And I should not see "Position framework idnumber"
    And I set the following fields to these values:
      | Parent       | Not specified |
    And I press "Next"
    And I should see "When hierarchy is selected, the 'Parent' column must have mapping"
    And I set the following fields to these values:
      | Parent       | parent |
    And I press "Next"
    And I should see "Instances (25)"
    And I log out

  @_file_upload
  Scenario: Import departments using departments CSV exporter into a new framework
    Given "2" tenants exist with "1" users and "0" courses in each
    When I log in as "tenantadmin1"
    And I change window size to "large"
    And I perform a new import with these options:
      | Step 1 | Upload a file              | admin/tool/organisation/tests/fixtures/departments.csv |
      | Step 2 | Departments importer (CSV) | 1                                                      |
      | Step 3 | Create a generic framework | 1                                                      |
    And I press "View import" action in the "Departments importer (CSV)" report row
    And I should see "Show 22 more..."
    And I click on "[data-action=show-log]" "css_element"
    And I should see "Created new department framework 'departments'"
    And I should see "Created new department 'Sales'"
    And I follow "departments"
    And I should see "Organisation structure" in the "#page-navbar" "css_element"
    And I click on "Expand department framework 'departments'" "button"
    And I should see "Sales"
    And I log out

  @_file_upload
  Scenario: Import departments using departments CSV exporter into an existing framework
    Given "2" tenants exist with "1" users and "0" courses in each
    And the following departments exist in organisation structure:
      | tenant     | name            | parent         |
      | Tenant1    | Framework11_    |                |
      | Tenant1    | Framework12_    |                |
    When I log in as "tenantadmin1"
    And I change window size to "large"
    And I perform a new import with these options:
      | Step 1 | Upload a file              | admin/tool/organisation/tests/fixtures/departments.csv |
      | Step 2 | Departments importer (CSV) | 1                                                      |
      | Step 3 | Select existing framework  | 1                                                      |
      | Step 3 | Department framework       | Framework12_                                           |
    And I press "View import" action in the "Departments importer (CSV)" report row
    And I should see "Show 22 more..."
    And I click on "[data-action=show-log]" "css_element"
    And I should not see "Created new department framework"
    And I should see "Created new department 'Sales'"
    And I navigate to "Organisation structure" in workplace launcher
    And I follow "Departments"
    And I click on "Expand department framework 'Framework12_'" "button"
    And I should see "Sales"
    And I log out

  @_file_upload
  Scenario: Import positions using positions CSV exporter into a new framework
    Given "2" tenants exist with "1" users and "0" courses in each
    When I log in as "tenantadmin1"
    And I change window size to "large"
    And I perform a new import with these options:
      | Step 1 | Upload a file              | admin/tool/organisation/tests/fixtures/positions.csv |
      | Step 2 | Positions importer (CSV) | 1                                                      |
      | Step 3 | Create a generic framework | 1                                                    |
    And I press "View import" action in the "Positions importer (CSV)" report row
    And I should see "Show 22 more..."
    And I click on "[data-action=show-log]" "css_element"
    And I should see "Created new position framework 'positions'"
    And I should see "Created new position 'Chief Executive Officer'"
    And I follow "positions"
    And I should see "Organisation structure" in the "#page-navbar" "css_element"
    And I click on "Expand position framework 'positions'" "button"
    And I should see "Chief Executive Officer"
    And I log out

  @_file_upload
  Scenario: Import positions using positions CSV exporter into an existing framework
    Given "2" tenants exist with "1" users and "0" courses in each
    And the following positions exist in organisation structure:
      | tenant     | name            | parent         |
      | Tenant1    | Framework11_    |                |
      | Tenant1    | Framework12_    |                |
    When I log in as "tenantadmin1"
    And I change window size to "large"
    And I perform a new import with these options:
      | Step 1 | Upload a file              | admin/tool/organisation/tests/fixtures/positions.csv   |
      | Step 2 | Positions importer (CSV)   | 1                                                      |
      | Step 3 | Select existing framework  | 1                                                      |
      | Step 3 | Position framework         | Framework12_                                           |
    And I press "View import" action in the "Positions importer (CSV)" report row
    And I should see "Show 22 more..."
    And I click on "[data-action=show-log]" "css_element"
    And I should not see "Created new position framework"
    And I should see "Created new position 'Chief Executive Officer'"
    And I navigate to "Organisation structure" in workplace launcher
    And I follow "Positions"
    And I click on "Expand position framework 'Framework12_'" "button"
    # Check permissions have been imported correctly.
    And "Manager" "text" should exist in the "Chief Executive Officer" "tool_wp > Table tree node"
    And "Department lead" "text" should exist in the "Chief Executive Officer" "tool_wp > Table tree node"
    And "Manager" "text" should exist in the "Project Manager" "tool_wp > Table tree node"
    And "Department lead" "text" should not exist in the "Project Manager" "tool_wp > Table tree node"
    And "organisation:allocateuserstoprogramcertificationsglob" permission should be "enabled" in the "Project Manager" "tool_wp > Table tree node"
    And "organisation:viewusersreportglob" permission should be "enabled" in the "Project Manager" "tool_wp > Table tree node"
    And "organisation:receivenotificationsglob" permission should be "disabled" in the "Project Manager" "tool_wp > Table tree node"

  @_file_upload
  Scenario: Importing jobs into Shared space should not be possible
    Given shared space is enabled
    When I log in as "admin"
    And I navigate to "Export and import" in workplace launcher
    And I navigate to "Import" in current page administration
    And I press "Import"
    And I upload "admin/tool/organisation/tests/fixtures/jobs.zip" file to "Upload a file" filemanager
    And I press "Next"
    And I click on "Select tenant..." "radio"
    And I set the field "Choose tenant" to "Shared space"
    And I press "Next"
    And I should see "Job assignments"
    And I press "Next"
    And I should see "Jobs can not be imported into Shared space"
    And I log out

  Scenario: Export organisation structure from Shared space
    Given shared space is enabled
    And the following departments exist in organisation structure:
      | tenant     | name            | parent         |
      | -          | FrameworkS1_    |                |
      | -          | DepartmentS11_  | FrameworkS1_   |
      | -          | DepartmentS112_ | DepartmentS11_ |
      | -          | DepartmentS12_  | FrameworkS1_   |
    And the following positions exist in organisation structure:
      | tenant   | name          | parent       | globalmanager | globalpermissions | departmentmanager | departmentpermissions |
      | -        | FrameworkS1_  |              | 0             | 0                 | 0                 | 0                     |
      | -        | FrameworkS2_  |              | 0             | 0                 | 0                 | 0                     |
      | -        | PositionS11_  | FrameworkS1_ | 1             | 0                 | 1                 | 0                     |
      | -        | PositionS112_ | PositionS11_ | 0             | 0                 | 0                 | 0                     |
      | -        | PositionS12_  | FrameworkS1_ | 0             | 0                 | 0                 | 0                     |
    When I log in as "admin"
    And I switch to tenant "Shared space"
    And I navigate to "Export and import" in workplace launcher
    And I press "Export"
    And I click on "Organisation structure frameworks" "radio"
    And I press "Next"
    And I should see "Select all department and position frameworks"
    And I press "Next"
    And I should see "FrameworkS1_ (Department framework)"
    And I should see "FrameworkS1_ (Position framework)"
    And I should see "FrameworkS2_ (Position framework)"
    And I press "Export"
    And I click on "Proceed" "button" in the "Confirm" "dialogue"
    And I should see "Scheduled" in the "Status" "table_row"
    And I click on "Close" "link_or_button" in the "region-main" "region"
    Then the following should exist in the "reportbuilder-table" table:
      | Exporter                          | Tenant name  | Status    |
      | Organisation structure frameworks | Shared space | Scheduled |
    And I log out

  @_file_upload
  Scenario: Importing organisation structure into the Shared space
    Given shared space is enabled
    When I log in as "admin"
    # The default tenant still does not have any positions or departments.
    And I navigate to "Users > Organisation > Organisation structure" in site administration
    And I follow "Departments"
    And I should see "Nothing to display"
    And I follow "Positions"
    And I should see "Nothing to display"
    And I perform a new import with these options:
      | Step 1 | Upload a file | admin/tool/organisation/tests/fixtures/orgstructure.zip |
      | Step 2 | Select tenant | 1                                                       |
      | Step 2 | Choose tenant | Shared space                                            |
    And the following should exist in the "reportbuilder-table" table:
      | Exporter                          | Tenant name  | Status  |
      | Organisation structure frameworks | Shared space | Success |
    # The default tenant now has shared positions and shared departments.
    And I navigate to "Users > Organisation > Organisation structure" in site administration
    And I follow "Departments"
    And I should not see "Nothing to display"
    And I follow "Positions"
    And I should not see "Nothing to display"
    And I switch to tenant "Shared space"
    And I follow "Departments"
    And I should see "Department frameworks"
    And I should see "country"
    And I follow "Positions"
    And I should see "Position frameworks"
    And I should see "Main hierarchy"
    And I log out

  @_file_upload
  Scenario: Importing export created when jobs DR conditions were single-value selects
    Given I log in as "admin"
    And I perform a new import with these options:
      | Step 1 | Upload a file | admin/tool/organisation/tests/fixtures/dr_export_using_singleselects.zip |
    And the following "tool_tenant > users" exist:
      | username | firstname | lastname | email              | tenant                |
      | u2       | User      | 2        | u2@address.invalid | Test DR export import |
      | u3       | User      | 3        | u3@address.invalid | Test DR export import |
    When I navigate to "All tenants" in workplace launcher
    And I switch to tenant "Test DR export import"
    And I navigate to "Dynamic rules" in workplace launcher
    And I should see "Users who have a job in the department 'dep1'" in the "indep1" "table_row"
    And I should see "Users who have position 'pos1'" in the "inpos1" "table_row"
    And I should see "Users who don't have a job in the department 'dep1'" in the "not in dep1" "table_row"
    And I should see "Users who don't have position 'pos1'" in the "not in pos1" "table_row"
    Then I follow "indep1"
    And I should see "1 total matches"
    And I navigate to "Dynamic rules" in workplace launcher
    Then I click on "Enable rule" "field" in the "indep1" "table_row"
    And I click on "Enable" "button" in the "Confirm" "dialogue"
    Then I click on "Enable rule" "field" in the "inpos1" "table_row"
    And I click on "Enable" "button" in the "Confirm" "dialogue"
    Then I click on "Enable rule" "field" in the "not in dep1" "table_row"
    And I click on "Enable" "button" in the "Confirm" "dialogue"
    Then I click on "Enable rule" "field" in the "not in pos1" "table_row"
    And I click on "Enable" "button" in the "Confirm" "dialogue"
    And I run all adhoc tasks
    # In the following assertions, "DR1 U1" is a user created during import.
    And I press "View report" action in the "indep1" report row
    And I should see "DR1 U1" in the "reportbuilder-table" "table"
    And I should not see "User 2" in the "reportbuilder-table" "table"
    And I should not see "User 3" in the "reportbuilder-table" "table"
    And I navigate to "Dynamic rules" in workplace launcher
    And I press "View report" action in the "not in dep1" report row
    And I should not see "DR1 U1" in the "reportbuilder-table" "table"
    And I should see "User 2" in the "reportbuilder-table" "table"
    And I should see "User 3" in the "reportbuilder-table" "table"
    And I navigate to "Dynamic rules" in workplace launcher
    And I press "View report" action in the "inpos1" report row
    And I should see "DR1 U1" in the "reportbuilder-table" "table"
    And I should not see "User 2" in the "reportbuilder-table" "table"
    And I should not see "User 3" in the "reportbuilder-table" "table"
    And I navigate to "Dynamic rules" in workplace launcher
    And I press "View report" action in the "not in pos1" report row
    And I should not see "DR1 U1" in the "reportbuilder-table" "table"
    And I should see "User 2" in the "reportbuilder-table" "table"
    And I should see "User 3" in the "reportbuilder-table" "table"

  @_file_upload
  Scenario: Import departments using departments CSV importer with idnumber as parentid into an existing framework
    Given "2" tenants exist with "1" users and "0" courses in each
    And the following departments exist in organisation structure:
      | tenant     | name            | parent         | idnumber |
      | Tenant1    | Depframework    |                |          |
      | Tenant1    | Japan           | Depframework   | japan    |
    When I log in as "tenantadmin1"
    And I perform a new import with these options:
      | Step 1 | Upload a file              | admin/tool/organisation/tests/fixtures/departments_with_idnumbers.csv |
      | Step 2 | Departments importer (CSV) | 1                                                                     |
      | Step 3 | Select existing framework  | 1                                                                     |
      | Step 3 | Department framework       | Depframework                                                          |
    And I press "View import" action in the "Departments importer (CSV)" report row
    And I should see "Show 2 more..."
    And I click on "[data-action=show-log]" "css_element"
    And I should not see "Created new department framework"
    And I should see "Created new department 'Sales'"
    And I navigate to "Organisation structure" in workplace launcher
    And I follow "Departments"
    And I click on "Expand department framework 'Depframework'" "button"
    And I should see "Tokyo"
    And "Tokyo" "text" should appear after "Japan" "text"
    And "Tokyo" "text" should appear before "Sales" "text"
    And I click on "Edit department 'Tokyo'" "link"
    And I should see "Japan" in the ".form-autocomplete-selection" "css_element"
    And I click on "Cancel" "button" in the "Edit department 'Tokyo'" "dialogue"
    And I should see "Shibuya"
    And "Shibuya" "text" should appear after "Tokyo" "text"
    And "Shibuya" "text" should appear before "Another" "text"
    And I click on "Edit department 'Shibuya'" "link"
    And I should see "Tokyo" in the ".form-autocomplete-selection" "css_element"
    And I click on "Cancel" "button" in the "Edit department 'Shibuya'" "dialogue"
    And I should see "Another"
    And "Another" "text" should appear after "Shibuya" "text"
    And "Another" "text" should appear before "Editorial" "text"
    And I click on "Edit department 'Another'" "link"
    And I should see "Shibuya" in the ".form-autocomplete-selection" "css_element"
    And I click on "Cancel" "button" in the "Edit department 'Another'" "dialogue"
    And I should see "Editorial"
    And "Editorial" "text" should appear after "Another" "text"
    And I click on "Edit department 'Editorial'" "link"
    And I should see "Top" in the ".form-autocomplete-selection" "css_element"
    And I click on "Cancel" "button" in the "Edit department 'Editorial'" "dialogue"
    And I log out

  @_file_upload
  Scenario: Import positions using positions CSV importer with idnumber as parentid into an existing framework
    Given "2" tenants exist with "1" users and "0" courses in each
    And the following positions exist in organisation structure:
      | tenant     | name            | parent         | idnumber |
      | Tenant1    | Posframework    |                |          |
      | Tenant1    | Manager           | Posframework   | manager    |
    When I log in as "tenantadmin1"
    And I perform a new import with these options:
      | Step 1 | Upload a file              | admin/tool/organisation/tests/fixtures/positions_with_idnumbers.csv |
      | Step 2 | Positions importer (CSV) | 1                                                                   |
      | Step 3 | Select existing framework  | 1                                                                   |
      | Step 3 | Position framework         | Posframework                                                        |
    And I press "View import" action in the "Positions importer (CSV)" report row
    And I should see "Show 2 more..."
    And I click on "[data-action=show-log]" "css_element"
    And I should not see "Created new position framework"
    And I should see "Created new position 'Sales'"
    And I navigate to "Organisation structure" in workplace launcher
    And I follow "Positions"
    And I click on "Expand position framework 'Posframework'" "button"
    And "Lead" "text" should appear after "Manager" "text"
    And "Senior developer" "text" should appear after "Lead" "text"
    And "Junior developer" "text" should appear after "Senior developer" "text"
    And "UX Expert" "text" should appear after "Junior developer" "text"

  @_file_upload
  Scenario: Import departments using departments CSV importer with idnumber as parentid into a new framework
    Given "2" tenants exist with "1" users and "0" courses in each
    And the following departments exist in organisation structure:
      | tenant     | name            | parent         | idnumber |
      | Tenant1    | Depframework    |                |          |
    When I log in as "tenantadmin1"
    And I perform a new import with these options:
      | Step 1 | Upload a file               | admin/tool/organisation/tests/fixtures/departments_with_idnumbers.csv |
      | Step 2 | Departments importer (CSV)  | 1                                                                     |
      | Step 3 | Create a generic framework  | 1                                                                     |
      | Step 3 | Hierarchy of departments... | 1                                                                     |
    And I press "View import" action in the "Departments importer (CSV)" report row
    And I should see "Show 2 more..."
    And I click on "[data-action=show-log]" "css_element"
    And I should see "Created new department framework"
    And I should see "Created new department 'Sales'"
    And I navigate to "Organisation structure" in workplace launcher
    And I follow "Departments"
    And "departments with idnumbers" "text" should appear after "Depframework" "text"
    And I click on "Expand department framework 'departments with idnumbers'" "button"
    And I should see "Tokyo"
    And "Tokyo" "text" should appear after "departments with idnumbers" "text"
    And "Tokyo" "text" should appear after "Sales" "text"
    And "Tokyo" "text" should appear before "Shibuya" "text"
    And I click on "Edit department 'Tokyo'" "link"
    And I should see "Top" in the ".form-autocomplete-selection" "css_element"
    And I click on "Cancel" "button" in the "Edit department 'Tokyo'" "dialogue"
    And I should see "Shibuya"
    And "Shibuya" "text" should appear after "Tokyo" "text"
    And "Shibuya" "text" should appear before "Another" "text"
    And I click on "Edit department 'Shibuya'" "link"
    And I should see "Tokyo" in the ".form-autocomplete-selection" "css_element"
    And I click on "Cancel" "button" in the "Edit department 'Shibuya'" "dialogue"
    And I should see "Another"
    And "Another" "text" should appear after "Shibuya" "text"
    And "Another" "text" should appear before "Editorial" "text"
    And I click on "Edit department 'Another'" "link"
    And I should see "Shibuya" in the ".form-autocomplete-selection" "css_element"
    And I click on "Cancel" "button" in the "Edit department 'Another'" "dialogue"
    And I should see "Editorial"
    And "Editorial" "text" should appear after "Another" "text"
    And I click on "Edit department 'Editorial'" "link"
    And I should see "Top" in the ".form-autocomplete-selection" "css_element"
    And I click on "Cancel" "button" in the "Edit department 'Editorial'" "dialogue"
    And I log out

  Scenario: Export departments to a CSV file
    Given "1" tenants exist with "1" users and "0" courses in each
    And the following departments exist in organisation structure:
      | tenant     | name            | idnumber | parent         |
      | Tenant1    | FrameworkS1_    | frmS1_   |                |
      | Tenant1    | FrameworkS2_    | frmS2_   |                |
      | Tenant1    | FrameworkS3_    | frmS3_   |                |
      | Tenant1    | DepartmentS11_  | depS11_  | FrameworkS1_   |
      | Tenant1    | DepartmentS112_ | depS112_ | DepartmentS11_ |
      | Tenant1    | DepartmentS12_  | depS12_  | FrameworkS3_   |
    When I log in as "tenantadmin1"
    And I navigate to "Migration" in workplace launcher
    And I press "Export"
    And I click on "Organisation structure departments (CSV)" "radio"
    And I press "Next"
    And I should see "FrameworkS1_" in the "select_framework" "select"
    And I should see "FrameworkS3_" in the "select_framework" "select"
    And I press "Next"
    # An option must be default selected.
    And I should not see "Required"
    And I should see "FrameworkS1_ (Department framework)"
    And I press "Export"
    And I click on "Proceed" "button" in the "Confirm" "dialogue"
    And I should see "Scheduled" in the "Status" "table_row"
    And I click on "Close" "link_or_button" in the "region-main" "region"
    Then the following should exist in the "reportbuilder-table" table:
      | Exporter                                 | Status    |
      | Organisation structure departments (CSV) | Scheduled |
    And I log out

  Scenario: Export positions to a CSV file
    Given "1" tenants exist with "1" users and "0" courses in each
    And the following positions exist in organisation structure:
      | tenant   | name          | idnumber | parent       | globalmanager | globalpermissions | departmentmanager | departmentpermissions |
      | Tenant1  | FrameworkS1_  | frmS1_   |              | 0             | 0                 | 0                 | 0                     |
      | Tenant1  | FrameworkS2_  | frmS2_   |              | 0             | 0                 | 0                 | 0                     |
      | Tenant1  | FrameworkS3_  | frmS3_   |              | 0             | 0                 | 0                 | 0                     |
      | Tenant1  | PositionS11_  | posS11_  | FrameworkS1_ | 1             | 0                 | 1                 | 0                     |
      | Tenant1  | PositionS112_ | posS112_ | PositionS11_ | 0             | 0                 | 0                 | 0                     |
      | Tenant1  | PositionS12_  | posS12_  | FrameworkS3_ | 0             | 0                 | 0                 | 0                     |
    When I log in as "tenantadmin1"
    And I navigate to "Migration" in workplace launcher
    And I press "Export"
    And I click on "Organisation structure positions (CSV)" "radio"
    And I press "Next"
    And I should see "FrameworkS1_" in the "select_framework" "select"
    And I should see "FrameworkS3_" in the "select_framework" "select"
    And I press "Next"
    # An option must be default selected.
    And I should not see "Required"
    And I should see "FrameworkS1_ (Position framework)"
    And I press "Export"
    And I click on "Proceed" "button" in the "Confirm" "dialogue"
    And I should see "Scheduled" in the "Status" "table_row"
    And I click on "Close" "link_or_button" in the "region-main" "region"
    Then the following should exist in the "reportbuilder-table" table:
      | Exporter                               | Status    |
      | Organisation structure positions (CSV) | Scheduled |
    And I log out

  Scenario: Export jobs to a CSV file
    Given "1" tenants exist with "3" users and "0" courses in each
    And the following departments exist in organisation structure:
      | tenant     | name            | idnumber | parent         |
      | Tenant1    | FrameworkS1_    | frmS1_   |                |
      | Tenant1    | DepartmentS11_  | depS11_  | FrameworkS1_   |
      | Tenant1    | DepartmentS112_ | depS112_ | DepartmentS11_ |
      | Tenant1    | DepartmentS12_  | depS12_  | FrameworkS1_   |
    And the following positions exist in organisation structure:
      | tenant   | name          | idnumber | parent       | globalmanager | globalpermissions | departmentmanager | departmentpermissions |
      | Tenant1  | FrameworkS1_  | frmS1_   |              | 0             | 0                 | 0                 | 0                     |
      | Tenant1  | PositionS11_  | posS11_  | FrameworkS1_ | 1             | 0                 | 1                 | 0                     |
      | Tenant1  | PositionS112_ | posS112_ | PositionS11_ | 0             | 0                 | 0                 | 0                     |
      | Tenant1  | PositionS12_  | posS12_  | FrameworkS1_ | 0             | 0                 | 0                 | 0                     |
    And the following job assignments exist in organisation structure:
      | user   | department      | position      |
      | user11 | DepartmentS11_  | PositionS112_ |
      | user12 | DepartmentS112_ | PositionS11_  |
    When I log in as "tenantadmin1"
    And I navigate to "Migration" in workplace launcher
    And I press "Export"
    And I click on "Organisation structure jobs (CSV)" "radio"
    And I press "Next"
    And I should see "Select all active jobs"
    And I should see "Select all active and past jobs"
    And I should see "Select all jobs in any of the selected frameworks..."
    And I click on "Select all jobs in any of the selected frameworks..." "radio"
    And I open the autocomplete suggestions list
    And "FrameworkS1_ (Department framework)" "autocomplete_suggestions" should exist
    And "FrameworkS1_ (Position framework)" "autocomplete_suggestions" should exist
    And I click on "FrameworkS1_ (Department framework)" item in the autocomplete list
    And I click on "FrameworkS1_ (Position framework)" item in the autocomplete list
    And I press the escape key
    And I press "Next"
    And I should see "User 11 - PositionS112_ (DepartmentS11_)"
    And I should see "User 12 - PositionS11_ (DepartmentS112_)"
    And I press "Export"
    And I click on "Proceed" "button" in the "Confirm" "dialogue"
    And I should see "Scheduled" in the "Status" "table_row"
    And I click on "Close" "link_or_button" in the "region-main" "region"
    Then the following should exist in the "reportbuilder-table" table:
      | Exporter                          | Status    |
      | Organisation structure jobs (CSV) | Scheduled |
    And I log out

  Scenario: Export departments to a CSV file from Shared space
    Given shared space is enabled
    And the following departments exist in organisation structure:
      | tenant | name            | idnumber | parent         |
      | -      | FrameworkS1_    | frmS1_   |                |
      | -      | FrameworkS2_    | frmS2_   |                |
      | -      | FrameworkS3_    | frmS3_   |                |
      | -      | DepartmentS11_  | depS11_  | FrameworkS1_   |
      | -      | DepartmentS112_ | depS112_ | DepartmentS11_ |
      | -      | DepartmentS12_  | depS12_  | FrameworkS3_   |
    When I log in as "admin"
    And I switch to tenant "Shared space"
    And I navigate to "Migration" in workplace launcher
    And I press "Export"
    And I click on "Organisation structure departments (CSV)" "radio"
    And I press "Next"
    And I should see "FrameworkS1_" in the "select_framework" "select"
    And I should see "FrameworkS3_" in the "select_framework" "select"
    And I press "Next"
    # An option must be default selected.
    And I should not see "Required"
    And I should see "FrameworkS1_ (Department framework)"
    And I press "Export"
    And I click on "Proceed" "button" in the "Confirm" "dialogue"
    And I should see "Scheduled" in the "Status" "table_row"
    And I click on "Close" "link_or_button" in the "region-main" "region"
    Then the following should exist in the "reportbuilder-table" table:
      | Exporter                                 | Tenant name  | Status    |
      | Organisation structure departments (CSV) | Shared space | Scheduled |
    And I log out

  Scenario: Export positions to a CSV file from Shared space
    Given shared space is enabled
    And the following positions exist in organisation structure:
      | tenant | name          | idnumber | parent       | globalmanager | globalpermissions | departmentmanager | departmentpermissions |
      | -      | FrameworkS1_  | frmS1_   |              | 0             | 0                 | 0                 | 0                     |
      | -      | FrameworkS2_  | frmS2_   |              | 0             | 0                 | 0                 | 0                     |
      | -      | FrameworkS3_  | frmS3_   |              | 0             | 0                 | 0                 | 0                     |
      | -      | PositionS11_  | posS11_  | FrameworkS1_ | 1             | 0                 | 1                 | 0                     |
      | -      | PositionS112_ | posS112_ | PositionS11_ | 0             | 0                 | 0                 | 0                     |
      | -      | PositionS12_  | posS12_  | FrameworkS3_ | 0             | 0                 | 0                 | 0                     |
    When I log in as "admin"
    And I switch to tenant "Shared space"
    And I navigate to "Migration" in workplace launcher
    And I press "Export"
    And I click on "Organisation structure positions (CSV)" "radio"
    And I press "Next"
    And I should see "FrameworkS1_" in the "select_framework" "select"
    And I should see "FrameworkS3_" in the "select_framework" "select"
    And I press "Next"
    # An option must be default selected.
    And I should not see "Required"
    And I should see "FrameworkS1_ (Position framework)"
    And I press "Export"
    And I click on "Proceed" "button" in the "Confirm" "dialogue"
    And I should see "Scheduled" in the "Status" "table_row"
    And I click on "Close" "link_or_button" in the "region-main" "region"
    Then the following should exist in the "reportbuilder-table" table:
      | Exporter                               | Tenant name  | Status    |
      | Organisation structure positions (CSV) | Shared space | Scheduled |
    And I log out
