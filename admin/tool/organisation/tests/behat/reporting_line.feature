...
Feature: Organisation structure reporting line management
  ...
  Background:
    ...
    And the following "tool_tenant > users" exist:
      ...
      | user1        | User      | 1        | user1@address.invalid        | Tenant1        | 0           |
      | user2        | User      | 2        | user2@address.invalid        | Default tenant | 0           |
      | user3        | User      | 3        | user3@address.invalid        | Tenant1        | 0           |
      | user11       | User      | 11       | user11@address.invalid       | Tenant1        | 0           |
      | user12       | User      | 12       | user12@address.invalid       | Tenant1        | 0           |
      | user13       | User      | 13       | user13@address.invalid       | Tenant1        | 0           |
      | user14       | User      | 14       | user14@address.invalid       | Tenant1        | 0           |
      | tenantadmin1 | Tenant    | admin    | tenantadmin1@address.invalid | Tenant1        | 1           |
  ...
  Scenario: Transfer job to a new one
    ...
    Then the following should exist in the "userjobs-section-jobsassigned" table:
      | -1-                            | -1-                                 |
      | Position112_ · Department112_  | From ##today##%d/%m/%y## to Present |
    And the following should not exist in the "userjobs-section-jobsassigned" table:
      | -1-                            | -1-                                 |
  ...
  Scenario Outline: Assign manually assigned manager
    ...
    And I press the escape key
    And I click on "Cancel" "button" in the "Assign manager" "dialogue"
    And I click on "Assign staff" "button"
    Then I open the autocomplete suggestions list in the "Assign staff" "dialogue"
    And "User 12" "autocomplete_suggestions" should not exist
    And I press the escape key
    And I click on "Cancel" "button" in the "Assign staff" "dialogue"
    Examples:
      | user         |
      | user1        |
      | tenantadmin1 |
  ...
  Scenario: Org manager who can only assign direct managers but not jobs
    Given the following job assignments exist in organisation structure:
      | user   | department     | position      |
      | user11 | Department112_ | Position1111_ |
      | user12 | Department111_ | Position111_  |
    ...
    Given the following "permission overrides" exist:
      | capability                   | permission | role       | contextlevel | reference |
      | tool/organisation:assignjobs | Prohibit   | orgmanager | System       |           |
    When I log in as "user1"
    And I navigate to "Users > Organisation > Organisation structure" in site administration
    And I press "View jobs and reporting lines" action in the "User 11" report row
    Then the following should exist in the "userjobs-section-jobsassigned" table:
      | -1-                             | -1-                                 |
      | Position1111_ Â· Department112_ | From ##today##%d/%m/%y## to Present |
    And ".action-menu" "css_element" should not exist in the ".userjobs-section-jobsassigned" "css_element"
    And I should not see "Assign job"
    And I should see "Assign staff"
    And I click on "Assign manager" "button"
    And I set the field "Select users" in the "Assign manager" "dialogue" to "User 3"
    And I click on "Save" "button" in the "Assign manager" "dialogue"
    And the following should exist in the "userjobs-section-reportsto" table:
      | -1-     |
      | User 12 |
      | User 3  |
    And I press "Un-assign manager" action in the "User 3" report row
    And I click on "Delete" "button" in the "Confirm" "dialogue"
    And I should not see "User 3"
    And I should see "Successfully un-assigned" in the ".toast-message" "css_element"

  Scenario: Org manager who can only assign jobs but not direct managers
    Given user "user12" is a manually assigned manager over users "user11" with permissions "1"
    Given the following "permission overrides" exist:
      | capability                   | permission | role       | contextlevel | reference |
      | tool/organisation:assignmanuallymgr | Prohibit   | orgmanager | System       |           |
    When I log in as "user1"
    And I navigate to "Users > Organisation > Organisation structure" in site administration
    And I press "View jobs and reporting lines" action in the "User 11" report row
    Then I should not see "Assign manager"
    And I should not see "Assign staff"
    And I open the action menu in "User 12" "table_row"
    And I should not see "Un-assign manager"
    And I click on "Assign job" "button"
    And I set the following fields in the "New job for 'User 11'" "dialogue" to these values:
      | positionid       | Position112_    |
      | departmentid     | Department112_  |
      ...
      | enddate[year]    | 2030            |
    And I click on "Proceed" "button" in the "New job for 'User 11'" "dialogue"
    And the following should exist in the "userjobs-section-jobsassigned" table:
      | -1-                             | -1-                                 |
      | Position111_ · Department111_ 	| From ##today##%d/%m/%y## to Present |
      | Position112_ · Department112_   | From ##today##%d/%m/%y## to Present |
    And I should see "Job created" in the ".toast-message" "css_element"

  Scenario: Observe people page as admin
    When I log in as "admin"
    And I navigate to "Organisation structure" in workplace launcher
    And I press "View jobs and reporting lines" action in the "User 2" report row
    Then I should see "Assign job"
    And I should see "Assign manager"
    And I should see "Assign staff"
    # When we switch to tenant "Tenant1", we cannot see any button in the User 2 people page.
    And I switch to tenant "Tenant1"
    Then I should not see "Assign job"
    And I should not see "Assign manager"
    And I should not see "Assign staff"
    # Now go to User 1 people page and check the buttons.
    And I navigate to "Organisation structure" in workplace launcher
    And I press "View jobs and reporting lines" action in the "User 1" report row
    Then I should see "Assign job"
    And I should see "Assign manager"
    And I should see "Assign staff"
    # If we get back to the "Default tenant" staying in the user 1 people page, we should not see the buttons.
