...
class behat_tool_organisation extends behat_base {
    ...
    public function the_following_job_assignments_exist_in_organisation_structure(TableNode $data) {
        ...
        foreach ($data->getHash() as $elementdata) {
            ...
            $generator->assign_job($data, false);
        }
    ...
    public function permission_should_be_in_the($permission, $status, $containerelement, $containerselectortype) {
        ...
        $elsmam = array_filter(tool_organisation\organisation::get_manually_assigned_manager_permissions(),
            function($el) use ($permission) {
                return $el['name'] === $permission;
            });
        if ($elsglob) {
            ...
            $managerclass = 'perm-manually-assigned';
            $el = reset($elsmam);
        } else {
            throw new Exception('Unknown permission: ' . $permission);
        }
    ...
    public function user_has_a_manager_position_over_users_with_permissions($manager, $users, $permission) {
        ...
        $generator->assign_job(['userid' => $managerid, 'departmentid' => $dep->id, 'positionid' => $pos1->id], false);
        foreach ($userids as $userid) {
            $generator->assign_job(['userid' => $userid, 'departmentid' => $dep->id, 'positionid' => $pos2->id], false);
        }
        $generator->execute_adhoc_build_reporting();
    }
    ...
    public function user_has_a_department_lead_position_over_users_with_permissions($manager, $users, $permission) {
        ...
        $generator->assign_job(['userid' => $managerid, 'departmentid' => $dep1->id, 'positionid' => $pos1->id], false);
        foreach ($userids as $userid) {
            $generator->assign_job(['userid' => $userid, 'departmentid' => $dep1->id, 'positionid' => $pos2->id], false);
        }
        $generator->execute_adhoc_build_reporting();
    }

    /**
     * Add manually assigned users that makes one user manager over others
     * phpcs:ignore
     * @Given /^user "(?P<manager_string>(?:[^"]|\\")*)" is a manually assigned manager over users "(?P<users_string>(?:[^"]|\\")*)" with permissions "(?P<courses_number>\d+)"$/
     * @param string $managerusername
     * @param string $users
     * @param int $permission
     * @throws Exception
     */
    public function user_is_a_manually_assigned_manager_over_users_with_permissions($managerusername, $users, $permission) {
        $manager = new stdClass();
        $manager->id = $this->get_user_id($managerusername);
        $manager->permissions = $permission;
        $userids = array_map([$this, 'get_user_id'], preg_split('/\s*,\s*/', trim($users), -1, PREG_SPLIT_NO_EMPTY));

        foreach ($userids as $userid) {
            user_manager::add_assigned_manager($userid, $manager);
        }
    }
     ...
     * | All jobs report | Job assignements page |
     *
    ...
    protected function resolve_page_url(string $page): moodle_url {
        switch ($page) {
            case 'All jobs report':
                return new moodle_url('/admin/tool/organisation/report.php');
            default:
                throw new Exception("Unrecognised people page type '{$page}'");
        }
    }
     ...
     * | Jobs page   | username    | People job page   |
     *
     * @param string $type
     * @param string $identifier
     * @return moodle_url
     * @throws Exception for unrecognised username or page type
     */
    protected function resolve_page_instance_url(string $type, string $identifier): moodle_url {
        if (!$userid = $this->get_user_id($identifier)) {
            throw new Exception("Unknown username '{$identifier}'");
        }

        switch ($type) {
            case 'Jobs page':
                return new moodle_url('/admin/tool/organisation/user.php', ['id' => $userid]);
            default:
                throw new Exception("Unrecognised people page type '{$type}'");
        }
    }
}
