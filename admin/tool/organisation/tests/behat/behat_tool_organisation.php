...
class behat_tool_organisation extends behat_base {
     ...
     *
     * @codingStandardsIgnoreLine
     * @Given /^user "(?P<manager_string>(?:[^"]|\\")*)" has a global manager position over users "(?P<users_string>(?:[^"]|\\")*)" with permissions "(?P<courses_number>\d+)"$/
     * @param string $manager
     * @param string $users
     * @param int $permission
     * @deprecated 3.9.2 No longer in use .See WP-1457
     */
    public function user_has_a_global_manager_position_over_users_with_permissions($manager, $users, $permission) {
        $this->user_has_a_manager_position_over_users_with_permissions($manager, $users, $permission);
    }
     ...
     *
     * @codingStandardsIgnoreLine
     * @Given /^user "(?P<manager_string>(?:[^"]|\\")*)" has a department manager position over users "(?P<users_string>(?:[^"]|\\")*)" with permissions "(?P<courses_number>\d+)"$/
     * @param string $manager
     * @param string $users
     * @param int $permission
     * @deprecated 3.9.2 No longer in use .See WP-1457
    ...
    public function user_has_a_department_manager_position_over_users_with_permissions($manager, $users, $permission) {
        $this->user_has_a_department_lead_position_over_users_with_permissions($manager, $users, $permission);
    }
