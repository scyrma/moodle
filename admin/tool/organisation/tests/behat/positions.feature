...
Feature: Organisation structure positions management
  ...
  Scenario: Create and edit positions and frameworks
    ...
    And I should see "Position ID numbers already exist"
    And I set the field "ID number" to "999"
    And I click on "Save" "button" in the "New position for framework 'New framework1'" "dialogue"
  ...
  Scenario: Using top as parent position
    When I log in as "user1"
    And I navigate to "Users > Organisation > Organisation structure" in site administration
    # Here I should be on a "Positions" tab because all other tabs are disabled.
    And I follow "New framework"
    And I set the following fields to these values:
      ...
      | ID number | 12345      |
    And I click on "Save" "button" in the "New position framework" "dialogue"
    And I follow "New position for framework 'Framework1'"
    And I set the following fields to these values:
      | Name      | Position1 |
      | ID number | 999       |
    ...
    And I click on "Save" "button" in the "New position for framework 'Framework1'" "dialogue"
    And I should see "Position1"
    And I follow "New subposition for position 'Position1'"
    And I set the following fields to these values:
      | Name      | Position2 |
      | ID number | 998       |
    And I should see "Position1"
    And I open the autocomplete suggestions list
    And "Top" "autocomplete_suggestions" should exist
    And "Position1" "autocomplete_suggestions" should exist
    And I click on "Top" "text" in the ".modal-dialog .form-autocomplete-suggestions" "css_element"
    And I click on "Save" "button" in the "New subposition for position 'Position1'" "dialogue"
    And I should see "Position2"
    And "Position2" "text" should appear after "Position1" "text"
    And I follow "New position for framework 'Framework1'"
    And I set the following fields to these values:
      | Name      | Position3 |
      | ID number | 997       |
    ...
    And "Top" "autocomplete_suggestions" should exist
    And "Position1" "autocomplete_suggestions" should exist
    And "Position2" "autocomplete_suggestions" should exist
    And I click on "Position2" "text" in the ".modal-dialog .form-autocomplete-suggestions" "css_element"
    And I click on "Save" "button" in the "New position for framework 'Framework1'" "dialogue"
    Then "Position3" "text" should appear after "Position2" "text"
    And "Position2" "text" should appear after "Position1" "text"
  ...
  Scenario: Changing position parent field
    ...
    And "Position1111_" "autocomplete_suggestions" should not exist
  ...
  Scenario: User with assigninlocked capability can create,edit and delete locked positions
    ...
    And I click on "Save" "button" in the "New position framework" "dialogue"
    And "Edit position framework 'Framework1'" "link" should exist
    And "New position for framework 'Framework1'" "link" should exist
    And "Delete position framework 'Framework1'" "link" should exist
    # Remove capability.
    And the following "permission overrides" exist:
      | capability                        | permission | role       | contextlevel | reference |
      | tool/organisation:assigninlocked  | Prohibit   | orgmanager | System       |           |
    And I navigate to "Users > Organisation > Organisation structure" in site administration
    Then "Edit position framework 'Framework1'" "link" should not exist
    And "New position for framework 'Framework1'" "link" should not exist
    And "Delete position framework 'Framework1'" "link" should not exist
    # Users without the capability can not lock frameworks.
    And I follow "New framework"
    And I should not see "Locked" in the "New position framework" "dialogue"
