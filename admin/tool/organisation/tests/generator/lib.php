...
class tool_organisation_generator extends component_generator_base {
    ...
    public function create_department($record = null) : stdClass {
        ...
        $record = (object)($record ? (array)$record : []);
     ...
     *    set to false if you are assigning multiple jobs in bulk and want to call
     *    {@see tool_organisation_generator::execute_adhoc_build_reporting()}
     *    manually after all jobs are assigned.
     * @return stdClass
     */
    public function assign_job($data, bool $rebuildtree = true): stdClass {
        ...
        $job = (new \tool_organisation\job_manager())->create_job($data, false)->to_record();
        if ($rebuildtree) {
            $this->execute_adhoc_build_reporting();
        }
        return $job;
    }
    ...
    /**
     * Execute adhoc task created to build reporting line.
     *
     */
    public function execute_adhoc_build_reporting (): void {
