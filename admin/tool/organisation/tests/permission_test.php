 ...
 * @covers     \tool_organisation\permission
...
class permission_test extends advanced_testcase {
    ...
    public function test_can_access_entity(): void {
        ...
        $positionframework1 = (new position_manager())->create_position((object)['name' => 'F1']);
        ...
        $positionframework2 = (new position_manager())->create_position((object)$params, false);
        ...
        $positionframework3 = (new position_manager())->create_position((object)$params, false);
    ...
    public function test_can_edit_with_shared(): void {
        ...
        $positionframework1 = (new position_manager())->create_position((object)$params, false);
    ...
    public function test_user_is_manager(): void {
        ...
        $positionframework2 = (new position_manager())->create_position((object)$params2, false);
        ...
        $positionframework3 = (new position_manager())->create_position((object)$params3, false);
    ...
    /**
     * Test for permission::can_edit_if_position_is_locked()
     */
    public function test_can_edit_if_position_is_locked(): void {
        [$tenant, [$user1, $user2]] = $this->tenantgenerator->create_tenant_and_users(2);

        // Create framework and position without global/department manager.
        $params = ['name' => 'General', 'shared' => 0, 'tenantid' => $tenant->id, 'locked' => 0];
        $positionframework = (new position_manager())->create_position((object)$params, false);

        $context = \context_system::instance();
        $roleid = create_role('Dummy role', 'dummyrole', 'dummy role description');
        assign_capability('tool/organisation:assigninlocked', CAP_ALLOW, $roleid, $context->id);
        role_assign($roleid, $user1->id, $context->id);

        // Position is not locked, both users should be able to edit the position.
        $this->setUser($user1);
        $this->assertTrue(permission::can_edit_if_position_is_locked($positionframework));

        $this->setUser($user2);
        $this->assertTrue(permission::can_edit_if_position_is_locked($positionframework));

        $positionframework->set('locked', true);
        $positionframework->update();

        // Position is locked, only the user with the capability should be able to edit the position.
        $this->setUser($user1);
        $this->assertTrue(permission::can_edit_if_position_is_locked($positionframework));

        $this->setUser($user2);
        $this->assertFalse(permission::can_edit_if_position_is_locked($positionframework));
    }

    /**
     * Test for permission::can_edit_if_department_is_locked()
     */
    public function test_can_edit_if_department_is_locked(): void {
        [$tenant, [$user1, $user2]] = $this->tenantgenerator->create_tenant_and_users(2);

        // Create framework and position without global/department manager.
        $params = ['name' => 'General', 'shared' => 0, 'tenantid' => $tenant->id, 'locked' => 0];
        $departmentframework = (new \tool_organisation\department_manager())->create_department((object)$params, false);
        ...
        // Department is not locked, both users should be able to edit the department.
        $this->setUser($user1);
        $this->assertTrue(permission::can_edit_if_department_is_locked($departmentframework));

        $this->setUser($user2);
        $this->assertTrue(permission::can_edit_if_department_is_locked($departmentframework));

        $departmentframework->set('locked', true);
        $departmentframework->update();

        // Department is locked, only the user with the capability should be able to edit the department.
        $this->setUser($user1);
        $this->assertTrue(permission::can_edit_if_department_is_locked($departmentframework));

        $this->setUser($user2);
        $this->assertFalse(permission::can_edit_if_department_is_locked($departmentframework));
    }

    /**
     * Test checking if user can view people page
     *
     * @covers \tool_organisation\permission::can_view_user_org_profile
     */
    public function test_can_view_people_page(): void {
        [$tenant1, [$user1, $tenantadmin]] = $this->tenantgenerator->create_tenant_and_users(2);
        [$tenant2, [$user2, $user3]] = $this->tenantgenerator->create_tenant_and_users(2);

        $manager = new \tool_tenant\manager();
        $manager->assign_tenant_admin_role($tenant1->id, [$tenantadmin->id]);

        // Admin user should be able to view people page.
        $this->setAdminUser();
        $this->assertTrue(permission::can_view_user_org_profile($user1->id));
        $this->assertTrue(permission::can_view_user_org_profile($user2->id));
        $this->assertTrue(permission::can_view_user_org_profile($user3->id));

        // Tenant admin should be able to view people page only for users in same tenant.
        $this->setUser($tenantadmin);
        $this->assertTrue(permission::can_view_user_org_profile($user1->id));
        $this->assertFalse(permission::can_view_user_org_profile($user2->id));
        $this->assertFalse(permission::can_view_user_org_profile($user3->id));
    }

    /**
     * Test checking if user can assign jobs to other users.
     *
     * @covers \tool_organisation\permission::can_assign_jobs_to_user
     *
     */
    public function test_can_assign_jobs_to_user(): void {
        $this->resetAfterTest();
        [$tenant1, [$user1, $tenantadmin]] = $this->tenantgenerator->create_tenant_and_users(2);
        [$tenant2, [$user2, $user3]] = $this->tenantgenerator->create_tenant_and_users(2);
        $user4 = $this->tenantgenerator->create_user(['username' => 'user4']);

        $manager = new \tool_tenant\manager();
        $manager->assign_tenant_admin_role($tenant1->id, [$tenantadmin->id]);

        // Check admin can assign jobs to users.
        $this->setAdminUser();
        $this->assertFalse(permission::can_assign_jobs_to_user($user1->id));
        $this->assertFalse(permission::can_assign_jobs_to_user($user2->id));
        $this->assertFalse(permission::can_assign_jobs_to_user($user3->id));
        $this->assertTrue(permission::can_assign_jobs_to_user($user4->id));

        // Let's switch to tenant1.
        \tool_tenant\tenancy::set_switched_tenant_id($tenant1->id);

        // Now check that users match with current tenantid (tenant1).
        $this->assertTrue(permission::can_assign_jobs_to_user($user1->id));
        $this->assertFalse(permission::can_assign_jobs_to_user($user2->id));
        $this->assertFalse(permission::can_assign_jobs_to_user($user3->id));
        $this->assertFalse(permission::can_assign_jobs_to_user($user4->id));

        // Check can assign jobs to users when a user belongs to the same tenant that current one,
        // but now as a tenant admin.
        $this->setUser($tenantadmin);
        $this->assertTrue(permission::can_assign_jobs_to_user($user1->id));
        $this->assertFalse(permission::can_assign_jobs_to_user($user2->id));
        $this->assertFalse(permission::can_assign_jobs_to_user($user3->id));
        ...
        $tenantadminrole = (object)['id' => manager::get_tenant_admin_role()];
        unassign_capability('tool/organisation:assignjobs', $tenantadminrole->id);

        // Tenant admin can't assign jobs to other users now.
        $this->assertFalse(permission::can_assign_jobs_to_user($user1->id));
        $this->assertFalse(permission::can_assign_jobs_to_user($user2->id));
        $this->assertFalse(permission::can_assign_jobs_to_user($user3->id));
        $this->assertFalse(permission::can_assign_jobs_to_user($user4->id));
    ...
    /**
     * Test checking if user can assign manually assigned managers to other users.
     *
     * @covers \tool_organisation\permission::can_assign_mam_to_user
     *
     */
    public function test_can_assign_mam_to_user(): void {
        $this->resetAfterTest();
        [$tenant1, [$user1, $tenantadmin]] = $this->tenantgenerator->create_tenant_and_users(2);
        [$tenant2, [$user2, $user3]] = $this->tenantgenerator->create_tenant_and_users(2);
        $user4 = $this->tenantgenerator->create_user(['username' => 'user4']);
        ...
        // Check admin can assign manually assigned managers to users.
        $this->setAdminUser();
        $this->assertFalse(permission::can_assign_mam_to_user($user1->id));
        $this->assertFalse(permission::can_assign_mam_to_user($user2->id));
        $this->assertFalse(permission::can_assign_mam_to_user($user3->id));
        $this->assertTrue(permission::can_assign_mam_to_user($user4->id));
        ...
        $this->assertTrue(permission::can_assign_mam_to_user($user1->id));
        $this->assertFalse(permission::can_assign_mam_to_user($user2->id));
        $this->assertFalse(permission::can_assign_mam_to_user($user3->id));
        $this->assertFalse(permission::can_assign_mam_to_user($user4->id));

        // Check can assign manually assigned managers to users when a user belongs to tenant same that current one,
        // but now as a tenant admin.
        $this->setUser($tenantadmin);
        $this->assertTrue(permission::can_assign_mam_to_user($user1->id));
        $this->assertFalse(permission::can_assign_mam_to_user($user2->id));
        ...
        // Unassign 'assignmanuallymgr' and assign 'assignjobs' capabilities to the tenant admin role.
        $tenantadminrole = (object)['id' => manager::get_tenant_admin_role()];
        unassign_capability('tool/organisation:assignmanuallymgr', $tenantadminrole->id);

        // Tenant admin still have the capability to assign mam, so all assert as previous.
        $this->assertFalse(permission::can_assign_mam_to_user($user1->id));
        $this->assertFalse(permission::can_assign_mam_to_user($user2->id));
        $this->assertFalse(permission::can_assign_mam_to_user($user3->id));
        $this->assertFalse(permission::can_assign_mam_to_user($user4->id));
