 ...
 * @covers     \tool_organisation\position_manager
 * @copyright  2018 Moodle Pty Ltd <support@moodle.com>
 * @author     2018 Marina Glancy
 * @license    Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class position_test extends advanced_testcase {
    ...
    public function test_get_potential_parents() {
        ...
        $manager = new \tool_organisation\position_manager();
        $position11 = $manager->get_position($this->position11->id);
        $framework1 = $manager->get_position($this->framework1->id);

        // When adding a new position, all the options should be shown.
        ...
        $potentialparents = $manager->get_potential_parents('', $position11);
        ...
        // Look for position21 potential parents with 'Top' and 'New position 2' search string (Framework 2).
        $potentialparents = $manager->get_potential_parents('Top', $manager->get_position($position21->id));
        $this->assertCount(1, $potentialparents);
        $this->assertArrayHasKey($this->framework2->id, $potentialparents);
        $potentialparents = $manager->get_potential_parents('New position 2', $manager->get_position($position21->id));
        $this->assertCount(1, $potentialparents);
        $this->assertArrayHasKey($this->framework2->id, $potentialparents);
    }
