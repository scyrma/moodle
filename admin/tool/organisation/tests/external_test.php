 ...
 * @covers     \tool_organisation_external
...
class external_test extends advanced_testcase {
    ...
    public function test_create_departments_with_warnings() {
        $this->resetAfterTest();
        $this->get_tenant_generator()->create_tenant(['idnumber' => 't1']);
        $this->setAdminUser();
        $records = [
            ['name' => 'Department 01', 'idnumber' => 'dep1'],
            ['name' => 'Department 02', 'idnumber' => 'dep2'],
            ['name' => 'Department 4', 'parent' => 'dep1', 'tenant' => 'meh'],
        ...
        $this->assertEquals('Department 4', $departments['warnings'][0]['item']);
        $this->assertEquals('tenantnotfound', $departments['warnings'][0]['warningcode']);
        $this->assertEquals('Department 31', $departments['warnings'][1]['item']);
        $this->assertEquals('errorparentnotfound', $departments['warnings'][1]['warningcode']);
    }
    /**
     * Test for funciton create_departments()
     */
    public function test_create_departments_without_warnings() {
        ...
        $this->setAdminUser();
        $records = [
            ['name' => 'Department 01', 'idnumber' => 'dep1', 'tenant' => 't1'],
            ['name' => 'Department 02', 'idnumber' => 'dep2', 'tenant' => 't1'],
            ['name' => 'Department 12', 'parent' => 'dep1', 'tenant' => 't1'],
            ['name' => 'Department 22', 'parent' => 'dep2', 'tenant' => 't1'],
            ['name' => 'Department 01 in Default tenant', 'idnumber' => 'dep1'],
        ];
        $departments = tool_organisation_external::create_departments($records);
        $departments = external_api::clean_returnvalue(tool_organisation_external::create_departments_returns(), $departments);
        $this->assertEquals('Department 01', $departments['result'][0]['name']);
        ...
        $this->assertEquals('Department 12', $departments['result'][2]['name']);
        $this->assertEquals('Department 22', $departments['result'][3]['name']);
        $this->assertEquals('Department 01 in Default tenant', $departments['result'][4]['name']);
    ...
    public function test_create_positions() {
        ...
        $this->assertTrue($pos0->get('globalmanager'));
        $this->assertFalse($pos0->get('departmentmanager'));

        $pos2 = position::get_record(['idnumber' => 'pos2']);
        $this->assertFalse($pos2->get('globalmanager'));
        $this->assertTrue($pos2->get('departmentmanager'));
    ...
    public function test_get_teams_tab_filters_is_deprecated() {
        $function = external_api::external_function_info('tool_organisation_get_teams_tab_filters');
        $this->assertEquals(1, $function->deprecated);
    }
    ...
    public function test_get_managed_users() {
        ...
        $job1 = $generator->assign_job((object) $data, false);
        ...
        $job2 = $generator->assign_job((object) $data, false);
        ...
        $job3 = $generator->assign_job((object) $data, false);
        ...
        $job4 = $generator->assign_job((object) $data, false);

        // Execute ad-hoc reporting line tasks.
        ...
        $result = tool_organisation_external::get_managed_users(0, 0, 0, 0,
            tool_organisation_external::FULLNAME_CONTAINS, 'Lionel');
        $cleanresult = external_api::clean_returnvalue(tool_organisation_external::get_managed_users_returns(), $result);
        $this->assertCount(1, $cleanresult['managedusers']);
        $this->assertEquals('Lionel Richie', $cleanresult['managedusers'][0]['fullname']);
        $this->assertEquals(1, $cleanresult['totalcount']);
    }

    public function test_get_managed_users_is_deprecated() {
        $function = external_api::external_function_info('tool_organisation_get_managed_users');
        $this->assertEquals(1, $function->deprecated);
    }
}
