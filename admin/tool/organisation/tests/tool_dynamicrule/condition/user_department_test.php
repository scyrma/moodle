 ...
 */
class user_department_test extends advanced_testcase {
    ...
    /**
     * Test that rule is executed when job start date is reached
     */
    public function test_watch_job_startdate() {
        ...
        if (extension_loaded('uopz')) {
            ...
            /** @var \tool_datewatch_generator $datewatchgenerator */
            $datewatchgenerator = $this->getDataGenerator()->get_plugin_generator('tool_datewatch');
            $datewatchgenerator->shift_dates('tool_organisation_job', 'startdate', -$delta);
        }
    ...
    /**
     * Test that user is marked as unmatched after the job enddate
     */
    public function test_watch_job_enddate_unmatch() {
        ...
        if (extension_loaded('uopz')) {
            ...
            /** @var \tool_datewatch_generator $datewatchgenerator */
            $datewatchgenerator = $this->getDataGenerator()->get_plugin_generator('tool_datewatch');
            $datewatchgenerator->shift_dates('tool_organisation_job', 'enddate', -$delta);
        }
    ...
    public function test_supports_rule_types(): void {
        $outcome = user_department::instance();
    ...
    public function test_get_matching_users_shared_rules() {
        /** @var \tool_tenant_generator $tenantgenerator */
        $tenantgenerator = $this->getDataGenerator()->get_plugin_generator('tool_tenant');
        /** @var \tool_dynamicrule_generator $drgenerator */
        ...
        $configdata = ['departmentid' => [$department1->id],
            'criteria' => user_department::CRITERIA_ALL, 'withsubdepartments' => 0];
        user_department::create($rule1->id, $configdata);
        // Check for department1 and that it finds both users from Tenant1.
        $this->assertEquals(2, \tool_dynamicrule\api::count_matching_users($rule1->id));
        $users = \tool_dynamicrule\api::get_matching_users($rule1->id);
        ...
        $configdata = ['departmentid' => [$departmentshared->id],
            'criteria' => user_department::CRITERIA_ALL, 'withsubdepartments' => 0];
        user_department::create($rule2->id, $configdata);
        // Check for shared department and that it finds just user11 from Tenant1.
