 ...
 * @license     Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class assign_job_test extends \advanced_testcase {
     ...
     * @return array
     */
    public static function validate_config_form_provider(): array {
        return [
            [[
    ...
    public static function get_description_provider(): array {
        ...
        [
            [
                ...
                [
                    'startdate' => get_string('ruleoutcomeassignjobstartruledate', 'tool_organisation'),
            ...
            [
                ...
                [
                    'startdate' => get_string('ruleoutcomeassignjobstartuserdate', 'tool_organisation'),
                    'enddate' => '1 weeks',
                ],
            ],
            [
                [
                    'startdate' => assign_job::START_ABSOLUTE,
                    'startdateabsolute' => strtotime('2023-01-01'),
                    'enddate' => assign_job::END_ABSOLUTE,
                    'enddateabsolute' => strtotime('2023-12-31'),
                ],
                [
                    'startdate' => userdate(strtotime('2023-01-01'), get_string('strftimedatefullshort')),
                    'enddate' => userdate(strtotime('2023-12-31'), get_string('strftimedatefullshort')),
                ],
            ],
        ];
    }
     ...
     */
    public function test_get_description(array $configdata, array $options): void {
        [$position, $department] = $this->get_plugin_generator()->create_position_and_department();
        $rule = $this->get_rule_generator()->create_rule();
        $configdata = ['departmentid' => $department->id, 'positionid' => $position->id] + $configdata;
        $outcome = assign_job::create($rule->id, $configdata);

        $options = ['department' => $department->name, 'position' => $position->name] + $options;
        $expected = get_string('ruleoutcomeassignjobdescwithdates', 'tool_organisation', $options);
     ...
     * @return array
     */
    public static function apply_to_users_provider(): array {
        return [
            [[
                'startdate' => assign_job::START_EXECUTION,
                'enddate' => assign_job::END_NONE,
    ...
    /**
     * Test applying to a user in another tenant
     */
