...
/**
 * Tests for conversion of job department filter
 *
 ...
 */
class job_department_test extends \core_reportbuilder_testcase {
    ...
    /**
     * Provider for job department conversion test
     *
     * @return array[]
     */
    public static function convert_provider(): array {
        return [
            'with subdepartments' => [1, ['A', 'C']],
            'without subdepartments' => [0, ['A']],
        ];
    }

    /**
     * Test for conversion of job department condition
     *
    ...
    public function test_convert(int $operator, array $expected): void {
        ...
        $orggenerator->assign_job(['userid' => $user2->id, 'positionid' => $pa->id, 'departmentid' => $dep2->id]);
        $orggenerator->assign_job(['userid' => $user3->id, 'positionid' => $pa->id, 'departmentid' => $dep3->id]);
        ...
        $generator->add_condition($report, 'tool_organisation_jobs:department');
        $DB->set_field('tool_reportbuilder', 'conditions',
            json_encode(['tool_organisation_jobs:department' => $dep1->id, 'tool_organisation_jobs:department_op' => $operator]),
            ['id' => $reportid]);
