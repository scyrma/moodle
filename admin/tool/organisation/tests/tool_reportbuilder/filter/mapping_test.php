...
class mapping_test extends \advanced_testcase {
     ...
     * @return string[]
     */
    public static function department_condition_provider(): array {
        return [
            ['tool_organisation_jobs:department', 'job:department_department'],
            ['user:jobdepartment', 'user:jobdepartment'],
        ];
    }
     ...
     *
     * @param string $conditionkey condition identifier in the tool reportbuilder (original report)
     * @param string $conditionkeynew condition identifier in the core reportbuilder (after conversion)
     * @dataProvider department_condition_provider
     */
    public function test_report_department_condition_mapping(string $conditionkey, string $conditionkeynew): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        // The imported condition field should be mapped to the new department (the report was converted to core reportbuilder).
        ...
        $this->assertEquals($newdepartment->id, $conditiondata[$conditionkeynew]);
    }
     ...
     * @return string[]
     */
    public static function position_condition_provider(): array {
        return [
            ['tool_organisation_jobs:position', 'job:position_position'],
     ...
     */
    public function test_report_position_condition_mapping(string $conditionkey, string $conditionkeynew): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        // The imported condition field should be mapped to the new position (the report was converted to core reportbuilder).
        $reports = report::get_records([], 'id');
        $newreport = \core_reportbuilder\manager::get_report_from_id(end($reports)->get('id'));
        $conditiondata = (array)json_decode($newreport->get_report_persistent()->get('conditiondata'));
        $this->assertEquals($newposition->id, $conditiondata[$conditionkeynew]);
    }
