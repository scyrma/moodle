...
abstract class hierarchy extends \core\persistent {
     ...
     * @return self|null
     */
    public static function get_by_idnumber(string $idnumber, int $tenantid): ?self {
        global $DB;

        [$sql, $params] = \tool_tenant\hierarchy::filter_own_or_parent_shared_entities_sql(
            'tenantid', 'shared=1', $tenantid);
        // We need a case insensitive comparison on the value.
        $equal = $DB->sql_equal('idnumber', ':idnumber', false);
        $query = "$equal AND $sql";
        $params += ['idnumber' => $idnumber];

        if ($record = $DB->get_record_select(static::TABLE, $query, $params)) {
