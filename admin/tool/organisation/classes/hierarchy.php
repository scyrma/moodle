...
abstract class hierarchy extends \core\persistent {
    ...
    protected static function define_properties() {
        return array(
            ...
            'locked' => [
                'type' => PARAM_BOOL,
                'description' => 'Whether the hierarchy element is locked',
                'default' => false,
            ],
        );
    }
    ...
    /**
     * Retrieve a record by idnumber (from the same or shared tenant)
     *
     * @param string $idnumber
     * @param int $tenantid
     * @param bool $includeshared include entities from shared space
     * @return self|null
     */
    public static function get_by_idnumber(string $idnumber, int $tenantid = 0, bool $includeshared = true): ?self {
        $record = \tool_tenant\hierarchy::find_entity_by_idnumber(static::TABLE,
            $idnumber, $tenantid, null, [], true, 'shared = 1');
        if ($record) {
     ...
     * @param int $tenantid tenant of the framework. Must be specified explicitly,
     *     this function does not look up the framework tenant for performance reasons.
     * @return self|null
     */
    public static function get_by_idnumber_in_framework(string $idnumber, int $frameworkid, int $tenantid): ?self {
        global $DB;
        $extraquery = "(" . $DB->sql_like('path', ':framework') . ")";
        $extraparams = ['framework' => "/{$frameworkid}/%"];
        $record = \tool_tenant\hierarchy::find_entity_by_idnumber(static::TABLE,
            $idnumber, $tenantid, $extraquery, $extraparams, true, 'shared = 1');
        if ($record) {
            return new static(0, $record);
        }
        return null;
    ...
    /**
     * Checks if entity idnumber is unique within a tenant and shared space
     *
     * @param int $entityid entity id if already exists
     * @param string|null $idnumber
     * @param int $tenantid tenant id where the entity is or where it will be created, 0 for the current tennat
     * @return bool
     */
    public static function is_idnumber_unique(int $entityid, ?string $idnumber, int $tenantid = 0): bool {
        return \tool_tenant\hierarchy::is_entity_idnumber_unique(static::TABLE, $idnumber, $entityid,
            $tenantid);
    }
