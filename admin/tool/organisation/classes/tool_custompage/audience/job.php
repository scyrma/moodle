...
/**
 * Custom page audience type based on a users jobs within the organisation structure
 *
...
class job extends base {
    ...
    public function get_config_form(MoodleQuickForm $mform): void {
        ...
        // For global pages, we show shared organisation structure.
        $page = $this->get_persistent()->get_page();
        if ($page->get('global')) {
            $tenantid = (int) \tool_tenant\sharedspace::get_shared_space_id();
        } else {
            $tenantid = $page->get('tenantid');
        }

        // Department select.
        $departments = organisation::get_all_departments_menu([self::DEPARTMENT_ANY => $strs->anydepartment], $tenantid);
        $department = [
        ...
        // Position select.
        $positions = organisation::get_all_positions_menu([self::POSITION_ANY => $strs->anyposition], $tenantid);
        $position = [
    ...
    public function get_sql(string $usertablealias): array {
        ...
        $departmentid = (int) ($departmentconfig['id']);
        ...
        $positionid = (int) ($positionconfig['id']);
    ...
    public function user_can_add(bool $global = false): bool {
        return (permission::has_assign_jobs_capability() || permission::has_manage_positions_capability() ||
            permission::has_manage_departments_capability()) && (tenancy::is_site_multi_tenant() || !$global);
    }
