...
class job_manager {
    ...
    public function create_job(\stdClass $data, bool $currenttenantonly = true) : job {
        ...
        if (!permission::is_entity_visible_in_list($department, $data->userid)) {
            throw new \moodle_exception('errorcreatingjob', 'tool_organisation');
        }
        ...
        if (!permission::is_entity_visible_in_list($position, $data->userid)) {
            throw new \moodle_exception('errorcreatingjob', 'tool_organisation');
        }
    ...
    /**
     * Move jobs for the given user that have both, department AND position, on shared space to the new user tenant
     *
     * Those jobs that have only one shared entity (the department or the position) will not be moved.
     *
     * @param int $userid
     ...
     */
    public static function move_shared_jobs_to_new_tenant(int $userid, int $oldtenantid, int $newtenantid): void {
        global $DB;

        $sharedtenantid = \tool_tenant\sharedspace::get_shared_space_id();
        // Check if Shared space is enabled.
        if (!$sharedtenantid) {
            return;
        }

        $sql = 'SELECT job.* FROM {tool_organisation_job} job
                LEFT JOIN {tool_organisation_department} dep
                ON dep.id = job.departmentid
                LEFT JOIN {tool_organisation_position} pos
                ON pos.id = job.positionid
                WHERE pos.tenantid = :tenantposition AND dep.tenantid = :tenantdepartment
                AND userid = :userid AND job.tenantid = :tenantjob';
        $params = [
            'tenantposition' => $sharedtenantid,
            'tenantdepartment' => $sharedtenantid,
            'userid' => $userid,
            'tenantjob' => $oldtenantid
        ];
        $jobs = $DB->get_records_sql($sql, $params);

        foreach ($jobs as $jobrecord) {
            $job = new \tool_organisation\job(0, $jobrecord);
