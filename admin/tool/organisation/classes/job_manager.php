...
class job_manager {
    ...
    /**
     * Returns all jobs for given user.
     *
     * @param int $userid
     * @return array
     */
    public static function get_user_jobs(int $userid) {
        return job::get_records(['userid' => $userid]);
    }
    ...
    public function update_job(int $jobid, \stdClass $newdata) : job {
        ...
        if ($haschanges) {
            ...
            reporting::schedule_reporting_line_reindex($entity->get('tenantid'));
    ...
    public function create_job(\stdClass $data, bool $currenttenantonly = true) : job {
        ...
        // Schedule building reporting line ad-hoc task.
        reporting::schedule_reporting_line_reindex($data->tenantid);

        return $entity;
    }

    /**
     * Creates a new job and updates the end date of the original job.
     *
     * @param int $jobid
     * @param \stdClass $newdata
     * @return job
     */
    public function update_and_create_job(int $jobid, \stdClass $newdata) : job {
        $entity = $this->get_job(['id' => $jobid]);

        $newdata->id = 0;
        $newdata->userid = $entity->get('userid');
        $newjob = $this->create_job($newdata);

        $newdata = $entity->to_record();
        $newdata->enddate = strtotime('-1 day', $newjob->get('startdate'));
        $this->update_job($jobid, $newdata);
