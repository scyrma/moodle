...
/**
 * The base class for user position conditions.
 *
 ...
 */
abstract class condition_position_base extends \tool_dynamicrule\condition_sql {
    ...
    public function validate_config_form(array $data): array {
        ...
        if (empty($data['positionid'])) {
            $errors['positionid'] = get_string('required');
            return $errors;
        }

        [$list, $params] = $DB->get_in_or_equal($data['positionid']);
        $sql = "SELECT COUNT(1) FROM {tool_organisation_position} WHERE archived = 0 AND id " . $list;
        if ($DB->count_records_sql($sql, $params) !== count($data['positionid'])) {
    ...
    /**
     * Return the configured positionids
     *
     * @return array
     */
    protected function get_positionid(): array {
        return (array) $this->get_configdata()['positionid'];
    }

    /**
     * Return the configured subpositions flag
     *
     * @return bool
     */
    protected function get_with_subpositions(): bool {
    ...
    public function is_configuration_valid(): bool {
        global $DB;
        $positionids = $this->get_positionid();
        [$list, $params] = $DB->get_in_or_equal($positionids);
        $sql = "SELECT COUNT(1) FROM {tool_organisation_position} WHERE archived = 0 AND id " . $list;
        return $DB->count_records_sql($sql, $params) === count($positionids);
    }
    ...
    public function get_importer_mapping(importer_base $importer): void {
        ...
        foreach ($this->get_positionid() as $positionid) {
            $configdata['positionid'][] = $importer->get_mapping('tool_organisation_position',
                $positionid, IGNORE_MISSING) ?? 0;
        }
