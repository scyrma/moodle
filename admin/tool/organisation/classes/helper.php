...
class helper {
     ...
     */
    public static function job_time_and_tenant_select(int $time, string $jobalias = 'j') {
    ...
    public static function user_has_position_select(int $positionid, bool $withsubpositions = false,
            string $usertablealias = 'u', ?int $minstartdate = null) : array {
        global $DB;

        $j = db::generate_alias();
        list($timewhere, $timeparams) = self::job_time_and_tenant_select(0, $j);
        if ($minstartdate) {
