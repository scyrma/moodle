...
class helper {
     ...
     * @param int $time
     * @param string $fields
     * @deprecated since 4.3 TODO WP-4381 add deprecation warning
     * @return array
     */
     ...
     * @param int $withpermissions
     * @param int $time
     * @return array [$joinqueries, $params]
     */
    public static function get_all_direct_managed_users_join(user_with_jobs $managerjobs, int $withpermissions = 0,
                                                             int $time = 0): array {
        global $DB;
        ...
        foreach ($jobs as $job) {
            if ($job->get_position()->is_global_manager($withpermissions)) {
                ...
                $joinqueries[] = " SELECT {$jobalias}.userid, " .self::GLOBAL_MANAGER. " AS manager
            ...
            if ($job->get_position()->is_department_manager($withpermissions)) {
                ...
                $joinqueries[] = " SELECT {$jobalias1}.userid, " .self::DEPARTMENT_MANAGER. " AS manager
        ...
        if ($managerjobs->is_manually_assigned_manager($withpermissions)) {
            ...
            $userparam = database::generate_param_name();
            $params[$userparam] = $managerjobs->get('id');
            $joinqueries[] = "SELECT {$mgr}.userid, " .self::MANUALLY_ASSIGNED_MANAGER. " AS manager
                FROM {tool_organisation_manual_mgr} {$mgr}
                 WHERE {$mgr}.managerid = :{$userparam}";
        }

        if (empty($joinqueries)) {
            return [["SELECT 0 AS userid, 0 AS manager {$DB->sql_null_from_clause()} WHERE 1=0"], []];
        }

        return [$joinqueries, $params];
    }
    ...
    public static function get_direct_managed_users_select(user_with_jobs $manager, $usertablealias = 'u',
                                                    int $withpermissions = 0, int $time = 0) : array {
        return self::get_managed_users_select($manager, $usertablealias, $withpermissions, $time, true);
    }
     ...
     * @return string
     */
    public static function get_user_direct_managers_sql(string $usersql = 'u.id', string $extrafields = ''): string {
        global $DB;
        $j1 = database::generate_alias();
        ...
        return "SELECT {$j2}.userid {$extrafields}
     ...
     * @return string
     */
    public static function get_user_direct_dptleads_sql(string $usersql = 'u.id', string $extrafields = ''): string {
        $j1 = database::generate_alias();
        $j2 = database::generate_alias();
        $p1 = database::generate_alias();
        $p2 = database::generate_alias();
        ...
        return "SELECT {$j2}.userid {$extrafields}
            ...
            WHERE {$j1}.userid = $usersql AND ({$j2}.startdate < {$now} AND ({$j2}.enddate = 0 OR {$j2}.enddate > {$now}))
     ...
     * @return string
     */
    public static function get_user_manually_assigned_managers_sql(string $usersql = 'u.id', string $extrafields = ''): string {
        $mmgr = database::generate_alias();

        return "SELECT {$mmgr}.managerid {$extrafields}
                    FROM {tool_organisation_manual_mgr} {$mmgr}
    ...
    public static function get_user_direct_managers(int $userid): array {
        ...
        $currentuserparam = database::generate_param_name();
        $managersquery[] = self::get_user_direct_managers_sql(":{$currentuserparam}",
            ", " . self::GLOBAL_MANAGER . " AS type");
        $managerparams[$currentuserparam] = $userid;

        // Get direct department lead managers sql.
        $currentuserparam1 = database::generate_param_name();
        $managersquery[] = self::get_user_direct_dptleads_sql(":{$currentuserparam1}",
            ", " . self::DEPARTMENT_MANAGER . " AS type");
        $managerparams[$currentuserparam1] = $userid;

        // Get direct manually assigned managers sql.
        $currentuserparam2 = database::generate_param_name();
        $managersquery[] = self::get_user_manually_assigned_managers_sql(":{$currentuserparam2}",
            ", " . self::MANUALLY_ASSIGNED_MANAGER . " AS type");
        $managerparams[$currentuserparam2] = $userid;
