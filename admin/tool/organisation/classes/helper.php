...
class helper {
    ...
    /** @var int The constant for global manager */
    const GLOBAL_MANAGER = 1;

    /** @var int The constant for department lead manager */
    const DEPARTMENT_MANAGER = 2;

    /** @var int The constant for manually assigned manager */
    const MANUALLY_ASSIGNED_MANAGER = 3;
    ...
    protected static function build_menu(array $records, callable $displayfilter = null, callable $indentfunc = null) {
        ...
        $frameworks = [];
        $frameworkslocked = [];
        $hasassigninlockedcap = permission::has_assign_in_locked_capability();
        foreach ($records as $record) {
            ...
            // Check if is a locked framework and if the user is able to assign jobs to it (has assigninlocked
            // capability). If not, do not add the framework and its departments/positions into the menu.
            if (!$hasassigninlockedcap && (int)$record->locked === 1 && (int)$record->pathlevel === 1) {
                $frameworkslocked[] = $fid;
                continue;
            }
            if (in_array($fid, $frameworkslocked)) {
                continue;
            }
            ...
            if ($record->pathlevel > 1) {
                ...
                $frameworks[$fid] = self::get_unique_framework_name(format_string($record->name, true, $options), $frameworks);
            }
        }
        if (count($r) == 1) {
    ...
    /**
     * Gets unique framework name to show them all in optgroup.
     * @param string $name
     * @param array $frameworks
     * @return string
     */
    private static function get_unique_framework_name(string $name, array $frameworks): string {
        if (in_array($name, $frameworks)) {
            // When there are multiple frameworks with the same name, we change the names to show them all in the jobs dropdown.
            // Otherwise, the optgroup will treat them as the same and remove them from the dropdown.
            $name .= ' ';
            return self::get_unique_framework_name($name, $frameworks);
        }
        return $name;
    }
    ...
    public static function job_time_select(?int $time, string $jobalias = 'j') {
        if ($time !== null) {
            ...
            $ptime1 = database::generate_param_name();
            $ptime2 = database::generate_param_name();
     ...
     * Extra field is added to the select: ismanuallyassignedmgr (1 if user is a manually assigned manager, 0 otherwise)
     *
     * @param string $where
     * @param array $params
     * @param int|null $time
    ...
    public static function get_users_with_jobs_sql(string $where, array $params = [], ?int $time = 0, int $tenantid = 0) {
        // Site need to be upgraded, otherwise the following query will throw an exception.
        if (get_config('tool_organisation', 'version') < 2023091200) {
        ...
        // Add extra select field to check if user is a manually assigned manager.
        $mmgr = database::generate_alias();
        $manuallyassignedmgrfields = "(CASE WHEN (EXISTS(SELECT 1
                  FROM {tool_organisation_manual_mgr} {$mmgr}
                      WHERE {$mmgr}.managerid = u.id
                        AND {$mmgr}.tenantid = {$tenantid})) THEN 1 ELSE 0 END)
                      AS ismanuallyassignedmgr";

        [$timewhere, $timeparams] = self::job_time_select($time);
        $sql = "SELECT {$userfields}, {$jobfields}, {$manuallyassignedmgrfields}
          FROM {user} u
          $tjoin
     ...
     *
     * This function is deprecated without replacement because it is no longer used
     *
    ...
    public static function get_managed_users_with_jobs_sql(user_with_jobs $manager, int $withpermissions = 0,
        ...
        debugging(
            'The function ' . __FUNCTION__ . '() is deprecated without replacement because it is no longer used.',
            DEBUG_DEVELOPER
        );

        list($where, $params) = $manager->get_managed_users_select('u', $withpermissions, $time);
        list($sql, $params) = self::get_users_with_jobs_sql($where, $params);

        if ($fields) {
            $sql = "SELECT $fields FROM ($sql) " . database::generate_alias();
        }
    ...
    public static function get_managed_users_select(user_with_jobs $manager, $usertablealias = 'u',
        ...
        debugging('The function ' . __FUNCTION__ . '() is deprecated, please do not use it any more. ' .
            'See \'user_with_jobs::get_managed_users_select()\' function for replacement', DEBUG_DEVELOPER);

        global $DB;
        $jobs = $manager->get_jobs(true, $withpermissions);
        $queries = [];
        $params = [];
        $j = database::generate_alias();
        ...
        foreach ($jobs as $job) {
            if ($job->get_position()->is_global_manager($withpermissions)) {
                ...
                if ($childpositionsonly) {
                    $pid = database::generate_param_name();
            ...
            if ($job->get_position()->is_department_manager($withpermissions)) {
                $did = database::generate_param_name();
                $params[$did] = $job->get_department()->get('id');
                if ($childpositionsonly) {
                    $queries[] = "{$d}.id = :$did";
                } else {
                    $dpath = database::generate_param_name();
                    $queries[] = "({$d}.id = :$did OR " . $DB->sql_like("{$d}.path", ":$dpath") . ')';
                    $params[$dpath] = $job->get_department()->get('path') . '/%';
                }
                $j1 = database::generate_alias();
                $p1 = database::generate_alias();
                $queries[count($queries) - 1] .= " AND NOT EXISTS ".
    ...
    /**
     * Helps to create SQL to retrieve the complete managed users for a given manager.
     * @param user_with_jobs $managerjobs
     * @param string $usertablealias
     * @param int $withpermissions
     * @param int $time
     * @return array [$where, $params]
     */
    public static function get_all_direct_managed_users_default(user_with_jobs $managerjobs, string $usertablealias = 'u',
                                                        int $withpermissions = 0, int $time = 0): array {
        $jobs = $managerjobs->get_jobs(true);
        $queries = [];
        $params = [];
        $where = [];
        $j = database::generate_alias();
        ...
        if ($queries) {
            $selfuserid = database::generate_param_name();
            list($timewhere, $timeparams) = self::job_time_and_tenant_select($time, $j);
            $params[$selfuserid] = $managerjobs->get('id');
        ...
        // If the manager is manually assigned, then we need to check if they are a manager of the user.
        if ($managerjobs->is_manually_assigned_manager($withpermissions)) {
            $mgr = database::generate_alias();
            $tenantparam = database::generate_param_name();
            $userparam = database::generate_param_name();
            $params[$tenantparam] = tenancy::get_tenant_id();
            $params[$userparam] = $managerjobs->get('id');
            $wheremam = " EXISTS (SELECT 1
                FROM {tool_organisation_manual_mgr} {$mgr}
                 WHERE {$mgr}.tenantid = :{$tenantparam}
                  AND {$usertablealias}.id = {$mgr}.userid
                  AND {$mgr}.managerid = :{$userparam})";
            $where = $queries ? "({$where} OR {$wheremam})" : $wheremam;
        }
    ...
    /**
     * SQL to retrieve all users who are managed directly by a given manager.
     *
     * Note that results will have duplicates if there are several reporting relations between the manager and the user.
     *
     * @param user_with_jobs $managerjobs
     * @param int $withpermissions
     * @return array [$sql, $params]
     */
    public static function get_all_direct_managed_users_sql(user_with_jobs $managerjobs, int $withpermissions = 0): array {
        global $DB;
        ...
        $selfuserid = database::generate_param_name();
        $params[$selfuserid] = $managerjobs->get('id');
        $time = time(); // It is only possible to retrieve at the current time since assigned managers do not have start/end dates.

        foreach ($jobs as $job) {
            if (!$job->is_active($time)) {
                continue;
            }
            if ($job->get_position()->is_global_manager($withpermissions)) {
                $jobalias = database::generate_alias();
                $posalias = database::generate_alias();
                $posid = database::generate_param_name();
                [$timewhere, $timeparams] = self::job_time_and_tenant_select($time, $jobalias);
                $joinqueries[] = " SELECT {$jobalias}.userid, " . self::GLOBAL_MANAGER . " AS manager, {$jobalias}.id AS jobid
                                FROM {tool_organisation_job} {$jobalias}
                                JOIN {tool_organisation_position} {$posalias} ON {$posalias}.id = {$jobalias}.positionid
                                WHERE {$posalias}.parentid = :{$posid}
                                  AND {$timewhere} ";
                $params[$posid] = $job->get_position()->get('id');
                $params = $params + $timeparams;
            }
            if ($job->get_position()->is_department_manager($withpermissions)) {
                $jobalias1 = database::generate_alias();
                $jobalias2 = database::generate_alias();
                $deptalias = database::generate_alias();
                $deptid = database::generate_param_name();
                $posalias1 = database::generate_alias();
                [$timewhere1, $timeparams1] = self::job_time_and_tenant_select($time, $jobalias1);
                [$timewhere2, $timeparams2] = self::job_time_and_tenant_select($time, $jobalias2);
                $joinqueries[] = " SELECT {$jobalias1}.userid, " . self::DEPARTMENT_MANAGER . " AS manager, {$jobalias1}.id AS jobid
                                FROM {tool_organisation_job} {$jobalias1}
                                JOIN {tool_organisation_department} {$deptalias} ON {$deptalias}.id = {$jobalias1}.departmentid
                                WHERE {$deptalias}.id = :{$deptid}
                                  AND {$timewhere1} AND
                                   NOT EXISTS (SELECT 1 FROM {tool_organisation_job} {$jobalias2}
                                        JOIN {tool_organisation_position} {$posalias1} ON {$posalias1}.id={$jobalias2}.positionid
                                        WHERE {$jobalias2}.userid={$jobalias1}.userid
                                            AND {$jobalias2}.departmentid={$deptalias}.id
                                            AND {$timewhere2}
                                            AND {$posalias1}.departmentmanager=1)";
                $params[$deptid] = $job->get_department()->get('id');
                $params = $params + $timeparams1 + $timeparams2;
            }
        }
        ...
        if ($managerjobs->is_manually_assigned_manager($withpermissions)) {
            ...
            $userparam = database::generate_param_name();
            $params[$userparam] = $managerjobs->get('id');
            $joinqueries[] = "SELECT {$mgr}.userid, " . self::MANUALLY_ASSIGNED_MANAGER . " AS manager, 0 AS jobid
                FROM {tool_organisation_manual_mgr} {$mgr}
                 WHERE {$mgr}.managerid = :{$userparam}";
        }

        if (empty($joinqueries)) {
            return ["SELECT 0 AS userid, 0 AS manager, 0 AS jobid {$DB->sql_null_from_clause()} WHERE 1=0", []];
        }

        $allmanagedsql = implode(' UNION ALL ', $joinqueries);
        return [$allmanagedsql, $params];
    }
    ...
    public static function user_has_position_select(int $positionid, bool $withsubpositions = false,
        ...
        if ($withsubpositions) {
            ...
            $p = database::generate_alias();
            $pathparam = database::generate_param_name();
            ...
            $pos = database::generate_param_name();
            $where = " EXISTS (SELECT 1
                                 FROM {tool_organisation_job} $j
                                WHERE $j.userid = {$usertablealias}.id
                                  AND {$timewhere}
    ...
    public static function user_is_in_department_select(int $departmentid, bool $includesubdept = false,
        ...
        $j = database::generate_alias();
        $dep = database::generate_param_name();
        ...
        if ($includesubdept) {
            $d = database::generate_alias();
            $pdeppath = database::generate_param_name();
    ...
    public static function users_with_jobs_sql(string $usertablealias, ?int $minstarttime = null, int $tenantid = 0): array {
        $jobtablealias = database::generate_alias();
    ...
    public static function users_with_managerpermission_sql(string $usertablealias, array $managerparams = [],
        ...
        $organisationposition = database::generate_alias();
        $organisationjob = database::generate_alias();
        ...
        if (count($activemanagerparams) === 1) {
            $departmentmanager = database::generate_param_name();
            $globalmanager = database::generate_param_name();

            $wheremanager = "{$organisationposition}.departmentmanager = :{$departmentmanager} OR
                {$organisationposition}.globalmanager = :{$globalmanager}";

            $params = [
                $departmentmanager => $managerparams['departmentmanager'],
                $globalmanager => $managerparams['globalmanager'],
            ];
        } else {
            ...
            $activemanagerparamname = database::generate_param_name();

            $wheremanager = "{$organisationposition}.{$activemanagerparam} = :{$activemanagerparamname}";
            $params = [$activemanagerparamname => $managerparams[$activemanagerparam]];
        }
    ...
    public static function get_direct_managed_users_select(user_with_jobs $manager, $usertablealias = 'u',
        ...
        return $manager->get_managed_users_select($usertablealias, $withpermissions, $time, true);
    }
     ...
     * @param string $extrafields Add some extra fields in the returned select.
     * @param bool $includejobid
     * @return string
     */
    public static function get_user_direct_managers_sql(
        string $usersql = 'u.id',
        string $extrafields = '',
        bool $includejobid = false
    ): string {
        global $DB;
        $j1 = database::generate_alias();
        $j2 = database::generate_alias();
        $p1 = database::generate_alias();
        $p2 = database::generate_alias();
        $pathconcat = $DB->sql_concat("{$p2}.path", "'/'", "{$p1}.id");
        $now = time();

        $jobid = $includejobid ? ", {$j2}.id AS jobid" : '';

        return "SELECT {$j2}.userid {$extrafields} {$jobid}
                ...
                AND {$j1}.startdate <= {$now} AND ({$j1}.enddate = 0 OR {$j1}.enddate >= {$now})
                AND {$j2}.startdate <= {$now} AND ({$j2}.enddate = 0 OR {$j2}.enddate >= {$now})
                ";
    }
    ...
    public static function get_user_direct_managers_select(int $userid, $usertablealias = 'u'): array {
        ...
        $p = database::generate_alias();
        $sql = "{$usertablealias}.id in ($subselect)";
     ...
     * @return string
     */
    public static function get_user_direct_dptleads_sql(
        string $usersql = 'u.id',
        string $extrafields = '',
        bool $includejobid = false
    ): string {
        ...
        $p1 = database::generate_alias();
        $p2 = database::generate_alias();
        $j3 = database::generate_alias();
        $p3 = database::generate_alias();
        $now = time();
        ...
        return "SELECT {$j2}.userid {$extrafields} {$jobid}
            ...
            WHERE {$j1}.userid = $usersql
                ...
                AND ({$j2}.startdate <= {$now} AND ({$j2}.enddate = 0 OR {$j2}.enddate >= {$now}))
                AND NOT EXISTS (
                    SELECT 1 FROM {tool_organisation_job} {$j3}
                    JOIN {tool_organisation_position} {$p3} ON {$j3}.positionid = {$p3}.id
                    WHERE {$j3}.userid = {$j1}.userid
                        AND {$j3}.departmentid = {$j1}.departmentid AND {$p3}.departmentmanager = 1
                        AND {$j3}.startdate <= {$now} AND ({$j3}.enddate = 0 OR {$j3}.enddate >= {$now})
                )";
    }
    ...
    /**
     * Returns manager permissions in human-readable format from octal notation for CSV exportation
     *
     * @param int $permissions
     * @return string
     */
    public static function get_humanreadable_manager_permissions(int $permissions): string {
        if ($permissions === organisation::PERM_ALLOCATE_PROGRAMS + organisation::PERM_VIEW_REPORTS +
        ...
        $readablepermissions = [];
        if ($permissions & organisation::PERM_ALLOCATE_PROGRAMS) {
            $readablepermissions[] = 'allocate';
        }
        if ($permissions & organisation::PERM_VIEW_REPORTS) {
            $readablepermissions[] = 'reports';
        }
        if ($permissions & organisation::PERM_RECEIVE_NOTIFICATIONS) {
            $readablepermissions[] = 'notifications';
        }

        return implode(',', $readablepermissions);
    }

    /**
     * Returns SQL to get the list of the user's manually assigned managers ids.
     *
     ...
     * @return string
     */
    public static function get_user_manually_assigned_managers_sql(
        string $usersql = 'u.id',
        string $extrafields = '',
        bool $includejobid = false
    ): string {
        $mmgr = database::generate_alias();
        $jobid = $includejobid ? ", 0 AS jobid" : '';

        return "SELECT {$mmgr}.managerid AS userid {$extrafields} {$jobid}
                    FROM {tool_organisation_manual_mgr} {$mmgr}
                    WHERE {$mmgr}.userid = {$usersql}";
    }

    /**
     * Joins together all types of direct managers queries
     *
     * @param string $usersql SQL snippet that returns id of the user (will be directly used in SQL query, can not use parameters)
     * @param string|null $relationalias if specified, alias for the relation type column
     * @param bool $includejobid
     * @return string SQL query returning 'userid' and optionally second column with the name $relationalias
     */
    public static function get_user_all_direct_managers_sql(
        string $usersql,
        ?string $relationalias = null,
        bool $includejobid = false
    ): string {

        // Get direct global managers sql.
        $managersquery[] = self::get_user_direct_managers_sql($usersql,
            $relationalias ? ("," . self::GLOBAL_MANAGER . " AS {$relationalias}") : '', $includejobid);

        // Get direct department lead managers sql.
        $managersquery[] = self::get_user_direct_dptleads_sql($usersql,
            $relationalias ? ("," . self::DEPARTMENT_MANAGER . " AS {$relationalias}") : '', $includejobid);

        // Get direct manually assigned managers sql.
        $managersquery[] = self::get_user_manually_assigned_managers_sql($usersql,
            $relationalias ? ("," . self::MANUALLY_ASSIGNED_MANAGER . " AS {$relationalias}") : '', $includejobid);

        // Let's join them using union all each manager query and add as a joined table, also exclude the current user.
        $allmanagerssql = implode(' UNION ALL ', $managersquery);

        return $allmanagerssql;
    }

    /**
     * Retrieve all direct manager for a given user.
     *
     * @param int $userid
     * @return array
     */
    public static function get_user_direct_managers(int $userid): array {
        global $DB;

        $allmanagerssql = self::get_user_all_direct_managers_sql("" . $userid, 'type');

        $useralias = database::generate_alias();
        $reportuseralias = database::generate_alias();
        $manageduserparam = database::generate_param_name();
        $managerparams = [$manageduserparam => $userid];
        $reportstojoin = "JOIN ({$allmanagerssql}) {$reportuseralias}
                                ON {$reportuseralias}.userid = {$useralias}.id
                                 AND {$reportuseralias}.userid <> :{$manageduserparam}";

        [$tenantjoin, $where, $params] = tenancy::get_users_sql($useralias, tenancy::get_tenant_id($userid));
        $uniqueid = $DB->sql_concat("{$useralias}.id", "'_'", "{$reportuseralias}.type");
        return $DB->get_records_sql("SELECT {$uniqueid}, {$useralias}.*, {$reportuseralias}.type FROM {user}
                    {$useralias} {$reportstojoin} {$tenantjoin} WHERE {$where}", $managerparams + $params);
    }

    /**
     * Return a SQL subselect to be used by report builder "peoplewithmanagers" filter.
     *
     * @param string $usertablealias
     * @return string
     */
    public static function users_with_managers_sql(string $usertablealias = 'u'): string {
        $allmanagerssql = self::get_user_all_direct_managers_sql($usertablealias.".id");
        return "CASE WHEN EXISTS ({$allmanagerssql}) THEN 1 ELSE 0 END";
    }
}
