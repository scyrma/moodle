...
class organisation {
    ...
    public static function get_department_manager_permissions() {
        return [
            ...
            self::PERM_RECEIVE_NOTIFICATIONS => [
    ...
    /**
     * Returns hardcoded permissions for Manually assigned manager with icons.
     *
     * @return array
     */
    public static function get_manually_assigned_manager_permissions(): array {
        return [
            self::PERM_ALLOCATE_PROGRAMS => [
                'name' => 'organisation:allocateuserstoprogramcertificationsmam',
                'title' => get_string('organisation:allocateuserstoprogramcertificationsmam', 'tool_organisation'),
                'icon' => new \pix_icon('i/course', ''),
            ],
            self::PERM_VIEW_REPORTS => [
                'name' => 'organisation:viewusersreportmam',
                'title' => get_string('organisation:viewusersreportmam', 'tool_organisation'),
                'icon' => new \pix_icon('bar-chart', '', 'tool_wp'),
            ],
            self::PERM_RECEIVE_NOTIFICATIONS => [
                'name' => 'organisation:receivenotificationsmam',
                'title' => get_string('organisation:receivenotificationsmam', 'tool_organisation'),
                'icon' => new \pix_icon('i/notifications', ''),
            ],
        ];
    }
    ...
    public static function get_user_direct_managers(int $userid = 0): array {
        global $DB, $USER;
        $userid = $userid ?: $USER->id;
        $useridparam = database::generate_param_name();
    ...
    /**
     * Returns list of users that are direct managers for the user (position manager, department leads
     * and manually assigned managers).
     *
     * @param int $userid
     * @return array
     */
    public static function get_user_all_direct_managers(int $userid = 0): array {
        global $DB, $USER;
        $userid = $userid ?: $USER->id;
        $useridparam1 = database::generate_param_name();
        $useridparam2 = database::generate_param_name();
        $useridparam3 = database::generate_param_name();
        $subselect1 = helper::get_user_direct_managers_sql(":{$useridparam1}");
        $subselect2 = helper::get_user_direct_dptleads_sql(":{$useridparam2}");
        $subselect3 = helper::get_user_manually_assigned_managers_sql(":{$useridparam3}");
        $sql = "SELECT u.* FROM {user} u WHERE u.id IN ($subselect1 UNION $subselect2 UNION $subselect3)";

        return $DB->get_records_sql($sql, [$useridparam1 => $userid, $useridparam2 => $userid, $useridparam3 => $userid]);
    }
     ...
     *
     * We can retrieve if a user is a manger over another user and the permissions, also there is a flag to
     * indicate if the manager is manually assigned.
     *
    ...
    public static function get_users_with_jobs(string $where, array $params = [], ?int $time = 0, int $tenantid = 0) : array {
        ...
        foreach ($records as $record) {
            ...
            if (!array_key_exists($userrecord->id, $userrecords)) {
                $userrecord->ismanuallyassignedmgr = (bool)$record->ismanuallyassignedmgr;
        ...
        foreach ($userrecords as $userrecord) {
            $users[$userrecord->id] = new user_with_jobs($userrecord, [
                'time' => $time,
                'jobs' => $userjobs[$userrecord->id],
                'fulluserrecord' => null,
                'ismanuallyassignedmgr' => $userrecord->ismanuallyassignedmgr,
            ]);
        }
        return $users;
    }
    ...
    public static function get_managed_users_departments_menu(user_with_jobs $manager, int $withpermissions = 0,
        ...
        $sql = "SELECT d.*, md.departmentid AS ismanaged
            FROM {tool_organisation_department} d
            LEFT JOIN (
                SELECT DISTINCT j.departmentid
                    FROM {tool_organisation_reporting} r
                    JOIN {tool_organisation_job} j ON j.userid = r.userid
                    WHERE r.managerid = :managerid
            ) md ON md.departmentid = d.id
            WHERE $tenantsql
            ORDER by d.pathlevel, d.sortorder, d.id";
        $records = $DB->get_records_sql($sql, $tenantparams + ['managerid' => $manager->get('id')]);
    ...
    public static function get_managed_users_positions_menu(user_with_jobs $manager, int $withpermissions = 0,
                                                            array $firstoption = null) {
        global $DB;
        [$tenantsql, $tenantparams] = tenanthierarchy::filter_own_or_parent_shared_entities_sql("p.tenantid", "shared=1");
        $sql = "SELECT DISTINCT p.id, p.name
            FROM {tool_organisation_reporting} r
            JOIN {tool_organisation_job} j ON j.userid = r.userid
            JOIN {tool_organisation_position} p ON p.id = j.positionid
            WHERE $tenantsql AND r.managerid = :managerid";
        $records = $DB->get_records_sql_menu($sql, ['managerid' => $manager->get('id')] + $tenantparams);
     ...
     *
     * @param array $firstoption for example ['' => 'None'] or ['' => 'All'] or ['' => '']
     * @param int $tenantid ID of the tenant we should consider current
     * @return array Returns array of options that can be used in moodleform 'selectgroups' element
     */
    public static function get_all_departments_menu(array $firstoption = null, int $tenantid = 0) : array {
        global $DB;

        // Filter by same or parent tenant.
        [$sql, $params] = tenanthierarchy::filter_own_or_parent_shared_entities_sql("tenantid", "shared=1", $tenantid);
     ...
     * @return array Returns array of options that can be used in moodleform 'selectgroups' element
     */
    public static function get_all_positions_menu(array $firstoption = null, int $tenantid = 0): array {
        global $DB;
    ...
    /**
     * Returns list of manually assigned managers.
     *
     * @param int $userid
     * @return array
     */
    public static function get_user_assigned_managers(int $userid = 0): array {
        global $DB, $USER;
        ...
        $sql = "SELECT u.*
                  FROM {user} u
                  JOIN {tool_organisation_manual_mgr} {$mmgr} ON {$mmgr}.managerid = u.id
                 WHERE {$mmgr}.userid = :{$useridparam}";
