...
class job_position extends select {
    ...
    public function convert_condition_values(array $values,
        ...
        if ($newcondition instanceof \tool_organisation\reportbuilder\local\filters\position) {
            ...
            return [
                $newconditionname.'_position' => $operator,
                $newconditionname.'_subpositions' => $withsubpositions,
            ];
        }

        // The datasource mapped this condition to something else, not "position", so
    ...
    public function setup_form(\MoodleQuickForm $mform) {
        ...
        $objs['limiter'] = $mform->createElement('advcheckbox', $this->name.'_op', null,
            get_string('withsubpositions', 'tool_organisation'), ['class' => 'js-filter-advcheckbox-op']);
