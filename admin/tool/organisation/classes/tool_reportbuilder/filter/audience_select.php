...
class audience_select extends filter_base {
    ...
    public function convert_condition_values(array $values, \core_reportbuilder\local\filters\base $newcondition): array {
        if ($newcondition instanceof \tool_organisation\reportbuilder\local\filters\audience_select) {
            $operator = $values["{$this->name}_op"] ?? self::OPERATOR_ALLUSERS;
            $reporting = $values["{$this->name}_reporting"] ?? null;
            $reportingdirect = $values["{$this->name}_reportingdirect"] ?? false;
            $glue = $values["{$this->name}_glue"] ?? 0;
            $dept = $values["{$this->name}_dept"] ?? null;
            $subdept = $values["{$this->name}_subdept"] ?? null;

            $newconditionname = $newcondition->get_filter_persistent()->get('uniqueidentifier');
            return [
                $newconditionname.'_operator' => $operator,
                $newconditionname.'_reporting' => $reporting,
                $newconditionname.'_reportingdirect' => $reportingdirect,
                $newconditionname.'_glue' => $glue,
                $newconditionname.'_dept' => $dept,
                $newconditionname.'_subdept' => $subdept,
            ];
        }
    ...
    public function setup_form(\MoodleQuickForm $mform) {
        ...
        $mform->hideIf("{$this->name}_reportingdirect", "{$this->name}_op", 'neq', self::OPERATOR_CUSTOM);
    ...
    public function get_sql_filter(?array $currentvalues) : array {
        ...
        $reportingdirect = $currentvalues["{$this->name}_reportingdirect"] ?? false;
        ...
        // Managed users.
        if ($reporting) {
            [$manageduserswhere, $managedusersparams] = $userwithjobs->get_managed_users_select($usertablealias,
                0, $time, $reportingdirect);
