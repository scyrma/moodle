...
class reporting {
    ...
    protected static function insert_direct_relations(int $tenantid): int {
        ...
        // User added as manually assigned manager should be returned.
        $sqlmgr = $DB->sql_concat("'/'", "{$mgr}.managerid", "'/'", "{$mgr}.userid");
        $datatoinsert[] = "SELECT {$mgr}.tenantid, {$mgr}.userid, {$mgr}.managerid,
               {$mgr}.permissions AS permissions, 1 AS depth, {$sqlmgr} AS path, 1 AS isdirect
        FROM {tool_organisation_manual_mgr} {$mgr} WHERE $tenantid = {$mgr}.tenantid";
