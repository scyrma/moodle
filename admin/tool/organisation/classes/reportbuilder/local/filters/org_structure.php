 ...
 */
class org_structure extends base {
    ...
    public function setup_form(MoodleQuickForm $mform): void {
        ...
        $mform->setDefault("{$this->name}_op", self::OPERATOR_CUSTOM);
    ...
    public function get_sql_filter(array $values) : array {
        $operator = (int) ($values["{$this->name}_op"] ?? self::OPERATOR_CUSTOM);

        if ($operator === self::OPERATOR_DIRECT_ONLY || $operator === self::OPERATOR_EVERYONE) {
            $manager = organisation::get_user_with_jobs();
            return $manager->get_managed_users_select('u', 0, 0, $operator === self::OPERATOR_DIRECT_ONLY);
        } else {
            // Custom selector.
            $wheres = $params = [];

            // This filter needs re-factoring to remove hard-coded value below, and also accept some actual SQL/params to filter.
            $usertablealias = 'u'; // TODO SP-422 replace hardcoded value.

            $department = (int) ($values["{$this->name}_department"] ?? 0);
            $subdepartments = !empty($values["{$this->name}_department_op"]);

            $position = (int) ($values["{$this->name}_position"] ?? 0);
            $subpositions = !empty($values["{$this->name}_position_op"]);
    ...
    public function applies_to_values(array $values): bool {
        ...
        return $operator !== self::OPERATOR_CUSTOM || ($department > 0 || $position > 0);
    }
