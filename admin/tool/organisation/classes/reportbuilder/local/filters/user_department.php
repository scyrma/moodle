 ...
 */
class user_department extends \core_reportbuilder\local\filters\base implements condition_with_mapping {
    ...
    public function setup_form(\MoodleQuickForm $mform): void {
        ...
        $options = organisation::get_all_departments_menu(
            [0 => get_string('anydepartment', 'tool_organisation')]);

        $objs[$this->name] = $mform->createElement('selectgroups', $this->name, null,
            $options, ['class' => 'js-filter-select-val']);
    ...
    public function get_importer_mapping(importer_base $importer, array &$values): void {
        if (!empty($values[$this->name])) {
            $values[$this->name] = $importer->get_mapping('tool_organisation_department',
