...
class jobs extends system_report {
    ...
    protected function initialise(): void {
        ...
        $depalias = $jobentity->get_table_alias('tool_organisation_department');
        $posalias = $jobentity->get_table_alias('tool_organisation_position');
        ...
        $this->add_base_fields("{$jobalias}.id, {$jobalias}.userid, {$jobalias}.departmentid, {$jobalias}.tenantid,
            ...
            {$depalias}.path as deppath, {$posalias}.path as pospath");
    ...
    private function add_actions(): void {
        ...
        $this->add_action((new action(
            ...
            [
                ...
                'data-reportselector' => '.jobslistreport-wrapper',
            ],
            false,
        ...
        $this->add_action((new action(
            ...
            ->add_callback(function(stdClass $row): bool {
                $department = new department(0, (object) ['id' => $row->departmentid, 'path' => $row->deppath]);
                $position = new position(0, (object) ['id' => $row->positionid, 'path' => $row->pospath]);
                $caneditlocked = permission::can_edit_if_department_is_locked($department) &&
                    permission::can_edit_if_position_is_locked($position);

                return permission::can_assign_job_to_user((int) $row->userid) &&
                    $this->is_user_in_job_tenant($row) &&
                    $caneditlocked;
            })
        );
