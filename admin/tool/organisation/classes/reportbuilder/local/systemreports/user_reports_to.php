...
class user_reports_to extends system_report {
    ...
    protected function initialise(): void {
        ...
        $managersquery[] = helper::get_user_direct_managers_sql(":{$currentuserparam}",
            "," .helper::GLOBAL_MANAGER. " AS {$relationalias}");
        $managerparams[$currentuserparam] = $usermanagedid;

        // Get direct department lead managers sql.
        $currentuserparam1 = database::generate_param_name();
        $managersquery[] = helper::get_user_direct_dptleads_sql(":{$currentuserparam1}",
            "," .helper::DEPARTMENT_MANAGER. " AS {$relationalias}");
        $managerparams[$currentuserparam1] = $usermanagedid;

        // Get direct manually assigned managers sql.
        $currentuserparam2 = database::generate_param_name();
        $managersquery[] = helper::get_user_manually_assigned_managers_sql(":{$currentuserparam2}",
            "," .helper::MANUALLY_ASSIGNED_MANAGER. " AS {$relationalias}");
        $managerparams[$currentuserparam2] = $usermanagedid;
        ...
        // Needed for actions.
        $this->add_base_fields("{$useralias}.id AS managerid, {$relationalias} AS relation");
    ...
    protected function add_columns(user $userentity): void {
        ...
        $this->add_column((new column(
            ...
            ->add_fields("{$useralias}.id as managerid")
    ...
    public function format_manager_permissions(?string $value, stdClass $row): string {
        global $OUTPUT;
        // Get the manager with jobs object.
        $managerjob = organisation::get_user_with_jobs((int)$row->managerid, null);
        $permissionsout = '';
        $permissions = [];

        // If the manager is not manually assigned, we add the permissions of the relevant position.
        $relevantjobs = $managerjob->get_relevant_manager_jobs($this->usermanagedjobs);
        if ($relevantjobs && in_array($this->relationbasefield, [helper::GLOBAL_MANAGER, helper::DEPARTMENT_MANAGER])) {
            // Set the filter method for each case to get the relevant job.
            $filtermethod = ($this->relationbasefield == helper::GLOBAL_MANAGER)
            ...
            $permissions = reset($relevantjob)->get_position()->get_node_roles_permissions();
        } else if ($this->relationbasefield == helper::MANUALLY_ASSIGNED_MANAGER) {
            // If the manager is manually assigned, we add the manually assigned permissions.
            $detailedpermissions = [];
            ...
            foreach ($globalpermissions as $index => $globalpermission) {
                $detailedpermissions[] = [
                    ...
                    'set' => $managerjob->is_manually_assigned_manager($index, (int)$this->usermanagedjobs->get('id')),
                ];
            }
            $permissions[] = [
        ...
        array_walk($permissions, function(&$roleperm) {
