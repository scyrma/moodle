...
class tool_organisation_external extends external_api {
        ...
        return new external_function_parameters([
            'departments' => new external_multiple_structure(
                new external_single_structure([
                    ...
                    'tenant' => new external_value(PARAM_RAW, 'Tenant idnumber where the department is created', VALUE_DEFAULT, ''),
    ...
    public static function create_departments($departments) {
        ...
        foreach ($params['departments'] as $d) {
            // If tenant is specified, check if it exists and accessible and switch to this tenant.
            if (strlen($d['tenant'] ?? '')) {
                try {
                    $tenant = $tenantmanager->get_by_idnumber($d['tenant'], MUST_EXIST);
                    \tool_tenant\permission::require_can_access_tenant($tenant->get('id'));
                } catch (Exception $e) {
                    $item = !empty($p['idnumber']) ? $d['idnumber'] : $d['name'];
                    $warnings[] = [
                        'item' => $item,
            ...
            // If parent is specified, find it in the current tenant.
            $parent = null;
            if (!empty($d['parent'])) {
                $parent = \tool_organisation\department::get_by_idnumber($d['parent'], 0, false);
                if ($parent) {
            ...
            // Verify idnumber uniqueness.
            if (!\tool_organisation\department::is_idnumber_unique(0, $d['idnumber'] ?? null)) {
                $warnings[] = [
                    'item' => $item,
                    'warningcode' => 'errorsameidnumberdepartment',
                    'message' => get_string('errorsameidnumberdepartment', 'tool_organisation'),
                ];
                continue;
            }
            ...
            'positions' => new external_multiple_structure(
                new external_single_structure([
                    ...
                    'parent' => new external_value(PARAM_RAW,
                        ...
                        VALUE_DEFAULT, null),
                    'idnumber' => new external_value(PARAM_RAW, 'IDNUMBER', VALUE_OPTIONAL),
                    'tenant' => new external_value(PARAM_RAW, 'Tenant idnumber where the position is created', VALUE_DEFAULT, ''),
    ...
    public static function create_positions($positions) {
        ...
        foreach ($params['positions'] as $p) {
            // If tenant is specified, find it and check it is accessible, switch to this tenant.
            if (strlen($p['tenant'] ?? '')) {
                try {
                    $tenant = $tenantmanager->get_by_idnumber($p['tenant'], MUST_EXIST);
                    \tool_tenant\permission::require_can_access_tenant($tenant->get('id'));
                } catch (Exception $e) {
            ...
            $parent = null;
            if (!empty($p['parent'])) {
                $parent = \tool_organisation\position::get_by_idnumber($p['parent'], 0, false);
                if ($parent) {
            ...
            // Check that idnumber is unique.
            if (!\tool_organisation\position::is_idnumber_unique(0, $p['idnumber'] ?? null)) {
                $warnings[] = [
                    'item' => $item,
                    'warningcode' => 'errorsameidnumberposition',
                    'message' => get_string('errorsameidnumberposition', 'tool_organisation'),
                ];
                continue;
            }
    ...
    public static function sum_permissions($permissions) : int {
        $sum = 0;
        if (!empty($permissions['allocateprograms'])) {
            $sum += \tool_organisation\organisation::PERM_ALLOCATE_PROGRAMS;
        }
        if (!empty($permissions['viewreports'])) {
            $sum += \tool_organisation\organisation::PERM_VIEW_REPORTS;
        }
        if (!empty($permissions['receivenotifications'])) {
     ...
     *
     * @deprecated since 4.1, replaced with block_myteams_get_team_overview
     *
    ...
    public static function get_managed_users(int $departmentid = 0, int $includesubdepts = 0, int $positionid = 0,
        ...
        [$where, $params] = $manager->get_managed_users_select();
        ...
        if (!empty($fullnametype)) {
            ...
            [$field, $paramsfullname] = fields::get_sql_fullname('u', $canviewfullnames);
    ...
    /**
     * Mark get_managed_users as deprecated.
     *
     * @return bool
     */
    public static function get_managed_users_is_deprecated(): bool {
        return true;
    }
     ...
     *
     * @deprecated since 4.1, replaced with block_myteams_get_team_overview_filters_options
     *
     * @return array
     */
    ...
    /**
     * Mark get_teams_tab_filters as deprecated.
     *
     * @return bool
     */
    public static function get_teams_tab_filters_is_deprecated(): bool {
        return true;
    }
