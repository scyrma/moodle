...
class add_department_form extends dynamic_form {
    ...
    public function definition() {
        ...
        // Add locked checkbox if user has permission to create/edit locked departments.
        if ($this->is_department_framework_form() && permission::has_assign_in_locked_capability()) {
            $mform->addElement('advcheckbox', 'locked', get_string('locked', 'tool_organisation'));
            $mform->addHelpButton('locked', 'locked', 'tool_organisation');
        }
        ...
        if ($department || $parent) {
            $frameworkid = $department ? $department->get_framework_id() : $parent->get_framework_id();
        }
        if (!$this->is_department_framework_form()) {
            ...
            $options = [
                ...
                'multiple' => false,
                'data-departmentid' => $department ? $department->get('id') : 0,
                'data-frameworkid' => $frameworkid,
                'valuehtmlcallback' => function ($value) {
                    $department = (new department_manager())->get_department($value);
                    return $department->is_framework() ? get_string('top') : $department->get_formatted_name();
                },
            ];
    ...
    public function validation($data, $files): array {
        ...
        if (!department::is_idnumber_unique($data->id, $data->idnumber)) {
            $errors['idnumber'] = get_string('departmentidnumberconflict', 'tool_organisation');
        }
