...
abstract class hierarchy_manager {
     ...
     * @return array
     */
    public function get_potential_parents(string $search, hierarchy $entity): array {
        global $DB;

        if ($entity->is_framework()) {
            return [];
        }

        $params = ['entityid' => $entity->get('id')];

        $frameworkid = $entity->get_framework_id();
        ...
        foreach ($results as $result) {
            // Format name.
            $result->name = format_string($result->name, true, $formatparams);
            // Format path.
