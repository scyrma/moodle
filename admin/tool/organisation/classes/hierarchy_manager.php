...
abstract class hierarchy_manager {
     ...
     */
    protected function get_hierarchy_entity(array $conditions) : hierarchy {
        global $DB;
        $class = $this->get_class();
    ...
    protected function get_hierarchy_entities(array $conditions, string $sortorder = 'sortorder, id') : array {
        ...
        $class = $this->get_class();
        $conditions['tenantid'] = tenancy::get_tenant_id();
        $records = $DB->get_records($this->get_table(), $conditions, $sortorder);
        $entities = [];
     ...
     * @return hierarchy[]
     */
    protected function get_all_children(hierarchy $entity, bool $includearchived = false) : array {
        global $DB;
        $class = $this->get_class();
    ...
    protected function get_hierarchy_structure(int $frameworkid, bool $includearchived = false) : hierarchy {
        $framework = $this->get_hierarchy_entity(['id' => $frameworkid] + ($includearchived ? [] : ['archived' => 0]));
        ...
        do {
            ...
            foreach ($children as $id => $child) {
                if (array_key_exists($child->get('parentid'), $entities)) {
                    $entities[$id] =
                        $entities[$child->get('parentid')]->add_child($child);
                    unset($children[$id]);
                    $found = true;
                }
            }
        ...
        if ($children) {
            debugging('Orphaned records found in the hierarchy structure, we will attempt to fix them',
                DEBUG_DEVELOPER);
     ...
     * @throws \coding_exception
     */
    protected function create_hierarchy(hierarchy $parent, \stdClass $data) : hierarchy {
        global $DB;
        if (isset($data->parentid) && ($parent->get('id') != $data->parentid)) {
            throw new \coding_exception('Parent argument does not match');
        }
        $data->tenantid = $parent->get('tenantid') ?: tenancy::get_tenant_id();
        if ($parent->get('id')) {
     ...
     */
    protected function fix_hierarchy_paths() {
        $processed = [];
        $needfixing = false;
     ...
     * @return string
     */
    public abstract static function get_description_filearea() : string;
