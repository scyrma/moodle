...
class add_position_form extends dynamic_form {
    ...
    public function definition() {
        ...
        // Add locked checkbox if user has permission to create/edit locked positions.
        if ($this->is_position_framework_form() && permission::has_assign_in_locked_capability()) {
        ...
        if ($this->is_position_framework_form()) {
            ...
            $mform->addElement('advcheckbox', 'globalmanager', '',
                ...
                ['class' => "permission-header global"], [0, 1]);
            ...
            $mform->addElement('advcheckbox', 'departmentmanager', '',
                ...
                ['class' => "permission-header department"], [0, 1]);
            ...
            foreach ($departmentpermissions as $index => $departmentpermission) {
                ...
                $mform->addElement('advcheckbox', "deptmgrpermission[$i]", '', $title ,
                    ['class' => "permission department"], [0, $index]);
    ...
    public function validation($data, $files): array {
        ...
        if (!position::is_idnumber_unique($data->id, $data->idnumber)) {
            $errors['idnumber'] = get_string('positionidnumberconflict', 'tool_organisation');
        }
