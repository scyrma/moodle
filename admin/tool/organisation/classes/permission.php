...
class permission {
    ...
    /**
     * Can edit if position is locked
     *
     * @param position $position
     * @return bool
     */
    public static function can_edit_if_position_is_locked(position $position): bool {
        $frameworkid = $position->get_framework_id();
        $framework = new position($frameworkid);

        return !($framework->get('locked') && !self::has_assign_in_locked_capability());
    }

    /**
     * Can edit if department is locked
     *
     * @param department $department
     * @return bool
     */
    public static function can_edit_if_department_is_locked(department $department): bool {
        $frameworkid = $department->get_framework_id();
        $framework = new department($frameworkid);
    ...
    public static function can_edit_position(position $position): bool {
        return self::has_manage_positions_capability() &&
            ...
            self::can_edit_if_position_is_locked($position);
    }
    ...
    /**
     * has_assignmanuallymgr_capability
     *
     * @return bool
     */
    public static function has_assignmanuallymgr_capability(): bool {
        return has_capability('tool/organisation:assignmanuallymgr', \context_system::instance());
    }

    /**
     * has_assign_in_locked_capability
     *
     * @return bool
     */
    public static function has_assign_in_locked_capability(): bool {
        return has_capability('tool/organisation:assigninlocked', \context_system::instance());
    }
    ...
    public static function can_edit_department(department $department): bool {
        return self::has_manage_departments_capability() &&
            ...
            self::can_edit_if_department_is_locked($department);
    }
    ...
    /**
     * Check if a user can assign jobs to another user, also checks if the user is from the same tenant.
     *
     * @param int $userid
     * @return bool
     */
    public static function can_assign_jobs_to_user(int $userid): bool {
        return self::has_assign_jobs_capability() && tenancy::get_actual_tenant_id($userid) === tenancy::get_tenant_id();
    }

    /**
     * Check if a user can assign manually assigned manager to another user,
     * also checks if the user is from the same tenant.
     *
     * @param int $userid
     * @return bool
     */
    public static function can_assign_mam_to_user(int $userid): bool {
        return self::has_assignmanuallymgr_capability() && tenancy::get_actual_tenant_id($userid) === tenancy::get_tenant_id();
    }
    ...
    public static function can_edit_job(job $job): bool {
        ...
        $caneditlocked = self::can_edit_if_department_is_locked($department) &&
            self::can_edit_if_position_is_locked($position);
    ...
    public static function can_view_jobs(): bool {
        ...
        return self::has_assign_jobs_capability() || self::has_assignmanuallymgr_capability();
    }
    ...
    public static function can_view_user_jobs(stdClass $user): bool {
        debugging('tool_organisation\permission::can_view_user_jobs() is deprecated, use ' .
            'tool_organisation\permission::can_view_user_org_profile() instead.', DEBUG_DEVELOPER);
        return self::can_view_user_org_profile($user->id);
    }

    /**
     * Can view jobs, managers and staff of another user
     *
     * this is checked
     * - on user profile page when displaying "jobs" widget
     * - on admin/tool/organisation/user.php page for this user
     * - in all reports used on the user.php page
     * - every time when we display links to this page
     *
     * @param int $userid
     * @return bool
     */
    public static function can_view_user_org_profile(int $userid): bool {
        global $USER;

        return ($userid == $USER->id) ||
            ((self::has_assign_jobs_capability() || self::has_assignmanuallymgr_capability()) &&
                \tool_tenant\permission::can_access_tenant(tenancy::get_tenant_id($userid))) ||
            (($userorg = \tool_organisation\organisation::get_user_with_jobs()) &&
                $userorg->is_manager_over_user($userid));
    }
    ...
    /**
     * require_can_view_jobs
     *
     * @throws \moodle_exception
     */
    public static function require_can_view_jobs(): void {
        if (!self::can_view_jobs()) {
    ...
    /**
     * Check if a user can view jobs/managers/subordinates of another user, allow the following cases:
     * 1 - User can view his own jobs
     * 2 - User can view jobs of another user if he has the capability to assign jobs/assign manually assigned managers
     *     and the other user is in the same tenant
     * 3 - User can view jobs of another user if he is a manager of the other user
     *
     * @param int $userid
     *
     * @throws \moodle_exception
     */
    public static function require_can_view_user_org_profile(int $userid): void {
        if (!self::can_view_user_org_profile($userid)) {
            throw new \required_capability_exception(\context_system::instance(),
                    'tool/organisation:assignjobs', 'nopermissions', '');
        }
    }
    ...
    /**
     * require_can_assign_manually_assigned_manager
     *
     * @throws \moodle_exception
     */
    public static function require_can_assign_manually_assigned_manager(): void {
        if (!self::has_assignmanuallymgr_capability()) {
            throw new \required_capability_exception(\context_system::instance(),
                'tool/organisation:assignmanuallymgr', 'nopermissions', '');
        }
    }
