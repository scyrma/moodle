...
class tool_organisation_observer {
    ...
    public static function tenant_user_updated(\tool_tenant\event\tenant_user_updated $event) {
        // Move user jobs that use a shared department AND a shared position to the new tenant.
        job_manager::move_shared_jobs_to_new_tenant($event->relateduserid, $event->other['oldtenantid'], $event->other['tenantid']);
    }
}
