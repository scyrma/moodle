...
class departments_csv extends importer_base {
    ...
    protected function initialise() {
        ...
        $this->register_potential_notice(self::CSV_DATA, 'parentidchanged',
            [
                self::NOTICE_LOG => static function(array $details, array $noticedetails) {
                    return get_string('parentidchanged', 'tool_organisation', s($noticedetails['parentid']));
                }
            ]);
    ...
    /**
     * Returns parentid for department creation
     *
     * @param array $record
     * @param array $map
     * @param int $frmid
    ...
    protected function get_parent_id(array $record, array $map, int $frmid): int {
        if (trim($record['parentid'] ?? '') !== '') {
            ...
            if (array_key_exists($record['parentid'], $map)) {
                ...
                // more than one department with the same idnumber, department framework ID will be used as parentid.
                $departments = department::get_records(
                    ['tenantid' => $this->get_import_tenant_id(), 'idnumber' => $record['parentid']]);
                // We only want departments from same framework.
                $departments = array_filter($departments, function ($dep) use ($frmid) {
                    return organisation_helper::get_framework_id_from_path($dep->get('path')) === $frmid;
                });
                if (count($departments) === 1 ) {
                    return (int) reset($departments)->get('id');
                }
