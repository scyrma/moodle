    ...
    public function add_to_options_form(import_settings_form $form): void {
        ...
        $this->add_csv_mapping_element($form,
            [
                ...
                'globalmanager' => get_string('globalmanager', 'tool_organisation'),
                'departmentmanager' => get_string('departmentmanager', 'tool_organisation'),
            ],
        ...
        foreach ($this->get_csv_reader()->get_rows() as $csvimportedentity) {
            $csvimportedentity
                ...
                ->set_import_callback(function(array $record, csv_imported_entity $caller)
                    ...
                    // Normalise the 'manager' fields and add permissions (assume all).
                    if (!empty($record['departmentmanager'])) {
                        $record['departmentmanager'] = 1;
                        $record['departmentpermissions'] = organisation::PERM_ALLOCATE_PROGRAMS
                            + organisation::PERM_RECEIVE_NOTIFICATIONS + organisation::PERM_VIEW_REPORTS;
                    } else {
                        $record['departmentmanager'] = 0;
                        $record['departmentpermissions'] = 0;
                    }
                    if (!empty($record['globalmanager'])) {
                        $record['globalmanager'] = 1;
                        $record['globalpermissions'] = organisation::PERM_ALLOCATE_PROGRAMS
                            + organisation::PERM_RECEIVE_NOTIFICATIONS + organisation::PERM_VIEW_REPORTS;
                    } else {
