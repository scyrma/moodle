...
class positions_csv extends importer_base {
    ...
    private function idnumber_taken(string $idnumber): bool {
        return !position::is_idnumber_unique(0, $idnumber, $this->get_import_tenant_id());
    }
    ...
    protected function get_parent_id(array $record, array $map, int $frmid): int {
        if (trim($record['parentid'] ?? '') !== '') {
            ...
            if (array_key_exists($record['parentid'], $map)) {
                ...
                // Try to find the parent in the same framework, if not found, use the framework id and show a notice.
                $position = position::get_by_idnumber_in_framework($record['parentid'],
                    $frmid, $this->get_import_tenant_id());
                if ($position) {
