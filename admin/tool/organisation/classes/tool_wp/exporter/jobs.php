...
class jobs extends \tool_wp\exporter_base {
    ...
    public function is_available(): bool {
        return !strlen($this->entrypoint) && permission::has_assign_jobs_capability();
    }
    ...
    protected function get_jobs(): array {
        ...
        $params += $paramsuser + ['tenantid' => $this->get_export_tenant_id()];
    ...
    public function perform_export(): void {
        ...
        if ($this->get_export_setting(self::EXPORT_FRAMEWORKS) && orgstructure::can_export_departments()
            ...
            $this->process_chained_entities(orgstructure::NAME_DEPARTMENT_FRAMEWORK, array_keys($depframeworks));
            $this->process_chained_entities(orgstructure::NAME_POSITION_FRAMEWORK, array_keys($posframeworks));
        }
    ...
    protected function get_all_position_frameworks(): array {
        ...
        if ($this->positionframeworks === null) {
            $this->positionframeworks = $DB->get_records_menu(position::TABLE,
                ['tenantid' => $this->get_export_tenant_id(), 'parentid' => null], '', 'id, name');
        }
