...
/**
 * Class tool_organisation_department
 *
...
/**
 * Mapper class for exporting/importing departments
 *
 * @package     tool_organisation
 * @copyright   2020 Moodle Pty Ltd <support@moodle.com>
 * @author      2020 Marina Glancy
 * @license     Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class tool_organisation_department extends export_import_mapper_base {
    ...
    protected function initialise(): void {
        ...
        $this->register_potential_error(self::NOTFOUND, [
            self::ERROR_CONFLICTHEADER => get_string('somedepartmentsdonotexist', 'tool_organisation'),
            self::ERROR_LOG => function(array $identifier) {
                return get_string('mappingerrordeptnotfound', 'tool_organisation',
        ...
        $this->register_potential_notice('namemapping', [
            self::NOTICE_LOG => get_string('mappingnoticenodeptidnumber', 'tool_organisation'),
        ]);

        $this->register_potential_notice('departmentcreated', [
            self::NOTICE_LOG => function(array $identifier, array $additionaldata) {
                return "Department '".s($identifier['name'])."' was created"; // TODO string, link.
            },
        ]);
    }
    ...
    public function get_mapping_data_for_workplace_export(int $id): ?array {
        global $DB;
        $obj = $DB->get_record('tool_organisation_department', ['id' => $id], 'id, idnumber, name');
     ...
     * @param int|null $tenantid strictly inside the given tenant (for entities that can be inside tenants)
    ...
    public function locate_mapping(array $identifier): ?int {
        ...
        if (!empty($identifier['idnumber'])) {
            $id = $DB->get_field('tool_organisation_department',
    ...
    /**
     * When conflict resolution settings allow and department does not exist, create a new one
     *
    ...
    protected function create_missing_if_needed(array $identifier): ?int {
        ...
        $frmexist = $DB->record_exists('tool_organisation_department', ['tenantid' => $identifier['tenantid'],
        ...
        if ($this->can_fully_apply_conflict_resolutions()) {
            ...
            $department = (new department_manager())->create_department((object)$data, false);
            $this->log_mapping_notice('departmentcreated', $identifier);
    ...
    public function add_to_conflict_form(import_conflict_form $form, array $importedentities,
        ...
        $frms = $DB->get_records_menu(department::TABLE,
        ...
        if ($frms) {
            ...
            $mform->addElement('select', $frmid, get_string('departmentframeworks', 'tool_organisation'),
