...
class user_with_jobs extends exporter {
    ...
    /**
     * Checks if user has a manager job or manually assigned manager.
     *
    ...
    public function is_manager(int $withpermissions = 0): bool {
        ...
        return !empty($jobs) || self::is_manually_assigned_manager($withpermissions);
    }
    ...
    /**
     * Checks if this user is a manually assigned manager
     *
     * @param int $withpermissions
     * @param int $userid
     * @return bool
     */
    public function is_manually_assigned_manager(int $withpermissions = 0, int $userid = 0): bool {
        global $DB;
        // If the user is not manually assigned as a manager, return false.
        if (empty($this->related['ismanuallyassignedmgr'])) {
            return false;
        }

        $params = ['managerid' => self::get_full_user_record()->id];
        ...
        $params['withmamperm'] = $withpermissions;
        $mamperm = $DB->sql_bitand('permissions', $withpermissions);
        $records = user_manager::count_records_select("{$select} AND {$mamperm} = :withmamperm", $params);
     ...
     * @param int $withpermissions
     * @param int $time this parameter is no longer used, since now is not possible to retrieve a tree at the past/future date
     ...
     */
    public function get_managed_users_select(string $usertablealias = 'u', int $withpermissions = 0, int $time = 0,
                                             bool $directonly = false): array {
        return reporting::get_managed_users_select($this->get('id'), $usertablealias, $withpermissions, $directonly);
    }
