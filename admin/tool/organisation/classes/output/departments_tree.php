...
class departments_tree extends table_tree {
     ...
     * @param department[] $toplevelnodes
     */
    ...
    protected function get_node_children($node = null) {
        ...
        } else if ($this->frameworkid && ($node instanceof department)) {
    ...
    protected function get_node_attributes($node = null): array {
        $attributes = [];
        $attributes['data-department-id'] = $node ? $node->get('id') : $this->frameworkid;
        return $attributes;
    }
    ...
    protected function export_cell(renderer_base $output, string $columnname, $node = null): string {
        ...
        if ($columnname === 'name') {
            ...
            if ($jobscount = \tool_organisation\job::get_department_jobs_count($node->get('id')) ) {
    ...
    protected function get_tree_attributes(): array {
        return ['data-framework-id' => $this->frameworkid, 'class' => 'tool-organisation-departments'];
    }
