...
class tab_departments extends tab {
    ...
    public function get_tab_label(): string {
        return get_string('departments', 'tool_organisation');
    }
    ...
    public function export_for_template(renderer_base $output) {
        ...
        foreach ($frameworks as $f) {
            ...
            $editablename = $this->get_department_editablename($f, $isshared, $output);
            $canedit = permission::can_edit_department($f);
            $editinsharedspace = '';
            if ($isshared && permission::can_edit_department_in_its_tenant($department)) {
                $editinsharedspace = helper::add_shared_space_button($f->get('tenantid'), $output, 'departments');
            }
            ...
            $disabledragdrop = !$canedit || $manager->too_many_children_disable_sorting($department);
            $frameworkinfo = [
                'moveicon' => $canedit ?
                    $output->render_from_template('core/drag_handle', ['movetitle' => $strmove]) : '',
                ...
                'candelete' => empty($department->get_jobs_count($frameworkid)->totalwithchildren) && $canedit,
                'editinsharedspace' => $editinsharedspace,
                'canedit' => $canedit,
                'locked' => $f->get('locked') && !permission::has_assign_in_locked_capability(),
            ];
    ...
    /**
     * Returns rendered department editable name
     *
     * @param department $f
     * @param bool $isshared
     * @param renderer_base $output
     ...
     */
    protected function get_department_editablename(department $f, bool $isshared, renderer_base $output): string {
        $shared = '';
        if ($isshared) {
            // Add 'Shared space' badge to the name.
            $shared = \html_writer::span(get_string('sharedspace', 'tool_tenant'),
                    'badge badge-secondary ml-1');
        }

        if (!empty($shared) || ($f->get('locked') && !permission::has_assign_in_locked_capability())) {
            return $f->get_formatted_name() . $shared;
        }

        // Get the Department inplace editable name.
