...
class job extends persistent_exporter {
    ...
    protected function get_other_values(\renderer_base $output) {
        ...
        $tenantid = \tool_tenant\tenancy::get_actual_tenant_id($this->get('userid'));
        return [
            ...
            'inusertenant' => (int) $this->get('tenantid') === $tenantid,
        ];
    }

    /**
     * Job is active at the given time
     *
     * @param int $time Timestamp (0 for current time)
     * @return bool
     */
    public function is_active(int $time = 0): bool {
        $time = helper::round_time($time ?: time());
        return $this->get('startdate') <= $time && (!$this->get('enddate') || $this->get('enddate') >= $time);
    }
}
