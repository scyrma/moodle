...
class add_jobassign_form extends dynamic_form {
    ...
    public function definition() {
        ...
        if (!in_array($action, ['addjobs', 'transferalltojob', 'setjobsfinished'])) {
        ...
        if ($action !== 'editjobdates' && $action !== 'setjobfinished' && $action !== 'setjobsfinished') {
            // Position.
        ...
        if ($action === 'setjobfinished' || $action === 'setjobsfinished') {
            ...
            if ($action === 'transfertojob' || $action === 'transferalltojob') {
                $previousjobnotestring = match ($action) {
                    'transfertojob' => get_string('previousjobdatenote', 'tool_organisation'),
                    'transferalltojob' => get_string('previousjobsdatenote', 'tool_organisation'),
                };
    ...
    public function definition_after_data() {
        $mform = $this->_form;
        $action = $mform->getElementValue('action');
        if (in_array($action, ['addjobs', 'transferalltojob', 'setjobsfinished'])) {
            return;
        }
    ...
    public function process_dynamic_submission() {
        ...
        $id = $data->id;
        if (!$id) {
            $users = !empty($data->userid)
                ? [$data->userid]
                : (!empty($data->users)
                    ? $data->users
                    : (!empty($data->userids)
                        ? explode(',', $data->userids)
                        : []
                    )
                );
            if ($data->action === 'setjobsfinished') {
                foreach ($users as $userid) {
                    if (!permission::can_assign_job_to_user($userid)) {
                        continue;
                    }
                    $userjobs = (new job_manager())->get_user_jobs($userid);
                    foreach ($userjobs as $userjob) {
                        // If the job is already finished, do nothing.
                        if (($userjob->get('enddate') && $userjob->get('enddate') <= helper::round_time(time()))) {
                            continue;
                        }
                        $id = $userjob->get('id');
                        $prepareddata = $this->prepare_data_for_storing($data, $id);
                        $prepareddata->startdate = $userjob->get('startdate');
                        $manager->update_job($id, $prepareddata);
                    }
                }
            } else if ($data->action === 'transferalltojob') {
                foreach ($users as $userid) {
                    if (!permission::can_assign_job_to_user($userid)) {
                        continue;
                    }
                    $userjobs = (new job_manager())->get_user_jobs($userid);
                    foreach ($userjobs as $userjob) {
                        // TODO: Add a note from the mockup.
                        $id = $userjob->get('id');
                        $prepareddata = $this->prepare_data_for_storing($data, $id);
                        $prepareddata->enddate = strtotime('-1 day', $data->startdate);
                        $prepareddata->startdate = $userjob->get('startdate');
                        $manager->update_job($id, $prepareddata);
                    }
                    $data->userid = $userid;
    ...
    public function set_data_for_dynamic_submission(): void {
        ...
        if ($job = $this->get_job()) {
            ...
            // Create new job form with a tenantid and users.
    ...
    public function validation($data, $files) {
        ...
        if ($action === 'setjobsfinished' || $action === 'transferalltojob') {
            $validatedfield = match ($action) {
                'setjobsfinished' => 'enddate',
                'transferalltojob' => 'startdate',
            };
            $users = !empty($data['userid'])
                ? [$data['userid']]
                : (!empty($data['users'])
            ...
            foreach ($users as $userid) {
                $userjobs = (new job_manager())->get_user_jobs($userid);
                foreach ($userjobs as $userjob) {
                    if ($userjob->get('startdate') >= $data[$validatedfield]) {
                        $fullname = fullname(core_user::get_user($userid));
                        $errors[$validatedfield] = get_string('validationmsgedateonenddatechange', 'tool_organisation', $fullname);
                        return $errors;
                    }
                }
            }
