    ...
    public static function process_new_user($user, $filecolumns, $upt) {
        foreach ($filecolumns as $column) {
            if (preg_match('/^jobposition(\d+)$/', $column, $matches)) {
                ...
                if (!$position || !permission::is_entity_visible_in_list($position)) {
                ...
                if (!$department || !permission::is_entity_visible_in_list($position)) {
                ...
                if ($job = job::get_record($params)) {
                    ...
                    if (strlen($user->{'jobenddate'.$i})) {
                        if ('' . $user->{'jobenddate'.$i} === '0') {
                            // Clear the end date.
                            $job->set('enddate', 0);
                        } else if (!self::validate_date($user->{'jobenddate'.$i})) {
                    ...
                    // TODO this should use job_manager::update() function but it does not work for another tenant.
                    $job->save();
                } else {
                    if (empty($user->{'jobstartdate'.$i})) {
