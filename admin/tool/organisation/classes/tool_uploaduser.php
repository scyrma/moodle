    ...
    public static function process_new_user($user, $filecolumns, $upt) {
        ...
        foreach ($filecolumns as $column) {
            if (preg_match('/^jobposition(\d+)$/', $column, $matches)) {
                ...
                $position = position::get_by_idnumber($user->{'jobposition'.$i}, $usertenantid);
                ...
                $department = department::get_by_idnumber($user->{'jobdepartment'.$i}, $usertenantid);
                ...
                // Delete all jobs for this user/department/position if jobdelete field is set to 1.
                if (!empty($user->{'jobdelete' . $i}) && (int)($user->{'jobdelete' . $i}) === 1
                    && $jobs = job::get_records($params)) {
                    foreach ($jobs as $job) {
                        if (permission::can_edit_job($job)) {
                            $jobmanager->delete_job($job->get('id'));
                        }
                    }
                    continue;
                }
