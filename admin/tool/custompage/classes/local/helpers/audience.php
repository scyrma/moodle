...
class audience {
    ...
    /**
     * Transform list of audience model records into corresponding audience instances
     *
     * @param model[] $audiences
     * @return base[]
     */
    public static function get_audiences_instances(array $audiences): array {
    ...
    public static function get_audiences_for_page(int $pageid): array {
        $records = model::get_records(['pageid' => $pageid], 'id');

        $audienceinstances = [];

        $showormessage = false;
        foreach (self::get_audiences_instances($records) as $instance) {
    ...
    public static function user_audience_sql(array $audiences, string $usertablealias = 'u'): array {
        ...
        foreach (self::get_audiences_instances($audiences) as $instance) {
            $instancetablealias = database::generate_alias();
            [$instancejoin, $instancewhere, $instanceparams] = $instance->get_sql($instancetablealias);

            if (empty($instancejoin)) {
                // If there are no joins, there is no need to create a complicated subquery,
                // we can apply "where" directly to the users table.
                [$instancejoin, $instancewhere, $instanceparams] = $instance->get_sql($usertablealias);
    ...
    public static function get_allowed_pages(?int $userid = null): array {
        ...
        // Use the userid and their current tenant as the cache key, some users may be able to switch tenants.
        $cachekey = $userid . '_' . tenancy::get_tenant_id($userid);
        $userallowedpages = $userallowedpagescache->get($cachekey);
        ...
        // Retrieve all audiences to determine potential matches for the given user.
        $audiences = model::get_records();
        if (isguestuser($userid) || empty($userid)) {
            // If the user is not authenticated, check if the audiences allow non-authenticated users.
            foreach ($audiences as $audience) {
                $instance = base::instance(0, $audience->to_record());
                if ($instance && $instance->allow_non_authenticated_users_access()) {
                    $allowedpages[] = $audience->get('pageid');
                }
            }
        } else {
            // Group our audiences by page for convenience.
            foreach ($audiences as $audience) {
            ...
            foreach ($pageaudiences as $pageid => $audiences) {
                ...
                // Ensure the given audience records produced some "where" clauses. If each was invalid and/or corrupted then no
                // such data is returned, and we shouldn't match any users at all.
                if (count($wheres) > 0) {
                    $userselect = implode(' OR ', $wheres);
        ...
        $userallowedpagescache->set($cachekey, $allowedpages);
