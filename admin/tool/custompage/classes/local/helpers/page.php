...
use moodle_url;
use stdClass;
use tool_custompage\local\models\{audience as audience_model, page as model};
...
class page {
    ...
    public static function create_page(stdClass $data): model {
        ...
        // Hack alert! Account for default global page creation during installation, before the tenant tool has been installed,
        // in which case the "tenantid" property was already set to zero (which is fine for this page).
        if (!isset($data->tenantid) ||
                (get_config('tool_tenant', 'version') && !\tool_tenant\permission::can_access_tenant((int) $data->tenantid))) {
    ...
    public static function duplicate_page(model $page, ?bool $global = null): model {
        ...
        if ($global === null) {
            $audiences = audience_model::get_records(['pageid' => $page->get('id')]);
            foreach (audience::get_audiences_instances($audiences) as $instance) {
                ...
                $audiencecopy = $instance->get_persistent()->to_record();
                unset($audiencecopy->id, $audiencecopy->usercreated);

                (new audience_model(0, $audiencecopy))
    ...
    private static function validate_block_configuration(block_base $blockinstance): ?object {
        ...
        $duplicatepage->set_pagetype('admin-tool-custompage');
        ...
        $customdata = ['page' => $duplicatepage, 'block' => $blockinstance];
        $mform = new $classname($duplicatepage->url->out(false), $customdata);
