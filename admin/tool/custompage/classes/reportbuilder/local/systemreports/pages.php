...
use tool_custompage\local\models\{audience, page};
...
class pages extends system_report {
    ...
    public function initialise(): void {
        ...
        if (permission::can_create_global_page()) {
            ...
            [$pageaudienceselect, $pageaudienceparams] = audience_helper::user_pages_list_sql('p');
            $pageselect = "(
    ...
    public function add_columns(): void {
        ...
        $this->add_column((new column(
            ...
            ->add_callback(function(string $value, stdClass $page): string {
                ...
                if ($page->global) {
                    $result .= ' ' . html_writer::span(get_string('globalpage', 'tool_custompage'),
        ...
        // Page audience summary.
        $this->add_column((new column(
            'audience',
            new lang_string('audience', 'tool_custompage'),
            self::PAGE_ENTITY_NAME
        ))
            ->set_type(column::TYPE_TEXT)
            // We select group concatenated list of audience IDs per page.
            ->add_field("(
                SELECT " . $DB->sql_group_concat('a.id', ',') . "
                  FROM {" . audience::TABLE . "} a
                 WHERE a.pageid = {$tablealias}.id
                )", 'audiences')
            ->set_is_sortable(false)
            ->add_callback(static function(?string $audiences): string {
                global $DB;
                ...
                // Now grab those audience records from group concatenated list.
                [$select, $params] = $DB->get_in_or_equal(explode(',', $audiences));
                $records = audience::get_records_select("id {$select}", $params, 'id');

                // Map audience instances to a list of their name/description.
                $list = array_map(static function(base $instance): string {
                    $title = $instance->get_name();
                    if ($instance->user_can_edit()) {
                        $title .= " ({$instance->get_description()})";
                    }
                    return $title;
                }, audience_helper::get_audiences_instances($records));

                return html_writer::alist($list, ['class' => 'fa-ul m-auto']);
            })
        );
    ...
    public function add_filters(): void {
        ...
        $this->add_filter((new filter(
            ...
            ->set_is_available(permission::can_create_global_page() && tenancy::is_site_multi_tenant())
        );
    ...
    protected function add_actions() {
        ...
        $this->add_action((new action(
            ...
            ->add_callback(static function(stdClass $data): bool {
                ...
                return !$page->get('global') || (permission::can_create_global_page() && tenancy::is_site_multi_tenant());
            })
        );
        ...
        $this->add_action((new action(
            new moodle_url('#'),
            new pix_icon('i/link', ''),
            ['data-action' => 'page-copy-url', 'data-page-id' => ':id'],
            false,
            new lang_string('copyurl', 'tool_custompage')
        )));
