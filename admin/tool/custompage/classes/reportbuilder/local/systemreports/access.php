...
class access extends system_report {
    ...
    protected function initialise(): void {
        ...
        if (!empty($wheres)) {
            ...
            // Exclude non-active users for all audiences.
            $paramguestuser = database::generate_param_name();
            $allwheres = "({$allwheres}) AND {$userentityalias}.suspended = 0 AND {$userentityalias}.deleted = 0
                    AND {$userentityalias}.id <> :{$paramguestuser}";
            $params[$paramguestuser] = $CFG->siteguest;
        } else {
            $allwheres = '1=0';
        }
