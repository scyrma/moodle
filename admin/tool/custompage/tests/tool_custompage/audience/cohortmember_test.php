 ...
 * @covers      \tool_custompage\tool_custompage\audience\cohortmember
...
class cohortmember_test extends advanced_testcase {
    ...
    public function test_user_can_add(): void {
        ...
        $page = $generator->create_page(['name' => 'My cohort page', 'weight' => -1]);
        $cohort = $this->getDataGenerator()->create_cohort();
        $context = context::instance_by_id($cohort->contextid);
        $audience = cohortmember::create($page->get('id'), ['cohorts' => [$cohort->id]]);

        // Admin user.
        $this->setAdminUser();
        $this->assertTrue($audience->user_can_add());

        // User without moodle/cohort:view capability.
        $user = $this->getDataGenerator()->create_user();
        $this->setUser($user);
        $this->assertFalse($audience->user_can_add());

        // Add capability to user (moodle/cohort:view).
        $roleid = create_role('Cohort viewer role', 'cohortviewer', 'Cohort viewer role');
    ...
    public function test_get_sql(): void {
        ...
        // Add userone into cohort.
        cohort_add_member($cohort->id, $userone->id);
        // Add usertwo into cohort.
        cohort_add_member($cohort->id, $usertwo->id);
