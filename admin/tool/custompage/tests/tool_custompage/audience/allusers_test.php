...
/**
 * All users audience type tests
 *
 * @package     tool_custompage
 * @covers      \tool_custompage\tool_custompage\audience\allusers
...
class allusers_test extends advanced_testcase {
    ...
    public function test_user_can_add(): void {
        ...
        $audience = allusers::create($page->get('id'), []);
    ...
    public function test_get_sql(): void {
        ...
        $user = $this->getDataGenerator()->create_user();
        $usersuspended = $this->getDataGenerator()->create_user(['suspended' => 1]);
        $userdeleted = $this->getDataGenerator()->create_user(['deleted' => 1]);

        $audience = allusers::create($page->get('id'), []);

        [$join, $where, $params] = $audience->get_sql('u');
        $whereuser = "u.suspended = 0 AND u.deleted = 0 AND u.id <> :guestuser";
        $params = $params + ['guestuser' => $CFG->siteguest];

        $users = $DB->get_fieldset_sql("SELECT u.id FROM {user} u {$join} WHERE {$where} AND {$whereuser}", $params);

        $this->assertEqualsCanonicalizing([get_admin()->id, $user->id], $users);
    }
}
