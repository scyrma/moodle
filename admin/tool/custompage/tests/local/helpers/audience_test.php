...
class audience_test extends advanced_testcase {
    ...
    /**
     * Test transforming list of audience model records into corresponding audience instances
     */
    public function test_get_audiences_instances(): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        // Create an "invalid" audience, should be subsequently filtered out.
        $audienceinvalid = $generator->create_audience(['pageid' => $page->get('id'), 'configdata' => []]);
        $audienceinvalid->get_persistent()->set('classname', 'invalid')->update();

        $instances = audience::get_audiences_instances([$audience->get_persistent(), $audienceinvalid->get_persistent()]);
        $this->assertCount(1, $instances);
        $this->assertContainsOnlyInstancesOf(allusers::class, $instances);
    }
    ...
    /**
     * Test getting users allowed pages where there exists a page with only invalid audiences
     */
    public function test_get_allowed_pages_invalid(): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $pageinvalid = $generator->create_page(['name' => 'Page with invalid audience', 'weight' => -1]);

        // Create an "invalid" audience, should be subsequently filtered out.
        $audienceinvalid = $generator->create_audience(['pageid' => $pageinvalid->get('id'), 'configdata' => []]);
        $audienceinvalid->get_persistent()->set('classname', 'invalid')->update();

        // Assert we get back only the page containing the valid/matching audience.
        $this->assertEquals([$page->get('id')], audience::get_allowed_pages());
    }
