@tool @tool_custompage @moodleworkplace @javascript
Feature: View custom page
  In order to view custom page
  As a user
  I need to be able to access the page from primary navigation

  Scenario: View custom pages in primary navigation
    Given "1" tenants exist with "2" users and "0" courses in each
    And the following "tool_custompage > Pages" exist:
      | name        | weight | tenant  |
      | Second page | 2      | Tenant1 |
      | First page  | -4     | Tenant1 |
      | Third page  | 3      | Tenant1 |
    And the following "tool_custompage > Audiences" exist:
      | page        | configdata |
      | First page  |            |
      | Second page |            |
    When I log in as "user11"
    And I am on site homepage
    Then I should see "First page" in the ".primary-navigation" "css_element"
    And I should see "Second page" in the ".primary-navigation" "css_element"
    And I should not see "Third page" in the ".primary-navigation" "css_element"
    And "Second page" "text" should appear after "First page" "text"

  Scenario: View custom pages by title in primary navigation
    Given the following "tool_custompage > Pages" exist:
      | name     | title        | weight |
      | Page one | My title one | 0      |
      | Page two |              | 1      |
    And the following "tool_custompage > Audiences" exist:
      | page     | configdata |
      | Page one |            |
      | Page two |            |
    And the following "users" exist:
      | username | firstname | lastname | email            |
      | user1    | John      | Smith    | john@example.com |
    When I log in as "user1"
    And I am on site homepage
    And I select "My title one" from primary navigation
    Then I should see "Page one" in the "page-header" "region"
    And I select "Page two" from primary navigation
    And I should see "Page two" in the "page-header" "region"

  Scenario: Viewing the default My teams custom page
    Given "1" tenants exist with "4" users and "0" courses in each
    And user "user11" has a manager position over users "user12,user13" with permissions "7"
    And I log in as "user11"
    When I select "My teams" from primary navigation
    Then I should see "User 12" in the "Team overview" "block"
    And I should see "User 13" in the "Team overview" "block"

  Scenario: Viewing empty My teams custom page
    Given "1" tenants exist with "2" users and "0" courses in each
    And user "tenantadmin1" has a manager position over users "user11" with permissions "7"
    And I log in as "tenantadmin1"
    When I select "My teams" from primary navigation
    Then I should see "User 11" in the "Team overview" "block"
    And I switch editing mode on
    And I open the "Team overview" blocks action menu
    And I follow "Delete Team overview block"
    And I click on "Delete" "button" in the "Delete block?" "dialogue"
    And I switch editing mode off
    Then I should see "Nothing to display"
    And I should not see "Team overview"
