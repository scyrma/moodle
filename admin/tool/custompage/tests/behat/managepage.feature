...
Feature: Manage single custom page
  ...
  Scenario: Edit block config in custom page and dashboard
    ...
    And "Dashboard page" "text" should exist in the "//div[contains(@id,'fitem_id_bui_staticpagetypepattern')]" "xpath_element"
    And "Display on page types" "select" should not exist in the "//div[contains(@id,'id_whereheader')]" "xpath_element"
    ...
    Then "Calendar" "block" should exist in the ".drawer-right" "css_element"
    And I configure the "Calendar" block
    And "Custom pages" "text" should exist in the "//div[contains(@id,'fitem_id_bui_staticpagetypepattern')]" "xpath_element"
    And "Display on page types" "select" should not exist in the "//div[contains(@id,'id_whereheader')]" "xpath_element"

  Scenario: Observe audiences in tenant
    ...
    And the following "permission overrides" exist:
      ...
      | tool/organisation:assignjobs        | Prohibit      | tool_tenant_admin | System       |           |
      | tool/organisation:managedepartments | Prohibit      | tool_tenant_admin | System       |           |
      | tool/organisation:managepositions   | Prohibit      | tool_tenant_admin | System       |           |
    When I log in as "admin"
    And I switch to tenant "Tenant1"
    And I am on the "My tenant page" "tool_custompage > Manage" page
    And I select "Audience" from secondary navigation
    And I click on "Add audience 'Member of cohort'" "link"
    And I set the field "Select members from cohort" to "Cohort test"
    And I press "Save changes"
    And I click on "Add audience 'Assigned system role'" "link"
    And I set the field "Select a role" to "Manager"
    And I press "Save changes"
    And I click on "Add audience 'Job assignments'" "link"
    And I set the following fields to these values:
      | Department | Department1 |
      | Position   | Position1   |
    And I press "Save changes"
    Then I should see "Cohort test" in the "audiences-container" "region"
    Then I should see "Manager" in the "audiences-container" "region"
    Then I should see "Users in department: Department1" in the "audiences-container" "region"
    And I log out
    And I log in as "tenantadmin1"
    And I am on the "My tenant page" "tool_custompage > Manage" page
    And I select "Audience" from secondary navigation
    Then I should not see "Cohort test" in the "audiences-container" "region"
    Then I should not see "Manager" in the "audiences-container" "region"
    Then I should not see "Users in department: Department1" in the "audiences-container" "region"
    Then I should see "You need permissions to view or modify this audience" in the "audiences-container" "region"
