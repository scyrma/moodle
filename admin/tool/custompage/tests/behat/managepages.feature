...
Feature: Manage custom pages
  ...
  Scenario: View list of custom pages with audience summary
    Given "3" "tool_custompage > Pages" exist with the following data:
      | name   | Page [count] |
      | weight | [count]      |
    And the following "tool_custompage > Audiences" exist:
      | page   | classname                                          | configdata  |
      | Page 1 | tool_custompage\tool_custompage\audience\adminrole | {"roles":1} |
      | Page 1 | tool_custompage\tool_custompage\audience\allusers  |             |
      | Page 2 | tool_custompage\tool_custompage\audience\allusers  |             |
    When I log in as "admin"
    And I navigate to "Custom pages" in workplace launcher
    Then the following should exist in the "reportbuilder-table" table:
      | Name   | Audience                                                                                    |
      | Page 1 | Administrators (Site administrators) All authenticated users (All authenticated site users) |
      | Page 2 | All authenticated users (All authenticated site users)                                      |
      | Page 3 |                                                                                             |
  ...
  Scenario: Copy URL of custom page from page listings
    Given the following "tool_custompage > Pages" exist:
      | name     | weight |
      | Page one | 0      |
    When I log in as "admin"
    And I navigate to "Custom pages" in workplace launcher
    And I press "Copy URL" action in the "Page one" report row
    And I click on "Copy to clipboard" "link" in the "Copy URL" "dialogue"
  ...
  Scenario: Add blocks as tenant admin
    ...
    And I set the following fields in the "New page" "dialogue" to these values:
      | Name   | My admin tenant page         |
      | Title  | This is my admin tenant page |
      | Weight | -10                          |
    And I click on "Save" "button" in the "New page" "dialogue"
    Then I should see "My admin tenant page"
    And I should not see "Global page"
    And I should see "Nothing to display"
    When I click on "Edit page" "link"
    And I switch editing mode on
    And I click on "Add a block" "link" in the ".drawer-right" "css_element"
    And I click on "Comments" "link" in the "Add a block" "dialogue"
    Then "Comments" "block" should exist in the ".drawer-right" "css_element"
    And I open the "Comments" blocks action menu
    And I click on "Delete Comments block" "link" in the "Comments" "block"
    And I click on "Delete" "button" in the "Delete block?" "dialogue"
    Then "Comments" "block" should not exist in the ".drawer-right" "css_element"
    And I click on "Add a block" "link" in the "region-main" "region"
    And I click on "Recently accessed items" "link" in the "Add a block" "dialogue"
    And "Recently accessed items" "block" should exist in the "region-main" "region"
    And I open the "Recently accessed items" blocks action menu
    And I click on "Delete Recently accessed items block" "link" in the "Recently accessed items" "block"
    And I click on "Delete" "button" in the "Delete block?" "dialogue"
    And "Recently accessed items" "block" should not exist in the "region-main" "region"

  Scenario: New page in single tenant site
    Given I log in as "admin"
    And I navigate to "Custom pages" in workplace launcher
    And I should see "Global page" in the "My teams" "table_row"
    When I click on "New page" "button"
    And I set the following fields in the "New page" "dialogue" to these values:
      | Name   | Single tenant page  |
      | Title  | Single tenant page  |
      | Weight | -10                 |
    And I click on "Save" "button" in the "New page" "dialogue"
    Then I should see "Single tenant page"
    And I should not see "Global page"
    And I should see "Nothing to display"
