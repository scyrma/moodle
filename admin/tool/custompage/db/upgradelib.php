...
function tool_custompage_create_myteams_page(): void {
    ...
    $myteampage->name = get_string('pluginname', 'block_myteams');
    $myteampage->weight = -10;
    $myteampage->tenantid = 0;
    $myteampage->global = true;

    // Create My teams custom page.
    $myteamscustompage = \tool_custompage\local\helpers\page::create_page($myteampage);

    // Add manager audience to myteams page.
    $manageraudience::create($myteamscustompage->get('id'), ['permissions' => 'anymanager']);

    // Add myteams block.
    $page = new moodle_page();
    $page->set_context(context_system::instance());
    $page->blocks->add_region('content');
    $page->blocks->add_block('myteams', 'content', 0, false,
        'admin-tool-custompage', $myteamscustompage->get('id'));
}
