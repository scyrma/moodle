...
`classes/tool_dynamicrule/[condition|action]/` plugin directory, and the
...
we check `moodle/cohort:manage` or `moodle/cohort:view`, for cohort action we
also check `moodle/cohort:assign`. The actual rule processing is performed by
