...
class rules extends \tool_wp\exporter_base {
    ...
    public function is_available(): bool {
        return (!strlen($this->entrypoint) || $this->is_chained_entrypoint()) && permission::can_manage_rules();
    }
        ...
        $this->register_entity('tool_dynamicrule', [
            self::ENTITY_INDIVIDUALEXPORT => function(array $ids, array $settings) {
                ...
                return [
                        ...
                        self::EXPORT_INSTANCES => self::EXPORT_INSTANCES_COMPONENT,
                        self::EXPORT_SELECT_COMPONENT => $settings['component'],
                        self::EXPORT_SELECT_COMPONENT_AREA => $settings['area'],
                        self::EXPORT_SELECT_COMPONENT_ITEMID => $settings['itemid'],
                    ];
            },
