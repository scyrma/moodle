    ...
    protected function initialise() {
        $this->register_entity(rule::TABLE, [
            ...
            self::ENTITY_INDIVIDUALIMPORT => function(array $ids, array $settings) {
                ...
                if (empty($settings['component']) || empty($settings['area']) || empty($settings['itemid'])
                        || empty($settings['mapper'])) {
                    throw new \coding_exception('Caller must specify component, area, itemid and mapper');
                }
                return [
                    self::IMPORT_CONTENT => 1,
                    self::IMPORT_INSTANCES => self::IMPORT_INSTANCES_COMPONENT,
                    self::IMPORT_SELECT_COMPONENT => $settings['component'],
                    self::IMPORT_SELECT_COMPONENT_AREA => $settings['area'],
                    self::IMPORT_SELECT_COMPONENT_ITEMID => $settings['itemid'],
                    self::IMPORT_SELECT_COMPONENT_MAPPER => $settings['mapper'],
                ];
            },
    ...
    public function perform_import(string $entity): void {
        ...
        foreach ($rules as $rule) {
            $rule
                ...
                ->set_validation_callback(function($record, wp_imported_entity $rule) {
                ...
                ->set_import_callback(function($data, wp_imported_entity $rule) {
