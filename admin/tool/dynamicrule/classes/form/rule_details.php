...
class rule_details extends modal_form {
    ...
    public function definition() {
        ...
        $duringoptions = [0 => get_string('ever', 'tool_dynamicrule'), 1 => get_string('during', 'tool_dynamicrule')];
        $restrictionrule[] = $mform->createElement('select', 'duringselector', '', $duringoptions);
        $restrictionrule[] = $mform->createElement('duration', 'matchinterval', '', ['defaultunit' => 86400, 'optional' => false]);
        $mform->addGroup($restrictionrule, 'limitingrule', '', '&nbsp;', false);
        $mform->setType('matchlimit', PARAM_INT);
        $mform->setDefault('matchlimit', 1);
