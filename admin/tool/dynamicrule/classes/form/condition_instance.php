 ...
 * @license     Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class condition_instance extends dynamic_form {
    ...
    protected function get_condition() :? condition_base {
        ...
        if (!$id) {
            ...
            $record->classname = '\\' . $plugin . '\\tool_dynamicrule\\condition\\' . $classname;
        }
        return condition_base::instance($id, $record);
    }
    ...
    protected function check_access_for_dynamic_submission(): void {
        ...
        permission::require_can_edit_rule_conditions($rule);

        // Validate condition instance.
        if (!$condition->get_id()) {
            // New instance. Validate adding permission.
            permission::require_can_add_condition($condition, $rule);
        } else {
            // Existing instance. Validate editing permission.
            permission::require_can_edit_condition($condition);
        }
    }
    ...
    public function process_dynamic_submission() {
        ...
        if (!$formdata->id) {
            // New condition. Validate new data for serious violations.
            permission::require_can_edit_condition($condition, $configdata);
            $condition = $condition::create($formdata->ruleid, $configdata);
        } else {
            // Editing condition. Validate new data for serious violations.
            permission::require_can_edit_condition($condition, $configdata);
            $condition->update_configdata($configdata, true);
        }
        return ['instanceid' => $condition->get_id(), 'description' => $condition->get_displayed_description()];
    }
