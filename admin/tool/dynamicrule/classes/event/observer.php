...
class observer {
    ...
    public static function process_event(\core\event\base $event) {
        ...
        foreach ($records as $ruleid => $userids) {
            if (api::requires_scheduled_task($ruleid)) {
                continue;
            }
    ...
    public static function tenant_user_updated(\tool_tenant\event\tenant_user_updated $event): void {
        ...
        foreach ($rules as $rule) {
            if (!api::requires_scheduled_task($rule->get('id'))) {
    ...
    public static function user_updated(\core\event\user_updated $event) {
        ...
        // If $SESSION value is created in precheck_update_user callback for users who change
        // the suspended status to unsuspended then re-evaluate all rules that are applicable to the given user.
        if (isset($SESSION->unsuspendinguserid) && $event->relateduserid === $SESSION->unsuspendinguserid) {
            // Delete $SESSION userid object.
            unset($SESSION->unsuspendinguserid);
            // Get all enabled rules that are applicable to the given user and rules that listen for the current event.
            $rules = api::get_rules_for_user($event->relateduserid);
            $ruleuserevents = self::get_rules_and_users_for_event($event);

            // Get rules ids that listen current event and have the given user as matched to skip in re-evaluate process.
            $rules = array_filter($rules, function($r) use ($ruleuserevents) {
                return in_array($r->get('id'), array_keys($ruleuserevents));
            });

            foreach ($rules as $rule) {
                // Check if rules applied to this user already listen current event
                // and rule isn't scheduled task then exclude it from re-evaluate rule process.
                if (!api::is_rule_scheduled_task($rule->get('id'))) {
                    api::process_rule($rule, $event->relateduserid);
                }
            }
        }
    }
