...
class permission {
    ...
    /**
     * User can delete rule conditions
     *
     * @param rule $rule
     * @return bool
     */
    public static function can_delete_rule_conditions(rule $rule): bool {
        return self::can_edit_rule_conditions($rule) &&
            (api::count_matched_users($rule->get('id')) === 0);
    }

    /**
     * Require user to be able to delete rule conditions
     *
     * @param rule $rule
     * @throws \moodle_exception
     */
    public static function require_can_delete_rule_conditions(rule $rule): void {
        if (!self::can_delete_rule_conditions($rule)) {
            throw new \moodle_exception('errorcannotmanage', 'tool_dynamicrule');
        }
    }
}
