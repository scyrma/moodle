...
class permission {
    ...
    public static function can_edit_condition(condition_base $condition, array $configdata = []) {
        ...
        return ($condition->is_broken() && $condition->user_can_add()) ||
            $condition->user_can_edit($configdata);
    }
    ...
    public static function can_edit_outcome(outcome_base $outcome, array $configdata = []) {
        ...
        return ($outcome->is_broken() && $outcome->user_can_add()) ||
            $outcome->user_can_edit($configdata);
    }
    ...
    /**
     * User is be able to add a condition
     *
     * @param condition_base $condition
     * @return bool
     */
    public static function can_add_condition(condition_base $condition) : bool {
        return $condition->user_can_add();
    }

    /**
     * Require user to be able to add a condition
     *
     * @param condition_base $condition
     * @throws \moodle_exception
     */
    public static function require_can_add_condition(condition_base $condition) {
        if (!self::can_add_condition($condition)) {
            throw new \moodle_exception('errorcannotmanagecondition', 'tool_dynamicrule');
        }
    }

    /**
     * User is be able to add an outcome
     *
     * @param outcome_base $outcome
     * @return bool
     */
    public static function can_add_outcome(outcome_base $outcome) : bool {
        return $outcome->user_can_add();
    }
     ...
     * @throws \moodle_exception
     */
    public static function require_can_add_outcome(outcome_base $outcome) {
        if (!self::can_add_outcome($outcome)) {
