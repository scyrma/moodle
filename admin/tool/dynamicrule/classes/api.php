...
class api {
     ...
     */
    public static function get_matching_users($rule, int $targetuserid = null, int $currenttime = null): array {
        global $DB;
        ...
        // TODO This method needs refactoring (WP-1681), current time is only used in tests.
        ...
        self::insert_matched_users($rule->get('id'), $users, $currenttime);
        return $users;
    }
     ...
     *
     * TODO Move this to generator as method is not used outside unittests (WP-1681).
     *
     * Example of usage:
    ...
    private static function get_dry_run_sql(int $ruleid, int $currenttime = null): array {
        ...
        list($restrictionjoin, $groupbyhaving, $restrictionparams) = self::get_rule_restriction_sql($rule, $currenttime);
    ...
    public static function process_rule(rule $rule, int $targetuserid = null) {
        if ($rule->is_enabled()) {
            if (self::is_rule_configuration_valid($rule)) {
                if ($users = self::get_matching_users($rule, $targetuserid)) {
     ...
     * @return bool
     */
    private static function validate_conditions_configuration(rule $rule): bool {
        $valid = true;
        foreach (self::get_rule_conditions($rule->get('id')) as $condition) {
            $configisvalid = $condition->is_configuration_valid();
            if (!$configisvalid) {
     ...
     * @return bool
     */
    private static function validate_outcomes_configuration(rule $rule): bool {
        $valid = true;
        foreach (self::get_rule_outcomes($rule->get('id')) as $outcome) {
            $configisvalid = $outcome->is_configuration_valid();
            if (!$configisvalid) {
