...
class api {
    ...
    public static function count_matching_users(int $ruleid): int {
        ...
        if (!condition::count_records(['ruleid' => $ruleid]) || !self::static_conditions_are_matching($ruleid)) {
            return 0;
        }
    ...
    private static function update_unmatched_users(int $ruleid, array $matchedusers, int $currenttime, $targetuserid = null) {
        ...
        if ($matchedusers) {
            if (count($matchedusers) == 1 && isset($matchedusers[$targetuserid])) {
                ...
                // by event. If user previously matched, this suggests event has happened
                // again, so unmatch user. New matching record for current event matching
                // will be created during outcome applying.
            ...
            if ($targetuserid !== null) {
                ...
                // again, so unmatch user. New matching record for current event matching
                // will be created during outcome applying.
                // TODO: Check if this is possible and refactor if needed, we reach this
                // point in observer_testacase::test_user_enrolled_trigger_rule_processing() only.
                $unmatechedparams = ['targetuserid' => $targetuserid];
                $unmatcheduseridsql = ' AND userid = :targetuserid';
            } else {
                $unmatcheduseridsql = '';
