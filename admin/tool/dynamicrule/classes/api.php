...
use tool_dynamicrule\event\rule_archived;
use tool_dynamicrule\event\rule_created;
use tool_dynamicrule\event\rule_deleted;
use tool_dynamicrule\event\rule_unarchived;
use tool_dynamicrule\event\rule_updated;
 ...
 */
class api {
    // Request caching helper trait.
    use \tool_dynamicrule\local\helpers\request_cache;
     ...
     */
    public static function get_rule(int $id, bool $bypasstenantcheck = false) : rule {
        if ($id && rule::record_exists($id)) {
            $rule = new rule($id);
            // Make sure rule belongs to current user tenant.
            if (!$bypasstenantcheck && !hierarchy::is_own_or_parent_shared_entity($rule->get('tenantid'), $rule->is_shared())) {
    ...
    public static function create_rule(\stdClass $data) : rule {
        ...
        // For now shared rules are only those created in the shared space.
        $data->shared = (int) sharedspace::is_shared_space($data->tenantid);

        $rule = new rule(0, $data);
        $rule->create();

        // Trigger event.
        rule_created::create_from_rule($rule)->trigger();
    ...
    public static function update_rule(int $ruleid, \stdClass $newdata) : rule {
        ...
        // Trigger event.
        rule_updated::create_from_rule($rule)->trigger();
    ...
    public static function delete_rule(int $ruleid) : bool {
        ...
        // Trigger event.
        $event = rule_deleted::create_from_rule($rule);
    ...
    public static function archive_rule(int $ruleid) : bool {
        ...
        // Trigger event.
        rule_archived::create_from_rule($rule)->trigger();
    ...
    public static function unarchive_rule(int $ruleid) : bool {
        ...
        // Trigger event.
        rule_unarchived::create_from_rule($rule)->trigger();
    ...
    public static function duplicate_rule(int $ruleid) : int {
        ...
        $newname = get_string('rulenamecopy', 'tool_dynamicrule', (object) ['name' => $newrule->name, 'number' => $i]);
        while ($DB->record_exists('tool_dynamicrule', ['name' => $newname, 'tenantid' => $tenantid])) {
            $newname = get_string('rulenamecopy', 'tool_dynamicrule', (object) ['name' => $newrule->name, 'number' => ++$i]);
        }
        $newrule->name = $newname;
        $newrule->enabled = 0;
        $newrule->tenantid = $tenantid;
        $newrule->shared = (int) sharedspace::is_shared_space();
        unset($newrule->id);
        ...
        $requiredtype = $newrule->shared ? rule::TYPE_SHARED : rule::TYPE_NORMAL;
        foreach ($conditions as $condition) {
            $conditionruletypes = $condition->supports_rule_types();
            if ($conditionruletypes & $requiredtype) {
        ...
        foreach ($outcomes as $outcome) {
            ...
            if ($outcomeruletypes & $requiredtype) {
    ...
    public static function get_matching_users($rule, int $targetuserid = null): array {
        ...
        $sql = "SELECT DISTINCT u.id, u.suspended, mlastmatch.id AS lastmatchid
                  FROM {user} u";
        ...
        if ($targetuserid !== null) {
            $pg = database::generate_param_name();
    ...
    public static function get_matching_join_sql(int $ruleid): array {
        ...
        $where = tenancy::get_users_subquery(false, false, 'u.id', $tenantid, true);
        $where .= ' AND u.deleted = 0 ';

        // If 'Include suspended users' rule setting is unchecked we just count non-suspended users.
        $rule = self::get_rule($ruleid, true);
        if (!$rule->includes_suspended()) {
    ...
    public static function process_rules() {
        ...
        foreach ($rules as $rule) {
            ...
            if (self::is_rule_scheduled_task($rule->get('id'))) {
    ...
    public static function process_rule(rule $rule, int $targetuserid = null) {
        ...
        if (!$rule->is_archived() && $rule->is_enabled()) {
            ...
            if ($targetuserid && !hierarchy::is_own_or_parent_shared_entity(
                    $rule->get('tenantid'), $rule->is_shared(), \tool_tenant\tenancy::get_tenant_id($targetuserid))) {
                // The rule can never apply for the target user (is from a different tenant), no need to do further checks.
                return;
            }

            // If user is suspended and 'Include suspended users' rule setting is unchecked skip rule process.
            if ($targetuserid !== null && \core_user::get_user($targetuserid)->suspended && !$rule->includes_suspended()) {
                return;
            }
            ...
            if (self::is_rule_configuration_valid($rule)) {
                ...
                foreach ($users as $user) {
                    ...
                    // For shared rules user tenant may be different from the rule tenant.
                    \tool_tenant\config::push_for_user($user->id);
     ...
     *
     * @deprecated since Moodle Workplace 4.2 - Dynamic rules does not support static conditions anymore
     *
     * @param int $ruleid
     * @return bool
     */
    public static function static_conditions_are_matching(int $ruleid): bool {
        ...
        foreach ($conditions as $condition) {
            if ($condition instanceof condition_static) {
                debugging('The base condition_static class has been deprecated - Dynamic rules does not
                    support static conditions anymore', DEBUG_DEVELOPER);
    ...
    public static function get_rule_restriction_sql(rule $rule, int $currenttime = null): array {
        ...
        $pruleid = database::generate_param_name();
        $ptimelimit = database::generate_param_name();
        $pcountlimit = database::generate_param_name();
        $m = database::generate_alias();
     ...
     *
     * @param rule $rule
     * @param \stdClass[] $matchedusers list of users who match the conditions, each record record has 'id' property
     * @param int|null $currenttime
    ...
    private static function filter_users_by_rule_restriction(rule $rule, array $matchedusers, int $currenttime = null): array {
        ...
        foreach ($userschunks as $chunk) {
            list($matchsql, $matchparams) = $DB->get_in_or_equal(array_column($chunk, 'id'), SQL_PARAMS_NAMED,
                database::generate_param_name() . 'user');
    ...
    public static function get_rule_conditions_sql(int $ruleid): array {
        ...
        foreach (self::get_rule_conditions($ruleid) as $condition) {
            if ($condition instanceof condition_sql) {
                ...
                if (debugging()) {
                    database::validate_params($newparams);
                }
                $joins[] = $newjoin;
                $where .= ' AND (' . $newwhere . ')';
                $params += $newparams;
    ...
    public static function is_rule_configuration_valid(rule $rule): bool {
        $result = static::get_cached_value('is_rule_configuration_valid', [$rule->get('id')]);
        if (is_null($result)) {
            ...
            $result = $tenantvalid && $conditionsvalid && $outcomesvalid;

            static::set_cached_value('is_rule_configuration_valid', $result, [$rule->get('id')]);
        }
        return $result;
    }
     ...
     *
     * If at least one of the rule conditions or actions requires cron processing,
     * then the rule is scheduled task.
     *
     * @param int $ruleid
     * @return bool
     */
    public static function is_rule_scheduled_task(int $ruleid) : bool {
        foreach (self::get_rule_conditions($ruleid) as $condition) {
            if ($condition->is_scheduled_task() && !$condition->is_broken()) {
                return true;
            }
        }
        return self::requires_scheduled_task($ruleid);
    }

    /**
     * Check if the rule is required to be run as scheduled task.
     *
     * If at least one outcome requires cron processing then the rule should
     * be run as scheduled task only.
     *
     * @param int $ruleid
     * @return bool
     */
    public static function requires_scheduled_task(int $ruleid) : bool {
        foreach (self::get_rule_outcomes($ruleid) as $outcome) {
            if ($outcome->is_scheduled_task() && !$outcome->is_broken()) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Do we need to show "Scheduled task" badge for this rule
     *
     * The badge should be shown if at least one condition or outcome in the rule is scheduled task.
     *
     * Some rules will still be evaluated regularly in the scheduled task but we don't show
     * the label for them because they also listen to the events.
     *
     * @param rule $rule
     * @return bool
     * @throws \coding_exception
     */
    public static function show_rule_scheduled_task_badge(rule $rule): bool {
        foreach (self::get_rule_conditions($rule->get('id')) as $condition) {
            if ($condition->show_scheduled_task_badge() && !$condition->is_broken()) {
                return true;
            }
        }
        // If outcome requires scheduled task, badge should be displayed.
        return self::requires_scheduled_task($rule->get('id'));
    }

    /**
     * Returns a list of enabled rules that contain the given conditions
     *
     * @param string $conditionclass
     * @param int|null $tenantid if specified returns only rules applicable to this tenant (i.e. either rules from
     *     this tenant or shared rules from parent tenant).
     * @return \stdClass[] array of rule records (only fields: id, tenantid, isshared)
     */
    public static function get_all_rules_with_condition(string $conditionclass, ?int $tenantid = null): array {
        global $DB;
        $cache = \cache::make('tool_dynamicrule', 'conditions');
        $cachekey = 'ruleswithcondition-' . $conditionclass;
        $rules = $cache->get($cachekey);
        if ($rules === false || !is_array($rules)) {
            list($sql, $params) = $DB->get_in_or_equal($conditionclass, SQL_PARAMS_NAMED);
            $sql = 'SELECT DISTINCT dr.id, dr.tenantid, dr.shared
        ...
        if ($tenantid) {
            $rules = array_filter($rules, function($rule) use ($tenantid) {
                return hierarchy::is_own_or_parent_shared_entity($rule->tenantid, $rule->shared, $tenantid);
            });
        }
    ...
    /**
     * Returns all instances of any condition in all enabled and non-archived rules
     *
     * @return condition_base[]
     */
    public static function get_all_rule_conditions(): array {
        global $DB;

        $sql = 'SELECT drc.*
                          FROM {tool_dynamicrule_condition} drc
                          JOIN {tool_dynamicrule} dr ON drc.ruleid = dr.id
                         WHERE dr.enabled = 1 AND dr.archived = 0
                      ORDER BY drc.ruleid, drc.id';
        $conditions = [];
        $records = $DB->get_records_sql($sql, []);
        foreach ($records as $record) {
            if ($condition = condition_base::instance(0, $record)) {
    ...
    /**
     * Returns the list of condition classes that might watch the given event
     *
     * @param string $eventclass
     * @return array
     */
    public static function get_conditions_watching_event(string $eventclass): array {
        $cache = \cache::make('tool_dynamicrule', 'conditions');
        $cachekey = 'eventwatchers';
        $eventconditionmap = $cache->get($cachekey);
        if ($eventconditionmap === false || !is_array($eventconditionmap)) {
            ...
            $cache->set($cachekey, $eventconditionmap);
        }

        return $eventconditionmap[$eventclass] ?? [];
    }

    /**
     * Get all enabled rules that are applicable to the given user based on their tenant
     *
     * @param int $userid
     * @return array
     */
    public static function get_rules_for_user(int $userid): array {
        $tenantid = \tool_tenant\tenancy::get_actual_tenant_id($userid);
        return self::get_rules_for_tenant($tenantid);
    }

    /**
     * Get all enabled rules that are applicable to the users in the given tenant
     *
     * @param int $tenantid
     * @return array
     */
    public static function get_rules_for_tenant(int $tenantid): array {
        [$sql, $params] = hierarchy::filter_own_or_parent_shared_entities_sql('tenantid', 'shared = 1', $tenantid);
        $rules = rule::get_records_select('enabled = :enabled AND '. $sql,
            $params + ['enabled' => 1]);
        return $rules;
    }

    /**
     * Checks if the entities from the given course category are allowed in the conditions/actions in the given rule
     *
     * In shared space we don't allow categories that belong to any tenant and their children.
     * In tenant we allow categories that belong to other tenants and their children.
     *
     * @param rule $rule
     * @param int $categoryid
     * @return bool
     */
    public static function is_course_category_allowed_in_rule(rule $rule, int $categoryid): bool {
        $ruletenantid = $rule->get('tenantid');
        $issharedspace = sharedspace::is_shared_space($ruletenantid);
        $issubcategoryof = function($childcategoryid, $parentcategoryid) {
            if (!$parentcategoryid) {
                return false;
            }
            ...
            $catcontext = \context_coursecat::instance($parentcategoryid);
            $parentcontexts = \context_coursecat::instance($childcategoryid)->get_parent_context_ids();
            return in_array($catcontext->id, $parentcontexts);
        };

        foreach (tenancy::get_tenants() as $tenant) {
            if ($issharedspace && $issubcategoryof($categoryid, $tenant->categoryid)) {
                return false;
            }
        }
        return true;
