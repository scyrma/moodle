...
class condition extends select {
    ...
    public function get_sql_filter(?array $values): array {
        ...
        $conditiontablealias = db::generate_alias();
        $conditionparam = db::generate_param_name();
        ...
        $containsconditionsql = "EXISTS (
            SELECT 1
             ...
             WHERE {$conditiontablealias}.classname = :{$conditionparam}
               AND {$conditiontablealias}.ruleid = {$rulefieldsql}
        )";

        return [$containsconditionsql, [$conditionparam => $condition]];
    }
}
