...
abstract class condition_base {
    ...
    final public static function create(int $ruleid, array $configdata) : condition_base {
        ...
        // Trigger event.
        condition_added::create_from_condition($instance->condition)->trigger();
    ...
    final public function delete() {
        // Trigger event.
        $event = condition_deleted::create_from_condition($this->condition);
     ...
     *
     * Indicates if condition supports multi-tenant (shared), normal or both
     * type of rules. Status constants need to be bitwise added if many types are supported,
     * e.g. rule::TYPE_NORMAL + rule::TYPE_SHARED.
     *
     * @return int Rule types bitwise added.
     */
    public function supports_rule_types(): int {
        return rule::TYPE_NORMAL;
    }
    ...
    public function update_configdata(array $configdata, bool $save = false) {
        ...
        if ($save) {
            ...
            // Trigger event.
            condition_updated::create_from_condition($this->condition)->trigger();
        }
    }
    ...
    /**
     * Return the description for the condition when current user can see all information about it
     *
     * @return string
     */
    ...
    public function get_uneditable_description(): string {
        return get_string('uneditabledescription', 'tool_dynamicrule', $this->get_title());
    }

    /**
     * Description that will be displayed in the list of rules
     *
     * @return string
     */
    final public function get_displayed_description(): string {
        if ($this->is_broken()) {
            return $this->get_broken_description();
        } else if (!$this->user_can_edit($this->get_configdata())) {
            return $this->get_uneditable_description();
        } else {
     ...
     * @param outcome_base $calleroutcome usually not used but some conditions may want to provide more specific data
     * @return array each element is an associative array with keys from $keys
     */
    public function get_custom_data_for_outcome(array $keys, \stdClass $user, outcome_base $calleroutcome): array {
        if (method_exists($this, 'get_data_for_outcome')) {
            // Fallback to the old method.
            // TODO WP-3229 remove this fallback.
            debugging('Calling legacy method ' . get_class($this) . '::get_data_for_outcome(),' .
                ' plugin should implement get_custom_data_for_outcome() instead.',
                DEBUG_DEVELOPER);
            return $this->get_data_for_outcome($keys, [$user], $calleroutcome)[$user->id];
        }
        return array_fill_keys($keys, null); // When overriding populate with actual values.
    }

    /**
     * Magic __call method. Used to provide backward compatibility.
     *
     * @param string $method
     * @param array $args
     * @return mixed
    ...
    public function __call($method, $args) {
        if ($method === 'get_data_for_outcome') {
            // TODO WP-3229 remove this fallback.
            debugging('\tool_dynamicrule\condition_base::get_data_for_outcome() is deprecated, ' .
                'please use get_custom_data_for_outcome() instead.',
                DEBUG_DEVELOPER);
            [$keys, $users, $calleroutcome] = $args;
            $data = [];
            $keys = array_intersect($keys, array_keys($this->get_available_data_for_outcome($calleroutcome)));
            foreach ($users as $user) {
                $data[$user->id] = array_fill_keys($keys, null);
            }
            return $data;
        }
        throw new coding_exception('Call to undefined method ' . get_class($this) . '::' . $method . '()');
    }
     ...
     *
     * This is the default implementation that can be overridden by individual condition classes.
     * Sometimes condition may have event subscriptions and still be a scheduled task,
     * sometimes condition may not have event subscriptions but does not need to be a scheduled task.
     *
     * @return bool
     */
    public function is_scheduled_task(): bool {
        return !$this->get_event_subscription();
    }

    /**
     * Should we show "Scheduled task" badge next to this condition (and the rule containing it)
     *
     * Usually when the condition has event listeners we should not show the badge even though
     * we may process it in the scheduled task to watch for some dates or external changes.
     *
     * @return bool
     */
    public function show_scheduled_task_badge(): bool {
        return !$this->get_event_subscription();
    }
    ...
    public static function get_event_related_userid(\core\event\base $event): int {
        ...
        // The user_loggedin event does not seem to store 'relateduserid'.
        if ($event instanceof \core\event\user_loggedin) {
            $data['relateduserid'] = $data['userid'];
        }

        if (!array_key_exists('relateduserid', $data) || $data['relateduserid'] === null) {
    ...
    /**
     * Return the list of date watchers for this particular condition in this particular rule
     *
     * The watchers do not define callback if the user field in the watched table is called 'userid'
     * and the callback is the same as the default one added in {@see tool_dynamicrule_datewatch()}
     *
