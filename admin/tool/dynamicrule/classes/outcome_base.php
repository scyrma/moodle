...
abstract class outcome_base {
    ...
    final public static function create(int $ruleid, array $configdata) : outcome_base {
        ...
        // Trigger event.
        outcome_added::create_from_outcome($instance->outcome)->trigger();
    ...
    public function delete() {
        // Trigger event.
        $event = outcome_deleted::create_from_outcome($this->outcome);

        $status = $this->outcome->delete();
     ...
     *
     * Indicates if outcome supports multi-tenant (shared), normal or both
    ...
    public function update_configdata(array $configdata, bool $save = false) {
        ...
        if ($save) {
            ...
            // Trigger event.
            outcome_updated::create_from_outcome($this->outcome)->trigger();
        }
    }
    ...
    public function get_data_from_conditions(array $keys, array $users): array {
        // TODO WP-3229 remove this method.
        debugging('\tool_dynamicrule\outcome_base::get_data_from_conditions() is deprecated, ' .
            'please use get_custom_data_from_conditions() instead.',
            DEBUG_DEVELOPER);
        $result = [];
        foreach ($users as $user) {
            $result[$user->id] = $this->get_custom_data_from_conditions($keys, $user);
        }
        return $result;
    }
     ...
     * @return array condition data.
     */
    final public function get_custom_data_from_conditions(array $keys, \stdClass $user): array {
        $conditionsdata = [];
        ...
        foreach ($conditions as $condition) {
            ...
            if ($thiskeys) {
                $data = $condition->get_custom_data_for_outcome($thiskeys, $user, $this);
                $conditionsdata = array_merge($conditionsdata, $data);
            }
        }
    ...
    /**
     * Should this outcome be processed as scheduled tasks.
     *
     * This is the default implementation that can be overridden by individual outcome classes.
     * Sometimes outcome should be processed as a scheduled task,for example when permissions
     * require so or when processing is resource intensive.
     *
     * @return bool
     */
