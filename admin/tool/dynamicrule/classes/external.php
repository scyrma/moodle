...
class external extends \external_api {
    ...
    protected static function duplicate_rule_parameters() {
        $params = ['id' => new \external_value(PARAM_INT, 'The ID of the rule to be duplicated', VALUE_REQUIRED)];
        return new \external_function_parameters($params);
    }
    ...
    public static function delete_condition($instanceid) {
        ...
        $condition = condition_base::instance($params['instanceid']);
        $rule = \tool_dynamicrule\api::get_rule($condition->get_ruleid());
        permission::require_can_delete_rule_condition($rule, $condition);
    ...
    public static function delete_outcome($instanceid) {
        ...
        $outcome = outcome_base::instance($params['instanceid']);
        $rule = \tool_dynamicrule\api::get_rule($outcome->get_ruleid());
        permission::require_can_delete_rule_outcome($rule, $outcome);
    ...
    public static function potential_badge_selector_returns(): \external_multiple_structure {
        return new \external_multiple_structure(new \external_single_structure([
            ...
            'name' => new \external_value(PARAM_TEXT, 'The name of the badge'),
        ]));
    }

    /**
     * Parameters for the cohort selector WS.
     *
     * @return \external_function_parameters
     */
    public static function potential_cohort_selector_parameters(): \external_function_parameters {
        return new \external_function_parameters([
            'search' => new \external_value(PARAM_NOTAGS, 'Search string', VALUE_REQUIRED),
            'instancetype' => new \external_value(PARAM_ALPHA, 'Instance type (condition or outcome)', VALUE_DEFAULT, 'condition'),
        ]);
    }
     ...
     * @return array
     */
    public static function potential_cohort_selector(string $search, string $instancetype): array {
        $params = self::validate_parameters(self::potential_cohort_selector_parameters(),
            ['search' => $search, 'instancetype' => $instancetype]);
        self::validate_context(\context_system::instance());
        return \tool_dynamicrule\api::get_potential_cohorts($params['search'], $params['instancetype']);
    }

    /**
     * Return for cohort selector.
     *
     * @return \external_multiple_structure
     */
    public static function potential_cohort_selector_returns(): \external_multiple_structure {
        return new \external_multiple_structure(new \external_single_structure([
    ...
    public static function potential_certificate_selector_returns(): \external_multiple_structure {
        return new \external_multiple_structure(new \external_single_structure([
            'id' => new \external_value(PARAM_INT, 'ID of the certificate'),
            'name' => new \external_value(PARAM_TEXT, 'The name of the certificate'),
        ]));
    }

    /**
     * Returns the structure of parameters for count_matched_users function.
     * @return \external_function_parameters
     */
    protected static function count_matched_users_parameters() {
        $params = ['id' => new \external_value(PARAM_INT, 'The ID of the rule to count already matched users', VALUE_REQUIRED)];
        return new \external_function_parameters($params);
    }

    /**
     * Count users that already matched the given rule
     *
     * @param int $ruleid The ID of the rule
     * @return bool
     */
    public static function count_matched_users($ruleid) {
        $params = self::validate_parameters(self::count_matched_users_parameters(), ['id' => $ruleid]);
        $rule = \tool_dynamicrule\api::get_rule($params['id']);
        permission::require_can_manage_rules($rule);

        return \tool_dynamicrule\api::count_matched_users($params['id']);
    }

    /**
     * Describes the return function of count_matched_users
     * @return \external_value
     */
    public static function count_matched_users_returns() {
        return new \external_value(PARAM_INT, 'The number of users that already matched the rule.');
    }
}
