...
class rules_list extends system_report {
    ...
    public function row_callback(\stdClass $row): void {
        ...
        $conditionsvalid = api::is_conditions_configuration_valid($this->currentrule->get('id'));
        $outcomesvalid = api::is_outcomes_configuration_valid($this->currentrule->get('id'));
        if (!$conditionsvalid || !$outcomesvalid) {
            api::mark_rule_as_broken($this->currentrule->get('id'));
            // Update current rule instance.
            $this->currentrule->set('enabled', 0);
            $this->currentrule->set('broken', 1);
        }
    }
    ...
    public function get_display_name($value, \stdClass $row, $additionalarguments) {
        global $OUTPUT;
        $editable = \tool_dynamicrule\api::get_name_inplace_editable($this->currentrule);
