...
use tool_dynamicrule\tool_reportbuilder\filter\condition as condition_filter;
use tool_dynamicrule\tool_reportbuilder\filter\outcome as outcome_filter;
...
class rules_list extends system_report {
        ...
        if (permission::can_enable_rule($rule)) {
            ...
            if ($rule->is_enabled()) {
                ...
                $link = new \action_link(
                    new \moodle_url('#'), '', null,
                    ['title' => $title, 'data-action' => 'disable',
                        'data-id' => $row->id, 'data-wp-toggle' => 'on'],
                    new \pix_icon('toggle-on', '', 'tool_wp')
                );
            } else {
                if ($rule->is_shared() && !\tool_tenant\sharedspace::is_shared_space()) {
    ...
    public function get_display_name($value, \stdClass $row, $additionalarguments) {
        ...
        if (in_array($this->currentrule->get('tenantid'), hierarchy::get_parent_tenants_ids())) {
            $content .= '&nbsp;' . \html_writer::span(get_string('sharedspace', 'tool_tenant'), 'badge badge-secondary');
        }
        return $content;
    }
