...
class cohort_not_member extends \tool_dynamicrule\condition_sql {
    ...
    public function get_config_form(\MoodleQuickForm $mform) {
        ...
        $str = get_string('cohort', 'tool_dynamicrule');
        $mform->addElement('autocomplete', 'cohortid', $str, $cohorts, ['multiple' => false]);
    ...
    /**
     * Returns list of available cohorts
     *
     * @return array
    ...
    private function get_cohorts_list(): array {
        global $DB;
        $cohorts = $DB->get_records('cohort', ['contextid' => \context_system::instance()->id, 'visible' => 1]);
        $cohortslist = [];
