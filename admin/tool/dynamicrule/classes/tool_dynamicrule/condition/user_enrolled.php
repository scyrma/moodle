...
class user_enrolled extends \tool_dynamicrule\condition_sql {
    ...
    public function get_sql(): array {
        ...
        $enrol = $this->get_enrol();
        if (!empty($enrol)) {
            $penrol = \tool_dynamicrule\api::generate_param_name();
            $where .= " AND {$e}.enrol = :{$penrol}";
            $params[$penrol] = $enrol;
        }
        ...
        if (!is_null($timeenrolled)) {
            ...
            $where .= " AND (({$ue}.timestart = 0 AND {$ue}.timecreated >= :{$ptimec})
    ...
    public function get_description(): string {
        ...
        if ($enrol = $this->get_enrol()) {
            ...
            $str = get_string('conditionuserenrolleddescriptionwithenrol', 'tool_dynamicrule', $a);
        } else {
            $str = get_string('conditionuserenrolleddescription', 'tool_dynamicrule', $a);
        }
        return $str;
    }
    ...
    public function get_event_subscription() {
        return '\core\event\user_enrolment_created';
    }
}
