...
class user_profile_field extends \tool_dynamicrule\condition_sql {
    ...
    public function add_text_field(\MoodleQuickForm $mform, array &$group, \stdClass $field): void {
        ...
        $elements = [];
        $elements[] = $mform->createElement('select', $shortname . '_op', $field->name, $this->get_text_operators());
    ...
    public function add_date_field(\MoodleQuickForm $mform, array &$group, \stdClass $field): void {
        ...
        // Hide value field for all date operators excluding DATE_LAST_X_DAYS and DATE_NEXT_X_DAYS.
        $operators = array_diff_key($this->get_date_operators(), array_flip([self::DATE_LAST_X_DAYS, self::DATE_NEXT_X_DAYS]));
        $mform->hideIf($shortname . '_value', $shortname . '_op', 'in', array_keys($operators));

        // Hide operator2 field for all date operators excluding DATE_CURRENT, DATE_PREVIOUS, DATE_UPCOMING.
        $operators = array_diff_key($this->get_date_operators(),
            array_flip([self::DATE_CURRENT, self::DATE_PREVIOUS, self::DATE_UPCOMING]));
        $mform->hideIf($shortname . '_op2', $shortname . '_op', 'in', array_keys($operators));
    ...
    public function get_text_operators() : array {
        return [
            ...
            self::TEXT_IS_NOT_EMPTY => get_string('filterisnotempty', 'core_reportbuilder'),
        ];
    }
    ...
    public function get_date_operators() : array {
        return [
            self::DATE_ANY_VALUE => get_string('filterisanyvalue', 'core_reportbuilder'),
            self::DATE_IS_NOT_EMPTY => get_string('filterisnotempty', 'core_reportbuilder'),
            self::DATE_IS_EMPTY => get_string('filterisempty', 'core_reportbuilder'),
            self::DATE_IS_IN_THE_PAST => get_string('dateinthepast', 'tool_dynamicrule'),
            self::DATE_IS_IN_THE_FUTURE => get_string('dateinthefuture', 'tool_dynamicrule'),
            self::DATE_LAST_X_DAYS => get_string('datelastdays', 'tool_dynamicrule'),
            self::DATE_NEXT_X_DAYS => get_string('datenextdays', 'tool_dynamicrule'),
            self::DATE_CURRENT => get_string('filterdatecurrent', 'core_reportbuilder'),
            self::DATE_PREVIOUS => get_string('filterdatelast', 'core_reportbuilder'),
            self::DATE_UPCOMING => get_string('filterdatenext', 'core_reportbuilder'),
        ];
    }
    ...
    public function get_time_operators() : array {
        return [
            ...
            self::TIME_QUARTER => get_string('quarter', 'tool_dynamicrule'),
            self::TIME_YEAR => get_string('year'),
        ];
    }

    /**
     * Return a mapping of relative time period constants used internally.
     *
    ...
    public function validate_config_form(array $data): array {
        ...
        if (empty($data[$fieldvalue]) &&
                (($datatype == 'text' && !in_array($data[$fieldop], [self::TEXT_IS_EMPTY, self::TEXT_IS_NOT_EMPTY])) ||
                ($datatype == 'datetime' && in_array($data[$fieldop], [self::DATE_LAST_X_DAYS, self::DATE_NEXT_X_DAYS])))) {
            // Empty fieldvalue where it should not be empty.
            $errors['userprofilefieldgroup'] = get_string('errorinvaliduserprofilefieldvalue', 'tool_dynamicrule');
        }

        if ($datatype == 'datetime' && in_array($data[$fieldop], [self::DATE_LAST_X_DAYS, self::DATE_NEXT_X_DAYS]) &&
                !(is_numeric($data[$fieldvalue]) && $data[$fieldvalue] > 0)) {
            // Zero or negative numeric value for last/next days datetime element.
            $errors['userprofilefieldgroup'] = get_string('errorinvaliduserprofilefieldvalue', 'tool_dynamicrule');
        }
    ...
    private function get_profile_fields_info(bool $checkcapability = true): array {
        ...
        $res['department'] = (object)['shortname' => 'department', 'name' => get_string('department'), 'datatype' => 'text',
            'paramtype' => core_user::get_property_type('department'), ];
        ...
        $res['lang'] = (object)['shortname' => 'lang', 'name' => get_string('language'), 'datatype' => 'menu',
            'param1' => get_string_manager()->get_list_of_translations(), ];
        ...
        foreach ($pfields as $pfield) {
            ...
            // Format string name (especially important if multilang filter is used).
            $field->name = format_string($field->name, true,
    ...
    public function get_sql(): array {
        ...
        if (in_array($field, self::$defaultfields)) {
            ...
            $ud = database::generate_alias();
            $uif = database::generate_alias();
            $uid = database::generate_alias();
            ...
            if (!empty($where)) {
                ...
                $userprofilefield = database::generate_param_name();
    ...
    public function get_datetime_sql(string $alias) : array {
        ...
        switch($operator) {
            ...
            case self::DATE_CURRENT: // Current [day/week/month/quarter/year/financial year].
                $res = "{$alias}.datetime >= :$param AND {$alias}.datetime <= :$param2";
                [$params[$param], $params[$param2]] = self::get_start_and_end_timestamp_for('current',
                    $this->get_time_period_name($operator2));
                return [$res, $params];
                break;
            case self::DATE_PREVIOUS: // Previous [day/week/month/quarter/year/financial year].
                $res = "{$alias}.datetime >= :$param AND {$alias}.datetime <= :$param2";
                [$params[$param], $params[$param2]] = self::get_start_and_end_timestamp_for('previous',
                    $this->get_time_period_name($operator2));
                return [$res, $params];
                break;
            case self::DATE_UPCOMING: // Upcoming [day/week/month/quarter/year/financial year].
                $res = "{$alias}.datetime >= :$param AND {$alias}.datetime <= :$param2";
                [$params[$param], $params[$param2]] = self::get_start_and_end_timestamp_for('upcoming',
                    $this->get_time_period_name($operator2));
                return [$res, $params];
                break;
            default:
    ...
    public function get_menu_checkbox_sql(string $alias, string $field, string $field2 = null) : array {
        global $DB;
        $userprofilevalue = database::generate_param_name();
        if (isset($field2)) {
    ...
    public function get_description(): string {
        ...
        if ($datatype == 'datetime' && in_array($opint, [self::DATE_LAST_X_DAYS, self::DATE_NEXT_X_DAYS])) {
            $identifier = ($opint == self::DATE_LAST_X_DAYS) ? 'datelast' : 'datenext';
            $identifier .= ($this->get_userprofilefield_value() > 1) ? 'plural' : '';
            $a->fieldvalue = get_string($identifier, 'tool_dynamicrule', $this->get_userprofilefield_value());
    ...
    private static function last_day_of(string $period, DateTime $date): int {
        $period = strtolower($period);
        $validperiods = ['year', 'quarter', 'month'];
