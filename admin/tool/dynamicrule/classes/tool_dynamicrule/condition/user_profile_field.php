...
class user_profile_field extends \tool_dynamicrule\condition_sql {
     ...
     */
    private static $defaultfields = ['lastname', 'firstname', 'username', 'email', 'city', 'idnumber', 'country'];
    ...
    private function get_profile_fields_info(): array {
        ...
        // Visibility checks. According to user profile fields User with moodle/user:viewalldetails but without
        // moodle/user:update can view all profile fields.
        $canviewallfields = has_capability('moodle/user:viewalldetails', \context_system::instance());
        ...
        $params = [];
        if (!$canviewallfields) {
            $sql .= ' AND f.visible = :visibility';
            $params = ['visibility' => PROFILE_VISIBLE_ALL];
        }
        $sql .= ' ORDER BY c.sortorder, f.sortorder';
        $res1 = $DB->get_records_sql($sql, $params);
    ...
    public function get_text_sql(string $alias, string $field) : array {
        ...
        if (is_null($operator) ||
            ($value === '' && $operator != self::TEXT_IS_EMPTY && $operator != self::TEXT_IS_NOT_EMPTY)) {
            // Configuration is invalid.
            return ['', []];
        }
    ...
    public function is_configuration_valid(): bool {
        ...
        return in_array($field, self::$defaultfields) ||
            $DB->record_exists('user_info_field', ['shortname' => $field]);
    }
    ...
    public function user_can_add(): bool {
        ...
        if (\tool_tenant\permission::can_browse_users()) {
            ...
            if (!empty($DB->get_records('user_info_field', ['visible' => PROFILE_VISIBLE_ALL]))) {
                return true;
            }
        }
        return false;
    ...
    public function user_can_edit(array $configdata): bool {
        ...
        if (\tool_tenant\permission::can_browse_users()) {
            ...
            if (!in_array($configdata['userprofilefield'], self::$defaultfields)) {
                ...
                $field = $DB->get_record('user_info_field', ['shortname' => $configdata['userprofilefield']]);
                require_once($CFG->dirroot . '/user/profile/lib.php');
                require_once($CFG->dirroot . '/user/profile/field/' . $field->datatype . '/field.class.php');
                $classname = 'profile_field_' . $field->datatype;
                $fieldobject = new $classname($field->id, null, $field);
                return $fieldobject->is_visible();
            }
        }
        return false;
    }
