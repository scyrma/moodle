...
class course_last_access extends \tool_dynamicrule\condition_sql {
    ...
    public function get_sql(): array {
        ...
        $e = database::generate_alias();
        $la = database::generate_alias();

        $courseid = database::generate_param_name();
        $time = database::generate_param_name();
    ...
    public function get_custom_data_for_outcome(array $keys, \stdClass $user, outcome_base $calleroutcome): array {
        ...
        foreach ($keys as $key) {
            if ($key === 'courseid') {
                ...
                $data[$key] = $course->fullname;
            } else if ($key === 'courseshortname') {
                $data[$key] = $course->shortname;
                ...
                $data[$key] = $lastaccesstime ? userdate($lastaccesstime, get_string('strftimedatefullshort')) : '';
