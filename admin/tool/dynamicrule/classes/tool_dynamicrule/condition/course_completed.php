...
class course_completed extends \tool_dynamicrule\condition_sql {
    ...
    /** @var string Any course grade */
    const GRADE_ANY = 'any';
    /** @var string Less than the given course grade */
    const GRADE_LESS_THAN = 'lessthan';
    /** @var string Greater than the given course grade */
    const GRADE_GREATER_THAN = 'greaterthan';
    /** @var string Course grade between the given range of values */
    const GRADE_RANGE = 'range';
    ...
    public function get_config_form(\MoodleQuickForm $mform) {
        ...
        $gradeoperator = [
            self::GRADE_ANY => get_string('any', 'tool_dynamicrule'),
            self::GRADE_LESS_THAN => get_string('lessthan', 'tool_dynamicrule'),
            self::GRADE_GREATER_THAN => get_string('greaterthan', 'tool_dynamicrule'),
            self::GRADE_RANGE => get_string('range', 'tool_dynamicrule'),
        ];
        // Static elements don't seem to work properly with hideIf, so we need to use groups (MDL-66251).
        $elements = $elements2 = [];
        $elements[] = $mform->createElement('select', 'coursegrade_selector', '', $gradeoperator);
        $elements2[] = $mform->createElement('text', 'coursegrade_value', '', ['size' => 3]);
        $elements2[] = $mform->createElement('text', 'coursegrade_value2', '', ['size' => 3]);
        $elements2[] = $mform->createElement('static', 'coursegrade_percentage', '', '%');
        $elements[] = $mform->createElement('group', 'gradegroup2', '', $elements2, '', false);
        $group[] = $mform->createElement('group', 'gradegroup', '', $elements, '', false);
        $mform->addGroup($group, 'gradegroup', get_string('coursegrade', 'completion'), '', false);
        $mform->setType('coursegrade_value', PARAM_FLOAT);
        $mform->setType('coursegrade_value2', PARAM_FLOAT);
        $mform->hideIf('coursegrade_value', 'coursegrade_selector', 'eq', self::GRADE_ANY);
        $mform->hideIf('coursegrade_value2', 'coursegrade_selector', 'neq', self::GRADE_RANGE);
        $mform->hideIf('gradegroup2', 'coursegrade_selector', 'eq', self::GRADE_ANY);
        $mform->addHelpButton('gradegroup', 'coursegrade', 'tool_dynamicrule');
    }
    ...
    public function validate_config_form(array $data): array {
        ...
        // Check grade values are valid.
        if (in_array($data['coursegrade_selector'], [self::GRADE_GREATER_THAN, self::GRADE_LESS_THAN, self::GRADE_RANGE]) &&
            (!is_numeric($data['coursegrade_value']) || $data['coursegrade_value'] < 0 || $data['coursegrade_value'] > 100)) {
            $errors['gradegroup'] = get_string('errorinvalidvalue', 'tool_dynamicrule');
        }
        if ((string)$data['coursegrade_selector'] === self::GRADE_RANGE && (!is_numeric($data['coursegrade_value2']) ||
            $data['coursegrade_value2'] < $data['coursegrade_value'] || $data['coursegrade_value2'] > 100)) {
            $errors['gradegroup'] = get_string('errorinvalidvalue', 'tool_dynamicrule');
        }
    ...
    public function get_sql(): array {
        ...
        $cc = database::generate_alias();

        $join = "JOIN {course_completions} {$cc}
                   ON ({$cc}.userid = u.id)";

        $courseparam = database::generate_param_name();
        ...
        if ($conditionoperator != self::OPERATOR_ANYTIME && $conditiondate > 0) {
            ...
            $timeparam = database::generate_param_name();
        ...
        // Check if grade selector is enabled.
        $gradeoperator = $this->get_coursegrade_selector();
        if ($gradeoperator !== self::GRADE_ANY) {
            $gradevalue = $this->get_coursegrade_value();
            $gradeparam = database::generate_param_name();
            $gi = database::generate_alias();
            $gg = database::generate_alias();
            ...
            $join .= implode(' ', $gradejoins);
            $params[$gradeparam] = $gradevalue;

            switch ($gradeoperator) {
                case self::GRADE_LESS_THAN:
                    $where .= " AND ROUND((({$gg}.finalgrade * 100) / {$gg}.rawgrademax), 5) < :{$gradeparam}";
                    break;
                case self::GRADE_GREATER_THAN:
                    $where .= " AND ROUND((({$gg}.finalgrade * 100) / {$gg}.rawgrademax), 5) > :{$gradeparam}";
                    break;
                case self::GRADE_RANGE:
                    $gradeparam2 = database::generate_param_name();
                    $gradevalue2 = $this->get_coursegrade_value2();
                    $where .= " AND ROUND((({$gg}.finalgrade * 100) / {$gg}.rawgrademax), 5)
                        BETWEEN :{$gradeparam} AND :{$gradeparam2}";
                    $params[$gradeparam2] = $gradevalue2;
                    break;
                default:
                    throw new \coding_exception('Invalid grade operator');
            }
        }
    ...
    /**
     * Return the configured coursegrade selector.
     *
     * @return string
     */
    private function get_coursegrade_selector(): string {
        return $this->get_configdata()['coursegrade_selector'] ?? self::GRADE_ANY;
    }

    /**
     * Return the configured course grade value.
     *
     * @return float
     */
    private function get_coursegrade_value(): float {
        return unformat_float($this->get_configdata()['coursegrade_value'] ?? 0);
    }

    /**
     * Return the configured second course grade value for the range option.
     *
     * @return float
     */
    private function get_coursegrade_value2(): float {
        return unformat_float($this->get_configdata()['coursegrade_value2'] ?? 0);
    }
    ...
    public function get_description(): string {
        ...
        $gradeoperator = $this->get_coursegrade_selector();
        if ($gradeoperator === self::GRADE_RANGE) {
            $description .= '<br />' . get_string('conditioncoursecompletedgraderange', 'tool_dynamicrule',
                ['value' => $this->get_coursegrade_value(), 'value2' => $this->get_coursegrade_value2()]);
        } else if ($gradeoperator !== self::GRADE_ANY) {
            $description .= '<br />' . get_string('conditioncoursecompletedgrade' . $gradeoperator, 'tool_dynamicrule',
                $this->get_coursegrade_value());
        }
     ...
     * @return array
     */
    public function get_custom_data_for_outcome(array $keys, \stdClass $user, outcome_base $calleroutcome): array {
        global $CFG, $DB;
        ...
        foreach ($keys as $key) {
            if ($key === 'courseid') {
                ...
                $data[$key] = format_string($course->fullname, true,
                    ['context' => \context_course::instance($course->id), 'escape' => false]);
            } else if ($key === 'courseshortname') {
                $data[$key] = format_string($course->shortname, true,
                    ['context' => \context_course::instance($course->id), 'escape' => false]);
            } else if ($key === 'courseurl') {
                $data[$key] = course_get_url($this->get_courseid())->out(false);
                ...
                $data[$key] = $completiondate ? userdate($completiondate, get_string('strftimedatefullshort')) : '';
        ...
        if ($coursecustomfieldfound) {
            ...
            foreach ($handler->get_instance_data($this->get_courseid(), true) as $instance) {
                ...
                if (in_array($issuefieldname, $keys)) {
                    $data[$issuefieldname] = $instance->export_value();
                }
            }
        }
        return $data;
