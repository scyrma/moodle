...
class course_completed extends \tool_dynamicrule\condition_sql {
    ...
    public function get_config_form(\MoodleQuickForm $mform) {
        ...
        $operators = [
            'after' => get_string('operatorafter', 'tool_dynamicrule'),
            'before' => get_string('operatorbefore', 'tool_dynamicrule'),
        ];
        $mform->addElement('select', 'operator', get_string('operator', 'tool_dynamicrule'), $operators);
        $mform->addRule('operator', null, 'required', null, 'client');

        $options = ['optional' => true];
        $mform->addElement('date_time_selector', 'timecompleted', get_string('timecompleted', 'tool_dynamicrule'), $options);
        $mform->setDefault('timecompleted', time());
    }
    ...
    public function validate_config_form(array $data): array {
        ...
        if (empty($data['operator']) || (($data['operator'] != 'after') && ($data['operator'] != 'before'))) {
    ...
    public function get_sql(): array {
        ...
        if (!is_null($timecompleted)) {
            ...
            if ($this->get_configdata()['operator'] == 'after') {
                $operator = '>';
            } else {
                $operator = '<';
            }
            ...
            $params[$timeparam] = $timecompleted;
        } else {
    ...
    private function get_conditiondate(): ?int {
        return $this->get_configdata()['timecompleted'] ?? null;
    }
    ...
    private function get_conditionoperator(): string {
        return $this->get_configdata()['operator'];
    }
    ...
    public function get_description(): string {
        if ($date = $this->get_conditiondate()) {
            $strid = 'conditioncoursecompleteddescriptionwithdate';
            $strparams = ['coursename' => $this->get_coursename()];
            switch ($this->get_conditionoperator()) {
                case 'before':
                    $strparams['datetype'] = get_string('operatorbefore', 'tool_dynamicrule');
                    break;
                case 'after':
                    $strparams['datetype'] = get_string('operatorafter', 'tool_dynamicrule');
                    break;
            }
            $strparams['conditiondate'] = userdate($date, get_string('strftimedatefullshort'));
            $description = get_string($strid, 'tool_dynamicrule', $strparams);
        } else {
            $strid = 'conditioncoursecompleteddescription';
            $description = get_string($strid, 'tool_dynamicrule', $this->get_coursename());
        }
     ...
     * @return int|null
     */
    private function get_timecompleted() {
        if (isset($this->get_configdata()['timecompleted'])) {
            return $this->get_configdata()['timecompleted'];
        }
        return null;
    }
    ...
    public function get_available_data_for_outcome(outcome_base $calleroutcome): array {
        $data = [
            ...
            'coursegrade' => new \lang_string('grade'),
        ];
        ...
        foreach ($handler->get_instance_data($this->get_courseid(), true) as $instancedata) {
            ...
            $data[$issuefieldname] = $instancedata->get_field()->get_formatted_name();
        }
        return $data;
    }
