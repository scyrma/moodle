...
class competency extends \tool_dynamicrule\outcome_base {
    ...
    public function get_uneditable_description(): string {
        $competency = new \core_competency\competency($this->get_configdata()['competency']);

        if (has_capability('moodle/competency:competencyview', $competency->get_context())) {
            // User can view this competency.
