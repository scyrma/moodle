...
class notification extends \tool_dynamicrule\outcome_base {
    ...
    public function apply_to_users(array $users) {
        ...
        foreach ($users as $userto) {
            $userto = \core_user::get_user($userto->id); // We need full user record to have all mail preferences.
            $placeholdersdata = $this->get_user_data_for_placeholders($userto) + $this->get_site_data_for_placeholders() +
                $conditionsdata[$userto->id];
     ...
     * @return array
     */
    public function get_available_site_placeholders() {
        return [
            'sitefullname' => get_string('fullsitename', 'moodle'),
            'siteshortname' => get_string('siteshortname', 'tool_dynamicrule'),
            'sitelink' => get_string('sitelink', 'tool_dynamicrule'),
        ];
    }
