...
class notification extends \tool_dynamicrule\outcome_base {
    ...
    public function get_config_form(\MoodleQuickForm $mform) {
        ...
        $elements = [
            ...
            $mform->createElement('advcheckbox', 'manualmanager', get_string('sendtomanualmanager', 'tool_dynamicrule')),
        ];
    ...
    public function apply_to_user(\stdClass $user): void {
        ...
        if (!empty($this->configdata['sendto']['manualmanager'])) {
            $managers = organisation::get_user_assigned_managers($user->id);
            $usersto = array_merge($usersto, $managers);
        }

        $langs = [];
        foreach ($usersto as $userto) {
            if (!array_key_exists($userto->lang, $langs)) {
                // Force recipient user language for message content rendering.
                // We also store previous language force state in case it is forced already elsewhere.
                ...
                $placeholdersdata = $this->get_user_data_for_placeholders($user) + $this->get_site_data_for_placeholders() +
                    $this->get_custom_data_from_conditions($this->conditionsplaceholders, $user);
                $usermessage = $this->get_body($placeholdersdata);
                $usersubject = $this->get_subject($placeholdersdata);

                // Restore language forcing state to the one we stored before.
                force_current_language($oldforcelang);

                $eventdata = clone($this->eventdata);
                ...
                // Store object to use with another receipient that has same language.
                $langs[$userto->lang] = $eventdata;
            } else {
                // We have the text rendered in this language already, use it.
                $eventdata = $langs[$userto->lang];
            }
    ...
    public function get_available_site_placeholders(): array {
        return [
            ...
            'sitelinkspecific' => new \lang_string('sitelinkspecific', 'tool_dynamicrule'),
        ] : []);
    }
    ...
    private function get_site_data_for_placeholders() {
        ...
        $wwwroot = tenancy::get_tenant_wwwroot(config::get_tenant_id());
        $extradata = [];
        if (tenancy::is_site_multi_tenant()) {
            $tenant = (new \tool_tenant\manager())->get_tenants()[config::get_tenant_id()];
            $loginurls = $tenant->get_login_urls(false);
            $extradata = ['sitelinkspecific' => $loginurls ? reset($loginurls) : $wwwroot];
        }
        return [
