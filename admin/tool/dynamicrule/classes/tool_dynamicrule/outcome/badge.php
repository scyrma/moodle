...
class badge extends \tool_dynamicrule\outcome_base {
    ...
    public function setup_for_applying(): void {
        ...
        // When issuing as admin, use the first role from the list.
        $this->roleid = array_key_first($this->badge->criteria[BADGE_CRITERIA_TYPE_MANUAL]->params);
    }
    ...
    public function apply_to_user(\stdClass $user): void {
        global $USER;
        if (process_manual_award($user->id, $USER->id, $this->roleid, $this->badge->id)) {
            // If badge was successfully awarded, review manual badge criteria.
            $data = new \stdClass();
            $data->crit = $this->badge->criteria[BADGE_CRITERIA_TYPE_MANUAL];
            $data->userid = $user->id;
    ...
    public function get_uneditable_description(): string {
        $badge = new \core_badges\badge($this->get_configdata()['badge']);
        if (has_capability('moodle/badges:viewbadges', $badge->get_context())) {
