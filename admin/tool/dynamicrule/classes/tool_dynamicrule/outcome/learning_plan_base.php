...
/**
 * The backend abstract class for the outcome for assigning and removing a learning plan.
 *
 ...
 */
abstract class learning_plan_base extends \tool_dynamicrule\outcome_base {
    ...
    public function get_broken_description(): string {
        return get_string('outcomelearningplanbroken', 'tool_dynamicrule', $this->get_configdata()['learningplan']);
    }
    ...
    public function is_configuration_valid(): bool {
        $configuration = $this->get_configdata();
        return array_key_exists('learningplan', $configuration) && template::record_exists($configuration['learningplan']);
    }
     ...
     */
    protected function get_lptemplate(): ?template {
    ...
    private function get_selected(): array {
        $selected = [];
        if ($this->is_configuration_valid() && $template = $this->get_lptemplate()) {
            $selected[$template->get('id')] = format_string($template->get('shortname'), true, [
    ...
    public function add_exporter_mapping(exporter_base $exporter): void {
        $exporter->add_mapping('learningplan', $this->get_configdata()['learningplan']);
    }
     ...
     *
     * Return true if there are learning plan templates.
     *
     * @return bool
     */
    public static function is_available(): bool {
        // Check if there are any learning plan templates.
        return (template::count_records() > 0);
    }

    /**
     * Should this rule be processed in scheduled tasks despite condition event status.
     *
     * We need admin permissions to process this rule.
     *
     * @return bool
     */
