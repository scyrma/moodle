...
function xmldb_tool_dynamicrule_upgrade($oldversion) {
    ...
    if ($oldversion < 2020040800) {
        ...
        $index = new xmldb_index('enarbr', XMLDB_INDEX_NOTUNIQUE, ['enabled', 'archived', 'broken']);
        ...
        // Re-adding index.
        $index = new xmldb_index('enabledarchived', XMLDB_INDEX_NOTUNIQUE, ['enabled', 'archived']);
        if (!$dbman->index_exists($table, $index)) {
    ...
    if ($oldversion < 2022040103) {
        ...
        upgrade_plugin_savepoint(true, 2022040103, 'tool', 'dynamicrule');
    }

    if ($oldversion < 2022042001) {

        // Define field includesuspendedusers to be added to tool_dynamicrule.
        $table = new xmldb_table('tool_dynamicrule');
        $field = new xmldb_field('includesuspendedusers', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '1', 'shared');

        // Conditionally launch add field includesuspendedusers.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Dynamicrule savepoint reached.
        upgrade_plugin_savepoint(true, 2022042001, 'tool', 'dynamicrule');
    }

    if ($oldversion < 2022091401) {
        // Update tool_organisation:user_not_in_department conditions criteria to "any".
        $results = $DB->get_records('tool_dynamicrule_condition',
            ['classname' => 'tool_organisation\tool_dynamicrule\condition\user_not_in_department']);
        foreach ($results as $result) {
            $configdata = @json_decode($result->configdata, true);
            $configdata['criteria'] = 'any';
            $result->configdata = json_encode($configdata);
            $DB->update_record('tool_dynamicrule_condition', $result);
        }

        // Update tool_organisation:user_without_position conditions criteria to "any".
        $results = $DB->get_records('tool_dynamicrule_condition',
            ['classname' => 'tool_organisation\tool_dynamicrule\condition\user_without_position']);
        foreach ($results as $result) {
            $configdata = @json_decode($result->configdata, true);
            $configdata['criteria'] = 'any';
            $result->configdata = json_encode($configdata);
        ...
        upgrade_plugin_savepoint(true, 2022091401, 'tool', 'dynamicrule');
    }

    if ($oldversion < 2023053011) {

        // Fix manual issue badges.
        tool_dynamicrule_fix_manual_issue_badges();

        upgrade_plugin_savepoint(true, 2023053011, 'tool', 'dynamicrule');
    }
