...
class tool_dynamicrule_rule_testcase extends advanced_testcase {
    ...
    public function test_is_broken() {
        // Rule is not broken by default.
        $rule0 = \tool_dynamicrule\api::get_rule($this->get_generator()->create_rule()->id);
        $this->assertFalse($rule0->is_broken());

        $rule1 = \tool_dynamicrule\api::get_rule($this->get_generator()->create_rule(['broken' => 1])->id);
        $this->assertTrue($rule1->is_broken());
    }
