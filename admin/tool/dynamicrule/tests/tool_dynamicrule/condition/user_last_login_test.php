...
class user_last_login_test extends advanced_testcase {
    ...
    public function test_validate_config_form() {
        ...
        $condition = user_last_login::instance();
        // No type selected.
        $configform = ['lastlogintype' => user_last_login::LAST_LOGIN_TYPE_NONE];
        $this->assertArrayHasKey('lastloginformgroup', $condition->validate_config_form($configform));

        // 0 days/weeks/years.
        $configform = ['lastlogintype' => user_last_login::LAST_LOGIN_TYPE_INLAST, 'lastloginrelative' => '0 days'];
        $this->assertArrayHasKey('lastloginformgroup', $condition->validate_config_form($configform));

        $configform = ['lastlogintype' => user_last_login::LAST_LOGIN_TYPE_PAST, 'lastloginrelative' => '0 weeks'];
        $this->assertArrayHasKey('lastloginformgroup', $condition->validate_config_form($configform));

        // Valid cases.
        $configform = ['lastlogintype' => user_last_login::LAST_LOGIN_TYPE_EVER];
        $this->assertArrayNotHasKey('lastloginformgroup', $condition->validate_config_form($configform));

        $configform = ['lastlogintype' => user_last_login::LAST_LOGIN_TYPE_PAST, 'lastloginrelative' => '1 year'];
        $this->assertArrayNotHasKey('lastloginformgroup', $condition->validate_config_form($configform));
    }
