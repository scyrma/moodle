...
class course_completed_test extends \advanced_testcase {
    ...
    public function test_validate_config_form() {
        ...
        $condition = $this->get_generator()->create_condition(course_completed::class, $rule->id, []);
        $configform = ['operator' => course_completed::OPERATOR_AFTER, 'coursegrade_selector' => course_completed::GRADE_ANY];
        $this->assertArrayNotHasKey('courseid', $condition->validate_config_form(
            $configform + ['courseid' => $subcatcourse->id]));
        $this->assertArrayNotHasKey('courseid', $condition->validate_config_form(
            $configform + ['courseid' => $subtenant0catcourse->id]));
        ...
        $this->assertArrayNotHasKey('courseid', $condition->validate_config_form(
            $configform + ['courseid' => $subcatcourse->id]));
        $this->assertArrayHasKey('courseid', $condition->validate_config_form(
            $configform + ['courseid' => $subtenant0catcourse->id]));

        // Valid course with invalid operator.
        $configform = ['courseid' => $subcatcourse->id, 'operator' => 'invalid',
            'coursegrade_selector' => course_completed::GRADE_ANY, ];
    ...
    public function test_get_matching_users() {
        ...
        $completion = new completion_completion(['course' => $completedcourse->id, 'userid' => $user1->id]);
        $completion->mark_complete($timenow - DAYSECS);
        $completion = new completion_completion(['course' => $completedcourse->id, 'userid' => $user2->id]);
        $completion->mark_complete($timenow + DAYSECS);
    ...
    /**
     * Test condition matching with grade
     *
     * @uses \tool_dynamicrule\api::count_matching_users
     * @uses \tool_dynamicrule\api::get_matching_users
     */
    public function test_get_matching_users_with_grade(): void {
        global $DB;
        ...
        $assign = $this->getDataGenerator()->create_module('assign', ['course' => $completedcourse->id], ['completion' => 1]);
        $cm = get_coursemodule_from_id('assign', $assign->cmid);
        $context = \context_module::instance($cm->id);
        $assign = new \assign($context, $cm, $completedcourse);

        // Create and grade some submissions from the students.
        $cm = $assign->get_course_module();

        $teacher = $this->getDataGenerator()->create_and_enrol($completedcourse, 'teacher');
        $user1 = $this->getDataGenerator()->create_and_enrol($completedcourse, 'student');
        $user2 = $this->getDataGenerator()->create_and_enrol($completedcourse, 'student');

        $this->setUser($teacher);

        // Give the submission a grade for user1.
        $data = (object)[
            'attemptnumber' => 0,
            'grade' => '54.00',
            'assignfeedbackcomments_editor' => ['text' => 'Comment on attempt 1.', 'format' => FORMAT_MOODLE],
        ];
        $assign->save_grade($user1->id, $data);

        // Give the submission a grade for user2.
        $data->grade = '75.00';
        ...
        $completion->update_state($cm, COMPLETION_COMPLETE, $user1->id);

        $this->setUser($user2);
        $completion = new \completion_info($completedcourse);
        $completion->update_state($cm, COMPLETION_COMPLETE, $user2->id);
        ...
        // Create rule and condition.
        $rule1 = $this->get_generator()->create_rule();
        $configdata = ['courseid' => $completedcourse->id, 'timecompleted' => $timenow];
        $condition = course_completed::create($rule1->id, $configdata + ['coursegrade_selector' => course_completed::GRADE_ANY]);
        ...
        // Now match users with a grade more than 60.
        $condition->update_configdata($configdata + [
            'coursegrade_selector' => course_completed::GRADE_GREATER_THAN,
            'coursegrade_value' => '60',
        ], true);
        $this->assertEquals(1, \tool_dynamicrule\api::count_matching_users($rule1->id));
        $users = \tool_dynamicrule\api::get_matching_users($rule1->id);
        $this->assertEquals($user2->id, reset($users)->id);

        // Now match users with a grade less than 70.
        $condition->update_configdata($configdata + [
            'coursegrade_selector' => course_completed::GRADE_LESS_THAN,
            'coursegrade_value' => '70',
        ], true);
        $this->assertEquals(1, \tool_dynamicrule\api::count_matching_users($rule1->id));
        $users = \tool_dynamicrule\api::get_matching_users($rule1->id);
        $this->assertEquals($user1->id, reset($users)->id);

        // Now match users with a grade between 70 and 80.
        $condition->update_configdata($configdata + [
            'coursegrade_selector' => course_completed::GRADE_RANGE,
            'coursegrade_value' => '70',
            'coursegrade_value2' => '80',
        ], true);
        $this->assertEquals(1, \tool_dynamicrule\api::count_matching_users($rule1->id));
        $users = \tool_dynamicrule\api::get_matching_users($rule1->id);
        $this->assertEquals($user2->id, reset($users)->id);

        // Now match users with a grade less than 10.
        $condition->update_configdata($configdata + [
            'coursegrade_selector' => course_completed::GRADE_LESS_THAN,
            'coursegrade_value' => '10',
        ], true);
        $this->assertEquals(0, \tool_dynamicrule\api::count_matching_users($rule1->id));
    }
    ...
    public function test_get_matching_users_shared_rule() {
        ...
        $completion = new completion_completion(['course' => $completedcourse->id, 'userid' => $user1->id]);
        $completion->mark_complete($timenow - DAYSECS);
        $completion = new completion_completion(['course' => $completedcourse->id, 'userid' => $user21->id]);
        $completion->mark_complete($timenow + DAYSECS);
    ...
    public function test_trigger_rule_processing() {
        ...
        $ccompletion = new \completion_completion(['course' => $completedcourse->id, 'userid' => $user0->id]);
    ...
    /**
     * Test rule unmatches after course reset
     *
     * @return void
     */
    public function test_trigger_rule_processing_after_course_reset(): void {
        // Course with self-completion enabled.
        $completedcourse = $this->getDataGenerator()->create_course(['enablecompletion' => COMPLETION_ENABLED]);
        /** @var \completion_criteria_self $criterion */
        $criterion = \completion_criteria::factory(['criteriatype' => COMPLETION_CRITERIA_TYPE_SELF]);
        $criteriadata = (object)['id' => $completedcourse->id, 'criteria_self' => COMPLETION_CRITERIA_TYPE_SELF];
        $criterion->update_config($criteriadata);
        ...
        $this->get_generator()->assert_user_did_not_match_rule($this, $user0->id, $rule0->id);
        ...
        $this->get_generator()->assert_user_matched_rule($this, $user0->id, $rule0->id);

        // Reset the course completion.
        reset_course_userdata((object)['id' => $completedcourse->id, 'reset_completion' => true]);

        // Process the rule.
        $ruleinstance = new \tool_dynamicrule\rule(0, $rule0);
        \tool_dynamicrule\api::process_rule($ruleinstance);

        // User does not match the rules anymore.
        $this->get_generator()->assert_user_did_not_match_rule($this, $user0->id, $rule0->id, 1);
        ...
        // User matches the rule now.
        $this->get_generator()->assert_user_matched_rule($this, $user0->id, $rule0->id, 2);
    }
    ...
    public function test_get_description() {
        ...
        $description = get_string('conditioncoursecompleteddescription', 'tool_dynamicrule', $course1->fullname);
        $this->assertEquals($description, $condition1->get_description());
        $this->assertNotEmpty($condition1->get_description());

        $condition1->update_configdata($configdata + [
            'coursegrade_selector' => course_completed::GRADE_LESS_THAN,
            'coursegrade_value' => '60',
        ], true);
        $description1 = $description . '<br />' . get_string('conditioncoursecompletedgradelessthan', 'tool_dynamicrule', '60');
        $this->assertEquals($description1, $condition1->get_description());

        $condition1->update_configdata($configdata + [
            'coursegrade_selector' => course_completed::GRADE_GREATER_THAN,
            'coursegrade_value' => '60',
        ], true);
        $description1 = $description . '<br />' . get_string('conditioncoursecompletedgradegreaterthan', 'tool_dynamicrule', '60');
        $this->assertEquals($description1, $condition1->get_description());

        $condition1->update_configdata($configdata + [
            'coursegrade_selector' => course_completed::GRADE_RANGE,
            'coursegrade_value' => '60',
            'coursegrade_value2' => '90',
        ], true);
        $description1 = $description . '<br />' . get_string('conditioncoursecompletedgraderange', 'tool_dynamicrule',
            ['value' => 60, 'value2' => 90]);
        $this->assertEquals($description1, $condition1->get_description());
    }
