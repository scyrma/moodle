 ...
 */
class user_enrolled_test extends \advanced_testcase {
    ...
    public function test_validate_config_form_tenant_courses() {
        ...
        $condition = $this->get_generator()->create_condition(user_enrolled::class, $rule->id, []);
    ...
    public function test_get_matching_users_course_timeenrolled() {
        ...
        $timestart = strtotime('-3 days');

        $this->user4 = $this->getDataGenerator()->create_and_enrol($this->course1, 'student', null, 'manual', $timestart);

        // In a course, enrolment starting 3 days ago.
        $configdata = ['courseid' => $this->course1->id, 'timeenrolled' => $timestart];
        user_enrolled::create($this->rule1->id, $configdata);

        // User3 did not match condition because their start date was 7 days ago.  Users 1, 2 and 4 matched.
        $this->assertEquals(3, \tool_dynamicrule\api::count_matching_users($this->rule1->id));
        $users = \tool_dynamicrule\api::get_matching_users($this->rule1->id);
        $this->assertEqualsCanonicalizing([$this->user1->id, $this->user2->id, $this->user4->id], array_column($users, 'id'));
    }
    ...
    public function test_get_matching_users_shared_rule() {
        ...
        $this->getDataGenerator()->enrol_user($user1->id, $this->course1->id, 'student', 'manual');
        $this->getDataGenerator()->enrol_user($user21->id, $this->course1->id, 'student', 'manual');

        $rule2 = $this->get_generator()->create_rule(['tenantid' => $sharedspaceid]);
        $configdata = ['courseid' => $this->course1->id];
        user_enrolled::create($rule2->id, $configdata);

        // Both tenant users should appear in matching.
        $this->assertEquals(5, \tool_dynamicrule\api::count_matching_users($rule2->id));
        $users = \tool_dynamicrule\api::get_matching_users($rule2->id);
        $this->assertEqualsCanonicalizing([$user1->id, $user21->id, $this->user1->id, $this->user2->id, $this->user3->id],
    ...
    /**
     * Test user enrolled/enrolment updated events are triggering rule processing.
     */
    public function test_trigger_rule_processing_unmatch() {
        global $DB;

        $course0 = $this->getDataGenerator()->create_course();
        $user0 = $this->getDataGenerator()->create_user();

        // Create rule0 with course0 enrol conditon and dummy outcome.
        $rule0 = $this->get_generator()->create_rule(['enabled' => 1]);
        $configdata = ['courseid' => $course0->id, 'enrol' => 'manual'];
        user_enrolled::create($rule0->id, $configdata);
        $this->get_generator()->create_outcome_donothing($rule0->id);

        // Enrol user, he is immediately matched.
        $this->getDataGenerator()->enrol_user($user0->id, $course0->id, 'student', 'manual');
        $this->get_generator()->assert_user_matched_rule($this, $user0->id, $rule0->id);

        // Update enrolment and set start date in the future, user will be unmatched.
        $manual = $DB->get_record('enrol', ['courseid' => $course0->id, 'enrol' => 'manual'], '*', MUST_EXIST);
        enrol_get_plugin('manual')->update_user_enrol($manual, $user0->id, null, time() + 22 * DAYSECS, null);
        $this->get_generator()->assert_user_did_not_match_rule($this, $user0->id, $rule0->id, 1);
    }

    /**
     * Test watching enrolment start date
     */
    public function test_trigger_rule_datewatch() {
        ...
        // Enrol user, he is now matched.
        $this->getDataGenerator()->enrol_user($user0->id, $course0->id, 'student', 'manual', time() + 22 * DAYSECS);
        $this->get_generator()->assert_user_did_not_match_rule($this, $user0->id, $rule0->id);

        // Time travel to the future and run datewatch task, user will be matched.
        $delta = 25 * DAYSECS;
        if (extension_loaded('uopz')) {
            uopz_set_return('time', time() + $delta);
        } else {
            $this->getDataGenerator()->get_plugin_generator('tool_datewatch')
                ->shift_dates('user_enrolments', 'timestart', -$delta);
        }
