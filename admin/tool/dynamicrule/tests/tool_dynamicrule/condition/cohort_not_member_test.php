 ...
 * @covers     \tool_dynamicrule\tool_dynamicrule\condition\cohort_not_member
 * @copyright  2019 Moodle Pty Ltd <support@moodle.com>
 * @author     2019 Daniel Neis Araujo <daniel@moodle.com>
 * @license    Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class cohort_not_member_test extends \advanced_testcase {
    ...
    public function test_supports_rule_types(): void {
        $condition = cohort_not_member::instance();
    ...
    public function test_get_matching_users() {
        ...
        cohort_not_member::create($rule1->id, $configdata);
    ...
    public function test_get_description() {
        ...
        $rule1 = $this->get_generator()->create_rule();
        $configdata = ['cohortid' => $cohort1->id];
        $condition1 = cohort_not_member::create($rule1->id, $configdata);
    ...
    public function test_user_can_add() {
        ...
        // Admin user.
        self::setAdminUser();
        $this->assertTrue(cohort_not_member::instance()->user_can_add());

        // Non-priveleged user.
        $user = self::getDataGenerator()->create_user();
        self::setUser($user);
        $this->assertFalse(cohort_not_member::instance()->user_can_add());
    ...
    public function test_user_can_edit() {
        ...
        // Admin user.
        self::setAdminUser();
        $this->assertTrue(cohort_not_member::instance()->user_can_edit($configform));

        // Non-priveleged user.
        $user = self::getDataGenerator()->create_user();
        self::setUser($user);
        $this->assertFalse(cohort_not_member::instance()->user_can_edit($configform));
    ...
    public function test_can_edit_cohort_condition_tenant() {
        ...
        // Sanity check.
        self::setAdminUser();
        $this->assertTrue(cohort_not_member::instance()->user_can_edit(['cohortid' => $syscohort->id]));
        $this->assertTrue(cohort_not_member::instance()->user_can_edit(['cohortid' => $catcohort->id]));
        $this->assertTrue(cohort_not_member::instance()->user_can_edit(['cohortid' => $subcatcohort->id]));

        // Tenant admin can't edit cohort in category.
        self::setUser($tenantadmin);
        $this->assertFalse(cohort_not_member::instance()->user_can_edit(['cohortid' => $syscohort->id]));
        $this->assertFalse(cohort_not_member::instance()->user_can_edit(['cohortid' => $catcohort->id]));
        $this->assertFalse(cohort_not_member::instance()->user_can_edit(['cohortid' => $subcatcohort->id]));
    ...
    public function test_validate_config_form() {
        ...
        $condition = $this->get_generator()->create_condition(cohort_not_member::class, $rule->id, ['cohortid' => $syscohort->id]);
    ...
    public function test_is_available() {
        // Sanity check.
        self::setAdminUser();
        $this->assertFalse(cohort_not_member::is_available());

        // Add system cohort.
        $this->getDataGenerator()->create_cohort(['contextid' => SYSCONTEXTID]);
        $this->assertTrue(cohort_not_member::is_available());
    ...
    public function test_field_mapping(): void {
        ...
        $this->get_generator()->create_condition(cohort_not_member::class, $rule->id, ['cohortid' => $cohort->id]);
        ...
        $condition = cohort_not_member::instance(0, condition::get_record(['ruleid' => end($rules)->get('id')])->to_record());
    ...
    /**
     * Test cohort_member_added and cohort_member_removed events are triggering rule processing.
     */
    public function test_trigger_rule_processing() {
        ...
        cohort_not_member::create($rule0->id, $configdata);
        ...
        // Remove from cohort.
        cohort_remove_member($cohort0->id, $user0->id);

        // Rule applied twie.
        $this->get_generator()->assert_user_matched_rule($this, $user0->id, $rule0->id, 2);

        // Add user0 into cohort0 again.
        cohort_add_member($cohort0->id, $user0->id);

        // User unmatched.
        $this->get_generator()->assert_user_did_not_match_rule($this, $user0->id, $rule0->id, 2);
    }
}
