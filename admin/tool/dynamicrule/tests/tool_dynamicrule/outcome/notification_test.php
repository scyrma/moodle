...
class notification_test extends \advanced_testcase {
    ...
    /**
     * Test apply_to_users with manual assigned manager notifications activated.
     */
    public function test_trigger_rule_processing_sendto_manually_assigned_managers() {
        ...
        // Create new rule setting only manually assigned manager notifications.
        $rule0 = $this->get_generator()->create_rule(['enabled' => 1, 'tenantid' => $tenant->id]);
        $this->get_generator()->create_condition_alwaystrue($rule0->id);

        $configdata = ['subject' => '{{userfullname}} matched!',
            'body' => ['text' => "{{userfullname}} matched!", 'format' => FORMAT_HTML],
            'sendto' => ['manualmanager' => 1], ];
        notification::create($rule0->id, $configdata);

        $sink = $this->redirectMessages();
        // Process rule manually only for employee1 as if we enabled it.
        $ruleinstance = new \tool_dynamicrule\rule(0, $rule0);
        \tool_dynamicrule\api::process_rule($ruleinstance, $employee1->id);
        $savedmessages = $sink->get_messages();
        $sink->close();

        $this->assertCount(1, $savedmessages);
        // Check notification was sent to manager (manager over employee1).
        $this->assertEquals($manager->id, $savedmessages[0]->useridto);
        // Check placeholder is using affected user (employee1) information.
        $this->assertEquals(fullname($employee1) . ' matched!', $savedmessages[0]->fullmessagehtml);

        $sink = $this->redirectMessages();
        // Process rule manually for rest of users as if we enabled it.
        $ruleinstance = new \tool_dynamicrule\rule(0, $rule0);
        \tool_dynamicrule\api::process_rule($ruleinstance);
        $savedmessages = $sink->get_messages();
        $sink->close();
        ...
        $this->assertCount(2, $savedmessages);
        // Check notification was sent to employee1 & employee2 (both are manager over other users).
        $this->assertEquals($employee1->id, $savedmessages[0]->useridto);
        $this->assertEquals($employee2->id, $savedmessages[1]->useridto);
        // Check placeholder is using affected user (employee2 & employee3) information.
        $this->assertEquals(fullname($employee2) . ' matched!', $savedmessages[0]->fullmessagehtml);
        $this->assertEquals(fullname($employee3) . ' matched!', $savedmessages[1]->fullmessagehtml);
    }
