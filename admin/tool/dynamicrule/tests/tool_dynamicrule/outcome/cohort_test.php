 ...
 * @covers     \tool_dynamicrule\tool_dynamicrule\outcome\cohort
...
class cohort_test extends \advanced_testcase {
    ...
    public function test_apply_to_user_in_shared_tenant() {
        ...
        $this->assertEqualsCanonicalizing([$user1->id, $user2->id, $user21->id, $user22->id, get_admin()->id],
            array_column($DB->get_records('cohort_members'), 'userid'));
    }

    /**
     * Test get_displayed_description.
     *
     * @uses \tool_dynamicrule\tool_dynamicrule\outcome\cohort::get_description
     * @uses \tool_dynamicrule\tool_dynamicrule\outcome\cohort::get_uneditable_description
     */
    ...
    public function test_is_available() {
        ...
        $category = $this->getDataGenerator()->create_category();
        $subcategory = $this->getDataGenerator()->create_category(['parent' => $category->id]);
        $tenant = $this->get_tenant_generator()->create_tenant(['categoryid' => $category->id]);
        $tenantadmin = $this->get_tenant_generator()->create_user(['tenantid' => $tenant->id, 'tenantadmin' => true]);
