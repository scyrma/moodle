...
class tool_dynamicrule_generator extends component_generator_base {
     ...
     *
     * @param int $ruleid
     * @return \tool_dynamicrule\tool_dynamicrule\condition\testable_condition_alwaystrue
     */
     ...
     * @return \tool_dynamicrule\tool_dynamicrule\outcome\testable_outcome_donothing
     */
    public function create_outcome_donothing(int $ruleid, bool $throwerror = false): outcome_base {
    ...
    /**
     * Creates testable condition "sharedonly".
     *
     * @param int $ruleid
     * @return condition_base
     */
    public function create_condition_sharedonly(int $ruleid): condition_base {
        require_once(__DIR__.'/../fixtures/testable_condition_sharedonly.php');
        $class = '\tool_dynamicrule\tool_dynamicrule\condition\testable_condition_sharedonly';
        return \tool_dynamicrule\api::create_rule_condition($ruleid, $class, [], true);
    }

    /**
     * Creates testable outcome "sharedonly".
     *
     * @param int $ruleid
     * @return outcome_base
     */
    public function create_outcome_sharedonly(int $ruleid): outcome_base {
        require_once(__DIR__.'/../fixtures/testable_outcome_sharedonly.php');
        $class = '\tool_dynamicrule\tool_dynamicrule\outcome\testable_outcome_sharedonly';
        return \tool_dynamicrule\api::create_rule_outcome($ruleid, $class, [], true);
    }
    ...
    public function create_badge(int $userid, int $status = 0, string $name = ''): badge {
        ...
        debugging('Function create_badge() should not be used, use core_badges generator instead.', DEBUG_DEVELOPER);
        $now = time();
    ...
    /**
     * Assert that user has a matched record for this rule and 'unmatchedtime' is empty
     *
     * @param advanced_testcase $test
     * @param int $userid
     * @param int $ruleid
     * @param int $recordscnt expected number of records in the {tool_dynamicrule_match} table
     * @throws dml_exception
     */
    public function assert_user_matched_rule(advanced_testcase $test, int $userid, int $ruleid, int $recordscnt = 1) {
        global $DB;
        $match = array_values($DB->get_records('tool_dynamicrule_match', ['userid' => $userid, 'ruleid' => $ruleid], 'id'));
        $test->assertEquals($recordscnt, count($match));
        $lastmatch = array_pop($match);
        $test->assertNotEmpty($lastmatch->matchedtime);
        $test->assertEmpty($lastmatch->unmatchedtime);
        $test->assertEquals(1, $lastmatch->status);
    }

    /**
     * Assert that user does not have matched records for this rule or is marked as unmatched
     *
     * @param advanced_testcase $test
     * @param int $userid
     * @param int $ruleid
     * @param int $recordscnt expected number of records in the {tool_dynamicrule_match} table
     *     0 (default) - user has never been matched before
     *     1 or more - user has been matched before but was marked as unmatched
     * @throws dml_exception
     */
    public function assert_user_did_not_match_rule(advanced_testcase $test, int $userid, int $ruleid, int $recordscnt = 0) {
        global $DB;
        $match = array_values($DB->get_records('tool_dynamicrule_match', ['userid' => $userid, 'ruleid' => $ruleid], 'id'));
        $test->assertEquals($recordscnt, count($match));
        if ($recordscnt >= 1) {
            $lastmatch = array_pop($match);
            $test->assertNotEmpty($lastmatch->matchedtime);
            $test->assertNotEmpty($lastmatch->unmatchedtime);
            $test->assertEquals(1, $lastmatch->status);
        }
    }
}
