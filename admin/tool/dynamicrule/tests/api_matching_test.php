 ...
 * @license    Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class api_matching_test extends advanced_testcase {
    /** @var stdClass */
    protected $user11;
    /** @var stdClass */
    protected $user12;
    ...
    protected $user23;
    /** @var stdClass */
    protected $ruledefaulttenant;
    /** @var stdClass */
    protected $ruleothertenant;
    /** @var stdClass */
    protected $rulesharedtenant;
    /** @var \tool_dynamicrule\rule */
    protected $ruledefaulttenantinstance;
    /** @var \tool_dynamicrule\rule */
    protected $ruleothertenantinstance;
    /** @var \tool_dynamicrule\rule */
    protected $rulesharedtenantinstance;
    ...
    private function get_matching_users_setup() {
        ...
        $tenantgenerator->allocate_user($this->user11->id, $defaulttenantid);
        $tenantgenerator->allocate_user($this->user12->id, $defaulttenantid);
        $tenantgenerator->allocate_user($this->user13->id, $defaulttenantid);
        $tenantgenerator->allocate_user($this->user14->id, $defaulttenantid);
        $tenantgenerator->allocate_user($this->user15->id, $defaulttenantid);
        ...
        $tenantgenerator->allocate_user($this->user21->id, $othertenant->id);
        $tenantgenerator->allocate_user($this->user22->id, $othertenant->id);
        $tenantgenerator->allocate_user($this->user23->id, $othertenant->id);
    ...
    /**
     * Test count_matching_users for rule based on "Include suspended users" setting.
     */
    public function test_count_matching_users_suspended() : void {
        global $DB;

        $user1 = $this->getDataGenerator()->create_user(['city' => 'XYZ']);
        $user2 = $this->getDataGenerator()->create_user(['city' => 'XYZ']);

        // We suspend the user to do not show as matching in rule (by default it's not consider suspended users).
        $user2->suspended = 1;
        $DB->update_record('user', $user2);

        $rule = $this->get_generator()->create_rule();
        $ruleinstance = new \tool_dynamicrule\rule($rule->id);
        $configdata = ['userprofilefield' => 'city', 'city_value' => 'XYZ', 'city_op' => 2];
        \tool_dynamicrule\tool_dynamicrule\condition\user_profile_field::create($rule->id, $configdata);

        $this->assertEquals(2, \tool_dynamicrule\api::count_matching_users($rule->id));

        // Then if includesuspendedusers is unchecked suspended users are included.
        $ruleinstance->set('includesuspendedusers', 0);
        $ruleinstance->update();
    ...
    /**
     * Test get_matching_users for rule based on "Include suspended users" setting.
     */
    public function test_get_matching_users_users_suspended() : void {
        global $DB;
        ...
        // Then if includesuspendedusers is checked suspended users are included.
        $ruleinstance->set('includesuspendedusers', 0);
        $ruleinstance->update();
