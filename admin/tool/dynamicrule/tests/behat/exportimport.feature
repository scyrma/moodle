...
Feature: Export and import dynamic rules
  ...
  Scenario: Export a rule
    ...
    And I click on "View export" "link" in the "Dynamic rules" "table_row"
  ...
  Scenario: Import a rule into current tenant
    ...
    And I click on "View import" "link" in the "Dynamic rules" "table_row"
    Then I should see "Success" in the "Status" "table_row"
    And I should see "Instances (1)"
    And I should see "Rule 1" in the "[data-region=\"exportimport-instances\"]" "css_element"
    ...
    And the following should exist in the "report-table" table:
      | Name   | Conditions                             | -4-                             |
      | Rule 1 | Users who have logged in at least once | Send notification 'Hi' to users |
