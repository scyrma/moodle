@tool @tool_dynamicrule @tool_certificate @javascript @moodleworkplace
Feature: Issue certificate with dynamic rules
  ...
  Scenario: Issue a certificate on course completion
    ...
    And I press "View" action in the "User 11" report row
  ...
  Scenario: Issue a certificate on certification completion
    Given "2" tenants exist with "3" users and "2" courses in each
