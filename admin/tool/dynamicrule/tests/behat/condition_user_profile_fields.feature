...
Feature: Creating and editing conditions for user profile fields
  ...
  Scenario Outline: Add a rule with user profile field datetime condition (empty, past future)
    ...
    And I press "Save changes"
    Then I should see "Users whose value for profile field 'Date Time field' <operatordesc>"
    And I should see "<matches> total matches"
    Examples:
      | operator | operatordesc  | date           | matches |
      | 0        | Is any value  | ##today##%s##  | 2       |
      | 1        | Is not empty  | ##today##%s##  | 1       |
      | 2        | Is empty      | ##today##%s##  | 1       |
      | 3        | Is in the past   | ##-1 day##%s## | 1       |
      | 3        | Is in the past   | ##+1 day##%s## | 0       |
      | 4        | Is in the future | ##-1 day##%s## | 0       |
      | 4        | Is in the future | ##+1 day##%s## | 1       |
  ...
  Scenario Outline: Add a rule with user profile field datetime condition (last, next)
    ...
    Examples:
      ...
      | 5        | Last 2 days  | ##-1 day##%s## | 1       | 2          |
      | 5        | Last 2 days  | ##+1 day##%s## | 0       | 2          |
      | 6        | Next 5 days  | ##-1 day##%s## | 0       | 5          |
      | 6        | Next 5 days  | ##+1 day##%s## | 1       | 5          |
  ...
  Scenario Outline: Add a rule with user profile field datetime condition (current, previous, upcoming)
    ...
    Examples:
      ...
      | 8        | Last Week     | ##-1 week##%s##    | 1       | 2          |
      | 8        | Last Week     | ##+1 week##%s##    | 0       | 2          |
      | 9        | Next year     | ##-1 year##%s##    | 0       | 5          |
      | 9        | Next year     | ##+1 year##%s##    | 1       | 5          |
  ...
  Scenario: Add a rule with user language preference condition
    Given the following "users" exist:
      | username | auth   | firstname | lastname | email               | lang |
      | userone  | ldap   | User      | One      | userone@example.com | en   |
      | usertwo  | manual | User      | Two      | usertwo@example.com |      |
    When I log in as "admin"
    And I navigate to "Dynamic rules" in site administration
    And I follow "New rule"
    And I set the field "Name" to "Users based on language preference"
    And I press "Save"
    And I follow "User profile field"
    And I set the following fields to these values:
      | Field    | Language     |
      | Language  | en |
    And I press "Save changes"
    Then I should see "Users whose value for profile field 'Language' is 'English ‎(en)‎'"
    And I should see "2 total matches"
    And I follow "View matching users"
    And I should see "userone@example.com" in the "View matching users" "dialogue"
    And I should see "moodle@example.com" in the "View matching users" "dialogue"
    And I should not see "usertwo@example.com" in the "View matching users" "dialogue"
