...
Feature: Check dynamic rules capabilities
  ...
  Scenario: Check capabilities for entity if entity was deleted and action is enrol into a course
    ...
    And I open the autocomplete suggestions list
    Then I click on "Course11" item in the autocomplete list
    And I press "Save changes"
    # Create second rule.
    And I navigate to "Dynamic rules" in workplace launcher
    And I follow "New rule"
  ...
  Scenario: Check capabilities for non existing action class
    ...
    And I click on "Enrol users into a course" "link" in the "#ruleoutcomes" "css_element"
    And I open the autocomplete suggestions list
    Then I click on "Course12" item in the autocomplete list
    And I press "Save changes"
    And I click on "Enable" "button"
    And I click on "Enable" "button" in the "Confirm" "dialogue"
    # Modify the DB record on one condition to use non-existing class
  ...
  Scenario: Check capabilities with user profile fields
    ...
    And the "userprofilefield" select box should contain "text_field1"
    And the "userprofilefield" select box should not contain "text_field_hidden"
    And I set the following fields to these values:
      | Field | text_field1 |
      | text_field1_op | 2 |
    ...
    And I follow "User profile field"
    And the "userprofilefield" select box should contain "text_field1"
    And the "userprofilefield" select box should contain "text_field_hidden"
    And I set the following fields to these values:
    ...
    And I press "Save changes"
    And I log out
    # Login as the tenant user again
