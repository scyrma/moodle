...
class tool_dynamicrule_condition_user_not_enrolled_testcase extends advanced_testcase {
    ...
    public function test_get_description() {
        ...
        $configdata = ['courseid' => 9999];
        $condition5 = \tool_dynamicrule\tool_dynamicrule\condition\user_not_enrolled::create($rule5->id, $configdata);
        $this->assertEquals(get_string('conditionisbroken', 'tool_dynamicrule'), $condition5->get_description());
    }
    ...
    public function test_trigger_rule_processing() {
        ...
        $configdata = ['subject' => 'Course0 unenrolled', 'body' => 'Congratulations, you are not enrolled in course0 anymore.'];
        ...
        // Unenrol user0 from course0, this is supposed to trigger rule0.
        $manualplugin = enrol_get_plugin('manual');
        $enrol = $DB->get_record('enrol', array('courseid' => $course0->id, 'enrol' => 'manual'), '*', MUST_EXIST);
        $manualplugin->unenrol_user($enrol, $user0->id);

        // Check outcomes.
        $messages = $sink->get_messages();
        $this->assertEquals(1, $sink->count());
        $this->assertEquals($messages[0]->subject, 'Course0 unenrolled');
        $sink->clear();
    ...
    public function test_with_multitenancy() {
        ...
        $configdata = ['subject' => 'Course1 unenrolled', 'body' => 'Congratulations, you are course1 unenrolled.'];
        ...
        // Unenrol user1 from course1, this is supposed to trigger rule1.
        $manualplugin = enrol_get_plugin('manual');
        ...
        $messages = $sink->get_messages();
        $this->assertEquals(1, $sink->count());
        $this->assertEquals($messages[0]->subject, 'Course1 unenrolled');
        $sink->clear();
        ...
        $configdata = ['subject' => 'Course2 unenrolled', 'body' => 'Congratulations, you are course2 unenrolled.'];
        ...
        // Unenrol user2 from course2, this is supposed to trigger rule2.
        ...
        $messages = $sink->get_messages();
        $this->assertEquals(1, $sink->count());
        $this->assertEquals($messages[0]->subject, 'Course2 unenrolled');
        $sink->clear();
