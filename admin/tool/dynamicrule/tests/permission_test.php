...
class permission_test extends advanced_testcase {
    ...
    public function test_can_edit_all_rule_conditions(): void {
        ...
        \tool_dynamicrule\tool_dynamicrule\condition\course_completed::create($rule->id, $configdata);
        ...
        // Expect permission to be cached.
        $this->assertFalse(permission::can_edit_all_rule_conditions(api::get_rule($rule->id)));
    ...
    public function test_can_edit_all_rule_outcomes(): void {
        ...
        \tool_dynamicrule\tool_dynamicrule\outcome\competency::create($rule->id, $configdata);
     ...
     * @return void
     */
    public function test_can_edit_rule(): void {
        $user1 = $this->getDataGenerator()->create_user();
        self::setUser($user1);

        $rule = $this->get_plugin_generator()->create_rule();

        // Add condition and outcome.
        $course1 = self::getDataGenerator()->create_course();
        self::getDataGenerator()->enrol_user($user1->id, $course1->id, 'editingteacher', 'manual');
        $configdata = ['courseid' => $course1->id];
        \tool_dynamicrule\tool_dynamicrule\condition\course_completed::create($rule->id, $configdata);
        ...
        $this->assertFalse(permission::can_edit_rule(api::get_rule($rule->id)));
        ...
        // Expect permission test to be cached.
        $this->assertFalse(permission::can_edit_rule(api::get_rule($rule->id)));

        // Purge cache and test again.
        self::purge_request_cache();
        $this->assertTrue(permission::can_edit_rule(api::get_rule($rule->id)));
    }
}
