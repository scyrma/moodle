...
class tool_dynamicrule_permission_testcase extends advanced_testcase {
    ...
    /*
     * Test can_edit_cohort_condition.
     */
    public function test_can_edit_cohort_condition() {
        $cohort1 = $this->getDataGenerator()->create_cohort();
        $context = \context::instance_by_id($cohort1->contextid);

        // Admin user.
        self::setAdminUser();
        $this->assertTrue(permission::can_edit_cohort_condition($cohort1->id));

        // Non-priveleged user.
        $user = $this->getDataGenerator()->create_user();
        self::setUser($user);
        $this->assertFalse(permission::can_edit_cohort_condition($cohort1->id));
    ...
    public function test_can_edit_cohort_condition_tenant() {
        ...
        $catcohort = $this->getDataGenerator()->create_cohort(['contextid' => $subcategory->get_context()->id]);
        ...
        // Sanity check.
        self::setAdminUser();
        $this->assertTrue(permission::can_edit_cohort_condition($catcohort->id));

        // Tenant admin.
        self::setUser($tenantadmin);
        $this->assertFalse(permission::can_edit_cohort_condition($catcohort->id));
