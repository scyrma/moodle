...
use tool_dynamicrule\tool_dynamicrule\outcome\badge;
 ...
 * @license    Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class process_rules_test extends advanced_testcase {
     ...
     */
    protected function create_badge(): \core_badges\badge {
        global $DB;
        $generator = $this->getDataGenerator()->get_plugin_generator('core_badges');
        // Create badge and criteria for manually awarding by role.
        $badge = $generator->create_badge(['image' => 'lib/tests/fixtures/gd-logo.png']);
        $managerrole = $DB->get_field('role', 'id', ['shortname' => 'manager']);
        $generator->create_criteria(['badgeid' => $badge->id, 'roleid' => $managerrole]);
        return $badge;
    }
     ...
     */
    public function test_process_rules_task(): void {
        global $DB;
     ...
     */
    public function test_process_rules_task_with_event_condition(): void {
        global $DB;
    ...
    /**
     * Test the process rules task with mixed conditions.
     *
     * @uses \tool_dynamicrule\tool_dynamicrule\outcome\notification
     * @uses \tool_dynamicrule\tool_dynamicrule\condition\cohort_member
     */
    public function test_process_rules_task_with_mixed_conditions(): void {
        global $DB;
    ...
    /**
     * Test the process rules task with scheduled task outcome.
     *
     * @uses \tool_dynamicrule\tool_dynamicrule\outcome\badge
     * @uses \tool_dynamicrule\tool_dynamicrule\condition\cohort_member
     */
    public function test_process_rules_task_with_scheduled_task_outcome(): void {
        global $DB;
        ...
        // Create rule0 with cohort0 membership conditon (event-based) and learning plan outcome (scheduled task required).
        $rule0 = $this->get_generator()->create_rule(['enabled' => 1]);
        $configdata = ['cohortid' => $cohort0->id];
        \tool_dynamicrule\tool_dynamicrule\condition\cohort_member::create($rule0->id, $configdata);
        $configdata = ['badge' => $this->create_badge()->id];
        ...
        // Check outcomes (rule has been triggered as there is scheduled task outcome).
        $this->assertEquals([$user0->id], array_column($DB->get_records('badge_issued'), 'userid'));
        $this->assertEquals([$user0->id], array_column($DB->get_records('badge_manual_award'), 'recipientid'));
    }
     ...
     * @uses \tool_dynamicrule\tool_dynamicrule\condition\cohort_member
     */
    public function test_enable_rule_trigger_process_rule_task(): void {
        global $DB;
     ...
     * Run adhoc tasks.
     */
    protected function run_adhock_tasks(): void {
