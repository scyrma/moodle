...
class tool_dynamicrule_api_testcase extends advanced_testcase {
    ...
    public function test_get_matching_users_two_conditions_same_type_same_rule() {
        ...
        $condcity->update_configdata(['userprofilefield' => 'city', 'city_value' => 'Florianópolis', 'city_op' => 2]);
    ...
    public function test_is_rule_configuration_valid() {
        ...
        // Break condition.
        $condition->update_configdata(['courseid' => 1000, 'enrol' => 'manual']);
        $this->assertFalse(\tool_dynamicrule\api::is_rule_configuration_valid($rule));

        // Fix condition.
        $condition->update_configdata(['courseid' => $course0->id, 'enrol' => 'manual']);
        $this->assertTrue(\tool_dynamicrule\api::is_rule_configuration_valid($rule));

        // Break outcome.
        $outcome->update_configdata(['coursetoenrol' => 1000, 'role' => $roleid]);
        $this->assertFalse(\tool_dynamicrule\api::is_rule_configuration_valid($rule));

        // Fix outcome.
        $outcome->update_configdata(['coursetoenrol' => $course0->id, 'role' => $roleid]);
        $this->assertTrue(\tool_dynamicrule\api::is_rule_configuration_valid($rule));
    ...
    public function test_process_rule_for_user() {
        ...
        $this->assertEquals($messages[0]->useridto, $user1->id);
        $this->assertEquals($messages[1]->subject, 'Test subject 1');
        $this->assertEquals($messages[1]->useridto, $user2->id);
        $sink->clear();
        ...
        // Unenrolling a user must trigger rule event and unmatch single user.
        $instance = $DB->get_record('enrol', ['courseid' => $course1->id, 'enrol' => 'manual']);
        $plugin = enrol_get_plugin('manual');
        $plugin->unenrol_user($instance, $user1->id);

        \tool_dynamicrule\api::process_rule($ruleinstance, $user1->id);
        ...
        $plugin->enrol_user($instance, $user3->id);
        $plugin->enrol_user($instance, $user4->id);

        $rule1 = $this->get_generator()->create_rule(['enabled' => 1]);
        $configdata = ['courseid' => $course2->id, 'enrol' => ''];
        ...
        $matchuser3 = $DB->get_record('tool_dynamicrule_match', ['ruleid' => $rule1->id, 'userid' => $user3->id]);
        $matchuser4 = $DB->get_record('tool_dynamicrule_match', ['ruleid' => $rule1->id, 'userid' => $user4->id]);

        $this->assertNull($matchuser3->unmatchedtime);
        $this->assertNull($matchuser4->unmatchedtime);

        $instance = $DB->get_record('enrol', ['courseid' => $course2->id, 'enrol' => 'manual']);

        $plugin->enrol_user($instance, $user5->id);

        $matchuser3 = $DB->get_record('tool_dynamicrule_match', ['ruleid' => $rule1->id, 'userid' => $user3->id]);
        $matchuser4 = $DB->get_record('tool_dynamicrule_match', ['ruleid' => $rule1->id, 'userid' => $user4->id]);
        $matchuser5 = $DB->get_record('tool_dynamicrule_match', ['ruleid' => $rule1->id, 'userid' => $user5->id]);

        $this->assertNull($matchuser5->unmatchedtime);
        $this->assertNull($matchuser3->unmatchedtime);
        $this->assertNull($matchuser4->unmatchedtime);
    }
