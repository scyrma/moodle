...
use tool_dynamicrule\event\condition_added;
use tool_dynamicrule\event\condition_deleted;
use tool_dynamicrule\event\condition_updated;
use tool_dynamicrule\event\outcome_added;
use tool_dynamicrule\event\outcome_deleted;
use tool_dynamicrule\event\outcome_updated;
...
class api_test extends \advanced_testcase {
     ...
     * Purge request cache.
     *
     * Each test is treated as a separate php session, so when we change capabilities
     * within the same test to assert change of behaviour, cache needs to be cleared.
     *
     * @return void
     */
    private static function purge_request_cache(): void {
        (\cache::make_from_params(\cache_store::MODE_REQUEST, 'tool_dynamicrule', 'requestcache'))->purge();
    }
    ...
    public function test_create_rule(): void {
        ...
        // Confirm that rule_created event is triggered.
        $event = array_pop($events);
        $this->assertInstanceOf(rule_created::class, $event);
        $this->assertEquals($rule->get('id'), $event->objectid);
    ...
    public function test_update_rule(): void {
        ...
        // Update rule.
        $sink = $this->redirectEvents();
        api::update_rule($rule->get('id'), (object)['name' => 'New name for Rule']);
        $events = $sink->get_events();
        $sink->close();

        $record = $DB->get_record(rule::TABLE, ['id' => $rule->get('id')]);
        $this->assertEquals('New name for Rule', $record->name);

        // Confirm that rule_updated event is triggered.
        $event = array_pop($events);
        $this->assertInstanceOf(rule_updated::class, $event);
        $this->assertEquals($rule->get('id'), $event->objectid);
        $this->assertEquals($USER->id, $event->userid);
    }
    ...
    public function test_create_rule_shared(): void {
        ...
        $this->assertFalse($rule1->get('shared'));
        ...
        $this->assertTrue($rule2->get('shared'));
        ...
        $this->assertFalse($newrule->get('shared'));
    }
    ...
    public function test_get_rule_wrong_tenant() {
        ...
        $othertenant = $this->get_tenant_generator()->create_tenant();
    ...
    /**
     * Test condition events.
     */
    public function test_condition_events(): void {
        global $DB, $USER;
        $this->resetAfterTest();
        ...
        $conditionclass = 'tool_dynamicrule\tool_dynamicrule\condition\testable_condition_alwaystrue';
        $sink = $this->redirectEvents();
        $condition = \tool_dynamicrule\api::create_rule_condition($rule0->id, $conditionclass, [], true);
        $events = $sink->get_events();
        $sink->close();
        $this->assertEquals(1, $DB->count_records('tool_dynamicrule_condition'));

        // Confirm that condition_added event is triggered.
        $event = array_pop($events);
        $this->assertInstanceOf(condition_added::class, $event);
        $this->assertEquals($condition->get_id(), $event->objectid);
        $this->assertEquals($USER->id, $event->userid);
        $this->assertEquals($rule0->id, $event->other['ruleid']);

        // Update condition.
        $sink = $this->redirectEvents();
        $condition->update_configdata(['test' => 'test'], true);
        $events = $sink->get_events();
        $sink->close();

        // Confirm that condition_updated event is triggered.
        $event = array_pop($events);
        $this->assertInstanceOf(condition_updated::class, $event);
        $this->assertEquals($condition->get_id(), $event->objectid);
        $this->assertEquals($USER->id, $event->userid);
        $this->assertEquals($rule0->id, $event->other['ruleid']);
        ...
        // Confirm that condition_deleted event is triggered.
        $event = array_pop($events);
        $this->assertInstanceOf(condition_deleted::class, $event);
        $this->assertEquals($conditionid, $event->objectid);
        $this->assertEquals($USER->id, $event->userid);
        $this->assertEquals($rule0->id, $event->other['ruleid']);
    }
     ...
     * Test outcome events.
     */
    public function test_outcome_events(): void {
        global $DB, $USER;
        $this->resetAfterTest();
        ...
        $configform = ['subject' => 'The notification subject', 'body' => 'Here comes the message.'];
        $sink = $this->redirectEvents();
        $outcome = \tool_dynamicrule\api::create_rule_outcome($rule0->id, $outcomeclass, $configform);
        $events = $sink->get_events();
        $sink->close();
        $this->assertEquals(1, $DB->count_records('tool_dynamicrule_outcome'));

        // Confirm that outcome_added event is triggered.
        $event = array_pop($events);
        $this->assertInstanceOf(outcome_added::class, $event);
        $this->assertEquals($outcome->get_id(), $event->objectid);
        $this->assertEquals($USER->id, $event->userid);
        $this->assertEquals($rule0->id, $event->other['ruleid']);

        // Update action.
        $sink = $this->redirectEvents();
        $outcome->update_configdata(['test' => 'test'], true);
        $events = $sink->get_events();
        $sink->close();

        // Confirm that outcome_updated event is triggered.
        $event = array_pop($events);
        $this->assertInstanceOf(outcome_updated::class, $event);
        $this->assertEquals($outcome->get_id(), $event->objectid);
        $this->assertEquals($USER->id, $event->userid);
        $this->assertEquals($rule0->id, $event->other['ruleid']);

        // Delete action.
        $actionid = $outcome->get_id();
        $sink = $this->redirectEvents();
        $outcome->delete();
        $events = $sink->get_events();
        $sink->close();

        // Confirm that outcome_deleted event is triggered.
        $event = array_pop($events);
        $this->assertInstanceOf(outcome_deleted::class, $event);
        $this->assertEquals($actionid, $event->objectid);
        $this->assertEquals($USER->id, $event->userid);
        $this->assertEquals($rule0->id, $event->other['ruleid']);
    }
    ...
    public function test_delete_rule() {
        ...
        // Confirm that rule_deleted event was triggered.
        $event = array_pop($events);
        $this->assertInstanceOf(rule_deleted::class, $event);
        $this->assertEquals($rule0->id, $event->objectid);
        $this->assertEquals($USER->id, $event->userid);
    ...
    public function test_archive_rule() {
        ...
        // Confirm that rule_archived event is triggered.
        $event = array_pop($events);
        $this->assertInstanceOf(rule_archived::class, $event);
        $this->assertEquals($rule0->id, $event->objectid);
        $this->assertEquals($USER->id, $event->userid);
    ...
    public function test_unarchive_rule() {
        ...
        // Confirm that rule_unarchived event is triggered.
        $event = array_pop($events);
        $this->assertInstanceOf(rule_unarchived::class, $event);
        $this->assertEquals($rule0->id, $event->objectid);
        $this->assertEquals($USER->id, $event->userid);
    ...
    /**
     * Test is_rule_configuration_valid for conditions
     *
     * @uses \tool_tenant\manager::archive_tenant
     * @uses \tool_tenant\manager::restore_tenant
     */
    public function test_is_rule_configuration_valid_conditions(): void {
        global $DB;

        $course0 = $this->getDataGenerator()->create_course();
        $tenant = $this->get_tenant_generator()->create_tenant();
        $tenantuser = self::getDataGenerator()->create_user();
        $this->get_tenant_generator()->allocate_user($tenantuser->id, $tenant->id);
        ...
        \enrol_dynamicrule\tool_dynamicrule\outcome\course_enrol::create($rulerecord->id, $configdata);
    ...
    /**
     * Test is_rule_configuration_valid for outcomes
     */
    public function test_is_rule_configuration_valid_outcomes(): void {
        global $DB;
        ...
        $rulerecord = $this->get_generator()->create_rule(['enabled' => 1, 'tenantid' => $tenant->id]);
        $configdata = ['courseid' => $course0->id, 'enrol' => 'manual'];
        \tool_dynamicrule\tool_dynamicrule\condition\user_not_enrolled::create($rulerecord->id, $configdata);
        $roleid = $DB->get_field('role', 'id', ['shortname' => 'student']);
        $configdata = ['coursetoenrol' => $course0->id, 'role' => $roleid];
    ...
    /**
     * Test is_rule_configuration_valid for tenants archiving.
     *
     * @uses \tool_tenant\manager::archive_tenant
     * @uses \tool_tenant\manager::restore_tenant
     */
    public function test_is_rule_configuration_valid_tenants_archive(): void {
        global $DB;
    ...
    /**
     * Test Matching users report
     *
     * @covers \tool_dynamicrule\reportbuilder\local\systemreports\users_matching_rule
     */
    public function test_matching_users_report() {
        ...
        $reportclass = \tool_dynamicrule\reportbuilder\local\systemreports\users_matching_rule::class;
        $report = \tool_tenant\system_report_factory::create($reportclass, ['ruleid' => $rule0->id]);
        $this->assertInstanceOf($reportclass, $report);
        ...
        $this->assertEquals([
            ...
            'user:username',
            'user:email',
            'matcheduser:matchedtime',
            'matcheduser:status',
        ], array_keys($report->get_columns()));
    ...
    /**
     * Test process_rule with large number of users.
     */
    public function test_process_rule_large_userbase(): void {
        global $DB;

        // Create number of users above real chunk limit.
        $aboveinlimit = (api::FILTER_USERS_CHUNK_SIZE * 2) + 5;
        ...
        // Check outcomes, we should have the same message subject sent to each of our test users plus admin.
        $messages = $sink->get_messages();
        $this->assertCount($aboveinlimit + 1, $messages);
        $sink->close();
    ...
    public function test_duplicate_rule() {
        ...
        $stripkeys = ['id', 'name', 'timecreated', 'timemodified'];
        $rule0 = (object) array_diff_key((array) $rule0, array_flip($stripkeys));

        $newrule1record = $newrule1->to_record();
        // We need to renew values using get to match persistent properties types, as to_record extracts raw value.
        $newrule1record = (object) array_diff_key((array) $newrule1record, array_flip($stripkeys));
        foreach ((array)$newrule1record as $property => $definition) {
            $newrule1record->$property = $newrule1->get($property);
        }

        $newrule2record = $newrule2->to_record();
        // We need to renew values using get to match persistent properties types, as to_record extracts raw value.
        $newrule2record = (object) array_diff_key((array) $newrule2record, array_flip($stripkeys));
        foreach ((array)$newrule2record as $property => $definition) {
            $newrule2record->$property = $newrule2->get($property);
        }
    ...
    /**
     * Test duplicate_rule containing non-applicable entities.
     */
    public function test_duplicate_shared_rule() {
        global $DB;
        ...
        \tool_dynamicrule\tool_dynamicrule\condition\user_profile_field::create($rulesharedtenant->id, $configdata);
        $this->get_generator()->create_condition_sharedonly($rulesharedtenant->id);
        $this->get_generator()->create_outcome_donothing($rulesharedtenant->id);
        $this->get_generator()->create_outcome_sharedonly($rulesharedtenant->id);
        ...
        // Duplicate rule to default tenant.
        $newruleid1 = \tool_dynamicrule\api::duplicate_rule($rulesharedtenant->id);
        $conditions = \tool_dynamicrule\api::get_rule_conditions($newruleid1);
        $outcomes = \tool_dynamicrule\api::get_rule_outcomes($newruleid1);

        // Only non-shared conditions and actions remained.
        $this->assertCount(1, $conditions);
        $this->assertCount(1, $outcomes);
        $this->assertInstanceOf('tool_dynamicrule\tool_dynamicrule\condition\user_profile_field', array_shift($conditions));
        $this->assertInstanceOf('tool_dynamicrule\tool_dynamicrule\outcome\testable_outcome_donothing', array_shift($outcomes));
    }
     ...
     * @return array
     */
    public static function provider_is_course_category_allowed_in_rule(): array {
        return [
    ...
    public function test_is_course_category_allowed_in_rule(string $rulename, string $catname, bool $result): void {
        ...
        $tenant1 = $this->get_tenant_generator()->create_tenant(['categoryid' => $cat1->id]);
        $tenant2 = $this->get_tenant_generator()->create_tenant(['categoryid' => $cat2->id]);
        $tenant3 = $this->get_tenant_generator()->create_tenant(['categoryid' => 0]);
