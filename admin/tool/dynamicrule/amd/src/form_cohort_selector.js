...
/**
 * Potential cohort selector module.
 *
 * @module     tool_dynamicrule/form_potential_cohort_selector
 * @class      form_potential_cohort_selector
...
const transport = (selector, search, success, failure) => {
    ...
    Ajax.call([{
        methodname: 'tool_dynamicrule_potential_cohort_selector',
        args: {
            search: search,
            instancetype: originalSelect.dataset.instancetype,
        }
    }])[0].then(async(results) => {
        if (results.length <= MAXSUGGESTIONS) {
            const promises = results.map((result) => {
                return Templates.render('tool_dynamicrule/form_cohort_selector_suggestion', result);
            });
            ...
            return await Str.get_string('toomanycohortstoshow', 'tool_dynamicrule', '>' + MAXSUGGESTIONS).then((result) => {
