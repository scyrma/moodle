    ...
    var initDuplicateLinksHandlers = function() {
        $('[data-region=ruleslistwrapper]').on('click', '[data-action=duplicate]', function(e) {
            ...
            if (typeof menuNode.data('limitreached') !== 'undefined') {
                ...
                Promise.all([Repository.preDuplicateValidation(menuNode.data('id')), Str.get_strings(langstrings)])
                .then(([message, [confirmstr, duplicatestr, cancelstr]]) => {
                    Notification.confirm(confirmstr, message, duplicatestr, cancelstr, function() {
