 ...
 *
 * @module     tool_dynamicrule/edit_details
...
import {getRuleEdit} from 'tool_dynamicrule/edit_rule';
...
const init = () => {
    ...
    const ruleEdit = getRuleEdit(document.querySelector('.wptabs').dataset.ruleid);
    ruleEdit.initEnableButton();

    // Initialize notification for changes saved.
    Tabs.initForm((data) => {
        if (data) {
            getString('changessaved', 'moodle')
                .then((string) => {
