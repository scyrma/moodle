...
function xmldb_tool_datastore_upgrade($oldversion) {
    ...
    if ($oldversion < 2021051211) {

        // Define index useractioncourse (not unique) to be added to tool_datastore_action.
        $table = new xmldb_table('tool_datastore_action');
        $index = new xmldb_index('useractioncourse', XMLDB_INDEX_NOTUNIQUE, ['relateduserid', 'action', 'originalcourseid']);

        // Conditionally launch add index useractioncourse.
        if (!$dbman->index_exists($table, $index)) {
            $dbman->add_index($table, $index);
        }

        // Datastore savepoint reached.
        upgrade_plugin_savepoint(true, 2021051211, 'tool', 'datastore');
    }
