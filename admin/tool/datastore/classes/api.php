...
class api {
     ...
     * @param int|null $fieldtype type of the field, default value is {@see column::TYPE_LONGTEXT}
    ...
    public static function get_datasource_field_sql(string $entitytype, string $fieldname, string $maintablealias,
        ...
        $paramentitytype = database::generate_param_name();
        $paramfieldname = database::generate_param_name();

        $fieldtable = database::generate_alias();
        $entitytable = database::generate_alias();
        ...
        $fieldtype = $fieldtype ?? column::TYPE_LONGTEXT;
        switch ($fieldtype) {
            ...
            case column::TYPE_FLOAT :
                $sql = $DB->sql_cast_char2real("({$sql})", true);
                break;
