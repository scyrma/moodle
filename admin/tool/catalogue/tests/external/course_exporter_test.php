...
class course_exporter_test extends advanced_testcase {
    ...
    public function test_export(): void {
        ...
        $expectedfiles = [
            [
                'isimage' => false,
                'url' => moodle_url::make_file_url($urlbase, $filebasepath . 'blank.pdf', true)->out(false),
                'name' => 'blank.pdf',
                'image' => $OUTPUT->pix_icon('f/pdf', 'blank.pdf'),
            ],
        ];
        $this->assertCount(1, $data->coursefiles);
        $this->assertEquals($expectedfiles, $data->coursefiles);

        // Check custom fields for Course2.
        $course2 = self::getDataGenerator()->create_course(['category' => $category->id]);
        $this->getDataGenerator()->enrol_user($user->id, $course2->id, 'editingteacher');
        $related = [
        ...
        // Assert that no custom fields are returned.
        $this->assertEmpty($data->customfields);
    }
    ...
    /**
     * Test course export images
     */
    public function test_export_images(): void {
        global $PAGE, $CFG;
        $this->resetAfterTest();

        // Create two courses, with and without a course image.
        ...
        $fs->create_file_from_string($course2filerecord, file_get_contents("{$CFG->dirroot}/lib/tests/fixtures/gd-logo.png"));

        // If the course hasn't an image, this image will be generated.
        $related = [
            'context' => $course1context,
            'course' => $course1,
        ];
        $exporter = new course_exporter(null, $related);
        $data = $exporter->export($PAGE->get_renderer('core'));
        $this->assertStringStartsNotWith('data:image/svg+xml;base64,', $data->course->courseimage);
        $this->assertEquals(manager::get_course_pattern_url((int)$course1->id), $data->course->courseimage);
        $this->assertTrue(filter_var($data->course->courseimage, FILTER_VALIDATE_URL) !== false);
        $this->assertStringContainsString('/tool_catalogue/course_generated_image/', $data->course->courseimage);

        // If the course has an image, this image won't be generated.
        $related = [
            'context' => $course2context,
            'course' => $course2,
        ];
        $exporter = new course_exporter(null, $related);
        $data = $exporter->export($PAGE->get_renderer('core'));
        $this->assertStringStartsNotWith('data:image/svg+xml;base64,', $data->course->courseimage);
        $this->assertNotEquals(manager::get_course_pattern_url((int)$course2->id), $data->course->courseimage);
        $this->assertTrue(filter_var($data->course->courseimage, FILTER_VALIDATE_URL) !== false);
        $this->assertStringContainsString('/course/overviewfiles/image.png', $data->course->courseimage);
    }
}
