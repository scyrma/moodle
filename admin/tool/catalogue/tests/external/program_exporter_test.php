...
class program_exporter_test extends advanced_testcase {
    ...
    public function test_export(): void {
        ...
        // Base set items.
        $image1 = course_summary_exporter::get_course_image($course1) ?: manager::get_course_pattern_url((int)$course1->id);
        ...
        $image2 = course_summary_exporter::get_course_image($course2) ?: manager::get_course_pattern_url((int)$course2->id);
    ...
    public function test_export_images(): void {
        ...
        // Test program without program image.
        $related = [
            'userid' => (int) $USER->id,
            'program' => $program1,
            'context' => $program1->get_context(),
            'allocations' => manager::get_user_allocations((int) $USER->id, $program1->get('id')),
        ];
        $exporter = new program_exporter($program1, $related);
        $data = $exporter->export($PAGE->get_renderer('core'));

        $this->assertStringStartsNotWith('data:image/svg+xml;base64,', $data->image);
        $this->assertEquals(program_api::get_program_pattern_url($program1->get('id')), $data->image);
        $this->assertTrue(filter_var($data->image, FILTER_VALIDATE_URL) !== false);
        $this->assertStringContainsString('/tool_program/program_generated_image', $data->image);

        // Test program with program image.
        $related = [
            'userid' => (int) $USER->id,
            'program' => $program2,
            'context' => $program2->get_context(),
            'allocations' => manager::get_user_allocations((int) $USER->id, $program2->get('id')),
        ];
        $exporter = new program_exporter($program2, $related);
        $data = $exporter->export($PAGE->get_renderer('core'));

        $this->assertStringStartsNotWith('data:image/svg+xml;base64,', $data->image);
        $this->assertNotEquals(program_api::get_program_pattern_url($program2->get('id')), $data->image);
        $this->assertTrue(filter_var($data->image, FILTER_VALIDATE_URL) !== false);
        $this->assertStringContainsString('/tool_program/program_image', $data->image);
    }
}
