...
Feature: My courses page with the Learning catalogue plugin
  ...
  Scenario: Viewing shared certification programs as a user and accessing its course
    ...
    And I should see "Shared program"
  ...
  Scenario: My courses page has hidden courses
    Given the following "courses" exist:
      | fullname      | shortname | format | category | visible |
      | Hidden Course | HC        | topics | CAT1     | 0       |
    And the following "course enrolments" exist:
      | user          | course  | role            |
      | user11        | HC      | student         |
      | user12        | HC      | editingteacher  |
    When I am on the "My courses" page logged in as "user11"
    Then I should not see "Hidden Course"
    # User with permissions can see hidden courses with a badge.
    And I am on the "My courses" page logged in as "user12"
    And I should see "Hidden from learners" in the "Hidden Course" "tool_catalogue > Catalogue item"

  Scenario: My courses page has hidden or archived programs
    Given the following "tool_program > programs" exist:
      | fullname   | tenant  | visible | archived |
      | Program 04 | Tenant1 | 1       | 0        |
      | Program 05 | Tenant1 | 0       | 0        |
      | Program 06 | Tenant1 | 1       | 1        |
    And the following "tool_program > program_users" exist:
      | user   | program     |
      | user11 | Program 04  |
      | user11 | Program 05  |
      | user11 | Program 06  |
    When I am on the "My courses" page logged in as "user11"
    And I should see "Program 04" in the "[data-region='catalogue']" "css_element"
    And I should not see "Program 05" in the "[data-region='catalogue']" "css_element"
    And I should not see "Program 06" in the "[data-region='catalogue']" "css_element"
