...
use tool_catalogue\constants as catalogueconstants;
...
class manager_test extends advanced_testcase {
    ...
    public function test_get_user_allocations(): void {
        ...
        /** @var \tool_certification_generator $certificationgenerator */
    ...
    /**
     * Test should_show_course_cover_modal
     */
    public function test_should_show_course_cover_modal(): void {
        $this->resetAfterTest();

        // Create course, users and enrolments.
        ...
        $method = new \ReflectionMethod(manager::class, 'should_show_course_cover_modal');
        $method->setAccessible(true);

        // Set the user preferences to indicate if the content modal has already been shown.
        set_user_preference('tool_catalogue_show_course_content_' . $course->id, false, $user1);
        set_user_preference('tool_catalogue_show_course_content_' . $course->id, true, $user2);
        set_user_preference('tool_catalogue_show_course_content_' . $course->id, false, $user3);

        // Set the config setting to never display the content modal.
        set_config('displaycoursecovermodals', catalogueconstants::NEVER_DISPLAY, 'tool_catalogue');

        $this->setUser($user1);
        $this->assertFalse($method->invoke(new manager(), $course));
        $this->setUser($user2);
        $this->assertFalse($method->invoke(new manager(), $course));
        $this->setUser($user3);
        $this->assertFalse($method->invoke(new manager(), $course));
        ...
        // Set the config setting to always display the content modal.
        set_config('displaycoursecovermodals', catalogueconstants::DISPLAY_FOR_EVERYBODY, 'tool_catalogue');

        $this->setUser($user1);
        $this->assertTrue($method->invoke(new manager(), $course));
        $this->setUser($user2);
        $this->assertFalse($method->invoke(new manager(), $course));
        $this->setUser($user3);
        $this->assertTrue($method->invoke(new manager(), $course));
        ...
        // Set the config setting to display the content modal only to students and guests.
        set_config('displaycoursecovermodals', catalogueconstants::DISPLAY_ONLY_FOR_STUDENTS_AND_GUESTS, 'tool_catalogue');
     ...
     * @return array
     */
    public static function get_duedate_badge_provider(): array {
        return [
    ...
    /**
     * Data provider for {@see test_can_view_program_cover}
     *
     * @return array
     */
    public static function should_show_program_cover_provider(): array {
        return [
            ['admin', catalogueconstants::DISPLAY_FOR_EVERYBODY, true],
            ['admin', catalogueconstants::DISPLAY_ONLY_FOR_NOT_ADMIN, false],
            ['admin', catalogueconstants::NEVER_DISPLAY, false],
            ['authuser', catalogueconstants::DISPLAY_FOR_EVERYBODY, true],
            ['authuser', catalogueconstants::DISPLAY_ONLY_FOR_NOT_ADMIN, true],
            ['authuser', catalogueconstants::NEVER_DISPLAY, false],
            ['tenantadmin', catalogueconstants::DISPLAY_FOR_EVERYBODY, true],
            ['tenantadmin', catalogueconstants::DISPLAY_ONLY_FOR_NOT_ADMIN, false],
            ['tenantadmin', catalogueconstants::NEVER_DISPLAY, false],
        ];
    }

    /**
     * Test for can_view_program_cover
     *
     * @param string $username
     * @param int $programviewsetting
     * @param bool $expected
     *
     * @dataProvider should_show_program_cover_provider
     */
    public function test_should_show_program_cover(string $username, int $programviewsetting, bool $expected): void {
        $this->resetAfterTest();
        ...
        // Set the displayprogramcoverpage config setting.
        set_config('displayprogramcoverpage', $programviewsetting, 'tool_catalogue');

        $this->assertEquals($expected, manager::should_show_program_cover($program));
    }
}
