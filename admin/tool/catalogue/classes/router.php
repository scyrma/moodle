...
class router {
    ...
    protected static function mycourses_page_program(program $program): string {
        ...
        // Check if it needs to show tabs (program content and program information) or just the program cover.
        if (!manager::should_show_program_cover($program)) {
    ...
    protected static function mycourses_page_program_set(program $program, program_set $set): string {
        ...
        if (!get_user_preferences('tool_catalogue_show_program_content_' . $program->get('id'), false)) {
            // Redirect to program cover page when user hasn't accessed the program before.
            redirect(self::build_program_url($program->get('id')));
        }
