...
class router {
    ...
    public static function mycourses_page(array &$coursemanagemenu): ?string {
        ...
        if (!empty($params['course'])) {
            ...
            $coursemanagemenu = [];
            if (($course = get_course($params['course'])) && permission::can_view_course_cover($course)) {
                return self::mycourses_page_course($course);
            }
            // Course not found or user is not enrolled - redirect to my courses page.
        ...
        if (!empty($params['program'])) {
            ...
            if ($program && $program->get('id') && permission::can_view_program_cover($program)) {
                if (!empty($params['set'])) {
    ...
    protected static function mycourses_page_list(): string {
        ...
        $sort = optional_param('sort', constants::SORT_DUEDATE, PARAM_ALPHA);
        $filter = optional_param('filter', constants::FILTER_ALL, PARAM_ALPHA);
    ...
    /**
     * Sets up the page and displays information about a single course
     *
     * @param stdClass $course
     * @return string
     */
    protected static function mycourses_page_course(stdClass $course): string {
        global $CFG, $PAGE;

        require_once("{$CFG->dirroot}/course/lib.php");

        if (get_user_preferences('tool_catalogue_show_course_content_' . $course->id, false)) {
        ...
        $PAGE->set_url(self::build_course_url((int) $course->id));
        $PAGE->set_heading($course->fullname);
        $PAGE->set_title(get_string('mycourses') . ' - '. $course->fullname);
        $PAGE->add_body_class('layout-option-nonavbar');
        $courseview = new \tool_catalogue\output\course_cover($course);
        return $output->render($courseview);
    }
    ...
    protected static function mycourses_page_program(program $program): string {
        ...
        if (get_user_preferences('tool_catalogue_show_program_content_' . $program->get('id'), false)) {
            ...
            // TODO improve this. It just needs to get certification alerts.
    ...
    public static function build_course_url(int $courseid): moodle_url {
        $url = new moodle_url('/my/courses.php');
        $url->set_slashargument('/course/' . $courseid, self::MYCOURSES_PATH_PARAM);
        return $url;
    }
    ...
    public static function get_mycourses_url_params(): array {
        ...
        if (count($args) > 2 && $args[1] === 'course' && is_numeric($args[2])) {
            return ['course' => (int)$args[2]];
        }
        return [];
    }
