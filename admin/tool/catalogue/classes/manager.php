...
class manager {
    ...
    public static function get_context_header(): ?context_header {
        ...
        if (!empty($params['program'])) {
            ...
            if (permission::can_view_program((int) $USER->id, $program) && !$showprogrampreference) {
                ...
                $imagedata = html_writer::tag('div', '', ['class' => 'rounded item-heading-image',
    ...
    public static function get_course_cover_modal(stdClass $course): string {
        ...
        // The 'disablecoursemodals' config is set to "true" in behat environments only.
        $disablecoursecovermodals = get_config('tool_catalogue', 'disablecoursemodals');
        $showcoursecoverpreference = get_user_preferences('tool_catalogue_show_course_content_' . $course->id, false);
        if (!$showcoursecoverpreference && !$disablecoursecovermodals) {
