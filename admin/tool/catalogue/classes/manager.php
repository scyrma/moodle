...
class manager {
     ...
     *
     * @deprecated since Moodle Workplace 4.2 - please use \tool_program\api::get_user_accessible_programs() instead
     *
     * @param int $userid
     * @return program[]
     */
    public static function get_user_accessible_programs(int $userid): array {
        debugging('This function is deprecated, please use \tool_program\api::get_user_accessible_programs() instead',
            DEBUG_DEVELOPER);

        return program_api::get_user_accessible_programs($userid);
    }
    ...
    /**
     * Returns a pattern image url for courses that have no image set.
     *
     * We cannot use core_renderer::get_generated_url_for_course() because the course will return a not null
     * course progress and course card will shown completion percentage, so our Behat tests will fail.
     * @param int $courseid
     * @return string
     */
    public static function get_course_pattern_url(int $courseid): string {
        // We need to use system context because tool_program pluginfile() validates system context.
        $context = \context_system::instance();
        return \moodle_url::make_pluginfile_url(
            $context->id, 'tool_catalogue', 'course_generated_image', $courseid, '/', 'course.svg')->out();
    }
}
