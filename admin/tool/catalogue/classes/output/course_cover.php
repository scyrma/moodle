 ...
 */
class course_cover implements named_templatable, renderable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $data->customfields = $coursedata->customfields;

        $showcoursedates = get_config('tool_catalogue', 'showcoursedates');
        if ($showcoursedates) {
            $data->pagesectiondates = (object) [];
            $data->pagesectiondates->collapse = $this->collapsesections;
            $dateformat = get_string('strftimedatefullshort', 'langconfig');
            $data->startdate = userdate($coursedata->course->startdate, $dateformat);
            $data->enddate = $coursedata->duedate > 0 ? userdate($coursedata->duedate, $dateformat) : null;
        }
        ...
        if (!empty($data->description) || !empty($data->customfields)) {
    ...
    public function get_template_name(renderer_base $renderer): string {
        return 'tool_catalogue/mycourses/cover/course/course_cover';
    }
}
