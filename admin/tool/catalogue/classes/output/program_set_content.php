 ...
 */
class program_set_content implements named_templatable, renderable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $contenthtml = $output->render_from_template('tool_catalogue/mycourses/cover/program/program_content', $data);

        $coverview = new \tool_catalogue\output\program_cover($this->program);
        $coverhtml = $output->render($coverview);

        // TODO WP-3743 Improve the way to get certification alerts for programs.
        $coverdata = $coverview->export_for_template($output);
        $tabs = new \tool_catalogue\output\program_tabs($coverhtml, $contenthtml, $coverdata->certifications);

        return $tabs->export_for_template($output);
    }
