...
class configuration {
    ...
    /** @var int leave only safe HTML tags */
    const OPTION_HTML_TAGS_SAFE = 0;
    /** @var int allow all HTML tags */
    const OPTION_HTML_TAGS_ALL = 1;
    /** @var int remove all HTML tags */
    const OPTION_HTML_TAGS_NONE = 2;
    /** @var array list of custom field types where HTML in the value should never be cleaned */
    const PLAIN_CUSTOM_FIELD_TYPES = ['checkbox', 'date', 'select', 'text'];
    ...
    public static function get_courses_per_page_frontpage(): int {
        return max(1, (int)self::get_setting_value(
    ...
    public static function get_categories_depth_limit(): int {
        $limit = (int)self::get_setting_value(
    ...
    /**
     * Returns true if the field has a setting to allow HTML tags (summary and 'textarea' custom fields)
     *
     * @param string $field
     * @return bool
     */
    public static function has_allow_html_tags_setting(string $field): bool {
        return ($field === self::FIELD_SUMMARY ||
            (($customfields = self::get_all_course_custom_fields()) &&
                array_key_exists($field, $customfields) &&
                !in_array($customfields[$field]->get('type'), self::PLAIN_CUSTOM_FIELD_TYPES)));
    }

    /**
     * Returns true if the field allows HTML tags in the 'list' view
     *
     * @param string $fieldname
     * @return int one of: OPTION_HTML_TAGS_SAFE, OPTION_HTML_TAGS_ALL, OPTION_HTML_TAGS_NONE
     */
    public static function get_allow_html_tags_list(string $fieldname): int {
        return self::get_allow_html_tags(self::SETTING_DISPLAYFIELDS_LIST, $fieldname);
    }

    /**
     * Returns true if the field allows HTML tags in the 'tiles' view
     *
     * @param string $fieldname
     * @return int one of: OPTION_HTML_TAGS_SAFE, OPTION_HTML_TAGS_ALL, OPTION_HTML_TAGS_NONE
     */
    public static function get_allow_html_tags_tiles(string $fieldname): int {
        return self::get_allow_html_tags(self::SETTING_DISPLAYFIELDS_TILES, $fieldname);
    }

    /**
     * Returns true if the field allows HTML tags
     *
     * @param string $listsettingname name of the setting that represents a list of fields (list or tiles)
     * @param string $fieldname
     * @return int one of: OPTION_HTML_TAGS_SAFE, OPTION_HTML_TAGS_ALL, OPTION_HTML_TAGS_NONE
     */
    private static function get_allow_html_tags(string $listsettingname, string $fieldname): int {
        if (self::has_allow_html_tags_setting($fieldname)) {
            $settingname = self::get_setting_name_allow_html_tags($listsettingname, $fieldname);
            return self::get_setting_value($settingname, self::OPTION_HTML_TAGS_SAFE, PARAM_INT);
        } else {
            // Other fields may already be formatted to contain HTML tags (i.e. tags, course contacts with links, etc)
            // and they should be preserved.
            return self::OPTION_HTML_TAGS_ALL;
        }
    }
