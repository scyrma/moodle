...
class course_exporter extends exporter {
    ...
    protected static function define_other_properties(): array {
        return [
            ...
            'hasrestrictions' => ['type' => PARAM_BOOL],
            'restrictions' => ['type' => PARAM_RAW],
        ];
    }
    ...
    protected function get_other_values(renderer_base $output): array {
        $duelimit = get_config('tool_catalogue', 'coursedisplayduelimit');
        $now = time();
        ...
        $days = round(($duedate - time()) / DAYSECS);
        // TODO Needs to show overdue?
        $duedatebadgestr = ($duedate !== 0 && $duelimit >= 0 && $duedate <= ($now + $duelimit * DAYSECS)) ?
            get_string('daysleft', 'tool_catalogue', $days) : '';
        ...
        foreach ($persistents as $persistent) {
            $linkedprograms[] = [
                ...
                'url' => router::build_program_url($persistent->get('id')),
            ];
        }

        // Get all restrictions for this course in all previous programs.
        [$hasrestrictions, $restrictions] = $this->get_restriction_messages($output, $persistents, $course);
    ...
    private function get_restriction_messages(renderer_base $output, array $persistents, stdClass $course): array {
        ...
        foreach ($persistents as $persistent) {
            ...
            $flatstructure = $this->get_flat_structure($programstructure->baseset->items);
            ...
            foreach ($courses as $courseitem) {
                if ($courseitem->islocked) {
                    $restrictions[] = $courseitem->unlockrequirement;
                } else {
     ...
     * @return array
     */
    private function get_flat_structure(array $items): array {
        $flatstructure = [];
        foreach ($items as $item) {
            if ($item->isset) {
                $flatstructure = array_merge($flatstructure, $this->get_flat_structure($item->items)); // Recursion.
                unset ($item->items);
            }
            $flatstructure[] = $item;
        }
