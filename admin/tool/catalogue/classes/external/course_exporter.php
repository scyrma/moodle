...
class course_exporter extends exporter {
    ...
    protected function get_other_values(renderer_base $output): array {
        ...
        foreach ($courselist->get_course_overviewfiles() as $file) {
            ...
            if (!$isimage) {
                $fileitem += [
                    ...
                    'image' => $output->pix_icon(file_file_icon($file), $file->get_filename()),
                ];
            }
            $allfiles[] = $fileitem;
        }
        ...
        foreach ($persistents as $persistent) {
            $linkedprograms[] = [
                ...
                'name' => \core_external\util::format_string($persistent->get('fullname'), context_system::instance()),
                'url' => (string) router::build_program_url($persistent->get('id')),
            ];
        }

        // Replace base64 course image with course image url, core is returning the image pattern in base64.
        $course->courseimage = $this->get_course_image_url();
        ...
        // Remove course image from course files.
        if (($key = array_search($course->courseimage, array_column($allfiles, 'url'))) !== false) {
            unset($allfiles[$key]);
        }
        ...
        return [
            ...
            'customfields' => $this->export_course_customfields(),
        ];
    }
    ...
    private function get_course_trainers(renderer_base $output): array {
        ...
        foreach ($contacts as $contact) {
            ...
            $trainers[] = (array) $exporter->export($output);
        }
     ...
     * @return array
     */
    private function export_course_customfields(): array {
        global $PAGE;
        ...
        $customfields = $handler->export_instance_data((int) $course->id);
     ...
     * @return string
     */
    private function get_course_image_url(): string {
        $course = $this->related['course'];
        $image = course_summary_exporter::get_course_image($course);
        if (!$image) {
            $image = manager::get_course_pattern_url((int)$course->id);
        }
