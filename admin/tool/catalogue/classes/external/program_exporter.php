...
class program_exporter extends exporter {
    ...
    protected static function define_other_properties(): array {
        return [
            ...
            'programstructure' => [
                'type' => program_content_exporter::read_properties_definition(),
                'optional' => true,
            ],
            ...
            'tags' => ['type' => PARAM_RAW, 'optional' => true, 'multiple' => true],
        ];
    }
    ...
    protected function get_other_values(renderer_base $output): array {
        ...
        $exporter = new program_content_exporter(null, [
            ...
            'courses' => $this->get_courses_with_lastaccess($userid),
        ]);
        $programstructure = $exporter->export($output);
        $data->programstructure = $programstructure;
        $data->basesetcriteria = $data->programstructure->baseset->setcriteriastr;
        $data->progress = $data->programstructure->baseset->progress;
        // TODO WP-3457 Try to avoid loading data with manager.
        $data->certifications = $this->get_certification_messages($userid, $this->related['allocations']);
        $data->id = $program->get('id');
        $data->fullname = $program->get_formatted_name();
        ...
        $data->description = format_text($description, $program->get('descriptionformat'), ['context' => $context]);
        ...
        // Retrieve program allocation dates.
        [$data->startdate, $data->startdatestr, $data->startdateshow] = $this->get_program_date(constants::STARTDATE);
        [$data->duedate, $data->duedatestr, $data->duedateshow] = $this->get_program_date(constants::DUEDATE);
        [$data->enddate, $data->enddatestr, $data->enddateshow] = $this->get_program_date(constants::ENDDATE);
        $duelimit = (int) get_config('tool_catalogue', 'programdisplayduelimit');
        [$data->duedatebadgetype, $data->duedatebadgestr] = manager::get_duedate_badge($data->duedate,
            $duelimit, 'duedateinfodays');
    ...
    private function export_program_customfields(): array {
        ...
        $customfields = $handler->export_instance_data($program->get('id'));
    ...
    private function export_program_tags(): array {
        $tags = core_tag_tag::get_item_tags_array('tool_program', 'tool_program', $this->related['program']->get('id'));
    ...
    /**
     * Return list of courses including the given user last access time to the course
     *
     * @param int $userid
     * @return array
     */
    private function get_courses_with_lastaccess(int $userid): array {
        global $DB;

        if ($courses = $this->related['program']->get_courses()) {
            [$sql, $params] = $DB->get_in_or_equal(array_keys($courses), SQL_PARAMS_NAMED, 'id');
            $query = "
                SELECT courseid, timeaccess
                FROM {user_lastaccess}
                WHERE courseid $sql AND userid = :userid
             ";
            $lastaccessrecords = $DB->get_records_sql($query, $params + ['userid' => $userid]);
            foreach ($courses as $course) {
                $course->lastaccess = $lastaccessrecords[$course->id]->timeaccess ?? 0;
            }
        }
