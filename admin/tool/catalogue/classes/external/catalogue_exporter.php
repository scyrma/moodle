...
class catalogue_exporter extends exporter {
    ...
    private function get_course_items(renderer_base $output): array {
        ...
        // Get all courses accesible for the current user.
        $courses = manager::get_user_accessible_courses();
        ...
        foreach ($courses as $course) {
            ...
            $data->duedate = $coursedata->duedate;
            $data->duedatebadgestr = $coursedata->duedatebadgestr;
            $data->category = $coursedata->course->coursecategory;
            $data->lastaccess = $lastaccess[$course->id]->timeaccess ?? 0;
            $data->progress = $coursedata->course->progress;
            $data->url = router::build_course_url((int) $course->id);
    ...
    private function get_program_items(renderer_base $output): array {
        ...
        $listitems = [];
        $allocations = manager::get_user_allocations();
        $programs = manager::get_user_accessible_programs();
        ...
        foreach ($programs as $program) {
            ...
            $data->progress = $programdata->programstructure->progress;
            $data->numcourses = $programdata->numcourses;
            $data->lastaccess = $programdata->lastaccess;
            $data->duedate = $programdata->duedate;
            $data->duedatebadgestr = $programdata->duedatebadgestr;
            $data->url = router::build_program_url($program->get('id'));
            $data->showprogress = get_user_preferences('tool_catalogue_show_program_content_' . $program->get('id'));
            $data->isprogram = true;
            $data->islocked = false;
    ...
    private function apply_filter(): void {
        switch ($this->related['filter']) {
            case constants::FILTER_COURSES:
                $this->listitems = array_filter($this->listitems, static function(stdClass $item) {
