...
/**
 * Clean up bad data in program users.
 *
 * It was used the userid from the xml instead the new created userid during the import process.
 */
function tool_certification_fix_wrong_program_users_allocation_when_importing_certification() {
    global $DB;

    // Find success imported certifications.
    $params = [
        'importer' => 'tool_certification\tool_wp\importer\certifications',
        'type' => 2,
    ];
    $importedcertifications = $DB->get_records('tool_wp_import_details', $params, 'id', 'data');

    // Find imported users id.
    $importedusers = [];
    foreach ($importedcertifications as $importedcertification) {
        $certification = json_decode($importedcertification->data);
        if (!isset($certification->entityname) || !isset($certification->importerdetails) ||
                !isset($certification->importerdetails->certificationid) || !isset($certification->importerdetails->userid)) {
            continue;
        }
        if ($certification->entityname === 'tool_certification_users') {
            $importedusers[$certification->importerdetails->certificationid][] = $certification->importerdetails->userid;
        }
    }

    foreach ($importedusers as $certificationid => $users) {
        // Find records to update.
        $sql = "SELECT tpu.id
                  FROM {tool_program_users} tpu
             LEFT JOIN {tool_certification_users} tcu
                    ON tcu.certificationid = tpu.certificationid
                   AND tcu.userid = tpu.userid
                 WHERE tcu.id IS NULL
                   AND tpu.certificationid = :certificationid
              ORDER BY tpu.id";
        $params = ['certificationid' => $certificationid];
        $programusers = $DB->get_fieldset_sql($sql, $params);

        if (count($programusers) != count($users)) {
            continue;
        }

        // Update the wrong user ids.
        array_map(function($programid, $userid) use ($DB) {
            $sql = "UPDATE {tool_program_users} SET userid = :userid WHERE id = :id";
            $params = ['userid' => $userid, 'id' => $programid];
