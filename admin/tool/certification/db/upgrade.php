...
function xmldb_tool_certification_upgrade($oldversion) {
    ...
    if ($oldversion < 2022111700) {
        // Clean up bad data in program users table.
        tool_certification_fix_wrong_program_users_allocation_when_importing_certification();

        // Certification savepoint reached.
        upgrade_plugin_savepoint(true, 2022111700, 'tool', 'certification');
    }
    ...
    if ($oldversion < 2023011701) {
        ...
        $results = $DB->get_records('tool_dynamicrule_outcome',
            ['classname' => 'tool_certification\tool_dynamicrule\outcome\allocation']);
        foreach ($results as $result) {
            $configdata = @json_decode($result->configdata, true);
            $configdata['action'] = \tool_certification\tool_dynamicrule\outcome\allocation::DONT_MODIFY_ALLOCATION;
            $result->configdata = json_encode($configdata);
            $DB->update_record('tool_dynamicrule_outcome', $result);
        }

        // Certification savepoint reached.
        upgrade_plugin_savepoint(true, 2023011701, 'tool', 'certification');
    }
