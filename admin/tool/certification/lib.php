...
use block_myteams\global_notification;
...
function tool_certification_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    if ($iscurrentuser || $canviewreports) {
        ...
        if (!empty($certs)) {
            $urlreport = new moodle_url('/admin/tool/certification/user_report.php',
                ['id' => $user->id, 'type' => constants::STATUS_OPEN]);
            $params = [
                'href' => $urlreport->out(),
                'count' => count($certs),
            ];
            $node = new core_user\output\myprofile\node('learning', 'ongoingcertifications', '', null, null,
                get_string('ongoingcertificationslink', 'tool_certification', $params));
        ...
        if (!empty($certs)) {
            $urlreport = new moodle_url('/admin/tool/certification/user_report.php',
                ['id' => $user->id, 'type' => constants::STATUS_OVERDUE]);
            $params = [
                'href' => $urlreport->out(),
                'count' => count($certs),
            ];
            $node = new core_user\output\myprofile\node('learning', 'overduecertifications', '', null, null,
                get_string('overduecertificationslink', 'tool_certification', $params));
            $category->add_node($node);
        }
        ...
        if (!empty($certs)) {
            $urlreport = new moodle_url('/admin/tool/certification/user_report.php',
                ['id' => $user->id, 'type' => constants::STATUS_EXPIRED]);
            $params = [
                'href' => $urlreport->out(),
                'count' => count($certs),
            ];
            $node = new core_user\output\myprofile\node('learning', 'expiredcertifications', '', null, null,
                get_string('expiredcertificationslink', 'tool_certification', $params));
            $category->add_node($node);
        }
        ...
        if (!empty($certs)) {
            $urlreport = new moodle_url('/admin/tool/certification/user_report.php',
                ['id' => $user->id, 'type' => constants::STATUS_CERTIFIED]);
            $params = [
                'href' => $urlreport->out(),
                'count' => count($certs),
            ];
            $node = new core_user\output\myprofile\node('learning', 'certifiedcertifications', '', null, null,
                get_string('certifiedcertificationslink', 'tool_certification', $params));
            $category->add_node($node);
        }
    }
}
 ...
 */
function tool_certification_block_myteams_user_section(int $userid): array {
    if (!permission::can_view_user_progress($userid)) {
        return [];
    }

    // Generate the "Certifications" userinfo section.
    $sectionname = get_string('certifications', 'tool_certification');
    $sectionlink = new moodle_url('/admin/tool/certification/user_report.php', ['id' => $userid]);
    $certificationssection = new userinfo_section($sectionname, $sectionlink, 10);

    // Get certification user data from certification API.
    $certificationusers = api::get_certifications_by_userid($userid);

    foreach ($certificationusers as $certification) {
        // Get program progress.
        $programtreeprogress = new program_tree_progress($certification->get_certification_program(), $userid);
        $percentage = $programtreeprogress->get_program_progress_as_percentage();

        $progress = get_string('progress', 'tool_certification', $percentage);
        $item = new userinfo_section_item($certification->get_formatted_name(), $progress);

        $certificationstatus = api::get_user_allocation_status($certification->get('id'), $userid);

        if ($certificationstatus[0]['status'] == constants::STATUS_EXPIRED) {
            $item->add_badge(get_string('expired', 'tool_certification'), 'danger');
            $item->set_overdue(true);
        } else if ($certificationstatus[0]['status'] == constants::STATUS_OVERDUE) {
            $item->add_badge(get_string('overdue', 'tool_certification'), 'danger');
            $item->set_overdue(true);
        }

        $certificationssection->add_item($item);
    }
...
/**
 * Callback for block_myteams, return a global report link to be added to the MyTeams block.
 *
 * @return global_report_link|null
 */
function tool_certification_block_myteams_global_report_link(): ?global_report_link {
    if (!permission::can_view_certifications_progress_report()) {
        return null;
    }
    return new global_report_link(
        get_string('fullcertificationreport', 'tool_certification'),
        new moodle_url('/admin/tool/certification/report.php')
    );
}

/**
 * Callback for block_myteams, return a notification to be added to the MyTeams block.
 *
 * @param int[] $manageduserids
 * @return global_notification|null
 */
function tool_certification_block_myteams_global_notification(array $manageduserids): ?global_notification {
    global $DB;

    // Certification tenant condition.
    [$tenantsql, $tenantparams] = hierarchy::filter_own_or_parent_shared_entities_sql("tc.tenantid", "tc.shared=1");
    // Certification status condition.
    [
        $certificationstatusjoin,
        $certificationstatuswhere,
        $certificationstatusparams
    ] = api::get_certification_status_sql_query(null, constants::STATUS_OVERDUE, false, 'u', 'tc', 'tcu', 'tcc', 'tp', null);
    // Managed users condition.
    [$insql, $inparams] = $DB->get_in_or_equal($manageduserids, SQL_PARAMS_NAMED, 'mu');

    // Perform the query to get the count of managed users that have any overdue certification.
    $sql = "SELECT COUNT(tc.id)
             ...
             WHERE u.deleted = 0
               AND tc.archived = 0
               AND $tenantsql
               AND $certificationstatuswhere
               AND u.id $insql";
    $params = $tenantparams + $certificationstatusparams + $inparams;
    $shownotification = $DB->count_records_sql($sql, $params) > 0;

    $url = new moodle_url('/admin/tool/certification/report.php', ['type' => constants::STATUS_OVERDUE]);
    $message = get_string('managedusersoverdue', 'tool_certification');

    return $shownotification ? new global_notification($message, $url) : null;
}
