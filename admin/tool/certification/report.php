...
if (!empty($userid)) {
...
} else if (!permission::check_access()) {
    // The user can view the report (as org manager) but does not have
    // access to the programs list. In this case we can not correctly set up the page.
    // TODO WP-4163 this will be properly addressed when we add a 'Reports' tab to the certifications list.
    $PAGE->set_context(context_system::instance());
    $PAGE->set_title($strcertificationprogress);
    $PAGE->set_heading($strcertificationprogress);
} else {
    ...
    if (!empty($certification)) {
        \tool_wp\admin_externalpage::setup_subpage($certification->get('fullname'),
            new moodle_url('/admin/tool/certification/edit.php', ['id' => $certification->get('id')]));
    }
