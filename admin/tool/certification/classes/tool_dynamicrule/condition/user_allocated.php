    ...
    public function get_sql(): array {
        ...
        $join = "JOIN {tool_certification_users} {$u}
                 ...
                 LEFT JOIN {tool_program_users} {$pu}
                   ON {$pu}.programid={$u}.currentprogramid AND {$pu}.userid={$u}.userid
        ...
        // Make sure the allocation is not suspended and not a future allocation.
        $now1 = database::generate_param_name();
        $where .= " AND ({$pu}.id IS NULL OR {$pu}.startdate <= :{$now1})".
            " AND {$u}.status = ". constants::STATUS_OVERRIDE_DEFAULT;
