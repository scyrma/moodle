 ...
 * @license    Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class recertification_grace_period_ended extends condition_base {
    /**
    ...
    public function get_title(): string {
        return get_string('conditionrecertificationgraceperiod', 'tool_certification');
    }
    ...
    public function get_sql(): array {
        $tc = database::generate_alias();
        $tcu = database::generate_alias();
        $tcc = database::generate_alias();
        $enabled = database::generate_param_name();
        ...
        $join = "
            ...
            ON {$tcu}.userid = {$tcc}.userid AND {$tcu}.certificationid = {$tcc}.certificationid
            AND {$tcc}.islast = 1 AND {$tcc}.timerevoked = 0
        ";

        $where = "
            {$tcc}.expirydate > 0 AND {$tcc}.expirydate <= :{$now}
        ...
        if ($this->get_conditiondateenabled()) {
            $where .= "AND {$tcu}.graceperiodends <= " . $this->get_conditiondate();
        }

        $params = [$enabled => constants::STATUS_OVERRIDE_DEFAULT, $now => time(),
    ...
    public function get_description(): string {
        ...
        if ($this->get_conditiondateenabled()) {
            ...
            $stringid = 'conditionrecertificationgraceperiodendsdescription';
        }
    ...
    public function get_event_subscription() {
        return '\tool_certification\event\deallocate_after_graceperiodends';
    }
     ...
     *
     * @param array $keys
     * @param \stdClass $user user objects
     * @param outcome_base $calleroutcome
     * @return array
     */
    public function get_custom_data_for_outcome(array $keys, \stdClass $user, outcome_base $calleroutcome): array {
        ...
        return $this->get_key_values($keys, $certification, $user->id, true);
    }
}
