...
class user_log {
    ...
    private function generate_log(): void {
        ...
        $previouskey = null;
        foreach ($completionrecords as $recordkey => $record) {
            ...
            if ($inarray) {
                ...
                // Only show "Certification expired" in the log if the expiration date is before the next certification date or
                // record is the last one.
                $islast = (bool)$record->islast;
                if ((int)$record->expirydate > 0 && time() > (int)$record->expirydate &&
                    ((isset($previouskey) && (int)$completionrecords[$previouskey]->expirydate < (int)$record->timecertified)
                        || $islast)) {
                    $date = $islast ? $record->expirydate : (int)($completionrecords[$previouskey]->expirydate);
                    $report[] = [
                        'user' => null,
            ...
            // Used to check previous record expirydate.
