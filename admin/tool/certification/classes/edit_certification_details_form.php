...
class edit_certification_details_form extends modal_form {
    ...
    protected function definition() {
        ...
        if (empty($this->_ajaxformdata['id']) || 0 === (int)$this->_ajaxformdata['id']) {
            ...
            $program = new program($certification->get('program'));
            $programname = format_string($program->get('fullname'));
            $mform->addElement('hidden', 'program');
            $mform->setType('program', PARAM_INT);
            $selectprogstr = get_string('selectprogram', 'tool_certification');
            $mform->addElement('static', 'programname', $selectprogstr, $programname);
            $mform->setType('programname', PARAM_RAW);
        }
        ...
        $mform->addElement('static', 'warning', '', html_writer::tag('div', $warningstr, array('class' => 'alert alert-warning')));
        ...
        $relatallocdatestr = get_string('relativeallocationdate', 'tool_certification');
        $choices = [
            constants::DATE_USER_ALLOCATION_DATE => $allocationdatestr,
            constants::DATE_ABSOLUTE => $selectdatestr,
            constants::DATE_RELATIVE_TO_ALLOCATION_DATE => $relatallocdatestr,
        ];
        $group = [];
