...
class edit_certification_users_edit_form_modal extends dynamic_form {
    ...
    protected function definition() {
        ...
        foreach ($statuses as $status) {
            $htmlstatuses .= api::get_user_allocation_status_badges($status['status']);
        }
        ...
        if ($iscertified && $certification->get('requirerecertification')) {
        ...
        } else if ($iscertified && !$currentprogramid && $certification->get('requirerecertification')) {
            $group = [];
            $group[] =& $mform->createElement('date_time_selector', 'startdate', '');
        ...
        } else if ($certification->get('requirerecertification')) {
        ...
        } else if ($iscertified && $certification->get('requirerecertification')) {
        ...
        // Grace period.
        if ($programisdifferent && $iscertified && $certification->get('requirerecertification')) {
            $options = [
        ...
        // Program dropdown.
        if ($iscertified && $currentprogramid && $certification->get('requirerecertification')) {
            ...
            if ((int)$currentprogramid !== (int)$certification->get('recertificationprogram')
                && $certification->get('recertdifferentprogram')) {
    ...
    public static function prepare_data_for_dynamic_submission(certification_user $certificationuser): array {
        ...
        $expirydate = $expirydatetype = $graceperiodends = $graceperiodendslocked = 0;
        $duedate = $programuser->get('duedate');
        // Check if user is certified.
        if ((bool)$lastcompletion) {
            ...
            // expiry date.
            $expirydate = $duedate = $lastcompletion->get('expirydate');
            if ($expirydate === 0) {
                $expirydatetype = constants::DATE_NEVER;
            } else {
                // Check if expiry date is the default one or is a custom one.
                $expirydatecalculated = api::recalculate_user_expiry_date($certificationuser->get_certification(),
                    $certificationuser, $certificationuser->get('timecreated'), $programuser->get('duedate'));
                $expirydatetype = ($expirydate !== $expirydatecalculated) ? constants::DATE_ABSOLUTE : constants::DATE_NONE;
            }
