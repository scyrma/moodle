...
class users_table extends system_report {
    ...
    protected function initialise() {
        ...
        $this->add_base_join("LEFT JOIN {tool_program_users} {$this->joinalias}
            ON {$this->joinalias}.certificationid = tcu.certificationid AND {$this->joinalias}.userid = tcu.userid");
        ...
        $this->add_base_fields($tcufields . ', '. "tcc.id AS completionid, $this->joinalias.programid");
    ...
    private function add_actions(): void {
        ...
        $action = new report_action($logurl, $logicon, [
            'class' => 'action-icon view_certification_user_log',
            'data-userid' => ':userid',
            'data-id' => ':certificationid'
        ]);
        $action->add_callback(function() {
