...
use renderer_base;
use templatable;
use tool_certification\local\reports\user_certifications;
...
class certifications_user_report implements templatable, renderable {
    ...
    public function export_for_template(renderer_base $output): array {
        ...
        if (class_exists('\\tool_reportbuilder\system_report_factory')) {
            ...
            $report = system_report_factory::create(user_certifications::class, $params);

            if (-1 !== $this->type) {
                $reportid = $report->get_id();
                $filter = new \stdClass();
                $filter->{'tool_certification_users:filterablestatus_op'} = 1;
                $filter->{'tool_certification_users:filterablestatus'} = $this->type;
        ...
        $exporter['certificationslisttable'] = $table;
        return $exporter;
    }
}
