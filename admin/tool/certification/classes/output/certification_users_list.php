...
class certification_users_list implements \renderable , \templatable {
    ...
    /** @var users_table $userreport The system report of certification users. */
    ...
    public function __construct(certification $certification) {
        ...
        if (class_exists('\\tool_reportbuilder\\system_report_factory')) {
            // Users list.
            $report = system_report_factory::create(users_table::class, ['id' => $certification->get('id')]);
            $this->userreport = $report->output();
        } else {
            $str = get_string('reportbuilderuserlist', 'tool_certification');
            $this->userreport = \html_writer::tag('div', $str, array('class' => 'alert alert-warning'));
        }
    }
