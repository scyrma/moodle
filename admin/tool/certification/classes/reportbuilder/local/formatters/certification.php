...
class certification {
     ...
     * @return string
     */
    public static function startdate(?int $value, stdClass $row): ?string {
     ...
     * @return string
     */
    public static function duedate(?int $value, stdClass $row): string {
        if (!$row->duedatetype) {
            return '';
        }
     ...
     * @return string
     */
    public static function expirydate(?int $value, stdClass $row): ?string {
        ...
        switch ($row->expirydatetype) {
            ...
            case constants::DATE_AFTER_DUE_DATE:
                if ((int) $row->startdatetype === constants::DATE_ABSOLUTE) {
                    $duedate = \tool_program\api::calculate_relative_date($row->duedaterelative, $row->startdateabsolute);
                    $expirydate = \tool_program\api::calculate_relative_date($row->expirydaterelative, $duedate);
    ...
    public static function actions(?int $value, stdClass $row): string {
        ...
        if (permission::can_view_users_progress($certification)) {
            $reporturl = new moodle_url('/admin/tool/certification/certification_report.php', ['id' => $row->id]);
