...
class progress extends system_report {
    ...
    protected function can_view(): bool {
        ...
        return permission::can_view_certifications_progress_report();
    }
    ...
    protected function add_columns(): void {
        ...
        $this->get_column('certification:fullnamewithlink')->set_is_available($certificationid === 0 || $userid !== 0);
        ...
        // Rename the certification name column title.
        $this->get_column('certification:fullnamewithlink')
            ->set_title(new lang_string('certificationname', 'tool_certification'));

        if (!$certificationid) {
            $this->set_initial_sort_column('certification:fullnamewithlink', SORT_ASC);
        } else if (!$userid) {
    ...
    /**
     * Preset the filter value for the status filter.
     *
     * @param int $status when -1 - remove the filter
     */
    public function prefilter_by_status(int $status) {
        if ($status === -1) {
            $this->set_filter_values([
                'certification_user:filterablestatus_operator' => select::ANY_VALUE,
            ]);
        } else {
            if (!array_key_exists($status, \tool_certification\api::get_certification_statuses_fieldset())) {
            ...
            $this->set_filter_values([
                ...
                'certification_user:filterablestatus_value' => $status,
            ]);
        }
    }
}
