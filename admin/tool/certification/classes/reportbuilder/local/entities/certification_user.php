 ...
 * join also with tool_certification_compltion (to check user allocation completions) and with tool_program_users to
 * get status.
 *
...
class certification_user extends base {
    ...
    protected function get_all_columns(): array {
        ...
        $columns[] = (new column(
            ...
            ->add_join("
                LEFT JOIN {tool_program_users} {$programuser}
                       ON {$programuser}.userid = {$certificationuser}.userid
                      AND {$programuser}.programid = tp.id
                      AND {$programuser}.certificationid = {$certificationuser}.certificationid
            ")
    ...
    protected function get_all_filters(): array {
        ...
        $filters[] = (new filter(
            ...
            new lang_string('certificationstatus', 'tool_certification'),
            $this->get_entity_name(),
            api::get_status_sql_cases(0, $certificationuser, $completion, $tablepualias, true)
        ))
            ->add_joins($this->get_joins())
