...
class certification_user_allocation extends datasource {
    ...
    protected function initialise(): void {
        ...
        $revokedbyalias = database::generate_alias();
        $revokebyentity = (new user())
            ->add_join($revokejoin)
            ->add_join("LEFT JOIN {user} {$revokedbyalias} ON {$revokedbyalias}.id = {$revoke}.revokedby")
            ->set_table_alias('user', $revokedbyalias)
            ->set_entity_name('tool_certification_revokedby')
            ->set_entity_title(new lang_string('revokedby', 'tool_certification'));
        $this->add_entity($revokebyentity);
        ...
        $certificationtenantentity
            ...
            ->add_join("JOIN {tool_tenant} {$tooltenant} ON {$tooltenant}.id = {$certification}.tenantid")
        ...
        $certifiedbyalias = database::generate_alias();
        $certifiedbyentity = (new user())
            ->add_join("LEFT JOIN {user} {$certifiedbyalias} ON {$certifiedbyalias}.id = {$certificationcompletion}.certifiedby")
            ->set_table_alias('user', $certifiedbyalias)
        ...
        $programtenantentity
            ...
            ->add_join("JOIN {tool_tenant} {$tooltenantalias} ON {$tooltenantalias}.id = {$program}.tenantid")
        ...
        $this->add_filters_from_entity('tool_certification_certifiedby', ['fullname', 'firstname', 'lastname', 'email']);
        $this->add_conditions_from_entity('tool_certification_certifiedby', ['fullname', 'firstname', 'lastname', 'email']);

        // Add "Revoked by" columns.
        $this->add_columns_from_entity('tool_certification_revokedby',
            ['fullname', 'fullnamewithlink', 'fullnamewithpicture', 'fullnamewithpicturelink', 'firstname', 'lastname', 'email']);
        $this->add_filters_from_entity('tool_certification_revokedby', ['fullname', 'firstname', 'lastname', 'email']);
        $this->add_conditions_from_entity('tool_certification_revokedby', ['fullname', 'firstname', 'lastname', 'email']);
    ...
    public function get_default_columns(): array {
        ...
        ], permission::can_show_tenant_report_column() ? ['certification_tenant:name'] : [],
