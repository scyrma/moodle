...
class report_certifications extends datasource {
     ...
     * @return array
     */
    private static function get_users_joins(): array {
        // Added tcc join here because status in certification user uses completion table.
        return [
            'LEFT JOIN {tool_certification_users} tcu ON tcu.certificationid = tc.id',
    ...
    protected function set_columns(): void {
        ...
        $this->add_entity((new certificationuser_entity())
            ->add_joins(self::get_users_joins()));
        ...
        $this->add_entity((new user())
            ->add_join(self::get_users_joins()[0])
            ->add_join(self::get_users_joins()[1]));
