...
class api {
    ...
    public static function allocate_user(certification $certification, stdClass $data): certification_user {
        ...
        // Trigger event.
        // TODO WP-2923 this event should be triggered before user is marked as certified
        // and before we send notifications. Allocation notification should also
        // be sent before user is marked as certified.
        // Ideally the certification event should be triggered after the program record
        // is created but before the program allocation event is triggered, so that
        // at the moment of the event all records already exist but the sequence of events
        // in the logs is natural.
     ...
     * @param bool $rollbackinitialcertdates Roll back to the initial certification allocation (with the original dates)
     * otherwise user dates will be recalculated based on current datetime.
     * @return program_user
     * @throws coding_exception
    ...
    public static function reallocate_user_into_initial_program(certification $certification,
                                                                ...
                                                                bool $rollbackinitialcertdates = false): program_user {
        $certificationid = $certification->get('id');
        ...
        // Recalculate certification user dates only when we don't need to keep the initial certification dates.
        if (!$rollbackinitialcertdates) {
            // Recalculate dates.
            self::recalculate_certification_user_dates($certification, $certificationuser);
        }

        // We only set as certified if user doesn't need to be restored to initial certification certified
        // and the certification program is already completed.
        if (!$rollbackinitialcertdates && self::is_program_completed($certification->get('program'), $userid)) {
            self::set_user_as_certified($userid, $certificationid);
        }
        ...
        // Trigger event.
        // TODO WP-2923 this is not the correct event here, user is already allocated to the certification.
    ...
    public static function allocate_user_recertification(certification $certification, int $programid, int $userid,
        ...
        // Stored certification user nextstartdate is when the program will start for this user.
        $recertificationstartdate = (int)$certificationuser->get('nextstartdate');
        // Check if we need to recalculate the default nextstartdate.
        if ($recertificationstartdate === 0) {
    ...
    public static function recalculate_certification_user_dates(certification $certification,
        ...
        if ($lastcompletion && $certification->get('requirerecertification')
        ...
        if (!$certification->get('requirerecertification') && (int)$certificationuser->get('graceperiodends') > 0
    ...
    public static function deallocate_user(int $certificationid, int $userid): void {
        ...
        self::send_certification_user_deallocated_notification($userid, $certificationuser->get('currentprogramid') ?? 0);
    }
    ...
    private static function recalculate_user_start_date(certification $certification,
        ...
        switch ($certification->get('startdatetype')) {
            ...
            case constants::DATE_ABSOLUTE:
                $userstartdate = $certification->get('startdateabsolute');
                break;
            case constants::DATE_RELATIVE_TO_ALLOCATION_DATE:
                $userstartdate = \tool_program\api::calculate_relative_date($certification->get('startdaterelative'),
                    $userallocationdate);
                break;
            default:
    ...
    private static function recalculate_user_due_date(certification $certification,
        ...
        if (constants::DATE_NONE === $userstartdate || (int)$certification->get('duedatetype') === constants::DATE_NEVER) {
            ...
            $userduedate = \tool_program\api::calculate_relative_date($certification->get('duedaterelative'), $userstartdate);
        }
    ...
    public static function recalculate_user_expiry_date(certification $certification,
        ...
        switch ($certification->get('expirydatetype')) {
            ...
            case constants::DATE_NEVER:
                ...
                break;
            case constants::DATE_ABSOLUTE:
                $userexpirydate = $certification->get('expirydateabsolute');
                break;
            case constants::DATE_AFTER_COMPLETION:
                ...
                if (!$certcompletion) {
                    return \tool_program\api::calculate_relative_date($certification->get('expirydaterelative'));
                }
                ...
                if (!empty($programcompletion)) {
                    ...
                    $userexpirydate = \tool_program\api::calculate_relative_date($expirydaterelative,
                        $programcompletion->get('timecreated'));
                } else if (!empty($certcompletion)) {
                    // If user has been certified manually apply relative date to certified date.
                    $expirydaterelative = $certification->get('expirydaterelative');
                    $userexpirydate = \tool_program\api::calculate_relative_date($expirydaterelative,
                        $certcompletion->get('timecertified'));
                }
                break;
            case constants::DATE_AFTER_ALLOCATION_DATE:
                $userexpirydate = \tool_program\api::calculate_relative_date($certification->get('expirydaterelative'),
                    $userallocationdate);
                break;
    ...
    private static function recalculate_recertification_user_expiry_date(certification $certification,
        ...
        // If $certcompletion is not passed it needs to retrieve the last completion record.
        // Also, in the RECERT_EXPIRY_DATE_AFTR_PREV_EXP and RECERT_EXPIRY_DATE_AFTR_LATEST options it always needs the previous
        // expiry date to calculate the new one.
        if (!$certcompletion
            || (int) $certification->get('recertexpirydatetype') === constants::RECERT_EXPIRY_DATE_AFTR_PREV_EXP
            || (int) $certification->get('recertexpirydatetype') === constants::RECERT_EXPIRY_DATE_AFTR_LATEST) {
        ...
        switch ($certification->get('recertexpirydatetype')) {
            case constants::RECERT_EXPIRY_DATE_NEVER_DATE:
                return 0;
            case constants::RECERT_EXPIRY_DATE_AFTR_PREV_COMPL:
                return \tool_program\api::calculate_relative_date($certification->get('recertexpirydaterelative'),
                    $certcompletion->get('timecertified'));
            case constants::RECERT_EXPIRY_DATE_AFTR_PREV_EXP:
                return \tool_program\api::calculate_relative_date($certification->get('recertexpirydaterelative'),
                    $certcompletion->get('expirydate'));
            case constants::RECERT_EXPIRY_DATE_AFTR_LATEST:
                $latest = max([$certcompletion->get('timecertified'), $certcompletion->get('expirydate')]);
                return \tool_program\api::calculate_relative_date($certification->get('recertexpirydaterelative'), $latest);
            default:
    ...
    public static function update_certification_user_dates_and_status(certification_user $certificationuser,
        ...
        if (!$hascompletionrecord) {
            ...
            if ($certification->get('requirerecertification')) {
                ...
                $validateddata->graceperiodends = 0;
            }
     ...
     * @param int $certificationid
     * @param int $userid
     * @return array allocation status and stringid for mobile app WS
     */
        ...
        switch ($status) {
            case constants::STATUS_FUTUREALLOCATION:
                return html_writer::span(
                    get_string('futureallocation', 'tool_certification'),
                    'badge badge-info'
                );
     ...
     *
     * @param int $userid
     * @return array certification_user
     */
    public static function get_user_allocations(int $userid): array {
        return certification_user::get_records(['userid' => $userid]);
    }
    ...
    public static function set_user_as_certified(int $userid, int $certificationid, ?int $expirydate = null, int $timecertified = 0,
        ...
        // If timecertified is not set OR if is set in the future change it to use $now instead.
        if ($timecertified === 0 || $timecertified > $now) {
            $timecertified = $now;
        }
        ...
        self::send_certification_completed_notification($userid, $certificationid, $currentprogramid);
    }
     ...
     *
     * @param int|null $certificationid Given certification id or null for all certifications
     ...
     * @param string $pr Table alias for table programs
     * @param string|null $cid Name for named param certification id or null for all certifications
     * @param string $pu Table alias for table program users
     * @return array
     */
    public static function get_certification_status_sql_query(?int $certificationid, int $statusid, bool $isnegated = false,
        ...
        $params = !is_null($certificationid) ? [$cid => $certificationid] : [];

        return [$join, $where, $params];
    }

    /**
     * Returns subquery that checks that a user has expected status on the given certification
     *
     * Example - select all users who are certified in the certification $cid:
     *     [$where,$params] = api::get_certifications_with_criteria_conditions($cid, constants::STATUS_CERTIFIED);
     *     $sql = "SELECT * FROM {user} u WHERE $where";
     *
     * @param int $certificationid Given certification id
     * @param int $status Expected status of the certification
     * @param int|null $datecreated Minimum certified date for the certification (if $status==constants::STATUS_CERTIFIED)
     * @param bool $withoutcurrentprogram only return users without current program set
     *     (i.e. users who are certified and recertification has not started)
     * @param string $user Table alias for table {user} from the main query
     * @return array array [$sqlsubquery, $params]
     */
    public static function get_certifications_with_criteria_conditions(int $certificationid, int $status,
        int $datecreated = null, bool $withoutcurrentprogram = false, string $user = 'u'): array {

        $wheredate = '';
        $user2 = database::generate_alias();
        $certification = database::generate_alias();
        $certificationuser = database::generate_alias();
        $certificationusercompletion = database::generate_alias();

        $program = database::generate_alias();
        $programuser = database::generate_alias();
        $certificationparam = database::generate_param_name();

        $join = self::get_status_sql_join($user2, $certification, $certificationuser, $certificationusercompletion,
            $program, $programuser, $certificationparam);
        $statuscase = self::get_status_sql_cases($status, $certificationuser, $certificationusercompletion, $programuser, true);

        if ($datecreated) {
            $wheredate = " AND {$certificationusercompletion}.timecertified >= " . $datecreated;
        }

        if ($withoutcurrentprogram) {
            $wheredate = " AND {$certificationuser}.currentprogramid IS NULL";
        }

        $where = " EXISTS (SELECT 1 FROM {user} {$user2}
                      {$join}
                WHERE {$user2}.id = {$user}.id
                AND {$statuscase} = {$status}
                {$wheredate})";

        $params = [$certificationparam => $certificationid];
        return [$where, $params];
    }
    ...
    public static function send_certification_completed_notification(int $userid, int $itemid, int $programid = 0): void {
        ...
        if ($expirydate) {
            $expirydatestr = userdate($expirydate, get_string_manager()->get_string('strftimedatefullshort', 'langconfig',
                null, $user->lang));
            $expiresonstr = $expirymessage = get_string_manager()->get_string('notificationcertificationwillexpireon',
                'tool_certification', $expirydatestr, $user->lang);
    ...
    private static function send_moodle_notification(stdClass $user, string $provider, string $subject, string $fullmessage,
        ...
        $message->customdata = ['notificationiconurl' => $program->get_image_url()];
    ...
    private static function calculate_certification_completion_for_all_certification_users(certification $certification): void {
        ...
        foreach ($allocations as $allocation) {
            ...
            if ($allocation->get('isrecertification') && $certification->get('recertdifferentprogram')) {
     ...
     * @return bool
     */
    public static function is_idnumber_unique(int $certificationid, string $idnumber, int $tenantid = 0): bool {
        return hierarchy::is_entity_idnumber_unique('tool_certification', $idnumber, $certificationid,
            $tenantid, 'archived = 0');
    }
    ...
    public static function get_certification_by_idnumber(string $idnumber, int $tenantid): ?certification {
        $record = hierarchy::find_entity_by_idnumber('tool_certification', $idnumber,
            $tenantid, 'archived = 0', [], true, 'shared = 1');
        return $record ? new certification(0, $record) : null;
    }
    ...
    public static function allocate_recertification_users(?certification_user $certuser = null,
        ...
        foreach ($records as $record) {
            ...
            // We need to reset the program progress before create the new allocation
            // to avoid complete the recertification program based on previous completion data.
            if ($resetprogram) {
    ...
    public static function deallocate_users_after_grace_period_end(?certification_user $certuser = null,
                                                                   ...
                                                                   bool $rollbackinitialcertdates = false): void {
        global $DB;
        ...
        if ($records) {
            foreach ($records as $record) {
                ...
                $programuser = self::reallocate_user_into_initial_program($certification, $certificationuser,
                    $rollbackinitialcertdates);

                // We need to restore user allocation dates to initial certification data.
                if ($rollbackinitialcertdates) {
                    // Current certification allocation is not a recertification, so we need to reset some values.
                    $certificationuser->set('isrecertification', 0);
                    $certificationuser->set('graceperiodends', 0);

                    // Since a new allocation is created when recertification isn't completed and those dates
                    // has been calculated based on current day, we need to update them keeping the initial certification dates.
                    $userdates = self::get_certification_user_dates($certificationuser);
                    $programuserdata = [
                        'id' => $programuser->get('id'),
                        'startdate' => $userdates->userstartdate,
                        'duedate' => $userdates->userduedate,
                        'timecreated' => $certificationuser->get('timecreated'),
                    ];
                    $DB->update_record('tool_program_users', (object)$programuserdata);
                }
    ...
    public static function recalculate_nextstartdate(certification $certification, int $expirydate): int {
        ...
        return \tool_program\api::calculate_relative_date('-' . $certification->get('recertstartdaterelative'), $expirydate);
    }
    ...
    public static function recalculate_graceperiodends(certification $certification, int $expirydate): int {
        ...
        return \tool_program\api::calculate_relative_date($certification->get('recertgraceperiod'), $expirydate);
    }
    ...
    public static function is_recertification_program_different(certification $certification,
        ...
        if ($currentprogram) {
            ...
            return $certification->get('recertdifferentprogram');
        }
    }
