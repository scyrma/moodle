...
class permission {
    ...
    /**
     * Check whether user has access to a given certification (based on whether they can view its details or reports)
     *
     * @param certification $certification
     * @return bool
     */
    public static function check_access_to_certification(certification $certification): bool {
        return self::can_view_details($certification)
            || self::can_view_users_progress($certification);
    }
     ...
     * @throws moodle_exception
     */
    public static function require_check_access_to_certification(certification $certification): void {
        if (!self::check_access_to_certification($certification)) {
            throw new moodle_exception('errornopermissionmanagecertifications', 'tool_certification');
        }
    }
     ...
     *
     * @param certification_user|null $certificationuser
     * @throws moodle_exception
     */
    ...
    public static function check_access(): bool {
        return self::can_view_list() || self::can_view_certifications_progress_report();
    }
    ...
    public static function can_certify_user_deep(certification_user $certificationuser,
        ...
        if (!$certification->get('requirerecertification') && $certcompletion !== false) {
            return false;
        }
     ...
     * @param certification_user $certificationuser
     * @param bool $certificationcompleted user has completed the certification
     * @param int|null $programid program where user is allocated
     * @return bool
     */
    ...
    /**
     * Require that current user can revoke the certification from a given user.
     *
     * @param certification_user $certificationuser
     * @param bool $certificationcompleted user has completed the certification
     * @param int|null $programid program where user is allocated
     */
    public static function require_can_revoke_user_certification(certification_user $certificationuser,
                                                         bool $certificationcompleted, ?int $programid): void {
        if (!self::can_revoke_user_certification($certificationuser, $certificationcompleted, $programid)) {
            $str = get_string('revokecertification', 'tool_certification');
            throw new moodle_exception('nopermissions', 'error', '', $str);
        }
    }
    ...
    /**
     * Checks if the current user can view "Certifications progress" report across all certifications
     *
     * That would be either somebody who can allocate to any program or an org manager
     * who can view reports.
     */
    public static function can_view_certifications_progress_report(): bool {
        return self::has_allocateuser_capability() ||
            self::can_view_any_report_as_organisation_manager();
    }

    /**
     * Current user can view "Certifications progress" report across all certifications
     */
    public static function require_can_view_certifications_progress_report(): void {
        if (!self::can_view_certifications_progress_report()) {
            throw new moodle_exception('errornopermissionviewreports', 'tool_certification');
        }
    }
}
