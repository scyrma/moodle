...
class certifications extends importer_base {
    ...
    public function add_to_options_form(import_settings_form $form): void {
        ...
        $mform->addElement('advcheckbox', self::IMPORT_PROGRAMS,
            $programsstr . ($canimportprograms ? '' : get_string('noavailablepostfix', 'tool_wp')));
        ...
        if (!$canimportprograms) {
            $mform->freeze(self::IMPORT_PROGRAMS);
            $mform->setConstant(self::IMPORT_PROGRAMS, 0);
        }
        ...
        $coursebackupsstr = get_string('import_course_backups', 'tool_certification');
        ...
        $mform->addHelpButton(self::IMPORT_COURSE_BACKUPS, 'import_course_backups', 'tool_certification');
        ...
        $certslistinfile = $this->get_entities_in_workplace_export_file('tool_certification')->get_menu('fullname');
        $certslistinfile = array_map('format_string', $certslistinfile);
        asort($certslistinfile);
        ...
        foreach ($certifications as $certification) {
            ...
            if ($this->get_import_setting(self::IMPORT_DYNAMICRULES) && $this->can_import_chained_entity('tool_dynamicrule')) {
                $this->process_chained_entities('tool_dynamicrule', [], [
                    ...
                    'area' => 'certification',
                    'itemid' => $certification->get_original_id(),
                    'mapper' => 'tool_certification',
                ]);
                ...
                api::add_default_dynamicrule_conditions_to_certification($certification->get_new_id(),
                    $this->get_import_tenant_id());
            }

            // Check if we need to import user allocations.
            if ($this->get_import_setting(self::IMPORT_USER_ALLOCATIONS) && permission::has_allocateuser_capability()) {
                $userallocations = $this->get_entities_in_workplace_export_file(certification_user::TABLE,
                static function(array $entity) use ($certification) {
                    return (int)$entity['certificationid'] === $certification->get_original_id();
                });

                // The new id can be "-1" if we are collecting errors.
                if ($certification->get_new_id() && !$this->is_collecting_errors()) {
                    $certificationpersistent = new certification($certification->get_new_id());
                } else {
                ...
                foreach ($userallocations as $userallocation) {
                    $userallocation
                        ...
                        ->set_validation_callback(function ($data, wp_imported_entity $entity) use ($certificationpersistent) {
                            ...
                            if ($certificationpersistent) {
                                $this->add_details_to_log([
                                    'certification' => $certificationpersistent->get('fullname'),
                                ]);
                            }
                        })
    ...
    private function idnumber_taken(string $idnumber): bool {
        global $DB;
        return $DB->record_exists(certification::TABLE, ['idnumber' => $idnumber, 'tenantid' => $this->get_import_tenant_id()]);
    }
