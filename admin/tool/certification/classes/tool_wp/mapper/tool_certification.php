...
/**
 * Class mapper for certifications
 *
...
/**
 * Mapper class for exporting/importing certifications
 *
 * @package     tool_certification
 * @copyright   2020 Moodle Pty Ltd <support@moodle.com>
 * @author      2020 David Matamoros <davidmc@moodle.com>
 * @license     Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class tool_certification extends export_import_mapper_base {
    ...
    protected function initialise(): void {
        $this->register_potential_error(self::NOTFOUND, [
            self::ERROR_CONFLICTHEADER => get_string('errorcertificationsdontexist', 'tool_certification'),
            self::ERROR_LOG => function(array $identifier) {
                return get_string('mappingerrorcertificationnotfound', 'tool_certification',
        ...
        $this->register_potential_notice('namemapping', [
            self::NOTICE_LOG => get_string('mappingnoticenoidnumber', 'tool_certification'),
        ]);
    }
    ...
    public function get_mapping_data_for_workplace_export(int $id): ?array {
        global $DB;
        $obj = $DB->get_record(certification::TABLE,
    ...
    public function locate_mapping(array $identifier): ?int {
        ...
        if (!empty($identifier['idnumber'])) {
            $certification = \tool_certification\api::get_certification_by_idnumber(
                $identifier['idnumber'], $tenantid ?? 0);
            if ($certification) {
            ...
            $id = $DB->get_field_select(certification::TABLE,
