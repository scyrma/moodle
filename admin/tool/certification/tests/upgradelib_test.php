...
/**
 * Tests for upgrade scripts in tool_certification
 *
...
class upgradelib_test extends advanced_testcase {
    ...
    public function setUp(): void {
        ...
        $generator = self::getDataGenerator()->get_plugin_generator('tool_certification');
     ...
     *
     * @covers ::tool_certification_upgrade_remove_orphaned_certifications
     */
    public function test_upgrade_remove_orphaned_certifications(): void {
        global $CFG, $DB;
     ...
     *
     * @covers ::tool_certification_remove_certification_tenant_groups_to_nonexisting_certification
     */
    public function test_remove_certification_tenant_groups_to_nonexisting_certification(): void {
        global $CFG, $DB;
        require_once($CFG->dirroot . '/' . $CFG->admin . '/tool/certification/db/upgradelib.php');
        self::setAdminUser();
     ...
     *
     * @covers ::tool_certification_upgrade_remove_users_from_orphan_groups
     */
    public function test_upgrade_remove_users_from_orphaned_groups_2_2_b(): void {
        global $CFG, $DB;
        require_once($CFG->dirroot . '/' . $CFG->admin . '/tool/certification/db/upgradelib.php');
        self::setAdminUser();
     ...
     * @covers ::tool_certification_upgrade_remove_users_from_orphan_groups
     */
    public function test_upgrade_remove_users_from_orphaned_groups_2_4(): void {
        global $CFG, $DB;
        require_once($CFG->dirroot . '/' . $CFG->admin . '/tool/certification/db/upgradelib.php');
        self::setAdminUser();
     ...
     * @covers ::tool_certification_upgrade_remove_users_from_orphan_groups
     */
    public function test_upgrade_remove_users_from_orphaned_groups_1_5(): void {
        global $CFG, $DB;
        require_once($CFG->dirroot . '/' . $CFG->admin . '/tool/certification/db/upgradelib.php');
        self::setAdminUser();
    ...
    /**
     * Tests for function tool_certification_fix_wrong_program_users_allocation_when_importing_certification()
     *
     * @covers ::tool_certification_fix_wrong_program_users_allocation_when_importing_certification
     */
    public function test_remove_fix_wrong_program_users_allocation_when_importing_certification(): void {
        global $CFG, $DB;
        require_once($CFG->dirroot . '/' . $CFG->admin . '/tool/certification/db/upgradelib.php');
        ...
        // Create a program, certification and allocate users.
        $program = $this->programgenerator->generate_program((object)(['tenantid' => $tenant->id]));
        $certification = $this->generator->generate_certification(['tenantid' => $tenant->id, 'program' => $program->get('id')]);
        $this->generator->allocate_user($users[1]->id, $certification->get('id'));
        $this->generator->allocate_user($users[2]->id, $certification->get('id'));

        // Create a new export containing all certifications.
        ...
        $certifications = $DB->get_records('tool_certification', ['tenantid' => $tenant->id]);
        foreach ($certifications as $certification) {
            \tool_certification\api::archive_certification($certification->id);
            $certificationtobedeleted = new \tool_certification\certification($certification->id);
            \tool_certification\api::delete_certification($certificationtobedeleted);
        }
        ...
        // We need to fake the userid field to test our upgrade step.
        $programusersbeforefakedata = $DB->get_records('tool_program_users');

        $certifications = $DB->get_records('tool_certification', ['tenantid' => $tenant->id]);
        list($insql, $inparams) = $DB->get_in_or_equal(array_keys($certifications));
        $sql = "UPDATE {tool_program_users} SET userid = (userid+10000) WHERE certificationid {$insql}";
        $DB->execute($sql, $inparams);
        $this->assertNotEquals($programusersbeforefakedata, $DB->get_records('tool_program_users'));

        // Fix bad data.
        tool_certification_fix_wrong_program_users_allocation_when_importing_certification();
        $this->assertEquals($programusersbeforefakedata, $DB->get_records('tool_program_users'));
    }
}
