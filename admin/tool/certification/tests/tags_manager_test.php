 ...
 * @covers     \tool_certification\tags_manager
...
class tags_manager_test extends advanced_testcase {
    ...
    public function test_get_tagged_certifications(): void {
        ...
        $certification = api::create_certification($certdata);
        $certdata->fullname = 'Program example two';
        $certdata->certification_tags = ['bye', 'dog'];
        $certification2 = api::create_certification($certdata);
        $certdata->fullname = 'Program example three';
        $certdata->certification_tags = ['bye', 'cat'];
        $certification3 = api::create_certification($certdata);

        $this->setAdminUser();

        $res = tags_manager::get_tagged_certifications(core_tag_tag::get_by_name(0, 'hello'),
        ...
        $res = tags_manager::get_tagged_certifications(core_tag_tag::get_by_name(0, 'bye'),
            /*$exclusivemode = */false, /*$fromctx = */0, /*$ctx = */0, /*$rec = */1, /*$page = */0);
        ...
        $res = tags_manager::get_tagged_certifications(core_tag_tag::get_by_name(0, 'dog'),
            /*$exclusivemode = */false, /*$fromctx = */0, /*$ctx = */0, /*$rec = */1, /*$page = */0);
        $this->assertStringNotContainsString($certification->get('fullname'), $res->content);
        $this->assertStringContainsString($certification2->get('fullname'), $res->content);
        $this->assertStringNotContainsString($certification3->get('fullname'), $res->content);
