...
class api_test extends advanced_testcase {
    ...
    public function test_allocate_user(): void {
        ...
        $nextstartdate = \tool_program\api::calculate_relative_date('-' . $certification->get('recertstartdaterelative'),
     ...
     * @return array
     */
    public static function recalculate_nextstartdate_provider(): array {
        return [
            ['0', '0', '0'],
     ...
     * @return array
     */
    public static function recalculate_graceperiodends_provider(): array {
        return [
            ['0', '0', '0'],
    ...
    public function test_allocate_recertification_users(): void {
        ...
        $nextstartdate = \tool_program\api::calculate_relative_date('-' . $certification->get('recertstartdaterelative'),
            $lastcompletion->get('expirydate'));
        $graceperiod = \tool_program\api::calculate_relative_date($certification->get('recertgraceperiod'),
            $lastcompletion->get('expirydate'));
    ...
    public function test_allocate_user_recertification_custom_nextstartdate(): void {
        ...
        $lastcompletion = api::get_last_completion_record($user->id, $certification->get('id'));
        $defaultnextstartdate = \tool_program\api::calculate_relative_date('-' . $certification->get('recertstartdaterelative'),
            $lastcompletion->get('expirydate'));
    ...
    /**
     * Test get certification status sql query.
     */
    public function test_get_certification_status_sql_query(): void {
        $certification = $this->generator->generate_certification();
        $certificationid = $certification->get('id');

        // Check equals to status certified.
        $status = constants::STATUS_CERTIFIED;
        [$join, $where, $params] = api::get_certification_status_sql_query($certificationid, $status);
        $this->assertNotFalse(strpos($where, ' = ' . $status));
        $this->assertArrayHasKey('certificationid', $params);
        $this->assertContains($certificationid, $params);

        // Check not equal to status overdue.
        $status = constants::STATUS_OVERDUE;
        [$join, $where, $params] = api::get_certification_status_sql_query($certificationid, $status, true);
        $this->assertNotFalse(strpos($where, ' <> ' . $status));
        $this->assertArrayHasKey('certificationid', $params);
        $this->assertContains($certificationid, $params);

        // Check for not specific certification.
        [$join, $where, $params] = api::get_certification_status_sql_query(null, $status, false, 'u', 'c', 'cu', 'cc', 'pr', null);
        $this->assertStringNotContainsString($join, 'AND cu.certificationid =');
        $this->assertArrayNotHasKey('certificationid', $params);
        $this->assertNotContains($certificationid, $params);
    }
}
