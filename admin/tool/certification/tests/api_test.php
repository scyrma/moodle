...
class tool_certification_api_testcase extends advanced_testcase {
    ...
    public function test_archive_certification(): void {
        ...
        $user1 = self::getDataGenerator()->create_user();
        $user2 = self::getDataGenerator()->create_user();
        $tenantgenerator->allocate_user($user1->id, $tenant1->id);
        $tenantgenerator->allocate_user($user2->id, $tenant1->id);
    ...
    public function test_get_latest_programuser_allocation() {
        global $DB;
        $params = [
            'expirydatetype' => constants::DATE_AFTER_COMPLETION,
            'expirydaterelative' => '1 day',
    ...
    /**
     * Data provider for translate_relativedate_string
     *
     * @return array
     */
    public function translate_relativedate_string_provider() : array {
        return [
            ['1 day', '1 day'],
            ['2 day', '2 days'],
            ['1 week', '1 week'],
    ...
    /**
     * Test_translate_relativedate_string
     *
     * @param string $relativetime
     * @param string $expected
     *
     * @dataProvider translate_relativedate_string_provider
     */
    public function test_translate_relativedate_string(string $relativetime, string $expected): void {
        $this->assertEquals($expected, api::translate_relativedate_string($relativetime));
    }
    ...
    public function test_send_certification_user_deallocated_notification(): void {
        ...
        $subject = get_string('notificationsubjectcertificationuserdeallocated', 'tool_certification', $certname);
        $fullmessage = get_string('notificationmsgcertificationuserdeallocated', 'tool_certification', $certname);

        // Sink should catch messages.
        $sink = $this->redirectMessages();
        api::send_certification_user_deallocated_notification($user->id, $certification->get('id'));
    ...
    public function test_send_certification_user_allocated_notification(): void {
        ...
        $context = context_system::instance();
        $certname = format_string($certification->get('fullname'), true, ['context' => $context, 'escape' => false]);
        $subject = get_string('notificationsubjectcertificationuserallocated', 'tool_certification', $certname);
        $fullmessage = get_string('notificationmsgcertificationuserallocated', 'tool_certification', $certname);

        // Sink should catch messages.
        $sink = $this->redirectMessages();
        api::send_certification_user_allocated_notification($user->id, $certification->get('id'));
        $messages = $sink->get_messages();
        $sink->close();
    ...
    public function test_send_certification_completed_notification(): void {
        ...
        $context = context_system::instance();
        $certname = format_string($certification->get('fullname'), true, ['context' => $context, 'escape' => false]);
        $subject = get_string('notificationsubjectcertificationcompleted', 'tool_certification', $certname);
        $fullmessage = get_string('notificationmsgcertificationcompleted', 'tool_certification', $certname);

        // Sink should catch messages.
        $sink = $this->redirectMessages();
        api::send_certification_completed_notification($user->id, $certification->get('id'));
        $messages = $sink->get_messages();
        $sink->close();
     ...
     */
    public function test_update_certification_name_in_calendar_events() {
        global $DB;
        $user1 = self::getDataGenerator()->create_user();
     ...
     */
    public function test_hide_certification_user_calendar_events() {
        global $DB;
        $user1 = self::getDataGenerator()->create_user();
        $user2 = self::getDataGenerator()->create_user();
     ...
     */
    public function test_show_certification_user_calendar_events() {
        global $DB;
        $user1 = self::getDataGenerator()->create_user();
        $user2 = self::getDataGenerator()->create_user();
