...
class behat_tool_certification extends behat_base {
        ...
        foreach ($data->getHash() as $elementdata) {
            ...
            if (!empty($switchids)) {
                foreach ($switchids as $element => $field) {
                    ...
                    if (array_key_exists($element, $elementdata)) {
                        if (strlen($elementdata[$element])) {
    ...
    protected function preprocess_certification($data): array {
        ...
        $data = $this->get_date_constant($data, 'expirydatetype');
        $data = $this->get_date_constant($data, 'duedatetype');
        $data = $this->get_date_constant($data, 'startdatetype');
        $data = $this->get_date_absolute($data, 'expirydateabsolute');
        $data = $this->get_date_absolute($data, 'duedateabsolute');
        $data = $this->get_date_absolute($data, 'startdateabsolute');
        return $data;
    }
    ...
    public function process_certification($record = null): certification {
        ...
        if (!empty($record['requirerecertification']) && empty($record['recertificationprogram'])) {
            // To prevent generator from automatically generating a new program for recertification.
            $record['recertificationprogram'] = $record['program'];
        }
        return $generator->generate_certification($record, !empty($record['requirerecertification']));
    }
    ...
    protected function get_program_id(string $programfullname): int {
        global $DB;
        return (int) $DB->get_field(program::TABLE, 'id', ['fullname' => $programfullname], MUST_EXIST);
    }
     ...
     * @return int
     */
    protected function get_recertificationprogram_id(string $programfullname): int {
        global $DB;
        return (int) $DB->get_field(program::TABLE, 'id', ['fullname' => $programfullname], MUST_EXIST);
    }
    ...
    public function the_following_user_allocations_to_certifications_exist(TableNode $data) {
        ...
        foreach ($data->getHash() as $elementdata) {
            if (array_diff_key($elementdata, ['certification' => 1, 'user' => 1])) {
                throw new coding_exception('This step only supports data for user and certification, for everything else use: '.
                    'Given the following tool certification data "certification_users" exist:');
            }
