...
Feature: Check capabilities for certifications
  ...
  Scenario: Manager has both capabilities and can edit and allocate users
    ...
    And "Users" "link" should exist in the "Certification1" "table_row"
    Then I click on "Certification1" "link"
    And the "class" attribute of "Calendar" "link" should not contain "disabled"
    ...
    And I set the field "Select users" to "User a,User b"
    Then I press "Save"
  ...
  Scenario: Organisation user has allocate user capabilities but can not edit certification and has no manager capabilities
    ...
    And I should not see "Suspended" in the "reportbuilder-table" "table"
    Then I press "Delete" action in the "User a" report row
    And I click on "Yes" "button" in the "Delete user allocation" "dialogue"
