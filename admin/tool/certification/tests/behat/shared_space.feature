...
Feature: It is possible to create certifications in shared space
  ...
  Scenario: Viewing and editing certifications in shared space as tenant administrator
    ...
    And I set the field "ID number" in the "New certification" "dialogue" to "cert4"
    Then I press "Save"
    And I navigate to "Certifications" in workplace launcher
  ...
  Scenario: Tenant administrator can see program progress overview modal on the shared certification
    ...
    And I should see "Program0" in the "Progress overview for User 11" "dialogue"
  ...
  Scenario: Export one shared certification with users
    ...
    And I press "Next"
    And I press "Next"
    And I should see "Problem: Certification with the same ID number already exists"
    And I set the field "Set ID number to empty string" in the "//div[starts-with(@id,'id_conflicttool_programidnumberconflict')]" "xpath_element" to "1"
    And I set the field "Set ID number to empty string" in the "//div[starts-with(@id,'id_conflicttool_certificationidnumberconflict')]" "xpath_element" to "1"
    And I press "Next"
  ...
  Scenario: Site admin can configure dynamic rules in shared certifications
    ...
    And I click on "Enable rule" "field" in the "Users allocated to certification 'Certification0'" "table_row"
  ...
  Scenario: Using Certifications datasource with certifications on different tenants
    ...
    And I set the field "certification_tenant:name_operator" to "Is equal to"
    And I set the field "certification_tenant:name_value" to "Tenant1"
    And I click on "Apply" "button" in the "[data-region='report-filters']" "css_element"
    And I should see "Certification1" in the "reportbuilder-table" "table"
