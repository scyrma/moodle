...
Feature: Edit certification users allocations
  ...
  Scenario: We edit one user allocation
    ...
    And I should see "First name" in the "table.reportbuilder-table thead" "css_element"
    And I should see "Allocation source" in the "table.reportbuilder-table thead" "css_element"
    And I should see "Certification status" in the "table.reportbuilder-table thead" "css_element"
    And I should see "Expiry date" in the "table.reportbuilder-table thead" "css_element"
    And I should see "Current program" in the "table.reportbuilder-table thead" "css_element"
    And I should see "Due date" in the "table.reportbuilder-table thead" "css_element"
    And I should see "Program status" in the "table.reportbuilder-table thead" "css_element"
    And I should see "Actions" in the "table.reportbuilder-table thead" "css_element"
    ...
    Then I press "Certify user" action in the "User a" report row
  ...
  Scenario: Check that expiry date form selector is set to the correct value
    Given the following "tool_certification > certifications" exist:
      | fullname       | tenant  | program   | expirydatetype | expirydaterelative | requirerecertification |
      | Certification3 | Tenant1 | Program1  | 5              | 1 week             | 1                      |
    And the following "tool_certification > certification_users" exist:
      | certification  | user   |
      | Certification3 | user1  |
      | Certification3 | user2  |
    When I log in as "manager1"
    And I navigate to "Certifications" in workplace launcher
    And I press "Users" action in the "Certification3" report row
    And I press "Certify user" action in the "User a" report row
    And I click on "Certify" "button" in the "Certify" "dialogue"
    Then I press "Edit" action in the "User a" report row
    # Selector should be set to the default option.
    Then the field "expirydatetype" matches value "1 week after completion (default)"
    And I press "Save changes"
    And I press "Certify user" action in the "User b" report row
    And I set the following fields to these values:
      | expirydatetype            | Select date |
    ...
    And I click on "Certify" "button" in the "Certify" "dialogue"
    And I press "Edit" action in the "User b" report row
    # Selector should be set to the select date option.
    And the field "expirydatetype" matches value "Select date"

  Scenario: Suspend user allocation
    Given the following "tool_certification > certifications" exist:
      | fullname       | archived | tenant  | program   | requirerecertification |
      | Certification3 | 0        | Tenant1 | Program1  | 0                      |
    And the following "tool_certification > certification_users" exist:
      | certification  | user   |
      | Certification3 | user1  |
    ...
    And I navigate to "Certifications" in workplace launcher
    And I press "Users" action in the "Certification3" report row
    Then I should not see "Suspended" in the "User a" "table_row"
    And I press "Edit" action in the "User a" report row
    And I select "Suspended" from the "Status" singleselect
    And I press "Save changes"
