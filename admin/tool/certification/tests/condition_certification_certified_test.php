...
class tool_certification_condition_certification_certified_testcase extends advanced_testcase {
    ...
    public function test_data_for_outcome() {
        ...
        $certificationgenerator->allocate_user($user2->id, $certification->get('id'));
        $certificationgenerator->allocate_user($user3->id, $certification->get('id'));
        $certificationgenerator->allocate_user($user4->id, $certification->get('id'));
    ...
    public function test_get_matching_users_shared_certification(): void {
        ...
        $program1 = $programgenerator->generate_program_with_course((object)['tenantid' => $sharedspaceid]);
        ...
        $certificationgenerator->allocate_users_to_certification($certification->get('id'),
        ...
        $programgenerator->complete_program($program1, $user11->id);
        $programgenerator->complete_program($program1, $user21->id);
        ...
        $users = api::get_matching_users($rule->id);
        $this->assertEquals([$user11->id], array_column($users, 'id'), '', 0, 10, true);
    }
}
