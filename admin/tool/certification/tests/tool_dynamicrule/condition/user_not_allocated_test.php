 ...
 * @covers     \tool_certification\tool_dynamicrule\condition\user_not_allocated
...
class user_not_allocated_test extends advanced_testcase {
    ...
    public function test_get_matching_users(): void {
        ...
        [$tenant, [$user1, $user2, $user3, $user4]] = $this->tenantgenerator->create_tenant_and_users(4);
     ...
     */
    public function test_get_matching_users_user_not_allocated_shared_rule(): void {
        ...
        $certification = $certificationgenerator->generate_certification(['tenantid' => $sharedspaceid]);
        ...
        user_not_allocated::create($rule->id, $configdata);
        $this->assertEquals(5, \tool_dynamicrule\api::count_matching_users($rule->id));
        $users = api::get_matching_users($rule->id);
        $this->assertEqualsCanonicalizing([$user12->id, $user13->id, $user22->id, $user23->id, get_admin()->id],
            array_column($users, 'id'));
    }

    /**
     * Test matching condition for the future allocations
     */
    public function test_get_matching_users_future_allocation(): void {
        ...
        // User should match because their start date is in the future and they are not yet allocated.
        ...
        // User now is considered allocated (does not match this condition).
        $this->assertEquals(0, \tool_dynamicrule\api::count_matching_users($rule->id));
    }

    /**
     * Event observer user deallocated
     */
    public function test_event_observer_user_deallocated(): void {
        [$tenant, [$user1]] = $this->tenantgenerator->create_tenant_and_users(2);
        $certification = $this->generator->generate_certification(
            ['tenantid' => $tenant->id]);
        ...
        // User does not match the rule yet.
        ...
        // Create new user, the rule will immediately be triggered for them.
        $user2 = $this->tenantgenerator->create_user(['tenantid' => $tenant->id]);
