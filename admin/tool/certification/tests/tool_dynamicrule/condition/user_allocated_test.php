...
class user_allocated_test extends advanced_testcase {
    ...
    protected $certificationgenerator = null;

    /** @var tool_program_generator  */
    protected $programgenerator = null;
     ...
     */
    public function test_event_observer_user_allocated() {
        [$tenant, [$user1, $user2]] = $this->get_tenant_generator()->create_tenant_and_users(2);
        $certification = $this->certificationgenerator->generate_certification(
            ['tenantid' => $tenant->id]);

        // Create a rule.
        $rule = $this->get_dynamicrule_generator()->create_rule(['tenantid' => $tenant->id, 'enabled' => 1]);
        $configdata = ['certificationid' => $certification->get('id')];
        user_allocated::create($rule->id, $configdata);
        $this->get_dynamicrule_generator()->create_outcome_donothing($rule->id);
        ...
        $this->get_dynamicrule_generator()->assert_user_matched_rule($this, $user1->id, $rule->id);

        // Deallocate the user.
        \tool_certification\api::deallocate_user($certification->get('id'), $user1->id);

        // Make sure the rule was immediately unmatched for him.
        $this->get_dynamicrule_generator()->assert_user_did_not_match_rule($this, $user1->id, $rule->id, 1);
    }
     ...
     */
    public function test_user_allocated_and_not_certified() {
        ...
        $certification = $this->certificationgenerator->generate_certification(['tenantid' => $tenant->id]);
        ...
        $this->get_dynamicrule_generator()->assert_user_did_not_match_rule($this, $user1->id, $rule->id);
        $this->get_dynamicrule_generator()->assert_user_matched_rule($this, $user2->id, $rule->id);
    }
}
