...
class tool_certification_user_log_testcase extends advanced_testcase {
    ...
    public function test_get_user_log(): void {
        ...
        $this->assertEquals('Was allocated to the certification', $log[0]['event']);
        $this->assertEquals($user->id, $log[0]['user']['id']);
        $this->assertEquals($certificationuser->get('timecreated'), $log[0]['date']);
        $this->assertEquals(fullname($user), $log[0]['user']['fullname']);
        $this->assertNotEmpty($log[0]['user']['picture']);
        $profileurl = (new \moodle_url('/user/profile.php', array('id' => $user->id)))->out(false);
        $this->assertEquals($profileurl, $log[0]['user']['profileurl']);
        ...
        $this->assertEquals('Manually certified '.fullname($user).' (Never expires)', $log[1]['event']);
        $this->assertEquals(2, $log[1]['user']['id']);
        $this->assertEquals($now, $log[1]['date']);

        // Revoke certification.
        \tool_certification\api::revoke_certification_from_user($user->id, $certificationid);
        $log = $userlog->get_user_log();
        $this->assertCount(3, $log);
        $this->assertEquals('Revoked '.fullname($user).'\'s certification', $log[2]['event']);
        $this->assertEquals(0, $log[2]['user']['id']);
        $record = $DB->get_record('tool_certification_compltion', ['userid' => $user->id, 'certificationid' => $certificationid]);
        $this->assertEquals($record->timerevoked, $log[2]['date']);
        ...
        $this->assertEquals('Manually certified '.fullname($user).' (expires on '.$expirydate.')', $log[3]['event']);
        $expirydate = userdate($expirytime2, get_string('strftimedatefullshort', 'langconfig'));
        $this->assertEquals('Manually certified '.fullname($user).' (expires on '.$expirydate.')', $log[4]['event']);
