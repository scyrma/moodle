...
class certifications_test extends advanced_testcase {
    ...
    /** @var string $importfixturecertifications Three tenants, each with a program and two courses,
     * and two certifications (a tenant without certification) files. */
    protected $importfixturecertifications = __DIR__ . '/../../fixtures/tenants-export-certifications.zip';
    ...
    public function test_import_single_certification_manually(): void {
        ...
        $this->assertCount(0, $DB->get_records(\tool_certification\certification::TABLE));
        ...
        $certifications = $DB->get_records(\tool_certification\certification::TABLE);
        $this->assertCount(1, $certifications);
     ...
     */
    public function test_export_import_shared_certification_allocations(): void {
        global $DB;
     ...
     */
    public function test_export_shared_certifications_in_tenant_export(): void {
        global $DB;
     ...
     */
    public function test_certification_import_idnumberconflict(): void {
        global $DB;
        $this->resetAfterTest();
    ...
    public function test_import_certification_different_tenant(): void {
        ...
        $this->assertCount(1, $DB->get_records(\tool_certification\certification::TABLE, ['tenantid' => $tenantid2]));
     ...
     * @return array
     */
    private function generate_tenant_and_users(): array {
        global $DB;
     ...
     */
    public function test_import_certification_with_dynamicrules(): void {
        global $DB;
     ...
     */
    public function test_import_settings_permissions(): void {
        global $DB;
    ...
    public function test_tenant_with_no_category_and_capability(): void {
        ...
        assign_capability('moodle/backup:backupcourse', CAP_ALLOW, $roleid, $context->id);
     ...
     */
    public function test_export_shared_certification(): void {
        global $DB;
     ...
     */
    public function test_export_tenant_certification_with_shared_program(): void {
        [$tenant, $users] = $this->tenantgenerator->create_tenant_and_users(1);
        $sharedtenantid = \tool_tenant\sharedspace::enable_shared_space();
    ...
    /**
     * Test importing all tenants with certifications, a tenant without certification
     */
    public function test_import_all_tenant_with_some_certifications(): void {
        global $DB;
        self::setAdminUser();

        $importid = $this->wpgenerator->perform_import_from_file($this->importfixturecertifications, [
            tenants_importer::IMPORT_INSTANCES => tenants_importer::IMPORT_INSTANCES_ALL,
            tenants_importer::IMPORT_DESTINATION => tenants_importer::IMPORT_DESTINATION_NEW,
            tenants_importer::IMPORT_PROGRAMS => 1,
            tenants_importer::IMPORT_CERTIFICATIONS => 1,
            tenants_importer::IMPORT_CATEGORIES => 1,
        ]);
        ...
        // Conflicts.
        $conflicts = $this->wpgenerator->get_import_conflict_review($import->get('id'));
        $this->assertEmpty($conflicts);

        // Analyse logs.
        $logs = $this->wpgenerator->get_import_logs($import->get('id'));
        $this->assertCount(19, $logs);

        // We should create all three programs from the test fixture.
        $programoneid = $DB->get_field(\tool_program\persistent\program::TABLE, 'id', ['idnumber' => 'program01'], MUST_EXIST);
        $programoneurl = (new moodle_url('/admin/tool/program/edit.php', ['id' => $programoneid]))->out();

        $programtwoid = $DB->get_field(\tool_program\persistent\program::TABLE, 'id', ['idnumber' => 'program02'], MUST_EXIST);
        $programtwourl = (new moodle_url('/admin/tool/program/edit.php', ['id' => $programtwoid]))->out();

        $programthreeid = $DB->get_field(\tool_program\persistent\program::TABLE, 'id', ['idnumber' => 'program03'], MUST_EXIST);
        $programthreeurl = (new moodle_url('/admin/tool/program/edit.php', ['id' => $programthreeid]))->out();

        $programs = array_filter($logs, function($log) {
            return (strpos($log['detail'], 'Created new program') === 0);
        });
        $this->assertEqualsCanonicalizing([
            ['detail' => "Created new program '<a href=\"{$programoneurl}\">Program one</a>' with 1 sets, 2 courses",
                'errors' => [], 'notices' => []],
            ['detail' => "Created new program '<a href=\"{$programtwourl}\">Program two</a>' with 1 sets, 2 courses",
                'errors' => [], 'notices' => []],
            ['detail' => "Created new program '<a href=\"{$programthreeurl}\">Program three</a>' with 1 sets, 2 courses",
                'errors' => [], 'notices' => []],
        ], $programs);

        // We should create both certifications from the test fixture (last tenant doesn't have certification).
        $certificationoneid = $DB->get_field(\tool_certification\certification::TABLE, 'id',
            ['idnumber' => 'certification01'], MUST_EXIST);
        $certificationoneurl = (new moodle_url('/admin/tool/certification/edit.php', ['id' => $certificationoneid]))->out();

        $certificationtwoid = $DB->get_field(\tool_certification\certification::TABLE, 'id',
            ['idnumber' => 'certification02'], MUST_EXIST);
        $certificationtwourl = (new moodle_url('/admin/tool/certification/edit.php', ['id' => $certificationtwoid]))->out();

        $certifications = array_filter($logs, function($log) {
            return (strpos($log['detail'], 'Created new certification') === 0);
        });
        $this->assertEqualsCanonicalizing([
            ['detail' => "Created new certification '<a href=\"{$certificationoneurl}\">Certification one</a>'",
                'errors' => [], 'notices' => []],
            ['detail' => "Created new certification '<a href=\"{$certificationtwourl}\">Certification two</a>'",
                'errors' => [], 'notices' => []],
        ], $certifications);

        // There should now be four tenants (default one, plus three new).
        $tenants = array_map(static function(\stdClass $tenant): string {
            return \tool_tenant\tenancy::get_tenant_name_from_id($tenant->id);
        ...
        // There should now be three programs.
        $programs = $DB->get_records(\tool_program\persistent\program::TABLE, null, '', 'fullname');
        $this->assertEqualsCanonicalizing([
            'Program one',
            'Program two',
            'Program three',
        ...
        // There should now be two certifications.
        $certifications = $DB->get_records(\tool_certification\certification::TABLE, null, '', 'fullname');
        $this->assertEqualsCanonicalizing([
            'Certification one',
            'Certification two',
        ], array_keys($certifications));
    }
}
