 ...
 */
function tool_reportbuilder_theme_workplace_menu_items(): array {
    global $OUTPUT;

    $menuitems = [];

    $tenantid = \tool_tenant\tenancy::get_tenant_id();
    $identifier = permission::can_create(true) ? 'pluginname' : 'customreports';
    if (permission::can_manage_reports($tenantid) || permission::can_view_reports_list()) {
        $menuitems[] = [
            'url' => new moodle_url("/admin/tool/reportbuilder/index.php"),
            'name' => get_string($identifier, 'tool_reportbuilder'),
            'imageurl' => $OUTPUT->image_url('icon', 'tool_reportbuilder')->out(false),
        ];
    }
    return $menuitems;
}
