...
class upgradelib_test extends advanced_testcase {
    ...
    /**
     * Test for delete tool reportbuilder system reports data
     *
     * @covers ::tool_reportbuilder_upgrade_delete_old_system_reports_data
     */
    public function test_upgrade_delete_old_system_reports_data(): void {
        global $DB;
        $this->setAdminUser();

        $customreport = $this->get_generator()->create_report([
            'source' => mock_report::class,
            'adddefault' => 0,
            'type' => constants::TYPE_DATASOURCE,
        ]);
        $customreportid = $customreport->get_id();
        $this->get_generator()->add_column($customreport, 'user:firstname');
        $this->get_generator()->add_condition($customreport, 'user:firstname');
        $this->get_generator()->add_filter($customreport, 'user:firstname');
        $this->get_generator()->set_report_filters_values($customreportid,
            ['user:firstname_op' => 1, 'user:firstname' => 'Spain']);

        $systemreport1 = $this->get_generator()->create_report([
            'source' => mock_report::class,
            'adddefault' => 0,
            'type' => constants::TYPE_SYSTEM,
        ]);
        $this->get_generator()->add_column($systemreport1, 'user:firstname');
        $this->get_generator()->add_condition($systemreport1, 'user:firstname');
        $this->get_generator()->add_filter($systemreport1, 'user:firstname');
        $this->get_generator()->set_report_filters_values($systemreport1->get_id(),
            ['user:firstname_op' => 1, 'user:firstname' => str_repeat('A', 2000)]);

        $systemreport2 = $this->get_generator()->create_report([
            'source' => mock_report::class,
            'adddefault' => 0,
            'type' => constants::TYPE_SYSTEM,
        ]);
        $this->get_generator()->add_column($systemreport2, 'user:firstname');
        $this->get_generator()->add_condition($systemreport2, 'user:firstname');
        $this->get_generator()->add_filter($systemreport2, 'user:firstname');
        $this->get_generator()->set_report_filters_values($systemreport2->get_id(),
            ['user:firstname_op' => 1, 'user:firstname' => 'Spain']);

        $this->assertCount(3, $DB->get_records('tool_reportbuilder'));
        $this->assertCount(3, $DB->get_records('tool_reportbuilder_column'));
        $this->assertCount(3, $DB->get_records('tool_reportbuilder_cond'));
        $this->assertCount(3, $DB->get_records('tool_reportbuilder_filter'));
        // Preferences for systemreport1 are split in 2 parts because value is bigger than 1333 characters.
        $this->assertCount(4, $DB->get_records('user_preferences'));

        tool_reportbuilder_upgrade_delete_old_system_reports_data();

        $this->assertEquals([$customreportid], array_keys($DB->get_records('tool_reportbuilder')));

        $columns = $DB->get_records('tool_reportbuilder_column');
        $this->assertCount(1, $columns);
        $this->assertEquals($customreportid, reset($columns)->reportid);

        $conditions = $DB->get_records('tool_reportbuilder_cond');
        $this->assertCount(1, $conditions);
        $this->assertEquals($customreportid, reset($conditions)->reportid);

        $filters = $DB->get_records('tool_reportbuilder_filter');
        $this->assertCount(1, $filters);
        $this->assertEquals($customreportid, reset($filters)->reportid);

        $userprefs = $DB->get_records('user_preferences');
        $this->assertCount(1, $userprefs);
        $this->assertEquals("filters_report_$customreportid", reset($userprefs)->name);
    }
