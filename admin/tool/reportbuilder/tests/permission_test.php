...
class tool_reportbuilder_permission_testcase extends advanced_testcase{
    ...
    public function test_can_view_reports_list() {
        ...
        $this->get_report_generator()->assign_job_with_report_permissions($user2->id, []);

        $report = $this->get_report_generator()->create_report([
            'source' => mock_report::class
        ]);
        $this->get_report_generator()->create_audience([
    ...
    public function test_can_view_report() {
        ...
        // Assign second user to a job, create an audience for the second report.
        $this->get_report_generator()->assign_job_with_report_permissions($user2->id, []);
        $this->get_report_generator()->create_audience([
     ...
     * @return void
     */
    public function test_can_create_no_capability() {
        $user = $this->getDataGenerator()->create_user();
        $this->setUser($user);
     ...
     */
    public function test_can_edit_in_report_tenant() {
        self::setAdminUser();
        $tenant = $this->get_tenant_generator()->create_tenant();
        $sharedspaceid = \tool_tenant\sharedspace::enable_shared_space();
