...
class tool_reportbuilder_helper_audience_testcase extends advanced_testcase {
    ...
    /**
     * Test that deleting a report clears associated audience data
     *
     * @return void
     */
    public function test_report_delete() {
        $report = $this->create_report(mock_report::class);
        $this->get_plugin_generator()->create_audience(['reportid' => $report->get_id()]);

        $this->assertEquals(1, audience::count_records($report));

        // Make sure that deleting the report also clears associated audience data.
        $report->get_persistent()->delete();
        $this->assertEquals(0, audience::count_records($report));
    }
