 ...
 * @covers      \tool_reportbuilder\tool_reportbuilder\audiences\allusers
...
class allusers_test extends advanced_testcase {
    ...
    public function test_get_title(): void {
        ...
        $audience = allusers::create($report->get_id(), []);
    ...
    public function test_get_description(): void {
        ...
        $audience = allusers::create($report->get_id(), []);
        $this->assertEquals(get_string('allusers', 'tool_reportbuilder'), $audience->get_description());
    }
    ...
    public function test_get_sql(): void {
        ...
        $user4 = $this->getDataGenerator()->create_user(['suspended' => 1]);
        $user5 = $this->getDataGenerator()->create_user(['deleted' => 1]);
        ...
        $this->assertEqualsCanonicalizing([$CFG->siteguest, get_admin()->id, $user1->id, $user2->id, $user3->id],
    ...
    public function test_convert_to_core_reportbuilder(): void {
        ...
        $this->get_generator()->audience_test_convert_to_core_reportbuilder(allusers::class, $this);
    }
    ...
    public function test_convert_to_core_reportbuilder_config(): void {
        ...
        $this->assertEquals(json_encode([]), $newaudience->get('configdata'));
        $this->assertEquals($newaudience->get('configdata'), $audience->get_persistent()->get('configdata'));

        // Create a user and make sure this user can view both original and converted report.
        $user1 = $this->getDataGenerator()->create_user();
        $this->setUser($user1);
        $this->assertTrue(permission::can_view(\tool_reportbuilder\manager::get_report($report->get_id())));
        $this->assertTrue(\core_reportbuilder\permission::can_view_report(
            \core_reportbuilder\manager::get_report_from_id($newid)->get_report_persistent()));
    }
}
