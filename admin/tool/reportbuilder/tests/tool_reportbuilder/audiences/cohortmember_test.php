...
/**
 * File containing tests for cohort member audience type
 *
 ...
 * @covers      \tool_reportbuilder\tool_reportbuilder\audiences\cohortmember
 * @copyright   2021 Moodle Pty Ltd <support@moodle.com>
 * @author      2021 David Matamoros <davidmc@moodle.com>
 * @license     Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class cohortmember_test extends advanced_testcase {
    ...
    public function test_get_title(): void {
        ...
        $audience = cohortmember::create($report->get_id(), ['cohorts' => [$cohort->id]]);
    ...
    public function test_get_description(): void {
        ...
        $audience = cohortmember::create($report->get_id(), ['cohorts' => [$cohort->id]]);
        $this->assertEquals($audience->get_title() . ' ' . $cohort->name, $audience->get_description());
    }
    ...
    /**
     * Test get_not_available_label()
     */
    public function test_get_not_available_label(): void {
        $report = $this->get_generator()->create_report([
            'source' => mock_report::class,
        ]);
        $audience = cohortmember::create($report->get_id(), ['cohorts' => []]);
        $this->assertEquals(get_string('noavailablecohorts', 'tool_reportbuilder'), $audience->get_not_available_label());
    }
    ...
    public function test_is_available(): void {
        ...
        $audience2 = cohortmember::create($report->get_id(), ['cohorts' => []]);
    ...
    public function test_convert_to_core_reportbuilder(): void {
        ...
        $this->get_generator()->audience_test_convert_to_core_reportbuilder(cohortmember::class, $this);
    }
    ...
    public function test_convert_to_core_reportbuilder_config(): void {
        ...
        $audience = cohortmember::create($report->get_id(), ['cohorts' => [$cohort->id, $cohort2->id]]);

        $newid = $report->convert(false);
        $newaudience = \core_reportbuilder\local\models\audience::get_record(['reportid' => $newid]);

        $this->assertEquals(json_encode(['cohorts' => [$cohort->id, $cohort2->id]]), $newaudience->get('configdata'));
        $this->assertEquals($newaudience->get('configdata'), $audience->get_persistent()->get('configdata'));

        // Make sure a cohort user can view both original and converted report, and user outside of cohort can not.
        $user3 = $this->getDataGenerator()->create_user();
