<?php
// This file is part of Moodle Workplace https://moodle.com/workplace based on Moodle
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.
//
// Moodle Workplace™ Code is the discrete and self-executable
// collection of software scripts (plugins and modifications, and any
// derivations thereof) that are exclusively owned and licensed by
// Moodle Pty Ltd (Moodle) under the terms of its proprietary Moodle
// Workplace License ("MWL") made available with Moodle's open software
// package ("Moodle LMS") offering which itself is freely downloadable
// at "download.moodle.org" and which is provided by Moodle under a
// single GNU General Public License version 3.0, dated 29 June 2007
// ("GPL"). MWL is strictly controlled by Moodle Pty Ltd and its Moodle
// Certified Premium Partners. Wherever conflicting terms exist, the
// terms of the MWL shall prevail.

/**
 * File containing tests for report_course_completion class.
 *
 * @package     tool_reportbuilder
 * @copyright   2019 Moodle Pty Ltd <support@moodle.com>
 * @author      2019 Marina Glancy
 * @license     Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */

namespace tool_reportbuilder\tool_reportbuilder\datasources;

use coding_exception;
use completion_completion;
use core_course_category;
use core_reportbuilder_testcase;
use testable_report_exporter;
use tool_reportbuilder\external;
use tool_reportbuilder_generator;
use tool_tenant_generator;

defined('MOODLE_INTERNAL') || die();

global $CFG;
require_once($CFG->dirroot . '/' . $CFG->admin . '/tool/reportbuilder/tests/fixtures/testable_report_exporter.php');
require_once("{$CFG->dirroot}/reportbuilder/tests/helpers.php");

/**
 * Tests for the datasource report_course_completion
 *
 * @package     tool_reportbuilder
 * @covers      \tool_reportbuilder\tool_reportbuilder\datasources\report_course_completion
 * @copyright   2019 Moodle Pty Ltd <support@moodle.com>
 * @author      2019 Marina Glancy
 * @license     Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class report_course_completion_test extends core_reportbuilder_testcase {

    /**
     * Get report builder generator
     *
     * @return tool_reportbuilder_generator
     * @throws coding_exception
     */
    protected function get_generator(): tool_reportbuilder_generator {
        return $this->getDataGenerator()->get_plugin_generator('tool_reportbuilder');
    }

    /**
     * Returns the tenant generator
     * @return tool_tenant_generator
     * @throws coding_exception
     */
    protected function get_tenant_generator() : tool_tenant_generator {
        return $this->getDataGenerator()->get_plugin_generator('tool_tenant');
    }

    /**
     * Set up for the
     */
    protected function set_up_for_report() {
        $this->resetAfterTest();

        // Create a course with completion enabled.
        set_config('enablecompletion', 1);
        $course = $this->getDataGenerator()->create_course(['enablecompletion' => true]);

        // Create one user and allocate them to the default tenant.
        $user = $this->getDataGenerator()->create_user(['firstname' => 'John', 'lastname' => 'Smith'])->id;
        $this->get_tenant_generator()->allocate_user($user, \tool_tenant\tenancy::get_default_tenant_id());

        $ccompletion = new completion_completion(['course' => $course->id, 'userid' => $user]);
        $ccompletion->mark_complete();

        // Create five more users and allocated them to the new tenant.
        $othertenantid = $this->get_tenant_generator()->create_tenant()->id;
        $users = [];
        for ($i = 0; $i < 5; $i++) {
            $users[$i] = $this->getDataGenerator()->create_user(
                ['firstname' => 'User', 'lastname' => $i + 1, 'username' => 'user' . $i])->id;
            $this->get_tenant_generator()->allocate_user($users[$i], $othertenantid);
            $this->getDataGenerator()->enrol_user($users[$i], $course->id);

            $ccompletion = new completion_completion(['course' => $course->id, 'userid' => $users[$i]]);
            $ccompletion->mark_complete();
        }

        return $users;
    }

    /**
     * Create a report
     *
     * @param int $tenantid
     * @param bool $adddefault
     * @return int
     */
    protected function create_report(int $tenantid, bool $adddefault) : int {
        return $this->get_generator()->create_report(
            ['source' => \tool_reportbuilder\tool_reportbuilder\datasources\report_course_completion::class,
                'tenantid' => $tenantid,
                'adddefault' => (int)$adddefault, ])->get_id();
    }

    /**
     * System capabilities and manager permissions
     */
    public function test_permissions() {
        $users = $this->set_up_for_report();

        // Create a report from the report_users_list datasource with default columns/conditions.
        $reportid = $this->create_report(\tool_tenant\tenancy::get_tenant_id($users[0]), true);

        // Admin user sees only the one user in their tenant.
        $this->setAdminUser();
        $exporter = new testable_report_exporter($reportid);
        $rows = $exporter->get_table_rows();
        $this->assertCount(1, $rows);

        // User 0 can see all five users in their tenant (but can not see users in other tenants).
        $this->setUser($users[0]);
        $exporter = new testable_report_exporter($reportid);
        $rows = $exporter->get_table_rows();
        $this->assertCount(5, $rows);
    }

    /**
     * Stress testing - add all available columns, try all possible aggregation methods.
     *
     * @coversNothing
     */
    public function test_stress_aggregation() {
        $users = $this->set_up_for_report();
        $reportid = $this->create_report(\tool_tenant\tenancy::get_tenant_id($users[0]), false);

        $generator = $this->get_generator();
        $generator->add_all_available_columns_to_report($reportid);
        $generator->datasource_stress_test_aggregation($reportid, $this);
    }

    /**
     * Stress testing - add all available conditions.
     *
     * @coversNothing
     */
    public function test_stress_conditions() {
        $users = $this->set_up_for_report();
        $reportid = $this->create_report(\tool_tenant\tenancy::get_tenant_id($users[0]), false);

        $generator = $this->get_generator();
        $generator->add_all_available_columns_to_report($reportid);
        $generator->datasource_stress_test_conditions($reportid, $this);
    }

    /**
     * Stress testing - add all available filters.
     */
    public function test_stress_filters() {
        $users = $this->set_up_for_report();
        $reportid = $this->create_report(\tool_tenant\tenancy::get_tenant_id($users[0]), false);

        $generator = $this->get_generator();
        $generator->add_all_available_columns_to_report($reportid);
        $generator->datasource_stress_test_filters($reportid, $this);
    }

    /**
     * Check that each column / filter / condition can be converted to the core reportbuilder
     * @return void
     */
    public function test_convert_to_core_reportbuilder(): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        $this->get_tenant_generator()->create_tenant(); // Make site multi-tenant.
        $this->get_generator()->datasource_test_convert_to_core_reportbuilder(report_course_completion::class, $this);
    }

    /**
     * Check that sorted columns will be converted to the core reportbuilder
     */
    public function test_convert_to_core_reportbuilder_sorting(): void {
        $this->resetAfterTest();
        $this->setAdminUser();

        $category1 = core_course_category::create(['name' => 'Child category']);
        $category2 = core_course_category::create(['name' => 'Last category']);
        $category3 = core_course_category::create(['name' => 'Parent category']);

        // Create courses.
        $course1 = $this->getDataGenerator()->create_course(['category' => $category1->id]);
        $course2 = $this->getDataGenerator()->create_course(['category' => $category2->id]);
        $course3 = $this->getDataGenerator()->create_course(['category' => $category3->id]);

        // Create some users and enrol them into some different courses.
        $user1 = $this->getDataGenerator()->create_and_enrol($course1, 'student', ['firstname' => 'User', 'lastname' => 'One']);
        $user2 = $this->getDataGenerator()->create_and_enrol($course2, 'student', ['firstname' => 'User', 'lastname' => 'Two']);
        $user3 = $this->getDataGenerator()->create_and_enrol($course1, 'student', ['firstname' => 'User', 'lastname' => 'Three']);
        $user4 = $this->getDataGenerator()->create_and_enrol($course2, 'student', ['firstname' => 'User', 'lastname' => 'Four']);
        $user5 = $this->getDataGenerator()->create_and_enrol($course3, 'student', ['firstname' => 'User', 'lastname' => 'Five']);

        $ccompletion1 = new completion_completion(['course' => $course1->id, 'userid' => $user1->id]);
        $ccompletion1->mark_complete();
        $ccompletion2 = new completion_completion(['course' => $course2->id, 'userid' => $user2->id]);
        $ccompletion2->mark_complete();
        $ccompletion3 = new completion_completion(['course' => $course1->id, 'userid' => $user3->id]);
        $ccompletion3->mark_complete();
        $ccompletion4 = new completion_completion(['course' => $course2->id, 'userid' => $user4->id]);
        $ccompletion4->mark_complete();
        $ccompletion5 = new completion_completion(['course' => $course3->id, 'userid' => $user5->id]);
        $ccompletion5->mark_complete();

        /** @var \tool_reportbuilder\datasource $report */
        $report = $this->get_generator()->create_report([
            'source' => report_course_completion::class,
            'adddefault' => (int)false,
        ]);

        $reportid = $report->get_id();

        // Add columns to report.
        $this->get_generator()->add_column($report, 'course:category');
        $this->get_generator()->add_column($report, 'course:fullname');
        $this->get_generator()->add_column($report, 'course:shortname');

        // Get report sortable columns.
        $columns = external::get_report_sortable_columns($reportid);
        $columns = external::clean_returnvalue(external::get_report_sortable_columns_returns(), $columns);
        $columns = $columns['columnsinuse'];

        // Enable sorting (ASC) on the column course category.
        $coursecat = external::toggle_report_sorting_column($reportid, $columns[0]['id']);
        $coursecat = external::clean_returnvalue(external::toggle_report_sorting_column_returns(), $coursecat);
        $this->assertEquals(1, $coursecat['newcolumnstate']);
        $this->assertEquals(SORT_ASC, $columns[0]['sortdirection']);

        // Assert new report return same sorted rows.
        $newreport = $report->convert(false);
        $newreportvalues = array_map('array_values', $this->get_custom_report_content($newreport));
        $categories = array_map(function($r) {
            return $r[0];
        }, $newreportvalues);
        $categoriesexpected = ['Child category', 'Child category', 'Last category', 'Last category', 'Parent category'];
        $this->assertEquals($categoriesexpected, $categories);

        // Change sorting (DESC) on the column course category.
        $coursecat = external::toggle_column_sorting_direction($reportid, $columns[0]['id']);
        $coursecat = external::clean_returnvalue(external::toggle_column_sorting_direction_returns(), $coursecat);
        $this->assertEquals(SORT_DESC, $coursecat['newcolumnstate']);

        // Assert new report return same sorted rows.
        $newreport = $report->convert(false);
        $newreportvalues = array_map('array_values', $this->get_custom_report_content($newreport));
        $categories = array_map(function($r) {
            return $r[0];
        }, $newreportvalues);
        $categoriesexpected = ['Parent category', 'Last category', 'Last category', 'Child category', 'Child category'];
        $this->assertEquals($categoriesexpected, $categories);
    }

    /**
     * Check that each custom field column/filter/condition can be converted to the core reportbuilder
     */
    public function test_convert_to_core_reportbuilder_custom_fields(): void {
        $this->resetAfterTest();
        $this->setAdminUser();

        /** @var \core_customfield_generator $corecustomfield */
        $corecustomfield = $this->getDataGenerator()->get_plugin_generator('core_customfield');

        // Create course custom fields available in report_course_completion datasource.
        $params = [
            'component' => 'core_course',
            'area' => 'course',
            'itemid' => 0,
            'name' => 'Course custom field',
            'contextid' => \context_system::instance()->id,
        ];
        $category = $corecustomfield->create_category($params);
        $categoryid = $category->get('id');

        $cftextarea = $corecustomfield->create_field(['shortname' => 'field1', 'name' => 'Name1',
            'type' => 'textarea', 'categoryid' => $categoryid, ]);
        $cftextareaid = $cftextarea->get('id');

        $cftext = $corecustomfield->create_field(['shortname' => 'field2', 'name' => 'Name2',
            'type' => 'text', 'categoryid' => $categoryid, ]);
        $cftextid = $cftext->get('id');

        $cfdatetime = $corecustomfield->create_field(['shortname' => 'field3', 'name' => 'Name3',
            'type' => 'date', 'categoryid' => $categoryid, ]);
        $cfdatetimeid = $cfdatetime->get('id');

        $cfcheckbox = $corecustomfield->create_field(['shortname' => 'field4', 'name' => 'Name4',
            'type' => 'checkbox', 'categoryid' => $categoryid, ]);
        $cfcheckboxid = $cfcheckbox->get('id');

        $cfmenu = $corecustomfield->create_field(['shortname' => 'field5', 'name' => 'Name5',
            'param1' => "a\nb\nc", 'type' => 'select', 'categoryid' => $categoryid, ]);
        $cfmenuid = $cfmenu->get('id');

        // Create a report.
        $report = $this->get_generator()->create_report([
            'source' => report_course_completion::class,
        ]);

        // Add course custom fields as report columns.
        $this->get_generator()->add_column($report, "course:cid{$cftextareaid}");
        $this->get_generator()->add_column($report, "course:cid{$cftextid}");
        $this->get_generator()->add_column($report, "course:cid{$cfdatetimeid}");
        $this->get_generator()->add_column($report, "course:cid{$cfcheckboxid}");
        $this->get_generator()->add_column($report, "course:cid{$cfmenuid}");

        // Add course custom fields as report filters.
        $this->get_generator()->add_filter($report, "course:cid{$cftextareaid}");
        $this->get_generator()->add_filter($report, "course:cid{$cftextid}");
        $this->get_generator()->add_filter($report, "course:cid{$cfdatetimeid}");
        $this->get_generator()->add_filter($report, "course:cid{$cfcheckboxid}");
        $this->get_generator()->add_filter($report, "course:cid{$cfmenuid}");

        // Add course custom fields as report conditions.
        $this->get_generator()->add_condition($report, "course:cid{$cftextareaid}");
        $this->get_generator()->add_condition($report, "course:cid{$cftextid}");
        $this->get_generator()->add_condition($report, "course:cid{$cfdatetimeid}");
        $this->get_generator()->add_condition($report, "course:cid{$cfcheckboxid}");
        $this->get_generator()->add_condition($report, "course:cid{$cfmenuid}");

        $this->get_generator()->datasource_test_convert_to_core_reportbuilder(report_course_completion::class, $this);
    }
}
