...
class tool_reportbuilder_helper_send_testcase extends advanced_testcase {
    ...
    public function test_sendemail_from_user() {
        ...
        $this->assertContains('Content-Type: text/csv; name="Test.csv"', $message->body);
        $this->assertContains('Content-Disposition: attachment; filename=Test.csv', $message->body);
    }
    ...
    public function test_sendemail_archived_tenant() {
        ...
        // Create a report schedule, with our test user as the audience.
        $audience = ['users' => [$this->user->id]];
        $schedule = $this->get_generator()->create_schedule(
            ['reportid' => $report->get_id(), 'audience' => json_encode($audience)]);
