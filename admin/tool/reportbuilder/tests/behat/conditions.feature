...
Feature: Manage conditions in report builder
  ...
  Scenario: Ensure the formatting is applied correctly to conditions names
    ...
    And I set the field "Select a condition" to "Last name"
    And I set the field "Edit condition name" in the "//div[contains(@id,'settingsconditions')]//div[contains(@class,'filter-name') and contains(.,'Last name')]" "xpath_element" to "<span lang=\"en\" class=\"multilang\">Test&\"3</span><span lang=\"es\" class=\"multilang\">Prueba&\"3</span>"
    And I should see "Test&\"3" in the "#settingsconditions" "css_element"
    And I should not see "Prueba"
    # Reload, recheck and test deleting
    When I navigate to "Reports > Report builder - outdated version > Manage custom reports" in site administration
    And I click on "Edit content" "link" in the "Report1" "table_row"
    And I click on "Show/hide filters sidebar" "button"
    And I click on "Expand conditions" "button"
    And I should see "Test&\"3" in the "#settingsconditions" "css_element"
    And I should not see "Prueba"
    And I click on "Delete condition" "link" in the "//div[contains(@id,'settingsconditions')]//div[contains(@class,'reportbuilder-card') and contains(.,'Test&\"3')]" "xpath_element"
  ...
  Scenario: Ensure that default conditions are applied when creating the report
    ...
    And I click on "Show/hide filters sidebar" "button"
    And I click on "Expand conditions" "button"
    And I should see "Email address" in the "#settingsconditions .tool_reportbuilder_report_active_conditions" "css_element"
    And I should not see "First name" in the "#settingsconditions .tool_reportbuilder_report_active_conditions" "css_element"
    And I should not see "ID number" in the "#settingsconditions .tool_reportbuilder_report_active_conditions" "css_element"
    And I should not see "Last name" in the "#settingsconditions .tool_reportbuilder_report_active_conditions" "css_element"
    And I should not see "Phone" in the "#settingsconditions .tool_reportbuilder_report_active_conditions" "css_element"
    And I should see "is equal to" in the "#settingsconditions .tool_reportbuilder_report_active_conditions" "css_element"
