...
Feature: Manage conditions in report builder
  ...
  Scenario: Ensure the formatting is applied correctly to conditions names
    ...
    And I set the field "Edit condition name" in the "//div[contains(@id,'settingsconditions')]//div[contains(@class,'filter-name') and contains(.,'Surname')]" "xpath_element" to "<span lang=\"en\" class=\"multilang\">Test&\"3</span><span lang=\"es\" class=\"multilang\">Prueba&\"3</span>"
  ...
  Scenario: Ensure that default conditions are applied when creating the report
    ...
    And I should not see "Surname" in the "#settingsconditions .tool_reportbuilder_report_active_conditions" "css_element"
