...
Feature: Manage a filter
  ...
  Scenario: Add a new filter to the report
    ...
    And I set the field "Select a filter" to "Last name"
    And I set the field "Select a filter" to "First name"
    And I set the field "Select a filter" to "ID number"
    And I should see "Last name" in the "ul.js-filters-list" "css_element"
    ...
    And I set the field "Last name field limiter" to "contains"
    And I should not see "Delete filter 'Last name'"
    And I should not see "Delete condition 'Last name'"
    And I should not see "Edit condition name"
    And "Reset 'Last name' field" "button" should exist
    And I log out
  ...
  Scenario: Change the order of a filter
    ...
    And I click on "Switch to preview view" "button"
    And I click on "Show/hide filters sidebar" "button"
    And "ID number" "text" should appear before "Last name" "text"
    ...
    And I click on "Switch to preview view" "button"
    And I click on "Show/hide filters sidebar" "button"
    And "ID number" "text" should appear after "Last name" "text"
    And I log out
  ...
  Scenario: Ensure the formatting is applied correctly to filter names
    ...
    And I set the field "Select a filter" to "Last name"
    And I set the field "Select a filter" to "First name"
    And I set the field "Edit filter name" in the "#settingsfilters [data-key=\"user:lastname\"]" "css_element" to "<span lang=\"en\" class=\"multilang\">Test&\"2</span><span lang=\"es\" class=\"multilang\">Prueba&\"2</span>"
    And I should see "Test&\"2" in the "#settingsfilters" "css_element"
  ...
  Scenario: Ensure adding filters to a report makes them available to the user
    ...
    And I navigate to "Table" in current page administration
    And I click on "Show/hide filters sidebar" "button"
    And I click on "Expand filters" "button"
    And I set the field "Select a filter" to "Country"
    And I set the field "Select a filter" to "Profile department"
  ...
  Scenario: Ensure that default filters are applied when creating the report
    ...
    And I click on "Show/hide filters sidebar" "button"
    And I click on "Expand filters" "button"
    And I should see "Email address" in the "//*[@id='settingsfilters']//*[@data-region='active-filters']" "xpath_element"
    And I should not see "First name" in the "//*[@id='settingsfilters']//*[@data-region='active-filters']" "xpath_element"
    And I should not see "ID number" in the "//*[@id='settingsfilters']//*[@data-region='active-filters']" "xpath_element"
    And I should not see "Last name" in the "//*[@id='settingsfilters']//*[@data-region='active-filters']" "xpath_element"
    And I should not see "Phone" in the "//*[@id='settingsfilters']//*[@data-region='active-filters']" "xpath_element"
    And I log out
