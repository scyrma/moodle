...
Feature: Export and import custom reports
  ...
  Scenario: Export a custom report
    ...
    And I press "Export"
    And I click on "Custom reports from outdated version" "radio"
    And I press "Next"
    ...
    And I press "View export" action in the "Custom reports from outdated version" report row
  ...
  Scenario: Import a custom report into current tenant
    ...
    And I press "Next"
    And I should see "Custom reports from outdated version (2)" in the "Content" "table_row"
    ...
    And I run all adhoc tasks
    And I click on "Close" "link_or_button" in the "region-main" "region"
    And I press "View import" action in the "Custom reports from outdated version" report row
    Then I should see "Success" in the "Status" "table_row"
    And I should see "Instances (1)"
    And I should see "Course completion" in the "[data-region=\"exportimport-instances\"]" "css_element"
    ...
    # Old 3.11 reports are converted to core report builder and deleted automatically. We need to check that core rb report exists.
    And I navigate to "Reports > Report builder > Custom reports" in site administration
    And the following should exist in the "reportbuilder-table" table:
      | Name              | Report source       | Modified by   |
      | Course completion | Course participants | Tenantadmin 1 |
  ...
  Scenario: Import a custom report into different tenant
    ...
    And the following should exist in the "reportbuilder-table" table:
      | Name              | Report source       | Modified by |
      | Course completion | Course participants | Admin User  |
