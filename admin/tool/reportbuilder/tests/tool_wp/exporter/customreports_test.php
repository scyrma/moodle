 ...
 * Test class
 *
 * TODO WP-3102 split into tests for import and export
 *
 * @package     tool_reportbuilder
...
class customreports_test extends advanced_testcase {
    ...
    public function test_export_single_report() {
        ...
        $audience = $this->get_plugin_generator()->create_audience([
        ...
        // Audience entities.
        $audienceentities = $importer->get_entities_in_workplace_export_file(audiences::TABLE);
        $this->assertCount(1, $audienceentities);
        $this->assertEquals($audience->get_id(), $audienceentities->current()->get_original_id());
    ...
    public function test_export_no_reports() {
        ...
        $this->assertEmpty($importer->get_entities_in_workplace_export_file(audiences::TABLE));
    ...
    public function test_import_single_report() {
        ...
        // Check that old tool reportbuilder report has been deleted after conversion.
        $this->assertCount(0, reportbuilder::get_records());
        // Check that report has been converted to core reportbuilder.
        ...
        $persistent = import_detail_persistent::get_records(['importid' => $importid], 'id');
        $oldid = ($persistent[1]->get('data'))['importerdetails']['oldid'];
        $reporturl = (new \moodle_url('/admin/tool/reportbuilder/manage.php', ['id' => $oldid]))->out();
        $this->assertEquals("Created new report '<a href=\"{$reporturl}\">UK Users</a>' with 3 columns, 1 conditions and 2 filters",
            $detail);
        $this->assertEmpty($errors);
        $this->assertEmpty($notices);

        $str = get_string('importlogconversionsuccess', 'tool_reportbuilder', ['name' => 'UK Users']);
    ...
    public function test_import_single_report_mapped_entities() {
        ...
        $persistent = import_detail_persistent::get_records(['importid' => $importid], 'id');
        $oldid = ($persistent[4]->get('data'))['importerdetails']['oldid'];
        ...
        $this->assertEquals([
            ...
            'Imported custom report \'Course completion\' was converted from the outdated version of report builder'
        ], array_column($logs, 'detail'));

        // Audience records (two original audience records, plus one created for the schedule).
        $audiences = audience::get_records(['reportid' => $reportid]);
        $this->assertCount(3, $audiences);

        // Schedule records.
        $schedules = \core_reportbuilder\local\models\schedule::get_records(['reportid' => $reportid]);
    ...
    public function test_import_all_reports_current_tenant() {
        ...
        $reports = report::get_records(['component' => 'tool_tenant', 'itemid' => $tenantid], 'name');
        $this->assertCount(2, $reports);
    ...
    public function test_export_import_field_mapping(): void {
        ...
        $report = $this->get_plugin_generator()->create_report([
            'source' => \tool_reportbuilder\tool_reportbuilder\datasources\report_course_enrolment_completion::class,
        ]);
        ...
        $newreport = \core_reportbuilder\manager::get_report_from_id(end($reports)->get('id'));
        $conditiondata = (array)json_decode($newreport->get_report_persistent()->get('conditiondata'));
        $this->assertEquals([$newcourse->id], $conditiondata['course:courseselector_values']);
    }
