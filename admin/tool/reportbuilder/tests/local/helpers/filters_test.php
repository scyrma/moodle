...
class filters_test extends advanced_testcase {
    ...
    /**
     * Test resetting some filter values greater than 1333 characters (user preference limit)
     */
    public function test_reset_filters_large_value(): void {
        $value = ['longvalue' => str_repeat('A', 2000), 'user:idnumber' => str_repeat('B', 2000)];
        filters_helper::set_filter($this->report->get_id(), (object) $value);

        // Sanity check.
        $instance = new filters_helper($this->report->get_id());
        $result = $instance->get_report_filters();
        $this->assertEquals($value, $result);

        // Reset user:idnumber.
        $filter = filters_helper::add_filter_from_key($this->report, 'user:idnumber');
        $instance->reset_filter($filter->get('id'));

        // Expect filter to contain longvalue only.
        $result = $instance->get_report_filters();
        unset($value['user:idnumber']);
        $this->assertEquals($value, $result);
    }

    /**
     * Test resetting all filter values greater than 1333 characters (user preference limit)
     */
    public function test_reset_all_filters_large_value(): void {
        $value = ['longvalue' => str_repeat('A', 2000), 'longvalueb' => str_repeat('B', 2000)];
        filters_helper::set_filter($this->report->get_id(), (object) $value);
