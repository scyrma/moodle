...
class columns_test extends advanced_testcase {
    ...
    /**
     * Test update visible columns
     *
     * @return void
     */
    public function test_update_visible_columns(): void {
        // The mock report has two columns by default.
        $columns = $this->report->get_active_columns();
        $this->assertCount(2, $columns);

        // Visibility is set to 1 when report is created.
        $settings = json_decode($this->report->get_persistent()->get('cardviewsettings'));
        $this->assertEquals(1, $settings->visibility);

        $settings->visibility = 8;
        $this->report->get_persistent()->set('cardviewsettings', json_encode($settings));
        $this->report->get_persistent()->update();

        // Ensure visibility has been changed to 8.
        $settings = json_decode($this->report->get_persistent()->get('cardviewsettings'));
        $this->assertEquals(8, $settings->visibility);

        columns_helper::update_visible_columns($this->report);

        // Ensure visibility has been set to the max number of active columns in the report.
        $settings = json_decode($this->report->get_persistent()->get('cardviewsettings'));
        $this->assertEquals(2, $settings->visibility);
    }
}
