...
class schedules_test extends advanced_testcase {
     ...
     */
    public function test_can_edit_schedule(): void {
        $this->resetAfterTest();
    ...
    /**
     * Test sending a schedule on a deleted report
     */
    public function test_send_on_deleted_report(): void {
        $this->resetAfterTest(true);
        ...
        // Delete the report.
        $persistent = $report->get_persistent()->delete();

        // Shouldn't be sent.
        $this->assertFalse(schedules::send($scheduleid));
    }
     ...
     * @return array
     */
    public static function needs_to_be_sent_provider(): array {
    ...
    /**
     * Provider for schedules conversion test
     *
     * @return array[]
     */
    public static function convert_provider(): array {
        return [
            'None' => [constants::RECURRENCE_NONE, model::RECURRENCE_NONE],
            'Daily' => [constants::RECURRENCE_DAILY, model::RECURRENCE_DAILY],
            'Weekly' => [constants::RECURRENCE_WEEKLY, model::RECURRENCE_WEEKLY],
            'Monthly' => [constants::RECURRENCE_MONTHLY, model::RECURRENCE_MONTHLY],
            'Annually' => [constants::RECURRENCE_ANNUALLY, model::RECURRENCE_ANNUALLY],
            'Daily - weekdays' => [constants::RECURRENCE_DAILY_WEEKDAY, model::RECURRENCE_WEEKDAYS],
        ];
    }

    /**
     * Convert report with a schedule from tool_reportbuilder to core_creportbuilder
     *
     * @dataProvider convert_provider
     * @param int $recurrence
     * @param int $expected
     */
    public function test_convert(int $recurrence, int $expected): void {
        $this->resetAfterTest();
        ...
        $user1 = $this->getDataGenerator()->create_user();
        $user2 = $this->getDataGenerator()->create_user();
        $audience1 = $this->get_generator()->create_audience([
            'reportid' => $report->get_id(),
            'classname' => manual::class,
            'configdata' => ['users' => [$user1->id, $user2->id]],
        ]);
        $user3 = $this->getDataGenerator()->create_user();
        $audience2 = $this->get_generator()->create_audience([
            'reportid' => $report->get_id(),
            'classname' => manual::class,
        ...
        $data->lastsenton = 123;
        $data->audiences = json_encode([$audience1->get_persistent()->get('id'), $audience2->get_persistent()->get('id')]);
        $oldsechedule = \tool_reportbuilder\local\helpers\schedules::add_schedule($data);

        $newreportid = $report->convert();

        $newaudiences = \core_reportbuilder\local\models\audience::get_records(['reportid' => $newreportid]);
        $newaudienceids = array_map(static function(\core_reportbuilder\local\models\audience $audience) {
            return $audience->get('id');
        }, $newaudiences);

        $schedule = \core_reportbuilder\local\models\schedule::get_record(['reportid' => $newreportid]);
        $this->assertEquals($oldsechedule->get('name'), $schedule->get('name'));
        $this->assertTrue($schedule->get('enabled'));
        $this->assertEqualsCanonicalizing($newaudienceids, json_decode($schedule->get('audiences')));
        $this->assertEquals($oldsechedule->get('format'), $schedule->get('format'));
        $this->assertEquals($oldsechedule->get('subject'), $schedule->get('subject'));
        $this->assertEquals($oldsechedule->get('message'), $schedule->get('message'));
        $this->assertEquals('1', $schedule->get('messageformat'));
        $this->assertEquals($oldsechedule->get('usercreated'), $schedule->get('userviewas'));
        $this->assertEquals($oldsechedule->get('scheduled'), $schedule->get('timescheduled'));
        $this->assertEquals($expected, $schedule->get('recurrence'));
        $this->assertEquals($oldsechedule->get('lastsenton'), $schedule->get('timelastsent'));
        $this->assertEquals($oldsechedule->get('nextsend'), $schedule->get('timenextsend'));
    }
}
