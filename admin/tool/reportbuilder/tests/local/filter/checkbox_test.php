...
/**
 * Tests for conversion of checkbox filter
 *
 * @package   tool_reportbuilder
 * @group     tool_reportbuilder
 * @category  test
 * @covers    \tool_reportbuilder\local\filter\checkbox
 * @copyright 2022 Moodle Pty Ltd <support@moodle.com>
 * @author    2022 Marina Glancy
 * @license   Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class checkbox_test extends \core_reportbuilder_testcase {
    ...
    /**
     * Provider for checkbox conversion test
     *
     * @return array[]
     */
    public static function convert_provider(): array {
        return [
            'any value' => [0, ['Admin', 'A', 'B']],
            'suspended' => [1, ['A']],
            'not suspended' => [2, ['Admin', 'B']],
        ];
    }

    /**
     * Test for conversion of checkbox condition
     *
    ...
    public function test_convert(int $operator, array $expected): void {
        ...
        $this->getDataGenerator()->create_user(['firstname' => 'A', 'suspended' => 1]);
        ...
        $generator->add_condition($report, 'user:suspended');
        $DB->set_field('tool_reportbuilder', 'conditions',
            json_encode(['user:suspended_op' => $operator, 'user:suspended_value' => 1]),
            ['id' => $reportid]);
