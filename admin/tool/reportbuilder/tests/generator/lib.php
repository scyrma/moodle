...
class tool_reportbuilder_generator extends component_generator_base {
    ...
    /**
     * Can be included in the audience testcases to make sure that they can be converted to core reportbuilder
     *
     * If an audience can not be converted, the test will fail and the description of the problem will be returned.
     *
     * @param string $classname
     * @param advanced_testcase $test
     */
    public function audience_test_convert_to_core_reportbuilder(string $classname, advanced_testcase $test): void {
        $test->assertTrue(is_subclass_of($classname, \tool_reportbuilder\audience_base::class),
            'Class '.$classname.' is not a subclass of '. \tool_reportbuilder\audience_base::class);

        /** @var \tool_reportbuilder\datasource $oldreport */
        $oldreport = $this->create_report(['source' => mock_report::class, 'adddefault' => 0]);
        $oldaudience = $classname::create($oldreport->get_persistent()->get('id'), []);

        // Make sure convert_get_audience_class() is implemented.
        $test->assertNotEmpty($oldaudience->convert_get_audience_class(),
            "Class $classname must implement function convert_get_audience_class() and return the name of ".
            "the corresponding class extending \core_reportbuilder\local\audiences\base");
        $newclassname = $oldaudience->convert_get_audience_class();
        $test->assertTrue(is_subclass_of($newclassname, \core_reportbuilder\local\audiences\base::class));
    }
}
