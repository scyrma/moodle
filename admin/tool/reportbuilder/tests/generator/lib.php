...
class tool_reportbuilder_generator extends component_generator_base {
     ...
     * @throws coding_exception
     */
    public function create_audience($record): audience {
        $record = (array)$record;
        ...
        return \tool_reportbuilder\local\helpers\audience::create_record((object) $record);
    }
    ...
    public function create_schedule($record): schedule {
        ...
        if (!array_key_exists('recipients', $record)) {
            $record['recipients'] = json_encode([]);
        }
