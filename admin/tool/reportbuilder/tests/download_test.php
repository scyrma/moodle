...
require_once($CFG->dirroot . '/admin/tool/reportbuilder/tests/fixtures/table_dataformat_export_format.php');
require_once($CFG->libdir . '/phpexcel/PHPExcel/IOFactory.php');
require_once($CFG->libdir . '/odslib.class.php');
require_once($CFG->dirroot . '/admin/tool/reportbuilder/tests/fixtures/pdf_writer.php');
...
class tool_reportbuilder_download_testcase extends advanced_testcase {
    ...
    public function test_download_cvs() {
        ...
        $this->get_reportbuilder_generator()->add_filter($reportid, $filters['user:country']);
    ...
    public function test_download_xlsx() {
        ...
        $excelobject = $this->get_excel($content);
        $excelsheet = $excelobject->getSheet(0);
        $this->assertEquals('Full name', $excelsheet->getCell('A1')->getValue()->getPlainText());
        $this->assertEquals('User Lastname 1', $excelsheet->getCell('A2')->getValue()->getPlainText());
        ...
        $excelsheet = $excelobject->getSheet(0);
        $this->assertEquals('Full name', $excelsheet->getCell('A1')->getValue()->getPlainText());
        $this->assertEquals('User Lastname 2', $excelsheet->getCell('A2')->getValue()->getPlainText());
        $this->assertEquals('User Lastname 3', $excelsheet->getCell('A3')->getValue()->getPlainText());
        $this->assertEquals('User Lastname 4', $excelsheet->getCell('A4')->getValue()->getPlainText());
     ...
     */
    private function get_excel(string $content): PHPExcel {
    ...
    private function get_ods_rows_content($content) {
        $reader = new Box\Spout\Reader\ODS\Reader();
        $reader->setGlobalFunctionsHelper(new \Box\Spout\Common\Helper\GlobalFunctionsHelper());
        ...
        foreach ($sheets as $sheet) {
            /** @var Box\Spout\Reader\ODS\RowIterator[] $rows */
            $rows = $sheet->getRowIterator();
            foreach ($rows as $row) {
                $rowscellsvalues[] = $row[0];
            }
        }
