...
class tool_reportbuilder_datasource_report_course_enrolment_completion_testcase extends advanced_testcase {
    ...
    public function test_grades_columns() : void {
        ...
        $this->assertEquals($this->course->fullname, $rows[0][0]);
        $this->assertStringContainsString(fullname($this->user1, true), $rows[0][1]);
        $this->assertNotEmpty($rows[0][2]);
        $this->assertEquals('Yes', $rows[0][3]);
        $this->assertEquals('60.00', $rows[0][4]);
        $this->assertEquals('50.00', $rows[0][5]);
        // Check the report data for user2, check course grade, completion and required grade.
        $this->assertEquals($this->course->fullname, $rows[1][0]);
        $this->assertStringContainsString(fullname($this->user2, true), $rows[1][1]);
        $this->assertNotEmpty($rows[1][2]);
        $this->assertEquals('No', $rows[1][3]);
        $this->assertEquals('40.00', $rows[1][4]);
        $this->assertEquals('50.00', $rows[1][5]);
    }
