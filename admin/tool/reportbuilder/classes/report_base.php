...
abstract class report_base {
    ...
    /** @var array */
    private $attributes = [];
    /** @var array list of active columns, calculated on first call to get_active_columns() */
    private $activecolumns = null;
    /** @var array list of active filters, calculated on first call to get_active_filters() */
    private $activefilters = null;
    /** @var array list of active conditions, calculated on first call to get_active_filters() */
    ...
    final protected function add_entity(entity_base $entity) {
        ...
        $this->addedentities[$entity->get_entity_name()] = $entity;
     ...
     * @param array $params named parameters for these queries (must be genrated with
     *     core_reportbuilder\local\helpers\database::generate_param_name()
     ...
     *     (must be genrated with core_reportbuilder\local\helpers\database::generate_param_name()
     * @param bool $validateparams Some queries might add non wp parameters and validation could fail
     */
    ...
    /**
     * For users who not have permission to view inactive users
     * add non-suspended and confirmed user conditions to users report
     *
     * @param string $usertablealias
     */
    final public function add_confirmed_user_condition(string $usertablealias = 'u'): void {
        $conditions = [$usertablealias.'.suspended' => 0, $usertablealias.'.confirmed' => 1];
        foreach ($conditions as $key => $row) {
            $this->add_base_condition_simple($key, $row);
        }
    }
}
