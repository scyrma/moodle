...
class report_column {
     ...
     * @param \lang_string|null $visiblename - the name and default title of the column
     ...
     * @return report_column
     */
    public function add_joins(array $joins) : report_column {
    ...
    /**
     * Get sql.
     * @param string|null $aggr The aggregation active.
     * @param int|null $dbtype Type of field.
     ...
     * @param string|null $aggr If the column has some aggregation applied
     * @param int $columnkey index of the column in the table (to generate unique fields prefixes)
     * @return string
     */
     ...
     * @return array|mixed
     */
    public function get_fields(?string $aggre, int $columnkey) {
        global $DB;
     ...
     * @param string|null $alias Alias for
     * @param array $params
     * @return report_column
     */
    ...
    public function get_callbacks(string $aggre = null) : array {
        ...
        if (!$aggre || $aggre === 'unique' || $aggre === 'min' || $aggre === 'max') {
            return $this->callbacks;
        }
        // For aggregation 'sum' use the same callback as for non-aggregated field (except for booleans).
        // Aggregation "sum" for booleans is pretty much the same as "count", it normally does not need
        // any callback at all, it's just a number.
        if ($aggre === 'sum' && ($this->get_type() != constants::DB_TYPE_BOOLEAN)) {
            return $this->callbacks;
        }
