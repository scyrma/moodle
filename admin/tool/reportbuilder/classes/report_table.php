...
class report_table extends \table_sql {
     ...
     */
    public function define_actions(array $actions = []) {
    ...
    public function print_nothing_to_display() {
        ...
        echo \html_writer::div(get_string('nothingtodisplay'), 'p-5 text-center text-gray-800 bg-gray-200');
    }
        ...
        if (!$this->started_output) {
            ...
            if (!$this->isuserview && !$this->display_table_data_during_editing()) {
                echo \html_writer::div(get_string('reportliveeditingdisabled', 'tool_reportbuilder'),
                    'p-5 text-center text-gray-800 bg-gray-200');
            }
        }
    }
    ...
    public function get_row_html($row, $classname = '') {
        ...
        $settings = json_decode($this->report->get_persistent()->get('cardviewsettings'), 1);
        if (isset($settings['visibility']) && $settings['visibility'] < count($this->columns)) {
            $togglebutton = \html_writer::tag('button', \html_writer::tag('i', '', ['class' => 'fa fa-angle-down']),
                ['type' => 'button', 'class' => 'btn', 'data-toggle' => 'collapsed', 'title' => get_string('expand', 'core')]);
            $html .= \html_writer::tag('td', $togglebutton, ['class' => 'card-toggle d-none']);
        }
