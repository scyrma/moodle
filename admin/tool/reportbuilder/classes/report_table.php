...
class report_table extends \table_sql {
    ...
    public function print_headers() {
        ...
        if ($this->isuserview) {
            // TODO: improve, we can't access $this->prefs from parent class so get preference manually.
            if (!$prefs = json_decode(get_user_preferences('flextable_' . $this->uniqueid), true)) {
                $prefs = ['sortby' => []];
            }

            $primarysortcolumn = '';
            $primarysortorder  = SORT_ASC;
            if (!empty($prefs) && reset($prefs['sortby'])) {
                $primarysortcolumn = key($prefs['sortby']);
                $primarysortorder  = current($prefs['sortby']);
            }
    ...
    protected function render_reset_button() {
        ...
        $html .= \html_writer::link('#', get_string('resettable'), $linkattributes);
