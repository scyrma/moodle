...
class helper {
    ...
    public static function get_sources($default = false): array {
        ...
        foreach ($datasources as $class => $path) {
            if (manager::report_source_valid($class, datasource::class)) {
                ...
                $sources[$pluginname][$class] = call_user_func([$class, 'get_name']);
            }
        }
    ...
    /**
     * Display a warning that there are legacy reports
     *
     * @return void
     * @throws \coding_exception
     */
    public static function legacy_reports_warning() {
        global $OUTPUT, $DB;
        if (permission::can_view_any()) {
            [$sql, $params] = hierarchy::filter_own_or_parent_shared_entities_sql('rb.tenantid', 'rb.shared=1');
            $sql = "SELECT 1 FROM {tool_reportbuilder} rb WHERE $sql AND type = :type";
            $params += ['type' => constants::TYPE_DATASOURCE];
            if (!$DB->record_exists_sql($sql, $params)) {
                return;
            }
        } else {
            if (!audience::user_reports_list()) {
                return;
            }
        }
        $url = new \moodle_url('/admin/tool/reportbuilder/index.php');
        echo $OUTPUT->notification(
            get_string('legacyreportswarning', 'tool_reportbuilder') . ' '.
            \html_writer::link($url, get_string('legacyreportsview', 'tool_reportbuilder')),
            'warning tool_reportbuilder-warning', false);
    }
}
