...
abstract class filter_base {
     ...
     *
     *    $oldconditionname = $this->name;
     *    $newconditionname = $newcondition->get_filter_persistent()->get('uniqueidentifier');
     *    return [
     *        $newconditionname => $values[$oldconditionname] ?? null,
     *        $newconditionname.'_operator' => $values[$oldconditionname.'_op'] ?? null,
     *    ];
     *
     * But depending on the filter type there may be additional values and also additional validations.
     * When conversion is not possible, the function should throw an exception {@see convert_not_possible}
     *
    ...
    public function convert_condition_values(array $values,
        ...
        throw new convert_not_implemented('Filter class '.get_class($this) . ' must implement ' . __FUNCTION__);
    }
