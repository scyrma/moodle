...
class schedule extends dynamic_form {
    ...
    public function definition() {
        ...
        if (!sharedspace::is_shared_space()) {
            ...
            // In shared space we limit 'View report data as' to the current user creating the schedule.
            $html = html_writer::tag('div', fullname($USER));
            $mform->addElement('static', 'usercreatedstatic', get_string('userviewreportas', 'tool_reportbuilder'), $html);
            $mform->addElement('hidden', 'usercreated', $USER->id);

            $html = html_writer::tag('div', get_string('userviewreportasnotice', 'tool_reportbuilder'),
