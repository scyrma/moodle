...
class manager {
    ...
    public static function get_tabs($reportid) {
        ...
        $tabsoutput = new \tool_wp\output\secondary_tabs($attributestab);

        $report = new \tool_reportbuilder\reportbuilder($reportid);
        $iscurrentsharedspace = sharedspace::is_shared_space();
        $isreportsharedspace = sharedspace::is_shared_space($report->get('tenantid'));

        $data = ['reportid' => $reportid];
        $tabsoutput->add_tab(new \tool_reportbuilder\output\tabs\table($data));
        if (!$isreportsharedspace || ($iscurrentsharedspace && $report->get('shared'))) {
            $tabsoutput->add_tab(new \tool_reportbuilder\output\tabs\schedule($data));
            $tabsoutput->add_tab(new \tool_reportbuilder\output\tabs\audiences($data));
        }
