...
abstract class entity_base {
     ...
     *
     * @return string name of the class extending {@see \core_reportbuilder\local\entities\base}
     * @throws convert_not_implemented
     */
    public function convert_get_entity_class(): string {
        throw new convert_not_implemented('Entity class '.get_class($this).' must implement '.__FUNCTION__);
    }
    ...
    public function convert_get_column_name(report_column $oldcolumn, \core_reportbuilder\datasource $newsource): string {
        return $this->convert_get_generic_field_name($oldcolumn->get_name());
    }
     ...
     * @return string
     */
    public function convert_get_filter_name(\tool_reportbuilder\report_filter $oldfilter,
                                            \core_reportbuilder\datasource $newsource): string {
        return $this->convert_get_generic_field_name($oldfilter->get_name());
    }
     ...
     *
     * @param \tool_reportbuilder\report_filter $oldcondition
     * @param \core_reportbuilder\datasource $newsource
     * @return string
     */
    public function convert_get_condition_name(\tool_reportbuilder\report_filter $oldcondition,
                                               \core_reportbuilder\datasource $newsource): string {
        return $this->convert_get_generic_field_name($oldcondition->get_name());
    }

    /**
     * The column/filter/condition name from the corresponding core reportbuilder entity
     *
     * To reduce code duplication some generic fields (i.e. custom fields) can be converted here
     *
    ...
    /**
     * Helper method to convert custom profile fields names
     *
     * In the workplace entities for the tool_reportbuilder the identifiers for the custom profile fields
     * used field id, where in the core_reportbuilder the identifiers are fields short names.
     *
     * @param \core_customfield\handler $cfhandler custom field handler for the current entity
     * @param string $oldfieldname the current column/filter/condition name
     * @param string $oldprefix current prefix in the identifiers ("{OLDPREFIX}{ID}")
     * @param string $newprefix new prefix in the identifiers ("{NEWPREFIX}{SHORTNAME}")
     * @return string expected name of the field in the converted report (by default the current name)
     */
    protected function convert_get_custom_field_name(\core_customfield\handler $cfhandler, string $oldfieldname,
                                                     string $oldprefix, string $newprefix = 'customfield_'): ?string {
        if (preg_match('/^'.preg_quote($oldprefix, '/').'(\d+)$/', $oldfieldname, $matches)) {
            if ($field = ($cfhandler->get_fields()[(int)$matches[1]] ?? null)) {
                return $newprefix . $field->get('shortname');
            }
        }
        return $oldfieldname;
    }
