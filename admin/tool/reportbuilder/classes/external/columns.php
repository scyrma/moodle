...
class columns extends \external_api {
    ...
    public static function sort_table_by_heading_parameters() {
        return new \external_function_parameters(
            [
                'tableid'           => new \external_value(PARAM_TEXT, 'The table unique ID'),
     ...
     * @param int $sortorder
     */
    public static function sort_table_by_heading($tableid, $sortcolumn, $sortorder) {
        ...
        if (!$prefs = json_decode(get_user_preferences('flextable_' . $params['tableid']), true)) {
            $prefs = ['sortby' => []];
        }

        // If key exists switch direction and remove it first.
        if (array_key_exists($params['sortcolumn'], $prefs['sortby'])) {
            $sortorder = $prefs['sortby'][$params['sortcolumn']] == SORT_ASC ? SORT_DESC : SORT_ASC;
            unset($prefs['sortby'][$params['sortcolumn']]);
        } else {
            $sortorder = $params['sortorder'];
        }

        // Add the new key and make sure that no more than 2 are present into the array.
        $prefs['sortby'] = array_slice(
            array_merge(array($params['sortcolumn'] => $sortorder), $prefs['sortby']),
            0,
            2
        );

        set_user_preference('flextable_' . $params['tableid'], json_encode($prefs));
    }
    ...
    /**
     * Describes the parameters for reset_table_heading_sort webservice.
     *
     * @return \external_function_parameters
     */
    public static function reset_table_heading_sort_parameters() {
        return new \external_function_parameters(
            [
                'tableid'   => new \external_value(PARAM_TEXT, 'The table unique ID')
            ]
     ...
     * @param string $tableid
     */
    public static function reset_table_heading_sort($tableid) {
        $params = self::validate_parameters(self::reset_table_heading_sort_parameters(), ['tableid' => $tableid]);

        $context = \context_system::instance();
        self::validate_context($context);

        set_user_preference('flextable_' . $params['tableid'], json_encode([]));
    }

    /**
     * Returns description of method reset_table_heading_sort.
     *
     * @return null
     */
    public static function reset_table_heading_sort_returns() {
        return null;
    }
}