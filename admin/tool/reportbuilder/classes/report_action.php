...
class report_action {
    ...
    public function get_icon(\stdClass $row) {
        ...
        foreach ($this->attributes as $name => $value) {
            $attributes[$name] = $this->replace_placeholder($value, $row);
        }

        // Class action-icon gets removed when we add classes manually to the report_action and should be always there.
        if (isset($attributes['class'])) {
            $attributes['class'] = 'action-icon ' . $attributes['class'];
        }

        return $OUTPUT->action_icon($url, $this->pix, null, $attributes);
    }
