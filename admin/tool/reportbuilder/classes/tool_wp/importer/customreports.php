...
use core_reportbuilder\permission as crbpermission;
use Exception;
use moodle_url;
...
class customreports extends importer_base {
    ...
    protected function initialise() {
        ...
        // Entity for the converted reports, it is a "pseudo" entity because it is created during import
        // but was not actually present in the import file.
        $this->register_entity(report::TABLE, [
            self::ENTITY_DEPENDENCIES => [reportbuilder::TABLE],
            self::ENTITY_LOGSUCCESS => static function (int $id, array $details) {
                return get_string('importlogconversionsuccess', 'tool_reportbuilder', $details);
            },
            self::ENTITY_LOGERROR => static function (array $details) {
                return get_string('importlogconversionfailure', 'tool_reportbuilder', $details);
            },
            // This string is hardcoded because is not used in UI.
            self::ENTITY_NAMEPLURAL => 'Custom reports',
        ]);
    }
    ...
    public function perform_import(string $entityname): void {
        ...
        if ($settingimportall !== null) {
            ...
            foreach ($reports as $report) {
                ...
                if ($this->is_collecting_errors() || !$report->get_new_id()) {
                    continue;
                }

                // Convert exported 3.11 reports into core reportbuilder.
                $toolreportid = $report->get_new_id();
                if ($toolreportid && $toolreportid > 0 && crbpermission::can_create_report()) {
                    try {
                        /** @var \tool_reportbuilder\datasource $newreport */
                        $newreport = \tool_reportbuilder\manager::get_report($toolreportid);
                        $corerbid = $newreport->convert();

                        // Log success.
                        $this->add_pseudo_entity_to_log(report::TABLE, $corerbid, [
                            'name' => $newreport->get_reportname(),
                            'oldid' => $toolreportid,
                            'newid' => $corerbid,
                        ]);
                        ...
                        // Log failure.
                        $this->add_pseudo_entity_to_log(report::TABLE, null, [
                            'name' => $newreport->get_reportname(),
                            'message' => $e->getMessage(),
                            'oldid' => $toolreportid,
                            'url' => (new moodle_url('/admin/tool/reportbuilder/manage.php', ['id' => $toolreportid]))->out(),
                        ]);
                    }
                }
            }
