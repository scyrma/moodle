...
class customreports extends exporter_base {
    ...
    public function is_available(): bool {
        return !strlen($this->entrypoint) && permission::can_create(true);
    }
    ...
    public function add_to_options_form(export_settings_form $form): void {
        ...
        $mform->addElement('advcheckbox', self::EXPORT_CONTENT, null,
        ...
        $mform->addElement('advcheckbox', self::EXPORT_AUDIENCES, null, new \lang_string('audiences', 'tool_reportbuilder'));
        ...
        $mform->addElement('advcheckbox', self::EXPORT_SCHEDULES, null, new \lang_string('schedules', 'tool_reportbuilder'));
        ...
        $mform->addElement('autocomplete', self::EXPORT_SELECT_REPORTS, new \lang_string('customreports', 'tool_reportbuilder'),
    ...
    public function get_summary_for_review_step(bool $exportcompleted): string {
        ...
        $settings = [
            ...
            ['name' => $strs->schedules, 'value' => !empty($data[self::EXPORT_SCHEDULES])],
        ];
    ...
    public function perform_export(): void {
        ...
        if (!empty($data[self::EXPORT_INSTANCES])) {
            ...
            if (!empty($data[self::EXPORT_AUDIENCES]) && count($reportids) > 0) {
                ...
                $audiences = audience::get_records_select("reportid {$select}", $params);

                foreach ($audiences as $audience) {
                    $this->prepare_data_for_workplace_export(audience::TABLE, (array) $audience->to_record())
            ...
            if (!empty($data[self::EXPORT_SCHEDULES]) && count($reportids) > 0) {
                list($select, $params) = $DB->get_in_or_equal($reportids, SQL_PARAMS_NAMED);
                $schedules = schedule::get_records_select("reportid {$select}", $params);

                foreach ($schedules as $schedule) {
                    $export = $this->prepare_data_for_workplace_export(schedule::TABLE, (array) $schedule->to_record())
                        ->add_mappings('usercreated', 'user')
                        ->add_mappings('departmentid', 'tool_organisation_department')
                        ->add_mappings('positionid', 'tool_organisation_position');

                    // We need to loop over manually added recipients of each schedule and add them too.
                    $recipients = json_decode($schedule->get('recipients'));
                    if ($recipients && !empty($recipients->users)) {
                        $scheduleusers = [];
                        foreach ($recipients->users as $userid) {
                            $scheduleusers[] = ['userid' => $userid];
                        }

                        $export->add_nested_entities('users', $scheduleusers)
                            ->add_mappings_for_nested_entities('users', 'userid', 'user');
                    }
