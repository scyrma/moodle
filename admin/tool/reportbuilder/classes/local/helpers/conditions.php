...
class conditions {
    ...
    public static function get_active_conditions($reportid) {
        $filters = reportbuilder_conditions::get_records(['reportid' => $reportid], 'sortorder', 'ASC');
        return $filters;
    }
    ...
    /**
     * Return values for given condition within the current report
     *
     * @param string $conditionname
     * @return array
     */
    public function get_condition_values(string $conditionname): array {
        $reportconditions = (array) $this->get_report_conditions();

        return array_filter($reportconditions, static function(string $conditionkey) use ($conditionname): bool {
            return strpos($conditionkey, $conditionname) === 0;
        }, ARRAY_FILTER_USE_KEY);
    }

    /**
     * Merge given values with the current conditions of the current report
     *
     * @param array $values
     * @return bool
     */
    public function merge_condition_values(array $values): bool {
        $reportconditions = array_merge((array) $this->get_report_conditions(), $values);

        return $this->add_report_conditions(json_encode($reportconditions));
    }
