...
class schedules {
    ...
    public function needs_to_be_sent() : bool {
        ...
        // If schedule has no recurrence and has never been sent, it can be sent now.
        $neversent = ($datelastsent == -1);
        if ($recurrence == constants::RECURRENCE_NONE) {
            return $neversent;
        }

        // Determine the time from which the recurrence should calculate the next send time.
        $fromlastdate = ($neversent ? $datescheduled : $datelastsent);

        return ($datenow >= self::calculate_next_send_time($recurrence, $fromlastdate));
    }

    /**
     * Calculate the next time a schedule should be sent, based on it's recurrence and the last time it was sent
     *
