...
class filters {
     ...
     *
     * @deprecated please use \tool_reportbuilder\local\helpers\filters::add_filter_from_key instead
     *
    ...
    public static function add_filter(int $reportid, report_filter $filter) : int {
        debugging('This function is deprecated. Please use \tool_reportbuilder\local\helpers\filters::add_filter_from_key instead',
                DEBUG_DEVELOPER);

        $report = manager::get_report($reportid);

        return self::add_filter_from_key($report, $filter->get_unique_identifier())->get('id');
    }
