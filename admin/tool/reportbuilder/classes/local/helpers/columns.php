...
class columns {
     ...
     */
    public static function add_column_from_key(report_base $report, string $key): reportbuilder_column {

        if (!array_key_exists($key, $report->get_columns())) {
            throw new \moodle_exception('Column not found'); // TODO SP-422 string.
        }
        // TODO SP-422 check column was not already added.
        $column = $report->get_columns()[$key];
    ...
    public static function export(report_base $report, \renderer_base $output) {
        ...
        $columnscount = [];
        $columnssorting = [];
        $columns = self::get_active_columns($report);
