...
class columns {
    ...
    public static function get_active_columns(report_base $report) {
        ...
        $columns = reportbuilder_column::get_records(['reportid' => $report->get_id()], 'columnorder, id');
        return $columns;
    }
    ...
    /**
     * Updates the number of columns always visible on the card view settings
     *
     * Value set in visibility can not be higher than the maximum number of active columns.
     * It will set the value to the max number of columns if is higher.
     *
     * @param report_base $report
     */
    public static function update_visible_columns(report_base $report): void {
        $persistent = $report->get_persistent();
        $reportsettings = (array) json_decode($persistent->get('cardviewsettings'), 1);
        $totalcolumns = count(self::get_active_columns($report));
        $visiblecolumns = $reportsettings['visibility'] ?? 0;
        if ((int)$visiblecolumns > $totalcolumns) {
            $reportsettings['visibility'] = $totalcolumns;
            $persistent->set('cardviewsettings', json_encode($reportsettings));
            $persistent->update();
        }
    }
}
