...
/**
 * Class containing the count aggregation
 *
 * @package   tool_reportbuilder
 * @copyright 2019 Moodle Pty Ltd <support@moodle.com>
 * @author    2019, Alberto Lara Hernández <albertolara@moodle.com>
 * @license   Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class count extends aggregation_base {
    /**
     * Get the SQL
     *
     * @param string $field
    ...
    public static function get_field(string $field, ?int $dbtype = null) : string {
        ...
        if ($dbtype == constants::DB_TYPE_LONGTEXT && $dbfamily === 'oracle') {
            return "COUNT(" . $DB->sql_compare_text($field, 255) . ")";
        }
        return "COUNT($field)";
    }
    ...
    public static function get_displayname(): string {
        return get_string('aggregation_count', 'tool_reportbuilder');
    }
