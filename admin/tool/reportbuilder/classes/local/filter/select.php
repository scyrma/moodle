...
class select extends filter_base {
    ...
    public function convert_condition_values(array $values, \core_reportbuilder\local\filters\base $newcondition): array {
        if ($newcondition instanceof \core_reportbuilder\local\filters\select) {
            $operator = array_key_exists("{$this->name}_op", $values) ? $values["{$this->name}_op"] : null;
            // Value in tool reportbuilder is using the same constants as in core reportbuilder.
        ...
        } else if ($newcondition instanceof \core_reportbuilder\local\filters\category) {
            $operator = array_key_exists("{$this->name}_op", $values) ? $values["{$this->name}_op"] : null;
            ...
            } else if ($operator != self::EQUAL_TO) {
                return [];
            }
            $value = array_key_exists($this->name, $values) ? $values[$this->name] : 0;
            $newconditionname = $newcondition->get_filter_persistent()->get('uniqueidentifier');
            return [
                $newconditionname.'_value' => $value,
                $newconditionname.'_subcategories' => 0,
            ];
        }

        // The datasource mapped this condition to something else, not "select", so
    ...
    public function get_operators() {
        return [self::ANY_VALUE => get_string('isanyvalue', 'filters'),
