...
class checkbox extends filter_base {
    ...
    public function convert_condition_values(array $values,
        ...
        if ($newcondition instanceof boolean_select) {
            $operator = array_key_exists("{$this->name}_op", $values) ? $values["{$this->name}_op"] : null;
            $newname = $newcondition->get_filter_persistent()->get('uniqueidentifier');

            switch($operator) {
                case 1: // Checked.
                    return [$newname.'_operator' => boolean_select::CHECKED];
                case 2: // Not checked.
                    return [$newname.'_operator' => boolean_select::NOT_CHECKED];
                default:
                    return [$newname.'_operator' => boolean_select::ANY_VALUE];
            }
        }
        // The datasource mapped this condition to something else, not "boolean_select", so
    ...
    public function get_operators() {
        return [0 => get_string('checkboxanyvalue', 'tool_reportbuilder'),
