...
class date_condition extends filter_base {
    ...
    public function convert_condition_values(array $values, \core_reportbuilder\local\filters\base $newcondition): array {
        if ($newcondition instanceof \core_reportbuilder\local\filters\date) {
            ...
            switch ($operator) {
                case self::DATE_NOT_EMPTY:
                    $newoperator = \core_reportbuilder\local\filters\date::DATE_NOT_EMPTY;
                    break;
                case self::DATE_EMPTY:
                    $newoperator = \core_reportbuilder\local\filters\date::DATE_EMPTY;
                    break;
                case self::DATE_LAST:
                    $newoperator = \core_reportbuilder\local\filters\date::DATE_LAST;
                    break;
                case self::DATE_NEXT:
                    $newoperator = \core_reportbuilder\local\filters\date::DATE_NEXT;
                    break;
                case self::DATE_CURRENT:
                    $newoperator = \core_reportbuilder\local\filters\date::DATE_CURRENT;
                    break;
                case self::DATE_PREVIOUS:
                    $newoperator = \core_reportbuilder\local\filters\date::DATE_PREVIOUS;
                    break;
                case self::DATE_PAST:
                    // TODO WP-3755 Add conversion for self::DATE_PAST.
                    throw new convert_not_possible(
                        get_string('conditionoptioncannotbeconverted', 'tool_reportbuilder',
                        (object)[
                            'option' => get_string('dateinthepast', 'tool_reportbuilder'),
                            'condition' => $this->get_formatted_header(),
                        ]));
                case self::DATE_FUTURE:
                    // TODO WP-3755 Add conversion self::DATE_FUTURE.
                    throw new convert_not_possible(
                        get_string('conditionoptioncannotbeconverted', 'tool_reportbuilder',
                        (object)[
                            'option' => get_string('dateinthefuture', 'tool_reportbuilder'),
                            'condition' => $this->get_formatted_header(),
                        ]));
                case self::DATE_UPCOMING:
                    throw new convert_not_possible(
                        get_string('conditionoptioncannotbeconverted', 'tool_reportbuilder',
                        (object)[
                            'option' => get_string('dateupcoming', 'tool_reportbuilder'),
                            'condition' => $this->get_formatted_header(),
                        ]));
                default:
                    // Otherwise it must be "DATE_ANY".
                    $newoperator = \core_reportbuilder\local\filters\date::DATE_ANY;
                    break;
            }

            switch ($operator2) {
                case self::DATE_RELATIVE_DAY:
                    $newoperator2 = \core_reportbuilder\local\filters\date::DATE_UNIT_DAY;
                    break;
                case self::DATE_RELATIVE_WEEK:
                    $newoperator2 = \core_reportbuilder\local\filters\date::DATE_UNIT_WEEK;
                    break;
                case self::DATE_RELATIVE_MONTH:
                    $newoperator2 = \core_reportbuilder\local\filters\date::DATE_UNIT_MONTH;
                    break;
                case self::DATE_RELATIVE_YEAR:
                    $newoperator2 = \core_reportbuilder\local\filters\date::DATE_UNIT_YEAR;
                    break;
                case self::DATE_RELATIVE_QUARTER:
                    // The case self::DATE_RELATIVE_QUARTER has no conversion and
                    // is being mapped here.
                    throw new convert_not_possible(
                        get_string('conditionoptioncannotbeconverted', 'tool_reportbuilder',
                        (object)[
                            'option' => $this->get_operators()[$operator] . ': ' . get_string('quarter', 'tool_reportbuilder'),
                            'condition' => $this->get_formatted_header(),
                        ]
                    ));
                default:
                    // This must be an operator that does not require the second part.
                    $newoperator2 = null;
            }

            return [
                $newconditionname.'_operator' => $newoperator,
                $newconditionname.'_unit' => $newoperator2,
                $newconditionname.'_value' => $value,
            ];
        }

        // The datasource mapped this condition to something else, not "date", so
