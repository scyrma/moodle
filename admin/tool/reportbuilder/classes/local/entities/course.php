...
class course extends entity_base {
    ...
    public function convert_get_entity_class(): string {
        return \core_reportbuilder\local\entities\course::class;
    }
    ...
    protected function convert_get_generic_field_name(string $oldfieldname): string {
        ...
        $oldprefix = clean_param($tablealias, PARAM_ALPHANUMEXT) . 'id';
        $cfhandler = \core_customfield\handler::get_handler('core_course', 'course');
    ...
    protected function get_all_columns() : array {
        ...
        foreach ($coursefields as $coursekey => $coursefield) {
            ...
            // Disable concat aggregation for summary.
            if ($coursekey === 'summary') {
                $column
    ...
    public static function get_options_for_category() {
        return [-1 => ''] + \core_course_category::make_categories_list();
    }
    ...
    /**
     * Formats the course field for display when aggregated
     *
