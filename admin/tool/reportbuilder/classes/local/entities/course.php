...
class course extends entity_base {
     ...
     * course constructor.
     *
     * @param string $coursejoin join with the course table or '' if the join is already added
     * @param string $coursetablealias alias for the {course} table used in the join above
     * @param array $excludefields list of course fields to exclude
     */
    public function __construct(string $coursejoin, string $coursetablealias = 'c', array $excludefields = []) {
        $this->coursejoin = $coursejoin;
        $this->coursetablealias = $coursetablealias;
        $this->excludefields = array_combine($excludefields, $excludefields);
        $this->customfields = new customfields($coursetablealias . '.id', $this->get_entity_name(),
            'core_course', 'course');
        $this->customfields->add_join($coursejoin);
    }
    ...
    public function get_columns() : array {
        ...
        foreach ($coursefields as $coursekey => $coursefield) {
            $columns[] = (new report_column(
                ...
                ->add_field($this->coursetablealias . '.' . $coursekey)
    ...
    public function get_conditions() : array {
        return array_merge($this->get_conditions_or_filters(true), $this->customfields->get_conditions());
    }

    /**
     * Returns all available filters on course fields
     *
     * @return report_filter[]
     */
    public function get_filters() : array {
        return array_merge($this->get_conditions_or_filters(false), $this->customfields->get_filters());
    }
