...
class user extends entity_base {
    ...
    public function convert_get_entity_class(): string {
        return \tool_wp\reportbuilder\local\entities\user::class;
    }
     ...
     * @return string
     */
    public function convert_get_filter_name(report_filter $oldfilter,
                                            \core_reportbuilder\datasource $newsource): string {
        if (in_array($oldfilter->get_name(), ['hascurrentjobs', 'jobposition', 'jobdepartment', 'audience'])) {
            // TODO WP-3629 implement.
            throw new convert_not_possible('User filter '.$oldfilter->get_name().' can not be converted');
        }
    ...
    public function convert_get_condition_name(\tool_reportbuilder\report_filter $oldcondition,
                                               \core_reportbuilder\datasource $newsource): string {
        if (in_array($oldcondition->get_name(), ['hascurrentjobs', 'jobposition', 'jobdepartment', 'audience'])) {
            // TODO WP-3629 implement.
            throw new convert_not_possible('User condition '.$oldcondition->get_name().' can not be converted');
        }
