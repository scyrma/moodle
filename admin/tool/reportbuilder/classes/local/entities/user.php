...
class user extends entity_base {
    ...
    protected function get_all_columns() : array {
        ...
        if ($this->hastenantcolumns) {
            $columns[] = (new report_column(
                ...
                ->add_joins($this->get_joins())
                ->add_joins($this->get_tenant_joins())
     ...
     *
     * @deprecated this function is no longer used, see get_tenant_joins()
     * @return string
     */
    ...
    /**
     * Joins for finding the user tenant
     *
     * @return string[]
     */
    protected function get_tenant_joins() {
        $ttu = $this->get_table_alias('tool_tenant_user');
        $tt = $this->get_table_alias('tool_tenant');
        $u = $this->get_table_alias('user');
        $defaulttenantid = tenancy::get_default_tenant_id();
        if ($this->tempalias === null) {
            ...
            // stays the same if function get_tenant_joins() is called several times.
            $this->tempalias = db::generate_alias();
        }
        return [
            "LEFT JOIN {tool_tenant_user} {$ttu} ON {$ttu}.userid = {$u}.id",
            "LEFT JOIN {tool_tenant} {$this->tempalias} ON " .
                "{$this->tempalias}.id = {$ttu}.tenantid AND {$this->tempalias}.archived = 0",
            "LEFT JOIN {tool_tenant} {$tt} ON {$tt}.id = COALESCE({$this->tempalias}.id, $defaulttenantid) " .
            // When there is no record in {tool_tenant_user} for an existing user it means that user belongs
            // to the default tenant. At the same time when there is a left join with {user} table and there is no user,
            // check that user.id is not null to make sure we return null in the tenant column and not the "Default tenant".
                "AND {$u}.id IS NOT NULL"
