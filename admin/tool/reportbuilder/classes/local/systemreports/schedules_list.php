...
class schedules_list extends system_report {
    ...
    protected function initialise() {
        ...
        $this->add_base_fields('sch.id, sch.name, sch.enabled, sch.reportid, rb.tenantid'); // Necessary for actions.
    ...
    protected function set_columns() {
        $tablealias = $this->get_main_table_alias('tool_reportbuilder_scheduled');
        ...
        $this->add_column((new report_column(
            ...
            ->add_field("{$tablealias}.reportid")
            ->set_is_default(true)
            ->set_is_sortable(true)
            ->set_is_available($this->get_reportid() === 0)
            ->add_callback([format::class, 'report_link'], ['url' => new \moodle_url('/admin/tool/reportbuilder/view.php')])
        );
    ...
    /**
     * Set row class, dimmed for disabled schedules.
     *
     * @param \stdClass $row
     * @return string
     */
