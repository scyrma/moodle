...
/**
 * Delete the old tool reportbuilder system reports data after conversion to core reportbuilder
 */
function tool_reportbuilder_upgrade_delete_old_system_reports_data(): void {
    global $DB;

    // Delete old system reports data after conversion to core reportbuilder.
    $systemreportids = $DB->get_fieldset_select('tool_reportbuilder', 'id', 'type = ?',
        ['type' => \tool_reportbuilder\constants::TYPE_SYSTEM]);

    if (empty($systemreportids)) {
        return;
    }

    [$sql, $params] = $DB->get_in_or_equal($systemreportids, SQL_PARAMS_NAMED, 'id');

    // Delete all system report columns.
    $DB->delete_records_select('tool_reportbuilder_column', "reportid $sql", $params);

    // Delete all system report conditions.
    $DB->delete_records_select('tool_reportbuilder_cond', "reportid $sql", $params);

    // Delete all system report filters.
    $DB->delete_records_select('tool_reportbuilder_filter', "reportid $sql", $params);

    // Delete all system reports user preferences. User preferences can be split into chunks of data if they are too long.
    foreach ($systemreportids as $reportid) {
        $DB->delete_records_select('user_preferences',
            'name = :name OR ' . $DB->sql_like('name', ':name2', false),
            [
                'name' => 'filters_report_' . $reportid,
                'name2' => $DB->sql_like_escape('filters_report_' . $reportid . '[') . '%',
            ]
        );
    }

    // Delete all system reports.
    $DB->delete_records_select('tool_reportbuilder', "id $sql", $params);
}
