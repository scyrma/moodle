...
function xmldb_tool_program_upgrade(int $oldversion) {
    ...
    if ($oldversion < 2022121300) {
        // Remove stored preferences that no longer exist.
        $DB->delete_records('user_preferences', ['name' => 'tool_program_program_status_filter']);
        $DB->delete_records('user_preferences', ['name' => 'tool_program_program_view_filter']);
        $DB->delete_records('user_preferences', ['name' => 'tool_program_program_sort_filter']);

        // Program savepoint reached.
        upgrade_plugin_savepoint(true, 2022121300, 'tool', 'program');
    }

    if ($oldversion < 2023011701) {
        // Set a default to the new field 'action' for all current allocation outcomes.
        $results = $DB->get_records('tool_dynamicrule_outcome',
            ['classname' => 'tool_program\tool_dynamicrule\outcome\allocation']);
        foreach ($results as $result) {
            $configdata = @json_decode($result->configdata, true);
            $configdata['action'] = \tool_program\tool_dynamicrule\outcome\allocation::DONT_MODIFY_ALLOCATION;
            $result->configdata = json_encode($configdata);
            $DB->update_record('tool_dynamicrule_outcome', $result);
        }

        // Program savepoint reached.
        upgrade_plugin_savepoint(true, 2023011701, 'tool', 'program');
    }
