...
class tool_program_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_bulk_deallocate_user(): void {
        ...
        $this->assertEquals(5, program_user::count_records());
        ...
        // Only user2, user4 and user5 should still be allocated into the program.
        $allocateduserids = array_map(static function($puser) {
            return $puser->get('userid');
        }, program_user::get_records());
        $this->assertEqualsCanonicalizing([$user2->id, $user4->id, $user5->id], $allocateduserids);
    ...
    public function test_bulk_reset_program_progress() {
        // Generate a program, allocate user to it and complete it as a user.
        $program = $this->generator->generate_program_with_course();
