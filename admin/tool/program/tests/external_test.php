...
class external_test extends externallib_advanced_testcase {
    ...
    /**
     * Test for get_user_programs WS function
     *
    ...
    public function test_get_user_programs(): void {
        ...
        $result = external::get_user_programs();
        $this->assertDebuggingCalledCount(19);
        $cleanresult = external_api::clean_returnvalue(external::get_user_programs_returns(), $result);
        $this->assertDebuggingNotCalled();
        $this->assertCount(count($cleanresult), $result);
        ...
        // Check now with the certification suspended.
        $certificationallocation->set('status', \tool_certification\constants::STATUS_SUSPENDED)->update();
        $result = external::get_user_programs();
        $this->assertDebuggingCalledCount(19);
        $cleanresult = external_api::clean_returnvalue(external::get_user_programs_returns(), $result);
        $this->assertEmpty($cleanresult['origins']);
    ...
    public function test_get_user_learning_statuses(): void {
        ...
        // Create a new allocation to a certification.
        /** @var \tool_certification_generator $certificationgenerator */
        $certificationgenerator = self::getDataGenerator()->get_plugin_generator('tool_certification');
        $certification = $certificationgenerator->generate_certification(['program' => $program1->get('id')]);
        $certificationgenerator->allocate_user($user1->id, $certification->get('id'));

        $result = external::get_user_learning_statuses($user1->id);
        $cleanresult = external_api::clean_returnvalue(external::get_user_learning_statuses_returns(), $result);

        $this->assertNotEmpty($cleanresult['certifications']);
        $this->assertEquals(0, $cleanresult['hasexpiredcertifications']);
    ...
    public function test_get_user_learning_statuses_is_deprecated() {
        $function = \external_api::external_function_info('tool_program_get_user_learning_statuses');
