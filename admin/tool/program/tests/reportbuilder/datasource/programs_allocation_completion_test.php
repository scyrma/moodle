...
class programs_allocation_completion_test extends core_reportbuilder_testcase {
    ...
    public function test_programs_allocation_completion_datasource(): void {
        ...
        $this->generator->allocate_user_to_program($program->get('id'), (int)$user1->id, 0, [
        ...
        // Add program user allocation suspended column to the report.
        $this->rbgenerator->create_column(['reportid' => $report->get('id'), 'uniqueidentifier' => 'program_user:suspended']);
        ...
        $this->assertEquals([
            ...
            'No', // User allocation suspended status.
        ], $contentrow);
    }

    /**
     * Test programs allocation completion datasource with reports column
     */
    public function test_programs_allocation_completion_reports_datasource(): void {
        $this->resetAfterTest();
        ...
        $this->generator->add_course_to_set((int)$course->id, $basesetid);
        $user1 = $this->getDataGenerator()->create_user(['firstname' => 'Lionel', 'lastname' => 'Kenobi']);
        $user2 = $this->getDataGenerator()->create_user(['firstname' => 'Luke', 'lastname' => 'Skywalker']);
        $this->generator->allocate_users_to_program($program->get('id'), [$user1->id, $user2->id]);
        ...
        // Add audience.
        $this->rbgenerator->create_audience([
        ...
        $this->rbgenerator->create_column(['reportid' => $report->get('id'), 'uniqueidentifier' => 'user:fullname'])
            ...
            ->update();
        // Add program user allocation progress column to the report.
        $this->rbgenerator->create_column(['reportid' => $report->get('id'), 'uniqueidentifier' => 'program_user:programprogress']);
        // Add program user allocation progress with overview column to the report.
        $this->rbgenerator->create_column(['reportid' => $report->get('id'),
            'uniqueidentifier' => 'program_user:programprogresswithoverview', ]);

        // Validate as admin. Admin should see progress for all users.
        self::setAdminUser();
        $content = $this->get_custom_report_content($report->get('id'));

        $this->assertCount(2, $content);
        $contentrow = array_values(array_pop($content));
        // Check program_user:programprogresswithoverview is not empty.
        $this->assertNotEmpty(array_pop($contentrow));
        ...
        // Validate as audience permitted user.
        self::setUser($user2);
        $content = $this->get_custom_report_content($report->get('id'));

        $this->assertCount(2, $content);
        // Report viewing user should show progress.
        $contentrow = array_values(array_pop($content));
        // Check program_user:programprogresswithoverview is not empty.
        $this->assertNotEmpty(array_pop($contentrow));
        // Check remaning columns.
        ...
        // Other user should not show progress.
        $contentrow = array_values(array_pop($content));
        // Check program_user:programprogresswithoverview is empty.
        $this->assertEmpty(array_pop($contentrow));
        // Check remaning columns.
        $this->assertEquals([
            'My program',
            'Lionel Kenobi',
