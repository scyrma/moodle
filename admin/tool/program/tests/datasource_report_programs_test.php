...
class tool_program_datasource_report_programs_testcase extends advanced_testcase {
    ...
    protected function set_up_for_report(): array {
        ...
        $this->get_generator()->generate_program($programparams);
        $users[] = $user1;
        ...
        for ($i = 0; $i < 5; $i++) {
            $this->get_generator()->generate_program($programparams)->get('id');
        }
    ...
    public function test_shared_programs() {
        ...
        $program = $this->get_generator()->generate_program((object)['tenantid' => $sharedspaceid, 'fullname' => 'Sharedprogram']);
        $program2 = $this->get_generator()->generate_program((object)['tenantid' => $sharedspaceid, 'fullname' => 'NOTvisible!']);
        $DB->update_record('tool_program', ['shared' => 0, 'id' => $program2->get('id')]);
        $this->get_generator()->allocate_users_to_program($program->get('id'),
