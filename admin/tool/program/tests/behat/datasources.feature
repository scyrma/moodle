...
Feature: Check datasources for programs
  ...
  Background:
    ...
    And the following "permission overrides" exist:
      ...
      | tool/reportbuilder:edit | Allow      | programmanager | System       |           |
  ...
  Scenario: Program manager can create a user allocation and a programs report
    Given the following "tool_reportbuilder > reports" exist:
      | name    | tenant  | source                                                                            |
      | Report1 | Tenant1 | tool_program\tool_reportbuilder\datasources\report_programs_allocation_completion |
  ...
  Scenario: System manager can create a user allocation and a programs report
    Given the following "tool_reportbuilder > reports" exist:
      | name    | tenant  | source                                                                            |
      | Report3 | Tenant1 | tool_program\tool_reportbuilder\datasources\report_programs_allocation_completion |
      | Report4 | Tenant1 | tool_program\tool_reportbuilder\datasources\report_programs                       |
  ...
  Scenario: Filter programs custom report by tag
    Given the following "tool_reportbuilder > reports" exist:
      | name    | tenant  | source                                                      |
      | Report1 | Tenant1 | tool_program\tool_reportbuilder\datasources\report_programs |
    When I log in as "manager1"
    And I navigate to "Report builder" in workplace launcher
    ...
    Then I should see "Program1" in the "report-table" "table"
