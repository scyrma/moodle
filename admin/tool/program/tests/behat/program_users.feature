@tool @tool_program @moodleworkplace @theme_workplace @javascript
Feature: Program users belong to same program tenant and have not been deleted
  In order to test that users belong to same program tenant and are active
  As a manager
  I need to create a program and allocate users

  Background:
    Given "2" tenants exist with "1" users and "0" courses in each
    Given the following "users" exist:
      | username | firstname | lastname | email                |
      | manager1 | Manager   | 1        | manager1@example.com |
      | user1    | User      | 1        | user1@example.com    |
      | user2    | User      | 2        | user2@example.com    |
      | user3    | User      | 3        | user3@example.com    |
    And the following users allocations to tenants exist:
      | user     | tenant  |
      | manager1 | Tenant1 |
      | user1    | Tenant1 |
      | user2    | Tenant2 |
      | user3    | Tenant1 |
    And the following "role assigns" exist:
      | user     | role                     | contextlevel | reference |
      | manager1 | tool_program_manager     | System       |           |
      | manager1 | tool_dynamicrule_manager | System       |           |
    Given the following "tool_program > programs" exist:
      | fullname | tenant  |
      | Program1 | Tenant1 |
    And the following "tool_program > program_users" exist:
      | program  | user   |
      | Program1 | user1  |
      | Program1 | user3  |

  Scenario: Create a program with basic information, allocate users and move one user to a different tenant
    When I log in as "manager1"
    Then I navigate to "Programs" in workplace launcher
    Then I press "Users" action in the "Program1" report row
    And I should see "Users"
    And I should see "First name"
    And I should see "User 1"
    And I should see "User 3"
    And I should not see "User 2"
    Then I log out
    Then I log in as "admin"
    And I navigate to "Users > Organisation > Manage tenants" in site administration
    And I click on "Tenant1" "link" in the "Tenant1" "tool_wp > Table tree node"
    And I set the field "Select user 'User 3'" to "1"
    And I set the field "With selected users..." to "Tenant2"
    And I press "Allocate users"
    Then I log out
    Then I log in as "manager1"
    Then I navigate to "Programs" in workplace launcher
    And I should see "Programs"
    Then I press "Users" action in the "Program1" report row
    And I should see "Users"
    And I should see "First name"
    And I should see "User 1"
    And I should not see "User 3"
    And I should not see "User 2"
    Then I log out

  Scenario: Create a program with basic information, allocate users and delete a user
    When I log in as "manager1"
    Then I navigate to "Programs" in workplace launcher
    Then I press "Users" action in the "Program1" report row
    And I should see "Users"
    And I should see "First name"
    And I should see "User 1"
    And I should see "User 3"
    And I should not see "User 2"
    Then I log out
    Then I log in as "admin"
    And I navigate to "Users > Accounts > Bulk user actions" in site administration
    And the "Available" select box should contain "User 3"
    And I set the field "Available" to "User 3"
    And I press "Add to selection"
    And I set the field "id_action" to "Delete"
    And I press "Go"
    And I press "Yes"
    And I should see "Changes saved"
    And I press "Continue"
    Then I log out
    Then I log in as "manager1"
    Then I navigate to "Programs" in workplace launcher
    Then I press "Users" action in the "Program1" report row
    And I should see "Users"
    And I should see "First name"
    And I should see "User 1"
    And I should not see "User 3"
    And I should not see "User 2"
    Then I log out

  Scenario: View user progress overview modal from user allocations
    When I log in as "manager1"
    And I navigate to "Programs" in workplace launcher
    And I press "Users" action in the "Program1" report row
    And I press "Progress overview" action in the "User 1" report row
    Then I should see "Program1" in the "Progress overview for User 1" "dialogue"
    And I should see "Complete all in order" in the "Progress overview for User 1" "dialogue"
    And I click on "Close" "button" in the "Progress overview for User 1" "dialogue"
    And I log out
