...
Feature: Export and import programs
  ...
  Scenario: Export one program
    ...
    And I press "View export" action in the "Programs" report row
  ...
  Scenario: Import a program into current tenant
    ...
    And I press "View import" action in the "Programs" report row
  ...
  Scenario: Importing export created when program complete DR conditions were single-value selects
    ...
    Then I should see "100% completed" in the "ProgramDR" "tool_catalogue > Catalogue item"
    And I log out
    When I log in as "admin"
    And I switch to tenant "TenantDR"
    Then I navigate to "Dynamic rules" in workplace launcher
    And I press "View report" action in the "DR PROGRAM" report row
