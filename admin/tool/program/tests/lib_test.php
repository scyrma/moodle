...
class lib_test extends advanced_testcase {
    ...
    public function test_section_block_myteams(): void {
        ...
        $this->assertCount(1, $userinprogram1);
        ...
        $this->assertEquals('Program active', $userinprogram1[0]->get_title());
        $this->assertEquals('0% completed', $userinprogram1[0]->get_subtitle());
        $this->assertEmpty($userinprogram1[0]->get_badges());

        $allocationoverdue = array_filter($userinprogram23, static function(object $program) {
            return $program->get_title() === 'Program overdue';
        });
        $allocationoverdue = reset($allocationoverdue);
        $this->assertEquals('0% completed', $allocationoverdue->get_subtitle());
        $this->assertEquals('Overdue', $allocationoverdue->get_badges()[0]['label']);
        $this->assertEquals('danger', $allocationoverdue->get_badges()[0]['type']);

        $allocationcompleted = array_filter($userinprogram23, static function(object $program) {
            return $program->get_title() === 'Program completed';
        });
        $allocationcompleted = reset($allocationcompleted);
        $this->assertEquals('100% completed', $allocationcompleted->get_subtitle());
        $this->assertEmpty($allocationcompleted->get_badges());
    }
}
