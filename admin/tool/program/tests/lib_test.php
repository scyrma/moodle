...
class tool_program_lib_testcase extends advanced_testcase {
    ...
    /**
     * Test user lists fragment generation.
     * @covers ::tool_program_output_fragment_programs_manager_users_list
     */
    public function test_tool_program_output_fragment_programs_manager_users_list(): void {
        global $PAGE, $CFG;
        $PAGE->set_url($CFG->dirroot . '/admin/tool/program/index.php');
        $context = context_system::instance();
        ...
        $this->assertEquals($data->defaulttenantid, tenancy::get_tenant_id());

        $program = $this->generator->generate_program_with_base_set((object) ['tenantid' => $data->defaulttenantid]);
        $this->generator->allocate_user_to_program($program->get('id'), $data->user->id);

        $params = ['context' => $context, 'id' => $program->get('id')];
        $res = tool_program_output_fragment_programs_manager_users_list($params);
    ...
    public function test_tool_program_inplace_editable(): void {
        ...
        try {
            ...
            $this->assertContains('Unexpected tool_program inplace editable item type', $e->getMessage());
        }
