...
class behat_tool_program_generator extends behat_generator_base {
    ...
    protected function get_set_id(string $name): int {
        global $DB;
        if (empty($name)) {
            return 0; // Generator will find the base set for the specified program.
        }
        // Locate the set by name. Note that if there are multiple programs, the name should be unique.
