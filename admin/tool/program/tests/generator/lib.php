...
class tool_program_generator extends testing_module_generator {
     ...
     *
     * @deprecated Please use generate_program instead
     *
     * @param stdClass $programdata
     * @return program
     */
    public function generate_program_with_base_set(stdClass $programdata = null): program {
        debugging('This function is deprecated. ' .
            'Please use generate_program() instead', DEBUG_DEVELOPER);
        return $this->generate_program($programdata);
    }
    ...
    public function generate_base_set(int $programid): program_set {
        debugging('Do not use, base set is generated automatically', DEBUG_DEVELOPER);
    ...
    public function enable_program_enrol_instance(program_course $programcourse): void {
        debugging('Do not use, enrol instance is enabled automatically', DEBUG_DEVELOPER);
     ...
     * @param int $userid
     * @param int $completedate
     * @return program_set_completion
     */
    public function complete_set(int $setid, int $userid, int $completedate = null): program_set_completion {
        debugging('Do not use, use complete_course() instead', DEBUG_DEVELOPER);

        $setcompletion = new program_set_completion(0, (object) [
            'setid' => $setid,
            'userid' => $userid,
            'completeddate' => $completedate ?? time(),
        ]);
        return $setcompletion->create();
    }
    ...
    public function allocate_user_to_program(int $programid, int $userid, int $certificationid = 0): program_user {
        $programuserdata = $this->get_dummy_program_user_data(
            ['userid' => $userid, 'certificationid' => $certificationid, 'programid' => $programid]);
    ...
    public function create_tenant_and_user() {
        debugging('Do not use', DEBUG_DEVELOPER);
