...
class tool_program_generator extends testing_module_generator {
    ...
    /**
     * Create a program set
     *
     * Used in Behat step 'Given the following "tool_program > program_sets" exist:'.
     *
     * @param array $record
     * @return program_set
     */
    public function create_program_set(array $record): program_set {
        return $this->generate_set((object) $record);
    }
    ...
    public function create_program_course(array $record): program_course {
        if (!empty($record['setid'])) {
            return $this->add_course_to_set($record['courseid'], $record['setid'], $record['sortorder'] ?? 1);
        }
