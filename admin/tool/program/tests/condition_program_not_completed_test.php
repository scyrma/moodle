...
/**
 * File contains the unit tests for condition program_not_completed class.
 *
...
use tool_program\tool_dynamicrule\condition\program_not_completed;
use tool_dynamicrule\rule;

defined('MOODLE_INTERNAL') || die();

/**
 * Unit tests for condition program_not_completed class.
 *
 * @covers     \tool_program\tool_dynamicrule\condition\program_not_completed
 * @package    tool_program
 * @group      tool_program
 * @copyright  2019 Moodle Pty Ltd <support@moodle.com>
 * @author     2019 Mitxel Moriana <mitxel@tresipunt.com>
 * @license    Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class tool_program_condition_program_not_completed_testcase extends advanced_testcase {
    ...
    public function test_supports_rule_types(): void {
        $condition = program_not_completed::instance();
    ...
    /**
     * Test shared program not completed condition matching in shared rule
     */
    public function test_get_matching_users_shared_program_shared_rule(): void {
        ...
        $users = api::get_matching_users($rule->id);
        $this->assertEqualsCanonicalizing([$user2->id, $user3->id, $user22->id, $user23->id], array_column($users, 'id'));
    }
    ...
    public function test_get_description(): void {
        ...
        $configdata = ['programid' => $program1->get('id')];
        /** @var program_not_completed $condition1 */
        $condition1 = program_not_completed::create($rule1->id, $configdata);

        $expectedstr = get_string('conditionprogramnotcompleteddescription', 'tool_program', $program1->get('fullname'));
    ...
    public function test_is_configuration_valid(): void {
        ...
        // Empty configuration.
        $condition1 = program_not_completed::create($rule1->id, []);
    ...
    public function test_user_can_add(): void {
        ...
        $rule1 = $this->drgenerator->create_rule(['tenantid' => $tenant->id]);
        $configdata = ['programid' => $program1->get('id')];
        program_not_completed::create($rule1->id, $configdata);
        $this->assertFalse(program_not_completed::instance()->user_can_add());

        $this->generator->assign_allocateuser_capability($user->id, $program1->get_context());
        $this->assertTrue(program_not_completed::instance()->user_can_add());
    }
    ...
    public function test_user_can_edit(): void {
        ...
        $configdata = ['programid' => $program1->get('id')];
        program_not_completed::create($rule1->id, $configdata);
        $this->assertFalse(program_not_completed::instance()->user_can_edit($configdata));

        $this->generator->assign_allocateuser_capability($user->id, $program1->get_context());
        $this->assertFalse(program_not_completed::instance()->user_can_edit($configdata));

        $this->tenantgenerator->allocate_user($user->id, $tenant->id);
        $this->assertTrue(program_not_completed::instance()->user_can_edit($configdata));
    }
}
