...
class provider_test extends provider_testcase {
    ...
    public function test_export_user_data(): void {
        ...
        $programdata->program_tags = ['Cool', 'Bananas'];
        $program1 = api::create_program($programdata);
        ...
        $this->assertEquals(['Cool', 'Bananas'], $tags);
    ...
    public function test_export_user_preferences() {
        $user1 = self::getDataGenerator()->create_user();
        set_user_preference('tool_program_program_status_filter', 'all', $user1->id);
        set_user_preference('tool_program_program_view_filter', 'viewcards', $user1->id);
        set_user_preference('tool_program_program_sort_filter', 'lastaccess', $user1->id);

        provider::export_user_preferences($user1->id);

        $context = context_system::instance();
        $preferences = writer::with_context($context)->get_user_preferences('tool_program');

        $this->assertCount(3, (array) $preferences);
        $this->assertEquals('all', $preferences->tool_program_program_status_filter->value);
        $this->assertEquals('viewcards', $preferences->tool_program_program_view_filter->value);
        $this->assertEquals('lastaccess', $preferences->tool_program_program_sort_filter->value);
    }
}
