...
class user_allocated_test extends advanced_testcase {
    ...
    /**
     * Make sure allocation event triggers the rule.
     */
    public function test_user_allocated(): void {
        ...
        // Create a rule with condition: User is allocated to the program.
    ...
    /**
     * Make sure allocation update event triggers the rule.
     */
    public function test_user_allocation_updated(): void {
        [$tenant, [$user1, $user2]] = $this->tenantgenerator->create_tenant_and_users(2);
        ...
        // Allocate user1 to the program.
        $programuser = \tool_program\api::allocate_user($program, (object) [
            'programid' => $program->get('id'),
            'userid' => $user1->id,
            'certificationid' => 0,
            'status' => \tool_program\constants::STATUS_OVERRIDE_SUSPENDED,
        ]);

        // Sanity check, no one matched the rule.
        $this->drgenerator->assert_user_did_not_match_rule($this, $user1->id, $rule->id);
        $this->drgenerator->assert_user_did_not_match_rule($this, $user2->id, $rule->id);

        // Change status to non-suspended.
        ...
        $unlockeddatesparams->status = \tool_program\constants::STATUS_OVERRIDE_DEFAULT;
        \tool_program\api::update_program_user_dates_and_status($programuser, $unlockeddatesparams);

        // Test user1 is allocated and status is active.
        $this->drgenerator->assert_user_matched_rule($this, $user1->id, $rule->id);
        $this->drgenerator->assert_user_did_not_match_rule($this, $user2->id, $rule->id);
    }
}
