...
/**
 * Unit tests for condition program_suspended class.
 *
 ...
 */
class program_suspended_test extends advanced_testcase {
    ...
    public function test_supports_rule_types(): void {
        $condition = program_suspended::instance();
    ...
    public function test_get_matching_users_given_suspended_status(): void {
        ...
        $programusers = $this->generator->allocate_users_to_program($program->get('id'), [$user1->id, $user2->id, $user3->id]);
        $programuser1 = $programusers[$user1->id];

        // Set program allocation to Suspended for this user.
        $now = time();
        $programuser1->set('status', constants::STATUS_OVERRIDE_SUSPENDED);
        $programuser1->set('timesuspended', $now);
        $programuser1->update();

        // Test users that have program1 with status Suspended.
        $rule = $this->drgenerator->create_rule(['enabled' => 1, 'tenantid' => $tenant->id]);
        $configdata = ['programid' => $program->get('id')];
        program_suspended::create($rule->id, $configdata);
    ...
    /**
     * Test shared program suspended status matching in shared rule
     */
    public function test_get_matching_users_given_suspended_status_shared_rule(): void {
        ...
        $programuser21 = $this->generator->allocate_user_to_program($program1->get('id'), $user21->id);
        $this->generator->allocate_users_to_program($program1->get('id'), [$user2->id, $user3->id, $user22->id, $user23->id]);

        // Set program allocation to Suspended.
        $now = time();
        $programuser1->set('status', constants::STATUS_OVERRIDE_SUSPENDED);
        $programuser1->set('timesuspended', $now);
        $programuser1->update();
        $programuser21->set('status', constants::STATUS_OVERRIDE_SUSPENDED);
        $programuser21->set('timesuspended', $now);
    ...
    public function test_get_description(): void {
        ...
        $configdata = ['programid' => $program->get('id')];
        /** @var program_suspended $condition */
        $condition = program_suspended::create($rule->id, $configdata);

        $expectedstr = get_string('conditionprogramsuspendeddescription', 'tool_program', $program->get('fullname'));
    ...
    public function test_is_configuration_valid(): void {
        ...
        // Empty configuration.
        $condition1 = program_suspended::create($rule1->id, []);
        $this->assertFalse($condition1->is_configuration_valid());

        // Users in program1.
        $configdata = ['programid' => $program1->get('id')];
        $condition1 = program_suspended::create($rule1->id, $configdata);
    ...
    public function test_user_can_add(): void {
        ...
        $rule1 = $this->drgenerator->create_rule(['tenantid' => $tenant->id]);
        $configdata = ['programid' => $program1->get('id')];
        program_suspended::create($rule1->id, $configdata);
        $this->assertFalse(program_suspended::instance()->user_can_add());

        $this->generator->assign_allocateuser_capability($user->id, $program1->get_context());
        $this->assertTrue(program_suspended::instance()->user_can_add());
    }
    ...
    public function test_user_can_edit(): void {
        ...
        $configdata = ['programid' => $program1->get('id')];
        program_suspended::create($rule1->id, $configdata);
        $this->assertFalse(program_suspended::instance()->user_can_edit($configdata));

        $this->generator->assign_allocateuser_capability($user->id, $program1->get_context());
        $this->assertFalse(program_suspended::instance()->user_can_edit($configdata));

        $this->tenantgenerator->allocate_user($user->id, $tenant->id);
        $this->assertTrue(program_suspended::instance()->user_can_edit($configdata));
    }
}
