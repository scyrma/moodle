...
class deallocation_test extends advanced_testcase {
    ...
    public function test_apply_to_user_in_other_tenant(): void {
        ...
        $configdata = ['programid' => $program1->get('id'), 'action' => outcome_base::DEALLOCATE_USER];
    ...
    public function test_apply_to_user_in_other_tenant_suspend(): void {
        ...
        $configdata = ['programid' => $program1->get('id'), 'action' => outcome_base::SUSPEND_USER];
        $outcome = deallocation::create($rule0->id, $configdata);
        ...
        $programusers = $DB->get_records('tool_program_users', [], 'id', 'userid, status, allocationtype');

        // Suspend allocation should have only acted upon the user(s) with a dynamic allocation type.
        ...
        $pu3 = array_filter($programusers, static function($programuser) use ($user3) {
            return $programuser->userid == $user3->id;
        });
        $expected = [
            'userid' => $user3->id,
        ...
        $this->assertEquals($expected, (array) reset($pu3));
    }
    ...
    public function test_apply_to_user_in_shared_tenant_suspend(): void {
        ...
        $pu21 = array_filter($programusers, static function($programuser) use ($user21) {
            return $programuser->userid == $user21->id;
        });
        $expected = [
            'userid' => $user21->id,
            'allocationtype' => constants::ALLOCATION_DYNAMIC,
        ...
        $this->assertEquals($expected, (array) reset($pu21));

        $pu22 = array_filter($programusers, static function($programuser) use ($user22) {
            return $programuser->userid == $user22->id;
        });
        $expected = [
            'userid' => $user22->id,
            'allocationtype' => constants::ALLOCATION_MANUAL,
        ...
        $this->assertEquals($expected, (array) reset($pu22));
    }
    ...
    public function test_get_description(): void {
        ...
        $outcomestr = get_string('outcomedeallocationdescriptionsuspend', 'tool_program', $program1->get('fullname'));
    ...
    public function test_apply_to_user_deallocate_several_times(): void {
        ...
        $configdata = ['programid' => $program1->get('id'), 'action' => outcome_base::DONT_MODIFY_ALLOCATION];
    ...
    public function test_apply_to_user_suspend_several_times(): void {
        ...
        $configdata = ['cohortid' => $cohort1->id];
        cohort_member::create($rule1->id, $configdata);
        $configdata = ['programid' => $program1->get('id'), 'action' => outcome_base::UNSUSPEND_AND_KEEP_DATES];
        allocation::create($rule1->id, $configdata);
