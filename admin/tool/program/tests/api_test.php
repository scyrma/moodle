...
use tool_program\event\user_allocation_updated;
...
class api_test extends advanced_testcase {
    ...
    public function test_create_shared_program(): void {
        ...
        $this->assertFalse($program1->get('shared'));
        ...
        $this->assertTrue($program2->get('shared'));
        ...
        $this->assertFalse($newprogram->get('shared'));
    }
    ...
    public function test_deallocate_user_triggers_event(): void {
        ...
        // First event in the array is user_allocation_deleted.
        $event = reset($events);
    ...
    public function test_get_user_allocations(): void {
        ...
        $this->assertEquals($programuser->get('id'), $userallocation->get('id'));
    }

    /**
     * Test get_program_customfields api method.
     * NOTE: this unittest cannot be modified without making sure that block_mylearning continues working.
     */
    public function test_get_program_customfields(): void {
        ...
        $field1 = $cfgenerator->create_field(['categoryid' => $category->get('id'),
            'type' => 'text', 'shortname' => 'fld1', ]);

        $programparams['customfield_fld1'] = 'Hello1';

        $program = $this->generator->generate_program((object)$programparams);
        $programid = $program->get('id');
        $customfields = api::get_program_customfields($programid);
        $customfield = reset($customfields);

        $this->assertCount(1, $customfields);
        $this->assertEquals($field1->get('shortname'), $customfield->get_shortname());
    }
    ...
    /**
     * Test trigger event for update program user dates and status.
     */
    public function test_update_program_user_dates_and_status_triggers_event(): void {
        // Test status suspended and absolute program dates.
        $program = $this->generator->generate_program((object) [
            'startdatetype' => constants::DATE_ABSOLUTE,
            'startdateabsolute' => strtotime('-7 day'),
        ...
        $course = self::getDataGenerator()->create_course(['enablecompletion' => true]);
        $programcourse = $this->generator->add_course_to_set($course->id,
            $program->get_base_set()->get('id'));
        $user = self::getDataGenerator()->create_user();
        $programuser = $this->generator->allocate_user_to_program($program->get('id'), $user->id);
        $this->generator->enrol_user_to_program_course($programcourse, $programuser);
        ...
        // First two events in the array are user_enrolment_updated, we need the third one.
        $event = $events[2];
        ...
        $this->assertEquals($programuser->get('id'), $event->objectid);
        $this->assertEquals($program->get('id'), $event->other['programid']);
        $this->assertEquals($unlockeddatesparams->status, $event->other['status']);

        // Test event get_name().
        $eventname = get_string('eventuserupdated', 'tool_program');
        $this->assertEquals($eventname, $event::get_name());

        // Test event get_description().
        $eventdescription = "The allocation for user id '$user->id' " .
            "on the program with id '" . $program->get('id') . "' got updated.";
        $this->assertEquals($eventdescription, $event->get_description());
    ...
    public function test_get_user_allocation_statuses(): void {
        ...
        $this->assertEquals(constants::STATUS_COMPLETED, $statuses[0]['status']);
        $this->assertEquals('completed', $statuses[0]['stringid']);
        ...
        $this->assertEquals(constants::STATUS_SUSPENDED, $statuses[0]['status']);
        $this->assertEquals('suspended', $statuses[0]['stringid']);
        $this->assertEquals(constants::STATUS_COMPLETED, $statuses[1]['status']);
        $this->assertEquals('completed', $statuses[1]['stringid']);
        ...
        $this->assertCount(1, $statuses);
        $this->assertEquals(constants::STATUS_FUTUREALLOCATION, $statuses[0]['status']);
        $this->assertEquals('futureallocation', $statuses[0]['stringid']);
        ...
        $this->assertCount(1, $statuses);
        $this->assertEquals(constants::STATUS_OVERDUE, $statuses[0]['status']);
        $this->assertEquals('overdue', $statuses[0]['stringid']);
        ...
        $this->assertCount(1, $statuses);
        $this->assertEquals(constants::STATUS_OPEN, $statuses[0]['status']);
        $this->assertEquals('open', $statuses[0]['stringid']);
    ...
    /**
     * Test get program image pattern url, not base64 image.
     * NOTE: this unittest cannot be modified without making sure that block_mylearning continues working.
     */
    public function test_get_program_pattern_url(): void {
        $program = $this->generator->generate_program();
        $imageurl = api::get_program_pattern_url($program->get('id'));

        $this->assertStringStartsNotWith('data:image/svg+xml;base64,', $imageurl);
        $this->assertTrue(filter_var($imageurl, FILTER_VALIDATE_URL) !== false);
        $this->assertStringContainsString('/tool_program/program_generated_image/', $imageurl);
    }
    ...
    public function test_get_program_status_sql_query(): void {
        ...
        // Check for not specific program.
        [$join, $where, $params] = api::get_program_status_sql_query(null, $status, false, $u, $pu, $p, $ps, $psc, null);

        $this->assertStringNotContainsString($join, 'AND pu.programid =');
        $this->assertArrayNotHasKey($pid, $params);
        $this->assertNotContains($programid, $params);
    }
    ...
    public function test_get_programs_by_status_and_userid(): void {
        ...
        // Mark Program1 as hidden and check is not included in the results.
        $program1->set('visible', constants::VISIBILITY_HIDDEN);
        $program1->update();
    ...
    /**
     * Test get_linked_programs_to_a_user_course
     */
    public function test_get_linked_programs_to_a_user_course(): void {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $this->assertEmpty($persistents);

        $program1 = $this->generator->generate_program();
        $program2 = $this->generator->generate_program();
        $program3 = $this->generator->generate_program();
        $program4 = $this->generator->generate_program();
        $program5 = $this->generator->generate_program();
        $this->generator->add_course_to_set((int) $course->id, $program1->get_base_set()->get('id'));
        $this->generator->add_course_to_set((int) $course->id, $program2->get_base_set()->get('id'));
        $this->generator->allocate_user_to_program($program1->get('id'), (int) get_admin()->id);
        $this->generator->allocate_user_to_program($program2->get('id'), (int) get_admin()->id);
        $this->generator->allocate_user_to_program($program3->get('id'), (int) get_admin()->id);
        // Add one suspended allocation to Program4.
        $this->generator->allocate_user_to_program($program4->get('id'), (int) get_admin()->id, 0 ,
            ['status' => constants::STATUS_SUSPENDED]);
        // Add one suspended allocation to Program5 which is hidden.
        $this->generator->allocate_user_to_program($program5->get('id'), (int) get_admin()->id, 0,
            ['visible' => constants::VISIBILITY_HIDDEN]);

        // It should only return information regarding program1 and program2.
        $persistents = api::get_linked_programs_to_a_user_course((int) $course->id);

        $programids = array_map(static function($program) {
            return $program->get('id');
        }, $persistents);
        $this->assertEqualsCanonicalizing([$program1->get('id'), $program2->get('id')], $programids);
    }

    /**
     * Test is_course_in_user_programs
     */
    public function test_is_course_in_user_programs(): void {
        $this->resetAfterTest();
        $this->setAdminUser();

        $course = self::getDataGenerator()->create_course();

        $this->assertFalse(api::is_course_in_user_programs((int) $course->id));
        ...
        $this->assertTrue(api::is_course_in_user_programs((int) $course->id));
    }

    /**
     * Test for core hack {@see core_course_category::can_view_course_info}
     */
    public function test_can_view_course_info(): void {
        global $DB;
        $this->resetAfterTest();
        ...
        $course1 = self::getDataGenerator()->create_course(['category' => $cat1->id]);
        $course2 = self::getDataGenerator()->create_course(['category' => $cat1->id]);
        $course3 = self::getDataGenerator()->create_course(['category' => $cat1->id]);
        ...
        $this->assertFalse(\core_course_category::can_view_course_info($course1));
        $this->assertFalse(\core_course_category::can_view_course_info($course2));
        $this->assertFalse(\core_course_category::can_view_course_info($course3));

        // Check for user2.
        $this->assertFalse(\core_course_category::can_view_course_info($course1, $user2));
        $this->assertFalse(\core_course_category::can_view_course_info($course2, $user2));
        $this->assertFalse(\core_course_category::can_view_course_info($course3, $user2));

        // If courses belong to a program where the user is allocated, can_view_course_info method should allow the user
        // to view the course info.
        $program = $this->generator->generate_program();
        $this->generator->add_course_to_set((int) $course1->id, $program->get_base_set()->get('id'));
        $this->generator->add_course_to_set((int) $course2->id, $program->get_base_set()->get('id'));
        $this->generator->allocate_user_to_program($program->get('id'), (int)$user1->id);
        $this->generator->allocate_user_to_program($program->get('id'), (int)$user2->id);

        // Check for current user.
        $this->assertTrue(\core_course_category::can_view_course_info($course1));
        $this->assertTrue(\core_course_category::can_view_course_info($course2));
        // Course3 does not belong to the program.
        $this->assertFalse(\core_course_category::can_view_course_info($course3));

        // Check for user2.
        $this->assertTrue(\core_course_category::can_view_course_info($course1, $user2));
        $this->assertTrue(\core_course_category::can_view_course_info($course2, $user2));
        // Course3 does not belong to the program.
        $this->assertFalse(\core_course_category::can_view_course_info($course3, $user2));

        // Check for user3 which is not allocated to the program.
        $this->assertFalse(\core_course_category::can_view_course_info($course1, $user3));
        $this->assertFalse(\core_course_category::can_view_course_info($course2, $user3));
        $this->assertFalse(\core_course_category::can_view_course_info($course3, $user3));
    }

    /**
     * Test for calculate_relative_date
     */
    public function test_calculate_relative_date(): void {
        $start = strtotime('midnight');

        $this->assertEquals(strtotime('+1 day', $start), api::calculate_relative_date('+1 day', $start));
        $this->assertEquals(strtotime('-1 day', $start), api::calculate_relative_date('-1 day', $start));
        $this->assertEquals(strtotime('+2 day', $start), api::calculate_relative_date('2 day', $start));
    }
}
