...
class program_user extends base {
    ...
    protected function get_all_columns(): array {
        ...
        $columns[] = (new column(
            ...
            ->add_field($DB->sql_bitxor("{$tablealias}.status", 1), 'status')
        ...
        $columns[] = (new column(
            ...
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_TIMESTAMP)
            ->add_field("CASE WHEN {$tablealias}.status = ".constants::STATUS_OVERRIDE_SUSPENDED."
                THEN {$tablealias}.timesuspended ELSE 0 END", 'timesuspended')
