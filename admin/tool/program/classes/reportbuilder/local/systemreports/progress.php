...
class progress extends system_report {
     ...
     *
     * @param int $status if -1 then remove the filter
     */
    public function prefilter_by_status(int $status) {
        if ($status === -1) {
            $this->set_filter_values([
                'program_user:filterablestatus_operator' => select::ANY_VALUE,
            ]);
        } else {
            if (!array_key_exists($status, \tool_program\api::get_program_statuses_fieldset())) {
            ...
            $this->set_filter_values([
                ...
                'program_user:filterablestatus_value' => $status,
            ]);
        }
    }
}
