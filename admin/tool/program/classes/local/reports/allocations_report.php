...
class allocations_report extends system_report {
    ...
    private function add_actions(): void {
        ...
        // User allocation program re-completion icon.
        $url = new moodle_url('/admin/tool/program/recompletion.php', ['id' => ':id']);
        $icon = new pix_icon('a/refresh', get_string('recalculateprogramcompletion', 'tool_program'), 'core');
        $action = new report_action($url, $icon, [
            'class' => 'confirm_recalculate_completion',
            'data-action' => 'recalculate_completion',
            'data-userid' => ':userid',
            'data-id' => ':programid',
            'data-programuserid' => ':id',
        ]);
        // Check if program completion can be recalculated for this user.
        $action->add_callback(function($row) use ($program) {
            $programuser = new program_user(0, $row);
            $programuser->set_program($program);
    ...
    public function get_row_class(stdClass $row): string {
        ...
        return $issuspendedorafterenddate || ($row->suspended || !$row->confirmed) ? 'dimmed_text' : '';
    }
}
