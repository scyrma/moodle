...
class program_entity extends entity_base {
    ...
    public function convert_get_entity_class(): string {
        return \tool_program\reportbuilder\local\entities\program::class;
    }
    ...
    protected function convert_get_generic_field_name(string $oldfieldname): string {
        $oldprefix = $this->get_table_alias('tool_program') . 'id';
        $cfhandler = \core_customfield\handler::get_handler('tool_program', 'program');
        return $this->convert_get_custom_field_name($cfhandler, $oldfieldname, $oldprefix) ?? $oldfieldname;
    }
    ...
    protected function get_default_table_aliases(): array {
        return ['tool_program' => 'tp', 'tool_tenant' => 'tttp'];
    }
    ...
    protected function get_all_columns(): array {
        ...
        // Prepare links for concat.
        $str = '<span>{{fullname}}</span data-program="{{id}}">';
        $concatfields = [
            '{{id}}' => "{$tablealias}.id",
            '{{fullname}}' => "{$tablealias}.fullname"
        ];
        [$sql, $params] = \tool_reportbuilder\db::sql_string_with_placeholders($str, $concatfields);

        $programwithlinkfunction = [program_format::class, 'programwithlink_concat'];
        ...
        $newcolumn = (new report_column(
            ...
            ->add_aggregation_callback('groupconcat', $programwithlinkfunction, 'fullnamewithimage')
            ->add_aggregation_callback('groupconcatdistinct', $programwithlinkfunction, 'fullnamewithimage')
        ...
        $newcolumn = (new report_column(
            ...
            ->add_field($sql, 'fullname', $params)
            ->add_field("{$tablealias}.id")
            ->add_callback($programwithlinkfunction, 'fullnamewithlink')
            ->add_aggregation_callback('groupconcat', $programwithlinkfunction, 'fullnamewithlink')
            ->add_aggregation_callback('groupconcatdistinct', $programwithlinkfunction, 'fullnamewithlink')
            ->set_groupby_sql(implode(', ', $concatfields))
            ->set_is_sortable(true);
        $columns[] = $newcolumn;
        ...
        // Column tags.
        [$tagsql, $tagparams] = db::sql_tag_field($tablealias, 'tool_program');
        ...
        $newcolumn = (new report_column(
            ...
            ->set_is_sortable(true, false, null, null, [
                "startdatetype",
                "startdateabsolute",
                "startdaterelative"
            ])
        ...
        $tpc = database::generate_alias();
        $tps = database::generate_alias();
        ...
        $groupconcatsql = db::sql_group_concat($placeholdersql, null, "{$c}.fullname");
        $sql = "(SELECT $groupconcatsql
            FROM {tool_certification} $c
            LEFT JOIN {tool_program} $p
            ON $c.program = $p.id
