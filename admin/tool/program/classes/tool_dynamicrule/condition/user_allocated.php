...
use MoodleQuickForm;
use tool_certification\tool_dynamicrule\outcome\outcome_base;
...
use tool_dynamicrule\outcome_base as dynamicrule_outcome_base;
...
class user_allocated extends condition_base {
    ...
    public function get_sql(): array {
        ...
        $programid = database::generate_param_name();
        ...
        // Make sure the allocation is currently active.
        $now1 = database::generate_param_name();
        $now2 = database::generate_param_name();
        $where .= " AND {$pu}.startdate <= :{$now1}".
            " AND ({$pu}.enddate = 0 OR {$pu}.enddate >= :{$now2}) ".
            " AND {$pu}.status = ". constants::STATUS_OVERRIDE_DEFAULT;
     ...
     *
     * @param dynamicrule_outcome_base $calleroutcome
     * @return array
     */
    public function get_available_data_for_outcome(dynamicrule_outcome_base $calleroutcome): array {
        return [
     ...
     * @return array
     */
    public function get_custom_data_for_outcome(array $keys, \stdClass $user, dynamicrule_outcome_base $calleroutcome): array {
        ...
        foreach ($keys as $key) {
            switch ($key) {
                case 'programid':
                    $data[$key] = $this->get_programid();
                    break;
                case 'programname':
                    $data[$key] = $program->get_formatted_name();
                    break;
                case 'programduedate':
                    $data[$key] = $this->get_program_user_duedate($program, $user->id);
                    break;
                default:
                    break;
            }
    ...
    public function get_event_subscription() {
        return [user_allocation_created::class, user_allocation_updated::class, user_allocation_deleted::class];
    }
