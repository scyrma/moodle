...
class program_tree_progress extends program_tree {
    ...
    /** @var array $userenrolments */
    private $userenrolments;

    /** @var array $coursecompletions */
    private $coursecompletions;
    ...
    public function __construct(program $program, int $userid) {
        ...
        $this->userid = $userid;
        $this->userenrolments = api::get_all_program_user_course_enrolments($program->get('id'), $userid);
        $this->coursecompletions = $this->get_program_courses_completions();
        $this->calculate_sets_progress($this->get_baseset());
        $this->calculate_items_locked_state($this->get_baseset());
    }

    /**
     * Retrieves all program course timecompleted.
     *
     * @return array
     * @throws \dml_exception
     * @throws coding_exception
     */
    private function get_program_courses_completions(): array {
        global $DB;

        $sql = '
            SELECT c.id, c.enablecompletion, cc.timecompleted
            FROM {course} c
    ...
    private function calculate_course_progress(program_item $courseitem, program_item $parentset): void {
        $cicourse = $courseitem->get_course();
        $courseitem->completionenabled = $this->is_course_completion_enabled($cicourse->id);

        $courseprogress = progress::get_course_progress_percentage($cicourse, $this->userid);
        if (!is_null($courseprogress)) {
            $courseitem->hasprogress = true;
        }
        if ($this->is_course_completed($cicourse->id)) {
            $courseitem->iscompleted = true;
            $courseitem->completion = 1;
            $courseitem->progresspercentage = 100;
        } else {
            $courseitem->iscompleted = false;
            $courseitem->completion = 0;
            // Adjust progress to maximum 95% if course is not completed.
            $courseitem->progresspercentage = (int)min(floor($courseprogress ?? 0), 95);
        }

        // We need completeditems to show if course is in progress.
        $courseitem->completeditems = ($courseitem->progresspercentage > 0) ? 1 : 0;
        $courseprogramenrolments = $this->get_course_program_enrolments($cicourse->id);
        $courseitem->isenrolled = !empty($courseprogramenrolments);
        // Add program enrolment if missing.
        if ($courseitem->isenrolled && !$this->has_program_enrolment($cicourse->id)) {
            // While this is useful to have to restore groups if one was accidentially deleted,
            // it may hide some issues in program API when gruups allocation or enrolment has not
            // been done correctly.
            // If you are fixing something in program API, it might be a good idea to comment the line below,
            // or even replace it with error throwing.
            api::enrol_in_program_course($this->program->get('id'), $cicourse, $this->userid);
        }
        $courseitem->weight = 1;
        ...
        $parentset->completeditems += $courseitem->iscompleted ? 1 : 0;
        $parentset->childrencompletionweightpairs[] = [
            'completion' => $courseitem->completion,
            'weight' => $courseitem->weight,
        ];

        // We store some info about all program's courses to improve performance in dashboad.
        $this->courses[$cicourse->id] = [
            'courseid' => $cicourse->id,
            'course' => $cicourse,
            'completionenabled' => $courseitem->completionenabled,
            'progress' => $courseitem->progresspercentage,
    ...
    private function save_set_as_completed(program_item $set): void {
        ...
        if ($set->is_base_set()) {
            $this->mark_program_as_completed($set->get_programid(), $setcompletion);
        }
    }
     ...
     *
     * @param int $programid
     * @param program_set_completion $setcompletion
     * @return void
     */
    private function mark_program_as_completed(int $programid, program_set_completion $setcompletion): void {
        // Delete Due date calendar event for this user and program.
        $data = (object) [
            'userid' => $setcompletion->get('userid'),
            'programid' => $programid,
        ];
        api::delete_calendar_events($data, constants::CALENDAR_EVENT_DUE_DATE);

        // Trigger program completed event.
        program_completed::create_from_program_completed($setcompletion, $programid)->trigger();

        // Send notification to user.
        api::send_program_completed_notification($setcompletion->get('userid'), $programid);
    }
    ...
    /**
     * Checks if course is marked as completed for this user.
     *
     * @param int $courseid
     * @return bool
     */
    private function is_course_completed(int $courseid): bool {
        return (bool)$this->coursecompletions[$courseid]->timecompleted;
    }

    /**
     * Checks if completion is enabled for a given course.
     *
     * @param int $courseid
     * @return bool
     */
    private function is_course_completion_enabled(int $courseid): bool {
        global $CFG;
        ...
        if ($this->coursecompletions[$courseid]->enablecompletion == COMPLETION_DISABLED) {
            return COMPLETION_DISABLED;
        }
     ...
     * @return array
     */
    private function get_course_program_enrolments(int $courseid): array {
        return array_filter($this->userenrolments, function($ue) use ($courseid) {
            return $ue->courseid == $courseid;
        });
    }

    /**
     * Checks if user has active current program enrolment in a course.
     *
     * @param int $courseid
     * @return bool
     */
    private function has_program_enrolment(int $courseid): bool {
        return !empty(array_filter($this->userenrolments, function($ue) use ($courseid) {
            return  $ue->courseid == $courseid && $ue->enrol == 'program' && $ue->customint1 == $this->program->get('id');
        }));
    }
    ...
    /**
     * Get user enrolments of this program tree program.
     *
     * @return array
     */
    ...
    /**
     * Get this program's courses basic information: course object, isenrolled and progress.
     *
