...
class edit_program_add_courses_form extends dynamic_form {
    ...
    protected function definition(): void {
        ...
        $mform->addElement('advcheckbox', 'recalculatecompletion', '', get_string('recalculateprogramcompletion', 'tool_program'));
        $mform->addHelpButton('recalculatecompletion', 'recalculateprogramcompletion', 'tool_program');
    }
    ...
    public function process_dynamic_submission() {
        ...
        // Check if completion needs to be re-calculated for all users allocated into this program.
        if (!empty($data->recalculatecompletion)) {
            $programset = program_set::get_record(['id' => $data->parentsetid]);
            $programusers = program_user::get_records(['programid' => $programset->get('programid')]);
