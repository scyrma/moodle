...
class permission {
     ...
     *
     * @param program $program
     * @param bool $checkallocationwindow Check if allocation window is open for this program
     * @param bool $checkallowdirectallocation Check if direct allocations are allowed for this program
     * @return bool
     */
    public static function can_allocate_anybody(program $program, bool $checkallocationwindow = true,
                                                bool $checkallowdirectallocation = true): bool {
        if (!$program->get('id')) {
            return false;
        }
        ...
        if ($checkallocationwindow && !api::is_allocation_window_open($program)) {
            return false;
        }

        if ($checkallowdirectallocation && !$program->get('allowdirectallocation')) {
            return false;
        }
    ...
    /**
     * Check if current user can edit a given program, if the program belongs to a different tenant than current tenant
    ...
    /**
     * Checks if user can edit the current program, if the program is from another tenant checks if user can switch
     * to that another tenant.
     *
     * @param program $program
     * @return bool
     */
    public static function can_edit_details_in_program_tenant(program $program): bool {
        ...
        if (!self::is_program_visible_in_list($program) ||
                !\tool_tenant\permission::can_access_tenant($program->get('tenantid'))) {
            return false;
        }
    ...
    public static function can_view_details(program $program): bool {
        return self::can_edit_details_in_program_tenant($program)
     ...
     *
     * User can view program progress if they can edit program or allocate any users OR
     * they are an org manager with a permission to view reports.
     *
     * @param program $program
     * @return bool
     */
     ...
     * @param bool $checkallocationwindow Check if allocation window is open for this program
     * @param bool $checkallowdirectallocation Check if direct allocations are allowed for this program
     * @param bool $checkprogramisvisible Checks if program is 'visible' to the current user based on the program tenant
     * @return bool
     */
    public static function can_allocate_user(program $program, int $userid, bool $checkallocationwindow = true,
                                             bool $checkallowdirectallocation = true, bool $checkprogramisvisible = true): bool {
        if (!$program->get('id')) {
            return false;
        }
        ...
        if ($checkprogramisvisible && !self::is_program_visible_in_list($program)) {
            return false;
        }
        ...
        if (!hierarchy::is_own_or_parent_shared_entity($program->get('tenantid'), $program->get('shared'), $usertenantid)) {
            // Program has to be "visible" to the user who is being allocated, which means the program belongs to the
            // same tenant or it is a shared program that belongs to the parent tenant.
            return false;
        }

        if (tenancy::is_user_hidden_by_tenancy($userid)) {
            // User must be "visible" to the current user.
            return false;
        }
    ...
    public static function can_view_allocated_users(program $program): bool {
        return self::can_edit_details_in_program_tenant($program) || self::can_allocate_anybody($program, false, false);
    }
     ...
     *
     * @param program $program
     * @param bool $skipcheckcertification If we can skip the check on 'belongs_to_non_archived_certification()'
     * @param bool $belongstocertification If $skipcheckcertification is true function will check this value instead.
     * @return bool
     */
    public static function can_archive(program $program, bool $skipcheckcertification = false,
                                       bool $belongstocertification = false): bool {
        if (!$program->get('id')) {
            return false;
        }
        ...
        if ($skipcheckcertification && $belongstocertification) {
             return false;
        }

        if (!$skipcheckcertification && api::belongs_to_non_archived_certification($program)) {
            return false;
        }
    ...
    public static function can_edit_user_allocation(program_user $programuser): bool {
        ...
        if (tenancy::is_user_hidden_by_tenancy($programuser->get('userid'))) {
            return false;
        }

        if (!self::is_program_visible_in_list($programuser->get_program())) {
            return false;
        }
        return true;
    }
    ...
    /**
     * Check if the current user can self enrol to a given course within a program.
     *
     * @param int $courseid
     * @param program $program
     * @return bool
     */
    public static function can_self_enrol_to_course(int $courseid, program $program): bool {
        global $USER;

        $userid = (int) $USER->id;
        $programid = (int) $program->get('id');
        // Check if user is allocated into the program.
        if (!$programuser = program_user::get_records(['userid' => $userid, 'programid' => $programid])) {
            return false;
        }
    ...
    public static function require_can_self_enrol_to_course(int $courseid, program $program): void {
        if (!self::can_self_enrol_to_course($courseid, $program)) {
     ...
     *
     * @param program_user|null $programuser
     */
    ...
    public static function check_access(): bool {
        ...
        || self::can_view_programs_progress_report();
    }

    /**
     * Check whether user has access to a given program (based on whether they can view its details or reports)
     *
     * @param program $program
     * @return bool
     */
    public static function check_access_to_program(program $program): bool {
        return self::can_view_details($program)
            || self::can_view_users_progress($program);
    }
     ...
     */
    public static function require_check_access_to_program(program $program): void {
        if (!self::check_access_to_program($program)) {
            throw new moodle_exception('errornopermissionviewprograms', 'tool_program');
        }
    }
     ...
     *
     * @param int $userid
     * @param program|null $program if specified will check if the progress for the given program can be viewed
     * @return bool
     */
    ...
    public static function can_view_user_programs_progress_as_organisation_manager(int $userid): bool {
        ...
        if (class_exists('\\tool_organisation\\organisation')) {
            if ($userorg = \tool_organisation\organisation::get_user_with_jobs()) {
                return $userorg->is_manager_over_user($userid, \tool_organisation\organisation::PERM_VIEW_REPORTS);
            }
        }
        return false;
    }
    ...
    public static function can_view_programs_overdue() {
        // TODO WP-4171 deprecate in 4.2 because it is not used.
        $orgpermission = false;
        // Check if can view reports if is subordinate.
        if (class_exists('\\tool_organisation\\organisation')) {
            if ($userorg = \tool_organisation\organisation::get_user_with_jobs()) {
    ...
    /**
     * Checks if current user can view any reports as organisation manager
     *
     * @return bool
     */
    protected static function can_view_any_report_as_organisation_manager(): bool {
        if (class_exists('\\tool_organisation\\organisation')) {
            $user = organisation::get_user_with_jobs();
            return $user && $user->is_manager(organisation::PERM_VIEW_REPORTS);
        }
     ...
     *
     * @param program_user $programuser
     * @param bool $deepcheck additionally check capabilities in the courses - should not be called in a loop
     *     for multiple users and/or programs!
     * @return bool
     */
    public static function can_reset_progress(program_user $programuser, $deepcheck = false): bool {
        ...
        if ($deepcheck && !course_reset_api::can_reset_courses($programuser->get_program()->get_courses_ids())) {
            return false;
        }

        if (!$programuser->is_manual_allocation()) {
            return false;
        }
     ...
     *     for multiple users and/or programs!
     */
    public static function require_can_reset_progress(program_user $programuser, bool $deepcheck = false): void {
        if (!self::can_reset_progress($programuser, $deepcheck)) {
     ...
     *
     * User should be able to view all allocated users in at least one program.
     * Since programs (currently) are only possible in system context, we can
     ...
     *
     * User should be able to view allocated users in the given program.
     *
     * @param program $program
     * @param rule $rule
     * @return bool
     */
    public static function can_edit_dynamicrule_condition(program $program, rule $rule): bool {
        // We can not call can_view_allocated_users() here because we need to check capability
        // to view ALL allocated users.
        return (self::has_edit_capability($program->get_context())
            || self::has_allocateuser_capability($program->get_context())) &&
            hierarchy::is_own_or_parent_shared_entity(
                $program->get('tenantid'), $program->get('shared'), $rule->get('tenantid'));
    }

    /**
     * Checks if user can add dynamic rule outcome
     *
     * User should be able to allocate users to programs.
     * Since programs (currently) are only possible in system context, we can
     * only check capabilities in system context.
     *
     ...
     *
     * User should be able to allocate users to given program.
     *
     * @param program $program
     * @param rule $rule
     * @return bool
     */
    public static function can_edit_dynamicrule_outcome(program $program, rule $rule): bool {
        // We check using has_allocateuser_capability() here rather than can_allocate_anybody()
        // to prevent case when organisation manager may allocate someone who does not
        // belong to her team.
        return self::has_allocateuser_capability($program->get_context()) &&
            hierarchy::is_own_or_parent_shared_entity($program->get('tenantid'), $program->get('shared'), $rule->get('tenantid'));
    }

    /**
     * Check if current user can recalculate the program completion for the given program user.
     *
     * @param program_user $programuser
     * @return bool
     */
    public static function can_recalculate_user_completion(program_user $programuser): bool {
        // Check if user has allocateuser capability OR allocate permission from Organisation.
        $canallocate = self::can_allocate_anybody_as_organisation_manager();
        if (!$canallocate && !self::has_allocateuser_capability($programuser->get_program()->get_context())) {
            return false;
    ...
    /**
     * Checks if the current user can view "Programs progress" report across all programs
     *
     * That would be either somebody who can allocate to any program or an org manager
     * who can view reports.
     */
    public static function can_view_programs_progress_report(): bool {
        return self::has_allocateuser_capability() ||
            self::can_view_any_report_as_organisation_manager();
    }

    /**
     * Current user can view "Programs progress" report across all programs
     */
    public static function require_can_view_programs_progress_report(): void {
        if (!self::can_view_programs_progress_report()) {
            throw new moodle_exception('errornopermissionviewreports', 'tool_program');
        }
    }
}
