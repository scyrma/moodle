...
class programs extends importer_base {
    ...
    public function add_to_options_form(import_settings_form $form): void {
        ...
        $coursebackupsstr = get_string('import_course_backups', 'tool_program');
        $mform->addElement('advcheckbox', self::IMPORT_COURSE_BACKUPS, $coursebackupsstr);
        $mform->setDefault(self::IMPORT_COURSE_BACKUPS, 1);
        $mform->addHelpButton(self::IMPORT_COURSE_BACKUPS, 'import_course_backups', 'tool_program');
        ...
        $group[] = $coursecategory = $mform->createElement('select', self::IMPORT_COURSE_CATEGORY, '', $categories);
        ...
        if (!$this->get_entities_in_workplace_export_file(program_user::TABLE)->count() ||
            ...
            $mform->hardFreeze(self::IMPORT_USER_ALLOCATIONS);
        }
        ...
        foreach ($programs as $program) {
            ...
            if ($this->get_import_setting(self::IMPORT_PROGRAM_DYNAMICRULES) &&
                ...
                $this->process_chained_entities('tool_dynamicrule', [], [
                    'component' => 'tool_program',
                    'area' => 'program',
                    'itemid' => $program->get_original_id(),
                    'mapper' => 'tool_program',
                ]);
            ...
            if ($this->get_import_setting(self::IMPORT_USER_ALLOCATIONS) && permission::has_allocateuser_capability()) {
                ...
                static function(array $entity) use ($program) {
                    return ((int)$entity['programid'] === $program->get_original_id()) && empty($entity['certificationid']);
                });

                // The new id can be "-1" if we are collecting errors.
                if ($program->get_new_id() && !$this->is_collecting_errors()) {
                    $programpersistent = new program($program->get_new_id());
                } else {
                ...
                foreach ($userallocations as $userallocation) {
                    $userallocation
                        ...
                        ->set_validation_callback(function ($data, wp_imported_entity $entity) use ($programpersistent) {
                            ...
                            if ($programpersistent) {
                                $this->add_details_to_log([
                                    'program' => $programpersistent->get('fullname'),
                                ]);
                            }
                        })
                        ->set_import_callback(function ($data, wp_imported_entity $entity) use ($programpersistent) {
                            ...
                            $userrecord['programid'] = $programpersistent->get('id');
    ...
    private function idnumber_taken(string $idnumber): bool {
        global $DB;
        return $DB->record_exists('tool_program', ['idnumber' => $idnumber, 'tenantid' => $this->get_import_tenant_id()]);
    }
