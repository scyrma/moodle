...
use \tool_dynamicrule\rules_list;
use tool_wp\output\tab;

defined('MOODLE_INTERNAL') || die();

/**
 * Class program_dynamic_rules_tab
 *
 * @package   tool_program
 * @copyright 2019 Moodle Pty Ltd <support@moodle.com>
 * @author    2019 David Matamoros <davidmc@moodle.com>
...
class program_dynamic_rules_tab extends tab {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        if (!class_exists('\\tool_reportbuilder\\system_report_factory')) {
            $str = get_string('reportbuilderdynamicrules', 'tool_program');
            $table = html_writer::tag('div', $str, ['class' => 'alert alert-warning']);
            $content = new content_with_heading($table, get_string('dynamicrules', 'tool_program'));
        ...
        if (!class_exists('\\tool_dynamicrule\\rules_list')) {
            $str = get_string('dynamicrulesplugincheck', 'tool_program');
        ...
        $content = new content_with_heading($report->output(), get_string('dynamicrules', 'tool_program'));
        $content->add_content_wrapper(['data-region' => 'ruleslistwrapper']);
        return $content->export_for_template($output);
    }
    ...
    public function get_tab_label(): string {
        return get_string('dynamicrules', 'tool_program');
    }
