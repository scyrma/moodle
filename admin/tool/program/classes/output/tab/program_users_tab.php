...
class program_users_tab extends tab {
    ...
    public function export_for_template(\renderer_base $output) {
        ...
        $canshowbutton = permission::can_allocate_anybody($this->get_program(), false, false);
        if ($canshowbutton) {
            $addbuttontitle = get_string('allocateusers', 'tool_program');
            $params = [];
            $program = $this->get_program();
            if ((int)$program->get('allowdirectallocation') === 0 || !api::is_allocation_window_open($program)) {
