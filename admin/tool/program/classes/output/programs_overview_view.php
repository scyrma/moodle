...
class programs_overview_view implements templatable, renderable {
    ...
    public function export_for_template(renderer_base $output): stdClass {
        ...
        // TODO WP-2164 Take sort order from the user preferences, uncomment this line.
        // @codingStandardsIgnoreLine
        // $userpreferencessort = get_user_preferences('tool_program_program_sort_filter', 'lastaccess');
        $userpreferencessort = 'lastaccess';
        ...
        $workplaceexcludedcourses = \tool_program\api::get_course_ids_with_only_enrol_program_instance($this->userid);
        $hiddencourses = array_unique(array_merge($hiddencourses, $workplaceexcludedcourses));
        $courses = enrol_get_my_courses('summary, summaryformat, enddate', null, 0, [], false, 0, $hiddencourses);
        $coursescompletion = $this->get_courses_completion();
        $coursesprogress = [];
        foreach ($courses as $course) {
            $courseprogress = progress::get_course_progress_percentage($course, $this->userid) ?? 0;
        ...
        $relateddata = [
            ...
            'lastcourseaccess' => $this->get_last_course_access($allcourseids),
            ...
            'coursestartdates' => $this->get_all_user_course_startdates($allcourseids) ?? [],
        ];
     ...
     * @return array
     */
    private function get_last_course_access(array $courseids) {
        global $DB;
        if (!$courseids) {
            return [];
        }
        ...
        return $DB->get_records_select('user_lastaccess', 'userid=:user AND courseid '.$sql,
            $params + ['user' => $this->userid], '', 'courseid, timeaccess');
    }
    ...
    private function get_course_info(array $programcourse): stdClass {
        ...
        $data->progress = round($programcourse['progress']);
    ...
    /**
     * Get non program courses completions
     *
     * @return array
     */
    private function get_courses_completion(): array {
        global $DB;

        $sql = 'SELECT c.id, cc.timecompleted
                FROM {course} c
     ...
     * @return array
     */
    private function get_all_user_course_startdates(array $courseids) {
        global $DB;
        if (!$courseids) {
            return [];
        }
