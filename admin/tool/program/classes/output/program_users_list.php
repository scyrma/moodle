...
use tool_program\reportbuilder\local\systemreports\user_allocations;
...
class program_users_list implements \renderable , \templatable {
    ...
    /** @var user_allocations $userreport The system report of program users. */
    ...
    public function __construct(program $program) {
        ...
        // Program users allocations list.
        $report = \tool_tenant\system_report_factory::create(user_allocations::class, ['id' => $program->get('id')]);
    ...
    public function export_for_template(renderer_base $output): array {
        ...
        if ($canshowbutton) {
            ...
            if (!$this->program->get('allowdirectallocation') || !api::is_allocation_window_open($this->program)) {
    ...
    private function get_bulk_actions(renderer_base $output): \stdClass {
        ...
        $bulkactions['actions'][$actions]['recalculateprogram'] = get_string('recalculateprogramcompletion', 'tool_program');
    ...
    private function get_data_for_modal(program $program) {
        ...
        if (!$program->get('allowdirectallocation')) {
        ...
        if ((int)$program->get('allocationstartdatetype') === constants::DATE_ABSOLUTE &&
            ...
            $enddate = api::calculate_relative_date($enddaterelative, $startdate);
