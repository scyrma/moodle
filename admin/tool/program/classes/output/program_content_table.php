...
class program_content_table extends table_tree {
    ...
    public function __construct(program $program, context $context) {
        ...
        $this->courses = $program->get_courses();
    }
    ...
    protected function export_course_name(renderer_base $output, stdClass $node): string {
        // If course is hidden we need to apply text-muted class to course name and show icon next to it.
        $class = null;
        $eyeslashicon = '';
        if (!$this->courses[$node->courseid]->visible) {
            $class = ['class' => 'text-muted'];
            $str = get_string('warningcourseinvisible', 'tool_program');
            $eyeslashicon = $output->pix_icon('i/show', $str, 'core', ['class' => 'text-muted pl-2']);
        }
        ...
        $displayvalue = \html_writer::link($courseurl, $displayvalue, $class);
        $columnname .= $displayvalue . $eyeslashicon . $node->warning;

        // Check if completion is enabled for this course, if not add an exclamation icon next to course name.
        if (!$this->is_course_completion_enabled($node->courseid)) {
            $str = get_string('coursecompletiondisabled', 'tool_program');
            $params = ['class' => 'icon fa fa-exclamation-triangle text-danger fa-fw pl-2'];
            $columnname .= $output->pix_icon('exclamation-triangle', $str, 'tool_wp', $params);
        }
    ...
    /**
     * Return if completion is enabled for this course
     *
     * @param int $courseid
     * @return bool
     */
    private function is_course_completion_enabled(int $courseid): bool {
        $completion = new \completion_info($this->courses[$courseid]);
