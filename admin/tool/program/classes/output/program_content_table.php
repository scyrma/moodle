...
class program_content_table extends table_tree {
    ...
    protected function export_course_name(renderer_base $output, stdClass $node): string {
        ...
        $strcompletion = '';
        $completion = new \completion_info($this->courses[$node->courseid]);
        // Check if completion is enabled for this course.
        if (!$completion->is_enabled()) {
            $strcompletion = 'coursecompletiondisabled';
        } else if (!$completion->has_criteria()) {
            // Check if course has criteria enabled.
            $strcompletion = 'coursecriteriadisabled';
        }

        // If completion warning is needed add an exclamation icon next to course name.
        if (!empty($strcompletion)) {
            $str = get_string($strcompletion, 'tool_program');
