...
class my_program_progress_exporter extends exporter {
    ...
    protected function get_other_values(renderer_base $output): array {
        ...
        $this->export_certifications_and_program_allocation_data($output, $userallocations, $certifications, $usercertifications,
     ...
     */
    private function fetch_related_user_certification(&$usercertifications, int $certificationid): ?certification_user {
        $relatedusercertification = null;
        foreach ($usercertifications as $key => $usercertification) {
            if ($certificationid === (int) $usercertification->get('certificationid')) {
                $relatedusercertification = $usercertification;
                unset($usercertifications[$key]);
                break;
            }
        }

        return $relatedusercertification;
    }
    ...
    private function export_progress_and_completion_data(renderer_base $output, program_tree_progress $programitemstree,
        ...
        foreach ($programitemslist as $programitem) {
            if ($programitem->is_base_set()) {
                ...
                $exporter = new program_set_progress_exporter(null, [
                    'context' => $context,
     ...
     * @param array $usercertifications
     * @param context $context
     * @param stdClass $exporteddata
     */
    private function export_certifications_and_program_allocation_data(renderer_base $output, array $userallocations,
        array $certifications, array $usercertifications, context $context, stdClass $exporteddata): void {
        foreach ($userallocations as $allocation) {
            ...
            if (0 !== (int) $certificationid) {
                ...
                $relatedcertificationallocation = $this->fetch_related_user_certification($usercertifications, $certificationid);
                $exporter = new program_certification_exporter(null, [
                    'context' => $context,
                    'certification' => $relatedcertification,
                    'certificationuser' => $relatedcertificationallocation,
                ]);
