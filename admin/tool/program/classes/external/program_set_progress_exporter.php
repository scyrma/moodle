 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class program_set_progress_exporter extends exporter {
    /**
    ...
    protected static function define_other_properties(): array {
        return program_set_exporter::read_properties_definition() + [
                'completeditems' => [
                    'type' => PARAM_INT,
                ],
                'completiontype' => [
    ...
    protected function get_other_values(renderer_base $output): array {
        ...
        $exporter = new program_set_exporter($programitemset->get_persistent(), ['context' => $context]);
        $exporteddata = (array) $exporter->export($output);

        $exporteddata['parentsetid'] = $programitemset->get_parent_id();
        $exporteddata['completiontype'] = $this->get_completion_type($programitemset);
        $exporteddata['completiontypestr'] = $this->get_completion_type_string($programitemset);
        ...
        $exporteddata['totalactivities'] = $programitemset->totalitems;
        $exporteddata['completedactivities'] = $programitemset->completeditems;
        $exporteddata['progress'] = $programitemset->progresspercentage;
        $exporteddata['iscompleted'] = false;
        $exporteddata['showprogress'] = true;
        $exporteddata['statusmessage'] = $this->get_completion_type($programitemset);
        $exporteddata['statusmessagestr'] = $this->get_completion_type_string($programitemset);
