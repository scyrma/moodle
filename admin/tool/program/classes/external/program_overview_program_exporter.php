...
class program_overview_program_exporter extends exporter {
        ...
        foreach ($programallocations as $programallocation) {
            ...
            if (0 !== $certificationid) {
                ...
                [$datemsg, $alertstyle, $iconstyle] = $this->get_certification_allocation_messages($certificationstatus,
                    $certificationcompletion, (int)$allocationsource->certificationduedate);
     ...
     * @return array
     */
    private function get_certification_allocation_messages($status, $completion, $allocationduedate): array {
        switch ($status) {
            ...
            case \tool_certification\constants::STATUS_OPEN:
                if ($allocationduedate === \tool_certification\constants::DATE_NONE) {
                    $msgstring = get_string('certificationduedatenotset', 'tool_program');
                } else {
                    $duedate = userdate($allocationduedate, get_string('strftimedatefullshort'));
