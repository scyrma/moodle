...
class api {
    ...
    private static function add_course_to_a_set(stdClass $data): program_course {
        ...
        // Enrol users who have completed already the course.
        $program = new program($data->programid);
        $programusers = $program->get_program_users();
        foreach ($programusers as $programuser) {
            self::enrol_in_program_course_if_course_is_completed($program, $course, $programuser->get('userid'));
        }
    ...
    public static function allocate_user(program $program, stdClass $programuserdata): program_user {
        ...
        // Enrol the user in already completed courses.
        $courses = $program->get_courses();
        foreach ($courses as $course) {
            self::enrol_in_program_course_if_course_is_completed($program, $course, $newuser->get('userid'));
        }
    ...
    /**
     * Check if course is already completed by user, then enrol user in course with program enrolment and mark it as completed
     *
     * @param program $program
     * @param stdClass $course
     * @param int $userid
     * @return void
     */
    private static function enrol_in_program_course_if_course_is_completed(program $program, stdClass $course, int $userid): void {
        global $CFG;
        require_once($CFG->libdir . "/completionlib.php");
        $completion = new \completion_info($course);
        if ($completion->is_enabled() && $completion->is_course_complete($userid)) {
            self::enrol_in_program_course($program->get('id'), $course, $userid);
    ...
    public static function suspend_all_allocated_users_enrolments(program $program): void {
        ...
        foreach ($coursesids as $courseid) {
            self::disable_course_enrol_program_instance($program->get('id'), $courseid);
        }
    }

    /**
     * Restores all allocated users enrolments for all courses related to the given program.
     *
     * @param program $program
     */
    ...
    public static function update_program_user_dates_and_status(program_user $programuser, stdClass $validateddata): bool {
        ...
        if ($suspendedstatus !== $oldstatus && $suspendedstatus === $newstatus) {
            // If just suspended, suspend program course enrolments for this user.
            self::suspend_allocated_user_enrolments($programuser);
        }
    ...
    public static function suspend_allocated_user_enrolments(program_user $programuser): bool {
        ...
        // Check if program_course enrolment is shared with more allocations. If all are suspended suspend it.
        $allallocationssuspended = true;
        $programallocations = program_user::get_records(['programid' => $programid, 'userid' => $userid]);
        foreach ($programallocations as $allocation) {
            if ((int)$allocation->get('status') === constants::STATUS_OVERRIDE_DEFAULT) {
                $allallocationssuspended = false;
            }
        }
        if (!$allallocationssuspended) {
            return true;
        }
    ...
    public static function reactivate_allocated_user_program_enrolments(program_user $programuser): bool {
        ...
        // Check if program_course enrolment is shared with more allocations. If any is active then activate it.
        $anyallocationsactive = false;
        $programallocations = program_user::get_records(['programid' => $programid, 'userid' => $userid]);
        foreach ($programallocations as $allocation) {
            if ((int)$allocation->get('status') === constants::STATUS_OVERRIDE_DEFAULT) {
                $anyallocationsactive = true;
                break;
            }
        }
        if (!$anyallocationsactive) {
            return true;
        }
    ...
    private static function send_moodle_notification(int $userid, string $provider, string $subject, string $fullmessage): void {
        ...
        $dashboardstr = get_string('myhome');
        $fullmessage .= '<br><br>' . get_string('notificationmsgfooter', 'tool_program', $url);
    ...
    private static function get_default_program_startdate(program $program): string {
        switch ((int) $program->get('startdatetype')) {
            ...
            case constants::DATE_ABSOLUTE:
                ...
                break;
            case constants::DATE_AFTER_USER_ALLOCATION:
                $str = \tool_certification\api::translate_relativedate_string($program->get('startdaterelative'));
    ...
    private static function get_default_program_duedate(program $program): string {
        switch ((int)$program->get('duedatetype')) {
            ...
            case constants::DATE_ABSOLUTE:
                ...
                break;
            case constants::DATE_AFTER_START:
                $str = \tool_certification\api::translate_relativedate_string($program->get('duedaterelative'));
    ...
    private static function get_default_program_enddate(program $program): string {
        switch ((int) $program->get('enddatetype')) {
            ...
            case constants::DATE_ABSOLUTE:
                ...
                break;
            case constants::DATE_AFTER_START:
                $str = \tool_certification\api::translate_relativedate_string($program->get('enddaterelative'));
                return get_string('afterstartdatewithrelativedate', 'tool_program', $str);
                break;
            case constants::DATE_AFTER_DUE:
                $str = \tool_certification\api::translate_relativedate_string($program->get('enddaterelative'));
