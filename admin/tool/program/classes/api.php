...
class api {
    ...
    public static function recalculate_program_user_dates(program $program, program_user $programuser): bool {
        ...
        if ($enddatehaschanged) {
            ...
            // We need to update the course end dates.
            if (!$enrolplugin = enrol_get_plugin('program')) {
                return false;
            }
            $enrolinstances = self::get_program_enrol_instances_by_programid_and_userid($program->get('id'), $userid);
            foreach ($enrolinstances as $enrolinstance) {
                $enrolplugin->update_user_enrol($enrolinstance, $userid, null, null, $userenddate);
            }
        }
