...
class api {
    ...
    public static function get_user_allocation_statuses(int $programid, int $userid, int $certificationid): array {
        ...
        if ($issuspended) {
            ...
            $statuses[] = ['status' => 'program_user_status_suspended', 'statusstr' => $statusstr, 'stringid' => 'suspended'];
        }

        $iscompleted = !empty($programcompletion);
        if ($iscompleted) {
            $statusstr = get_string('completed', 'tool_program');
            $statuses[] = ['status' => 'program_user_status_completed', 'statusstr' => $statusstr, 'stringid' => 'completed'];
        }
        ...
        if (!$iscompleted && !$issuspended) {
            ...
            if ($hasstartdate && $now < $startdate) {
                ...
                $statuses[] = ['status' => 'program_user_status_overdue', 'statusstr' => $statusstr, 'stringid' => 'overdue'];
            } else if ($isopen) {
                // Open - After start date and before due date.
                $statusstr = get_string('open', 'tool_program');
                $statuses[] = ['status' => 'program_user_status_open', 'statusstr' => $statusstr, 'stringid' => 'open'];
            }
        }
    ...
    public static function get_course_ids_with_only_enrol_program_instance(int $userid): array {
        ...
        // This method is only called from course/externallib.php. Handling this setting we changed the function functionality
        // wihtout changing its name.
    ...
    public static function get_programs_in_tenant_fieldset(int $userid = 0): array {
        ...
        if (sharedspace::is_shared_space() && \tool_tenant\permission::can_switch_tenant()) {
            $params = ['isshared' => 1, 'archived' => 0];
            [$tenantsql, $tenantparams] =
                hierarchy::filter_own_or_sub_or_parent_shared_entities_sql('tenantid', 'shared=:isshared');

            $sql = 'SELECT id,fullname from {tool_program} WHERE archived=:archived AND ' . $tenantsql . ' ORDER BY fullname';
            $programs = $DB->get_records_sql($sql, array_merge($params, $tenantparams));
        } else {
    ...
    public static function restore_user_in_program_course_groups(program_user $programuser): void {
        ...
        foreach ($tenantgroups as $tenantgroup) {
            $enrolinstance = self::get_program_enrol_instance_by_enrolled_user($programid, $tenantgroup->get('courseid'), $userid);
            if (!$enrolinstance) {
                continue;
            }
            groups_add_member($tenantgroup->get('groupid'), $userid, 'enrol_program', $enrolinstance->id);
        }
    }

    /**
     * Returns tenant groups for program/certification.
     *
     * Only tenant groups that have component reference are returned.
     *
    ...
    public static function get_user_courses_progress(int $userid, array $courses, ?array $coursescompletion = null): array {
        ...
        foreach ($courses as $course) {
            ...
            // Adjust 100% to 95% is course is not completed.
            if ($courseprogress >= 100 && is_null($coursescompletion[$course->id]->timecompleted)) {
                $courseprogress = 95;
            }
