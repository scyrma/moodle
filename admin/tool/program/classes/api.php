...
class api {
     ...
     *
     * @param program_item[] $programitems Program items to be duplicated recursively.
    ...
    private static function recalculate_user_start_date(program $program, program_user $programuser, int $userallocationdate): int {
        ...
        switch ($program->get('startdate' . 'type')) {
            ...
            case constants::DATE_AFTER_USER_ALLOCATION:
                $userstartdate = self::calculate_relative_date($program->get('startdaterelative'), $userallocationdate);
                break;
            default:
        ...
        switch ($program->get('duedate' . 'type')) {
            ...
            case constants::DATE_AFTER_START:
                if (constants::DATE_NONE === $userstartdate) {
                    ...
                    $userduedate = self::calculate_relative_date($program->get('duedaterelative'), $userstartdate);
                }
                break;
            case constants::DATE_AFTER_USER_ALLOCATION:
                $userduedate = self::calculate_relative_date($program->get('duedaterelative'), $userallocationdate);
                break;
            case constants::DATE_BEFORE_END:
                switch ($program->get('enddate' . 'type')) {
                    ...
                    case constants::DATE_ABSOLUTE:
                        ...
                        $userduedate = self::calculate_relative_date('-' . $program->get('duedaterelative'), $userenddate);
                        break;
                    case constants::DATE_AFTER_START:
                        if (constants::DATE_NONE === $userstartdate) {
                            $userduedate = constants::DATE_NONE;
                        } else {
                            $userenddate = self::calculate_relative_date($program->get('enddaterelative'), $userstartdate);
                            $userduedate = self::calculate_relative_date('-' . $program->get('duedaterelative'), $userenddate);
                        }
                        break;
                    case constants::DATE_AFTER_DUE:
                        ...
                        break;
                    case constants::DATE_AFTER_USER_ALLOCATION:
                        $userenddate = self::calculate_relative_date($program->get('enddaterelative'), $userallocationdate);
                        $userduedate = self::calculate_relative_date('-' . $program->get('duedaterelative'), $userenddate);
                        break;
                    default:
        ...
        switch ($program->get('enddate' . 'type')) {
            ...
            case constants::DATE_AFTER_DUE:
                if (constants::DATE_NONE === $userduedate) {
                    ...
                    $userenddate = self::calculate_relative_date($program->get('enddaterelative'), $userduedate);
                }
                break;
            default:
                throw new coding_exception('unexpected program end date type');
    ...
    public static function update_program_user_dates_and_status(program_user $programuser, stdClass $validateddata): bool {
        ...
        // Trigger event.
        user_allocation_updated::create_from_user_allocation_updated($programuser)->trigger();
     ...
     * @deprecated since 4.3, 4.2.1 Rolling
     *
     * @param int $programid
     * @return string datauri
     */
    public static function get_program_pattern(int $programid): string {
        debugging('get_program_pattern() is deprecated. An image in base64 format will not be returned for
            the course or program image. You can get an image URL with get_program_pattern_url()', DEBUG_DEVELOPER);
        global $OUTPUT;
    ...
    public static function is_allocation_window_open(program $program): bool {
        ...
        if ($enddatetype === constants::DATE_AFTER_ALLOCATION_STARTS) {
            ...
            $enddate = self::calculate_relative_date($enddaterelative, $startdate);
     ...
     *
     * @param int|null $programid Given program id or null for all programs
     ...
     * @param string $ps Table alias for table program sets
     * @param string $psc Table alias for table program set completions
     * @param string|null $pid Name for named param program id or null for all programs
     * @return array
     */
    public static function get_program_status_sql_query(?int $programid, int $status, bool $isnegated = false,
        ...
        $params = !is_null($programid) ? [$pid => $programid] : [];
    ...
    public static function get_programs_with_criteria_conditions(int $programid, int $status,
        ...
        $wheredate = '';
        $u2 = database::generate_alias();
        $pu = database::generate_alias();
        $p = database::generate_alias();
        $ps = database::generate_alias();
        $psc = database::generate_alias();
        $programparam = database::generate_param_name();
     ...
     * @return bool
     */
    public static function is_idnumber_unique(int $programid, string $idnumber, int $tenantid = 0): bool {
        return hierarchy::is_entity_idnumber_unique('tool_program', $idnumber, $programid,
            $tenantid, 'archived = 0');
    }
    ...
    public static function get_program_by_idnumber(string $idnumber, int $tenantid): ?program {
        if ($record = hierarchy::find_entity_by_idnumber('tool_program', $idnumber, $tenantid,
                'archived = 0', [], true, 'shared = 1')) {
    ...
    /**
     * Returns a program pattern image url for programs that have no image set.
     *
     * @param int $programid
     * @return string
     */
    public static function get_program_pattern_url(int $programid): string {
        $context = (new program($programid))->get_context();
        return \moodle_url::make_pluginfile_url(
            $context->id, 'tool_program', 'program_generated_image', $programid, '/', 'program.svg')->out();
    }

    /**
     * Returns a calculated relative date ensuring it will not be calculated with multiple signs (e.g. +-2).
     *
     * @param string $relativedate
     * @param int|null $basedate
     * @return int
     */
    public static function calculate_relative_date(string $relativedate, int $basedate = null): int {
        // Check if relative date starts with '+' or '-'.
        if (preg_match('/^[+-]/', $relativedate)) {
            return strtotime($relativedate, $basedate);
        }
