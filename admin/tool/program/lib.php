...
function tool_program_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    ...
    if ($filearea === 'program_description' || $filearea === 'program_image') {
        $fs = get_file_storage();
    ...
    } else if ($filearea === 'program_generated_image') {
        // Filearea to download generated files (like program or course image).
...
function tool_program_myprofile_navigation(tree $tree, $user, $iscurrentuser, $course) {
    ...
    if ($canviewreports) {
        ...
        if (!empty($programs)) {
            $urlreport = new moodle_url('/admin/tool/program/user_report.php',
        ...
        if (!empty($programs)) {
            $urlreport = new moodle_url('/admin/tool/program/user_report.php',
                ['id' => $user->id, 'type' => constants::STATUS_COMPLETED]);
...
function tool_program_block_myteams_user_section(int $userid): array {
    ...
    $sectionlink = new moodle_url('/admin/tool/program/user_report.php', ['id' => $userid]);
 ...
 */
function tool_program_block_myteams_global_report_link(): ?global_report_link {
    if (!permission::can_view_programs_progress_report()) {
        return null;
    }
    return new global_report_link(
        get_string('fullprogramreport', 'tool_program'),
        new moodle_url('/admin/tool/program/programsprogress.php')
    );
}
 ...
 */
function tool_program_block_myteams_global_notification(array $manageduserids): ?global_notification {
    global $DB;

    // Program tenant condition.
    [$tenantsql, $tenantparams] = hierarchy::filter_own_or_parent_shared_entities_sql("tp.tenantid", "tp.shared=1");
    // Program status condition.
    [
        $programstatusjoin,
        $programstatuswhere,
        $programstatusparams
    ] = api::get_program_status_sql_query(null, constants::STATUS_OVERDUE, false, 'u', 'tpu', 'tp', 'tps', 'tpsc', null);
    // Managed users condition.
    [$insql, $inparams] = $DB->get_in_or_equal($manageduserids, SQL_PARAMS_NAMED, 'mu');

    // Perform the query to get the count of managed users that have any overdue program.
    $sql = "SELECT COUNT(tp.id)
              FROM {user} u
              $programstatusjoin
             WHERE u.deleted = 0
               AND tp.archived = 0
               AND tp.visible = 1
               AND (tpu.allocationtype = :allocationmanual OR tpu.allocationtype = :allocationdr)
               AND $tenantsql
               AND $programstatuswhere
               AND u.id $insql";
    $allocationtypeparams = ['allocationmanual' => constants::ALLOCATION_MANUAL, 'allocationdr' => constants::ALLOCATION_DYNAMIC];
    $params = $tenantparams + $programstatusparams + $inparams + $allocationtypeparams;
    $shownotification = $DB->count_records_sql($sql, $params) > 0;

    $url = new moodle_url('/admin/tool/program/programsprogress.php', ['type' => constants::STATUS_OVERDUE]);
    $message = get_string('managedusersoverdue', 'tool_program');
