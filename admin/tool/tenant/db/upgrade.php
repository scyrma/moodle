...
function xmldb_tool_tenant_upgrade($oldversion) {
    ...
    if ($oldversion < 2019102103) {
        ...
        foreach ($tenants as $tenant) {
            if (!$DB->record_exists('course_categories', ['id' => $tenant->categoryid])) {
    ...
    if ($oldversion < 2022040100) {
        // This upgrade script was added right before 4.1 release but it is "backdated" so it is executed only on
        // upgrades from 3.11 to 4.1 and skipped on upgrades from 4.0 to 4.1.

        // Update tenant colour settings.
        tool_tenant_update_tenant_colour_settings();

        // Wp savepoint reached.
        upgrade_plugin_savepoint(true, 2022040100, 'tool', 'tenant');
    }

    if ($oldversion < 2022040101) {
        // Add capability to edit all custom reports to the tenant admin role.
        $context = context_system::instance();
        $roleswithcapability = get_roles_with_caps_in_context($context, ['tool/reportbuilder:edit']);

        $tenantadminrole = $DB->get_field('role', 'id', ['shortname' => 'tool_tenant_admin']);
        if ($tenantadminrole && array_key_exists($tenantadminrole, $roleswithcapability)) {
            assign_capability('moodle/reportbuilder:editall', CAP_ALLOW, $tenantadminrole, $context->id);
        }

        // Tenant savepoint reached.
        upgrade_plugin_savepoint(true, 2022040101, 'tool', 'tenant');
    }

    if ($oldversion < 2022090501) {
        update_capabilities('tool_tenant');

        // Update tenant admin role to allow ':addinstance' blocks capability.
        $capabilities = array_keys(array_filter(\tool_tenant\role::get_tenant_admin_capabilities()));
        \tool_tenant\manager::create_workplace_role('tool_tenant_admin', '',
            get_string('tenantadmindescription', 'tool_tenant'),
            $capabilities);

        // Tenant savepoint reached.
        upgrade_plugin_savepoint(true, 2022090501, 'tool', 'tenant');
    }
    ...
    if ($oldversion < 2022100600) {
        ...
        // Tenant savepoint reached.
        upgrade_plugin_savepoint(true, 2022100600, 'tool', 'tenant');
    }
