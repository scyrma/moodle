...
$ADMIN->add('appearance', new \tool_wp\admin_externalpage('tool_tenant_theme', new lang_string('appearance'),
    ...
    function() {
        return \tool_tenant\permission::can_access_tenant_settings_page();
    }));

// Manage tenant dashboard.
$ADMIN->add('appearance', new \tool_wp\admin_externalpage('tool_tenant_dashboard',
    new lang_string('mypage', 'admin'),
    \tool_tenant\manager::get_dashboard_url(),
    function() {
        return \tool_tenant\permission::can_edit_tenant_dashboard_blocks_anywhere();
    }, !\tool_tenant\permission::can_edit_tenant_dashboard_blocks()));

$mypagepagesettings = $ADMIN->locate('mypage');
if ($mypagepagesettings && isset($mypagepagesettings->visiblename)) {
    $mypagepagesettings->visiblename = new lang_string('defaultsitedashboardpage', 'tool_tenant');
}
...
// Change "Browse list of users" link name.
$editusers = $ADMIN->locate('editusers');
if ($editusers) {
    $editusers->visiblename = new lang_string('alluserslegacy', 'tool_tenant');
}

// Hide core site admin pages "Add new user" and "User management".
foreach (['addnewuser', 'usermanagement'] as $elname) {
...
// Add custom description to the site shortname setting.
/** @var admin_settingpage $frontpagesettings */
$frontpagesettings = $ADMIN->locate('frontpagesettings');
if ($frontpagesettings && isset($frontpagesettings->settings->shortname)) {
...
if ($hassiteconfig && ($modurlsettings = $ADMIN->locate('modsettingurl'))) {
    $modurlsettings->add(new admin_setting_heading('urlmodtenant',
        new lang_string('modurl:tenantdata', 'tool_tenant'), new lang_string('modurl:tenantdatadesc', 'tool_tenant')));

    $modurlsettings->add(new admin_setting_configcheckbox('url/tenantdata',
        new lang_string('modurl:tenantdata', 'tool_tenant'), new lang_string('modurl:configtenantdata', 'tool_tenant'), false));
}
