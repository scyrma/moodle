...
class config {
    ...
    public static function is_default_config_forced(string $plugin, string $name): bool {
        return (bool)self::get_config_default($plugin, $name.'_wforce');
    }

    /**
     * List of the settings inside the plugin that potentially allow overriding
     *
     * @param string $plugin full name of the plugin or 'core' for core settings
     * @return array
     */
    protected static function get_multitenant_settings_names(string $plugin) {
        $settings = [];
        if ($plugin === 'core') {
            $settings = array_merge($settings, array_keys(auth_manager::overridable_auth_settings()));
            $settings = array_merge($settings, array_keys(admin::overridable_admin_settings()));
        }
        $settings = array_merge($settings, auth_manager::get_multitenant_settings_names($plugin));
        $settings = array_merge($settings, tool_mobile::get_multitenant_settings_names($plugin));
        $settings = array_merge($settings, message_airnotifier::get_multitenant_settings_names($plugin));
        return $settings;
    }
    ...
    public static function add_flag_to_admin_setting(\admin_setting $setting): ?array {
        if (in_array($setting->name, self::get_multitenant_settings_names($setting->plugin ?? 'core'))) {
     ...
     */
    public static function get_tenant_id(): int {
    ...
    protected static function reset_cfg_for_tenant(): void {
        ...
        $cfg = (array)get_config('core');
        foreach (array_merge(self::get_multitenant_settings_names('core'), ['auth', 'showuseridentity']) as $key) {
            if (array_key_exists($key, $cfg) || isset($CFG->$key)) {
                $CFG->$key = $cfg[$key] ?? null;
            }
        }
        if (self::$defaultsite === null) {
            self::$defaultsite = $DB->get_record('course', ['category' => 0]);
        }
        // Purge user profile fields request cache that may be invalid now. Similar to profile_purge_user_fields_cache().
        \cache::make_from_params(\cache_store::MODE_REQUEST, 'core_profile', 'customfields',
            [], ['simplekeys' => true, 'simpledata' => true])->purge();
    ...
    public static function get_config_hook(string $plugin, array &$config): void {
        ...
        if ($overridablesettings = self::get_multitenant_settings_names($plugin)) {
            // Override the settings.
            $tenantoverrides = self::get_config_tenant_overrides($tenantid, $plugin);
            foreach ($overridablesettings as $key) {
                if (empty($config[$key.'_wforce']) && array_key_exists($key, $tenantoverrides)) {
                    $config[$key] = $tenantoverrides[$key];
                }
            }
            // Post-processing of the settings - some settings values need to be calculated.
            if ($plugin === 'core') {
                // Substitute auth-related config settings (auth, registerauth, auth_instructions, etc).
                auth_manager::calculate_config_for_tenant($config, $tenantoverrides);
