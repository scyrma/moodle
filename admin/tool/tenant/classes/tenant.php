...
class tenant extends \core\persistent implements \cacheable_object {
    ...
    public function get_editable_name() : inplace_editable {
        ...
        if (permission::can_view_tenant_details($this->get('id')) || permission::can_browse_users($this->get('id'))) {
    ...
    public function get_login_urls(bool $withcopylinks = true) {
        global $CFG, $OUTPUT;
        $urls = [];
        $wwwroot = tenancy::get_tenant_wwwroot($this->get('id'));
        if (tenancy::has_unique_custom_wwwroot($this->get('id'))) {
            $url = $wwwroot;
            if ($withcopylinks) {
