...
use tool_reportbuilder\tool_wp\importer\customreports;
...
use tool_wp\tool_wp\importer\reports;
...
class tenants extends importer_base {
    ...
    public function add_to_options_form(import_settings_form $form): void {
        ...
        $mform->hideIf(self::IMPORT_CERTIFICATES, self::IMPORT_CATEGORIES, 'eq', 0);
        ...
        $mform->addElement('advcheckbox', self::IMPORT_REPORTS, new lang_string('customreports', 'core_reportbuilder'));
        $mform->setType(self::IMPORT_REPORTS, PARAM_INT);
        $mform->setDefault(self::IMPORT_REPORTS, 1);
        $legacyreports = core_component::get_component_directory('tool_reportbuilder') &&
            $this->can_import_chained_entity('tool_reportbuilder') &&
            $this->get_entities_in_workplace_export_file('tool_reportbuilder')->count();
        $corereports = $this->can_import_chained_entity('reportbuilder_report') &&
            $this->get_entities_in_workplace_export_file('reportbuilder_report')->count();
        if (!$legacyreports && !$corereports) {
    ...
    public function get_summary_for_review_step(bool $importcompleted): string {
        ...
        if (!empty($settings[self::IMPORT_CATEGORIES])) {
            $settingssummary[] = [
                'name' => get_string('certificatetemplates', 'tool_wp'),
                'value' => !empty($settings[self::IMPORT_CERTIFICATES])
            ];
        }
        ...
        $settingssummary[] = [
            'name' => get_string('customreports', 'core_reportbuilder'),
    ...
    public function perform_import(string $entityname): void {
        ...
        foreach ($tenants as $tenant) {
            ...
            if (!empty($settings[self::IMPORT_PROGRAMS]) && $this->can_import_chained_entity('tool_program')) {
                ...
                // TODO WP-3928: This condition check may not be required.
            ...
            // Whether custom reports (tool_reportbuilder) should be included.
            if (!empty($settings[self::IMPORT_REPORTS]) && $this->can_import_chained_entity('tool_reportbuilder')) {
                ...
                $this->process_chained_entities('tool_reportbuilder', $reportids, [
                    customreports::IMPORT_AUDIENCES => $importusers,
                    customreports::IMPORT_SCHEDULES => $importusers,
                ]);
            }

            // Whether custom reports should be included.
            if (!empty($settings[self::IMPORT_REPORTS]) && $this->can_import_chained_entity('reportbuilder_report')) {
                $reports = $this->get_entities_in_workplace_export_file('reportbuilder_report',
                    static function(array $data) use ($tenant): bool {
                        return $data['itemid'] == $tenant->get_original_id() && $data['component'] === 'tool_tenant';
                    });
                ...
                $this->process_chained_entities('reportbuilder_report', $reportids, [
                    reports::IMPORT_AUDIENCES => $importusers,
                    reports::IMPORT_SCHEDULES => $importusers,
                ]);
            }
        }
