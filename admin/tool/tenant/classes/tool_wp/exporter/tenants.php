...
use tool_wp\tool_wp\exporter\reports;
...
class tenants extends exporter_base {
    ...
    public function add_to_options_form(export_settings_form $form): void {
        ...
        $mform->addElement('advcheckbox', self::EXPORT_REPORTS, new lang_string('customreports', 'core_reportbuilder'));
        ...
        if (!$this->can_export_chained_entity('tool_reportbuilder') &&
                !$this->can_export_chained_entity('reportbuilder_report')) {
    ...
    public function perform_export(): void {
        ...
        foreach ($this->get_tenants_for_export() as $tenant) {
            ...
            if (!empty($settings[self::EXPORT_REPORTS]) && $this->can_export_chained_entity('reportbuilder_report')) {
                $this->process_chained_entities('reportbuilder_report', [], [
                    reports::EXPORT_INSTANCES => reports::EXPORT_INSTANCES_ALL,
                    reports::EXPORT_AUDIENCES => $exportusers,
                    reports::EXPORT_SCHEDULES => $exportusers,
                ]);
            }
        }
