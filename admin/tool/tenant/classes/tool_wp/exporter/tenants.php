...
class tenants extends exporter_base {
    ...
    public function is_available(): bool {
        return !strlen($this->entrypoint) && permission::can_export_tenants();
    }
    ...
    public function add_to_options_form(export_settings_form $form): void {
        ...
        $mform->disabledIf(self::EXPORT_COURSE_CONTENT, self::EXPORT_CATEGORIES, 'eq', 0);
        ...
        $mform->disabledIf(self::EXPORT_CERTIFICATES, self::EXPORT_CATEGORIES, 'eq', 0);
    ...
    public function get_summary_for_review_step(bool $exportcompleted): string {
        ...
        $settingssummary = [
            ...
            ['name' => get_string('includecoursecontent', 'tool_wp'), 'value' => !empty($settings[self::EXPORT_COURSE_CONTENT])],
    ...
    public function perform_export(): void {
        ...
        foreach ($this->get_tenants_for_export() as $tenant) {
            ...
            $this->prepare_data_for_workplace_export(tenant::TABLE, (array) $record)
                ->export();
