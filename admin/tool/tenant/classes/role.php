...
class role {
    ...
    public static function get_tenant_admin_capabilities() : array {
        $capabilities = [
            ...
            'moodle/reportbuilder:edit' => CAP_ALLOW,
            'moodle/reportbuilder:editall' => CAP_ALLOW,
            'moodle/reportbuilder:scheduleviewas' => CAP_ALLOW,
            ...
            'moodle/restore:createuser' => CAP_ALLOW,
        ];
    ...
    /**
     * Validate role assignment, called from core function {@see assign_capability()}
     *
     * Checks if role assigments for special tenant roles are allowed, show debugging message otherwise
     * (throw exception in automated tests)
     *
     ...
     */
    public static function validate_assign_capability($capability, $permission, $roleid, $context) {
        global $CFG;
        if (during_initial_install()) {
            // Capabilities cache is not yet available during initial install, we can not validate them.
            return;
        }
        if (empty($CFG->tool_tenant_adminrole)) {
            // During LMS->Workplace upgrade (tool_tenant install) do not validate capabilities
            // before the tenant admin role is actually created and set up.
            return;
        }
        $istest = (defined('PHPUNIT_TEST') && PHPUNIT_TEST) || defined('BEHAT_SITE_RUNNING')
            || (defined('BEHAT_TEST') && BEHAT_TEST);
        if (!debugging() && !$istest) {
            return;
        }
        if ($roleid == (int)$CFG->tool_tenant_adminrole && $permission == CAP_ALLOW) {
            if ($context->contextlevel != CONTEXT_SYSTEM) {
                $error = 'Can not change role "Tenant administrator" outside of system context';
            } else if (!array_key_exists($capability, self::get_tenant_admin_capabilities())) {
                $error = "Capability \"$capability\" is not allowed in \"Tenant administrator\" role";
            }
        }
        if (isset($error)) {
            if ($istest) {
