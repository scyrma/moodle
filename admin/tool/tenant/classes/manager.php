...
class manager {
    ...
    public function archive_tenant(int $id) : tenant {
        ...
        // Remove all tenant admin roles from this tenant.
        $this->assign_tenant_admin_role($id, []);
    ...
    protected static function find_user_preallocation(\stdClass $newuser,
        ...
        foreach (self::$preallocatedusers as $key => $entry) {
            list($user, $tenantid, $component, $reason) = $entry;
            if (empty($user->id) && !empty($user->username) && !empty($newuser->username) &&
    ...
    public function allocate_user_on_creation(user_created $event) {
        ...
        if ($preallocation = self::find_user_preallocation($user)) {
            ...
            // Check if the user needs the tenant admin role.
            if (!empty($user->istenantadmin) && permission::can_assign_tenant_admin($tenantid)) {
                $this->assign_tenant_admin_roles([$user->id], $tenantid);
            }

            return;
        } else if (!empty($event->other['restoreid'])) {
            // For users created during restore allocate them to the current tenant.
            $this->allocate_user($userid, tenancy::get_tenant_id(), 'tool_tenant', 'restore');
            return;
        }
    ...
    public function get_css_config(int $tenantid, array $filemanageroptions) : ?\stdClass {
        ...
        $callbacks = get_plugins_with_function('tenant_get_css_config');
        foreach ($callbacks as $type => $plugins) {
            foreach ($plugins as $plugin => $pluginfunction) {
                $pluginfunction($info, $tenantid, $filemanageroptions);
            }
        }
        return (object) $info;
    }
     ...
     * @return \moodle_url|null The url.
     */
    public function get_tenant_file_url(int $tenantid, string $filearea): ?\moodle_url {
        $url = null;
        $fs = \get_file_storage();
    ...
    public function get_theme_scss(int $tenantid) : string {
        ...
        foreach (edit_css_form::get_colour_values() as $key) {
            if (array_key_exists($key, $storedcssconfig) && strlen($storedcssconfig[$key])) {
                ...
                // Validate colour pattern to avoid SCSS errors.
                $scss .= helper::is_valid_colour($storedcssconfig[$key])
                    ? '$' . $key . ': ' . $storedcssconfig[$key] . ';'
                    : '';
            }
        }

        // Add tenant grayscale.
        if (isset($storedcssconfig['brandgraytones']) && $storedcssconfig['brandgraytones'] === 1) {
            $scss .= file_get_contents($CFG->dirroot . '/theme/workplace/scss/workplace/graytones.scss');
        }
     ...
     *
     * If the role already exists the id of the role is returned, the role is not modified.
     *
     ...
     * @param array|null $contextlevels assignable context levels, by default [CONTEXT_SYSTEM]
    ...
    protected static function create_role(string $shortname, string $name,
        ...
        $systemroles = $DB->get_records_menu('role', [], 'sortorder ASC', 'shortname,id');
        $context = \context_system::instance();

        if (array_key_exists($shortname, $systemroles)) {
            return (int)$systemroles[$shortname];
        } else {
            // Create role.
    ...
    public static function set_tenant_cookie($tenantid) {
        ...
        if ($tenantid) {
            ...
            setcookie($cookiename, $tenantid, $sessionoptions);
        ...
        if (get_config('tool_tenant', 'showtenantselector')) {
            ...
            foreach ($this->get_tenants_without_shared() as $tenant) {
                if ($tenant->get('showinloginselector')) {
                    ...
                    $logo = $this->get_tenant_file_url($tenant->get('id'), 'tenantselectorlogo') ??
                        $this->get_tenant_file_url($tenant->get('id'), 'loginlogo');
                    $logourl = !is_null($logo) ? $logo->out(false) : '';

                    $tenant = [
                        'name' => \core_external\util::format_string($tenantname, \context_system::instance()),
                        'url' => reset($urls),
                        'logourl' => $logourl,
                    ];
    ...
    /**
     * Configured site tenant limit for number of custom reports threshold has been reached
     *
     * @return bool
     */
    public static function report_tenant_limit_reached(): bool {
        global $CFG;

        $params = ['type' => base::TYPE_CUSTOM_REPORT, 'itemid' => tenancy::get_tenant_id(), 'component' => 'tool_tenant'];

        return (!empty($CFG->tool_tenant_customreportslimit) &&
            (int) $CFG->tool_tenant_customreportslimit <= report::count_records($params));
    }

    /**
     * Returns a valid setting for the SameSite cookie attribute.
     * @see core_session_manager::should_use_samesite_none
     * Since the core method is private and it is needed here, the whole method was copied from core.
     *
     * @return bool The desired setting for the SameSite attribute on the cookie. False indicates the SameSite attribute
    ...
    /**
     * Looks up the tenant by idnumber (used in Web Services)
     *
     ...
     * @throws \moodle_exception
     */
    public function get_by_idnumber(string $idnumber, int $strictness = IGNORE_MISSING): ?tenant {
        $idnumber = strtolower(trim($idnumber));
        foreach ($this->get_tenants() as $tenant) {
            if (strtolower($tenant->get('idnumber') ?? '') === $idnumber) {
                return $tenant;
            }
        }
