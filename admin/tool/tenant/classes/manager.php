...
class manager {
    ...
    public function allocate_user_on_creation(user_created $event) {
        ...
        foreach (self::$preallocatedusers as $key => $entry) {
            ...
            if (empty($user->id) && !empty($user->username) &&
                    $user->username === $event->get_record_snapshot('user', $userid)->username) {
                unset(self::$preallocatedusers[$key]);
                $this->allocate_user($userid, $tenantid, $component, $reason, true);
                return;
            }
    ...
    public function reset_css_config(\stdClass $data) {
        ...
        if (!empty($data->resetimages)) {
            $fs = \get_file_storage();
            $fileareas = ['headerlogo', 'loginlogo', 'loginbackground', 'favicon'];
            foreach ($fileareas as $filearea) {
                $fs->delete_area_files(\context_system::instance()->id, 'tool_tenant', $filearea, $tenant->get('id'));
            }
        }
