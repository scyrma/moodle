...
class users_list implements \renderable , \templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        if (permission::can_confirm_anybody($this->tenantid)) {
            $actions = get_string('actions');
            $bulkactions['actions'][$actions]['confirm'] = get_string('confirmusers', 'tool_tenant');
        }
        if (permission::can_resend_email_users($this->tenantid)) {
            $actions = get_string('actions');
            $bulkactions['actions'][$actions]['resend'] = get_string('emailsconfirmationresend', 'tool_tenant');
        }
        // TODO WP-2375 remove check for empty tenant id.
        if (permission::can_assign_tenant_admin($this->tenantid) && $this->tenantid) {
            $bulkactions['admin'] = [
