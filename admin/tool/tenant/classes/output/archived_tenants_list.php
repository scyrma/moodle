...
class archived_tenants_list extends \tool_wp\output\table_tree {
    ...
    protected function export_cell(\renderer_base $output, string $columnname, $node = null): string {
        ...
        if ($columnname === 'name') {
            ...
            $actionmenulinks = $this->exportedtenant['actions'];
            if (!empty($actionmenulinks)) {
                $actionmenu = new action_menu($actionmenulinks);
                $icon = $output->pix_icon('i/settings', get_string('actions'));
                $actionmenu->set_menu_trigger($icon);
                $actionmenu->attributesprimary['class'] .= ' justify-content-end';
