...
class renderer extends \plugin_renderer_base {
    ...
    public function get_action_menu_links(tenant $tenant): array {
        ...
        if (permission::can_archive_tenant($tenantid)) {
            $confirmstr = get_string('confirmarchivetenant', 'tool_tenant', $tenantname);
            // Check if there are tenant admins in this tenant to show adittional warning message.
            if (!empty((new manager())->get_tenant_admins($tenantid))) {
                $confirmstr .= ' ' . get_string('confirmarchivetenantwarning', 'tool_tenant');
            }
