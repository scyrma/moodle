...
class users_report extends \tool_reportbuilder\system_report {
    ...
    protected function set_columns() {
        global $CFG;
        $this->add_entity((new user_entity())->set_allow_tenant_columns(true));
        ...
        $adminparams = ['comp' => 'tool_tenant', 'itemid' => $this->tenantid, 'roleid' => manager::get_tenant_admin_role()];
    ...
    private function add_actions() {
        ...
        $action = (new \tool_reportbuilder\report_action($blankurl, $icon, [
            ...
            ->add_callback(function(\stdClass $row) use ($tenantid) {
                $row->fullusername = format::fullname('', $row);
                return permission::can_confirm_user($row, $tenantid);
            });
        $this->add_action($action);
        ...
        $action = (new \tool_reportbuilder\report_action($blankurl, $icon, [
            ...
            ->add_callback(function(\stdClass $row) use ($tenantid) {
                return permission::can_resend_email_user($row, $tenantid);
            });
        $this->add_action($action);
