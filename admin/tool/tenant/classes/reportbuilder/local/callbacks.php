...
final class callbacks {
     ...
     * @param string|string[] $requiredcapability
     * @return array
     */
    public static function filter_by_tenant_category(string $categoryfield, $requiredcapability = ''): array {
        global $DB;
        ...
        // All the categories in which the current user has required capability, excluding the current tenant.
        $categories = core_course_category::make_categories_list($requiredcapability, (int) $categoryid);
        [$categorysql, $params] = $DB->get_in_or_equal(array_keys($categories), SQL_PARAMS_NAMED,
            database::generate_param_name() . '_', true, -1);

        $sql = "{$categoryfield} {$categorysql}";

        // Now append all tenant categories, regardless of capabilities.
        if (!empty($categoryid)) {
    ...
    /**
     * Callback executed in report builder datasources to filter cohorts by permission.
     *
     * - For shared space or non-multitenant sites, no filtering is applied.
     * - For multitenant sites when viewed from inside the tenant:
     *   - Show all cohorts in the tenant category or subcategories regardless of visibility or capabilities
     *   - In other categories, show cohorts only where current user has 'moodle/cohort:view' capability (assuming they are shared)
     *   - Show all system cohorts where
     *     - Cohort is visible; or
     *     - Current user has 'moodle/cohort:view' capability at system level
     *
     * @param string $cohortfield
     * @return array
     */
    public static function filter_by_tenant_cohort(string $cohortfield): array {
        global $DB;

        if (sharedspace::is_shared_space() || !tenancy::is_site_multi_tenant()) {
            // No need to additionally filter cohorts, let LMS do it.
            return ["1=1", []];
        }
        ...
        $pcoursecatctx = database::generate_param_name();
        $psyscontextid = database::generate_param_name();

        [$categoryselect, $categoryparams] = self::filter_by_tenant_category("$cc.id", 'moodle/cohort:view');
        $syscontext = \context_system::instance();
        $syscohortfilter = has_capability('moodle/cohort:view', $syscontext) ? '' : " AND $c.visible = 1";

        if (empty($syscohortfilter) && $categoryselect === '1=1') {
            // User can see everything.
            return ["1=1", []];
        }

        $select = "$cohortfield IN (
            SELECT $c.id FROM {cohort} $c
            JOIN {context} $ctx ON $ctx.id = $c.contextid
            LEFT JOIN {course_categories} $cc ON $cc.id = $ctx.instanceid AND $ctx.contextlevel = :$pcoursecatctx
            WHERE ($ctx.id = :$psyscontextid $syscohortfilter)
                OR
                ($cc.id IS NOT NULL AND $categoryselect)
            )";

        $params = $categoryparams + [
            $pcoursecatctx => CONTEXT_COURSECAT,
            $psyscontextid => $syscontext->id,
        ];
