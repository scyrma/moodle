...
class users extends system_report {
    ...
    protected function initialise(): void {
        ...
        // Check if report needs to show all tenant entities.
    ...
    public function add_columns(user $userentity): void {
        ...
        $additionaluserfields = fields::for_identity($context, true)->get_required_fields();
        $extra = preg_split('/,/', $CFG->showuseridentity, -1, PREG_SPLIT_NO_EMPTY);
        foreach ($extra as $userfield) {
            if ($column = $userentity->get_identity_column($userfield)) {
    ...
    protected function add_filters(user $userentity): void {
        ...
        $extra = preg_split('/,/', $CFG->showuseridentity, -1, PREG_SPLIT_NO_EMPTY);
        foreach ($extra as $userfield) {
            if ($filter = $userentity->get_identity_filter($userfield)) {
