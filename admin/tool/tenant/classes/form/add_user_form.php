...
class add_user_form extends dynamic_form {
    ...
    public function definition_after_data() {
        ...
        if ($user = $this->get_user()) {
            ...
            if ($user->id > 0 && is_mnet_remote_user($user)) {
        ...
        if ($caneditlockedfields) {
            foreach ($customfields as $customfield) {
                // Remove custom fields that are not in the form (not accessible for the user).
                if (!$mform->elementExists($customfield)) {
                    continue;
                }
    ...
    public function process_dynamic_submission() {
        ...
        if (!$data->id) {
            ...
            if ($createpassword || empty($data->newpassword)) {
                $data->password = '';
            } else {
        ...
        // Reload from db.
        $data = $DB->get_record('user', ['id' => $data->id]);
    ...
    public function set_data_for_dynamic_submission(): void {
        ...
        if ($data = $this->get_user()) {
            ...
            if (!empty($data->picture) && $hasuploadedpicture) {
                $data->currentpicture = $OUTPUT->user_picture($data, ['courseid' => SITEID, 'size' => 64]);
            } else {
