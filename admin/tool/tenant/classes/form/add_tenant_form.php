...
class add_tenant_form extends modal_form {
    ...
    public function definition() {
        ...
        if ($idnumber = $this->get_tenant()->get('idnumber')) {
            ...
            $urlidnumber .= $OUTPUT->render_from_template('tool_wp/copy_to_clipboard', ['text' => $urlidnumber]);
        } else {
            $urlidnumber .= '{IDNUMBER}';
        }
        ...
        $mform->setDefault('loginurlid', 1);
        $mform->setDefault('loginurlidnumber', 1);
        ...
        if (get_config('tool_tenant', 'showtenantselector')) {
            ...
            $mform->addHelpButton('showinloginselector', 'showinloginselector', 'tool_tenant');
        }
    ...
    protected function get_tenant() : tenant {
        if ($this->tenant === null) {
            if (!empty($this->_ajaxformdata['id'])) {
                ...
                $this->tenant = $manager->get_tenant($this->_ajaxformdata['id']);
            } else {
    ...
    public function render() {
        global $PAGE;
        $PAGE->requires->js_call_amd('tool_wp/copy_to_clipboard');
