...
class add_tenant_form extends dynamic_form {
    ...
    public function definition() {
        ...
        // Login URLs.
        $wwwroot = tenancy::get_tenant_wwwroot($this->get_tenant()->get('id'));
        $objs = [];
        if (tenancy::has_unique_custom_wwwroot($this->get_tenant()->get('id'))) {
            $urlcustom = $wwwroot . $OUTPUT->render_from_template('tool_wp/copy_to_clipboard', ['text' => $wwwroot]);
            $objs[] = $mform->createElement('advcheckbox', 'usecustomdomain', '', $urlcustom, ['disabled' => true]);
            $mform->setDefault('usecustomdomain', 1);
        }
        $urlid = $wwwroot.'/?tenantid=';
    ...
    public function validation($tenant, $files) {
        ...
        if (!empty($tenant['showinloginselector']) && empty($tenant['useloginurlid'])
                && empty($tenant['useloginurlidnumber'])
                && !tenancy::has_unique_custom_wwwroot($this->get_tenant()->get('id'))) {
