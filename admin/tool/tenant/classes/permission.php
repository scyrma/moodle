...
class permission {
     ...
     *
     * @param int $tenantid Tenant id (if empty the current tenant id will be used)
     * @param bool $checkadvanced Check advanced theme settings permission
     * @return bool True if the user has permission to edit the themes of tenants, else false.
     */
    public static function can_edit_tenant_theme(int $tenantid = 0, bool $checkadvanced = false) : bool {
        $tenantid = $tenantid ?: tenancy::get_tenant_id();
        if (!self::is_tenant_active($tenantid) || sharedspace::is_shared_space($tenantid)) {
            return false;
        }
        ...
        if ($tenantid !== tenancy::get_tenant_id()) {
            return false;
        }

        // Check for advanced permission.
        if (has_capability('tool/tenant:managethemeadvanced', \context_system::instance())) {
            return true;
        }

        return has_capability('tool/tenant:managetheme', \context_system::instance()) && !$checkadvanced;
    }
    ...
    /**
     * Can the user edit the theme advanced settings for the specified tenant
     *
     * @param int $tenantid Tenant id (if empty the current tenant id will be used)
     * @return bool True if the user has permission to edit the themes of tenants, else false.
     */
    public static function can_edit_tenant_theme_advanced(int $tenantid = 0) : bool {
        return self::can_edit_tenant_theme($tenantid, true);
    }
    ...
    public static function can_view_tenant_details(int $tenantid): bool {
        return self::is_tenant_active($tenantid) && !sharedspace::is_shared_space($tenantid) &&
            self::can_edit_tenant_theme($tenantid);
    ...
    /**
     * Can user edit tenant mobile settings
     *
     * @param int $tenantid
     * @return bool
     */
    public static function can_edit_tenant_mobile_settings(int $tenantid = 0): bool {
        global $CFG;
        $tenantid = $tenantid ?: tenancy::get_tenant_id();
        return !empty($CFG->enablemobilewebservice) &&
            has_capability('tool/tenant:mobileconfig', \context_system::instance()) &&
            !sharedspace::is_shared_space($tenantid) &&
            self::can_access_tenant($tenantid);
    }

    /**
     * require_can_edit_tenant_mobile_settings
     *
     * @param int $tenantid
     * @throws \required_capability_exception
     */
    public static function require_can_edit_tenant_mobile_settings(int $tenantid = 0): void {
        if (!self::can_edit_tenant_mobile_settings($tenantid)) {
            throw new \required_capability_exception(\context_system::instance(),
                'tool/tenant:mobileconfig', 'nopermissions', 'error');
        }
    }
    ...
    /**
     * Can this user view the tenant column
     *
     * @return bool
     */
    public static function can_show_tenant_report_column(): bool {
        // Checks that user can switch tenant and site is multitenant.
        return self::can_switch_tenant() && tenancy::is_site_multi_tenant();
    }

    /**
     * User should see the link to /admin/tool/tenant/details.php in wp launcher and be able to access it
     *
     * @param int $tenantid
     * @return bool
     */
    public static function can_access_tenant_settings_page(int $tenantid = 0): bool {
        $tenantid = $tenantid ?: tenancy::get_tenant_id();
        return self::can_edit_tenant_theme() ||
