...
class tenancy {
     ...
     *
     * @param int|null $userid userid, if omitted current user
     * @return int
     */
    public static function get_tenant_id(?int $userid = null) : int {
        global $USER, $CFG;

        // Check if tenantid has been set in the webservice override. Checking for WS_SERVER does not seem to work here.
        $cache = self::get_cache_instance();
        if ($tenantid = $cache->get('tenantid')) {
            return $tenantid;
        }
        ...
        if ($userid === null && (!isloggedin() || isguestuser())) {
            ...
            if ($tenantid = (array_flip($CFG->tool_tenant_wwwroot ?? [])[$CFG->wwwroot] ?? 0)) {
                return $tenantid;
            }
            return self::get_default_tenant_id();
        }
     ...
     * Example of usage:
     *
     * $ualias = \core_reportbuilder\local\helpers\database::generate_alias();
    ...
    public static function get_users_sql(string $usertablealias = 'u', int $tenantid = 0) : array {
        ...
        if (count($tenants) > 1) {
            ...
            $tu = database::generate_alias();
            $t = database::generate_alias();
     ...
     * @return array
     */
    public static function tenants_search_sql($search, $t = 't', $searchanywhere = true, array $extrafields = [],
    ...
    public static function get_theme_scss(int $tenantid) : string {
        debugging(
            'The function ' . __FUNCTION__ . '() is deprecated, please use manager::get_theme_scss()',
            DEBUG_DEVELOPER
        );
     ...
     *
     * @param int $tenantid
     * @param moodle_url|null $redirecturl Optional parameter to redirect after switching, defaults to current page URL
     * @return moodle_url
     */
    public static function get_tenantswitch_url(int $tenantid, ?moodle_url $redirecturl = null): moodle_url {
        global $PAGE;

        if ($redirecturl === null) {
            // If PAGE->url is an ajax request, then we need to redirect to a root path, otherwise continue in current PAGE->url.
            $redirecturl = $PAGE->url->compare(new moodle_url('/lib/ajax/service.php'), URL_MATCH_BASE) ?
                new moodle_url('/') : new moodle_url($PAGE->url);
        }

        return new moodle_url('/admin/tool/tenant/switchtenant.php', [
            'switchtenantid' => $tenantid,
            'redirecturl' => $redirecturl->out_as_local_url(false),
            'sesskey' => sesskey(),
        ]);
    }
    ...
    public static function tenant_menu($renderer): string {
        ...
        if (self::is_site_multi_tenant()) {
            ...
            $tenant = new tenant(0, $tenants[$currenttenantid]);
            $tenantname = $tenant->get_formatted_name();
            $url = self::get_tenantswitch_url($currenttenantid);
            $menu = ['haschildren' => [
                'url' => $url->out(false),
                'text' => \html_writer::span($tenantname, 'current-tenant-name d-inline-block', ['title' => $tenantname]),
                'children' => [],
            ], ];
            ...
            if (count($tenants) > self::MAX_TENANTS) {
                ...
                $menu['text'] = \html_writer::span($tenantname, 'current-tenant-name d-inline-block', ['title' => $tenantname]) .
                    $renderer->pix_icon('arrow-right-circle', get_string('switchtenant', 'tool_tenant'), 'tool_tenant');
    ...
    public static function load_tenant_config_from_tenant_url(string $functionname, array $parameters): void {
        if (isloggedin() && !isguestuser() && WS_SERVER) {
            // For Web Services the after_config hook is not executed and the tenant config is not
            // set for the current user. Let's set it here.
        ...
        if (($functionname === 'tool_mobile_get_public_config'
            ...
            if ($tenantid > 0) {
                ...
                // Store tenantid in an ad-hoc request cache to use it in get_tenant_id() method.
                $cache = self::get_cache_instance();
    ...
    /**
     * Returns wwwroot for this tenant
     *
     * @param int $tenantid optional tenant id (current tenant if not specified)
     * @return string
     */
    public static function get_tenant_wwwroot(int $tenantid = 0): string {
        global $CFG;
        $tenantid = $tenantid ?: self::get_tenant_id();
        return $CFG->tool_tenant_wwwroot[$tenantid] ?? $CFG->tool_tenant_wwwroot[0] ?? $CFG->wwwroot;
    }

    /**
     * Is there a custom domain (wwwroot) defined that is used only for this tenant
     *
     * @param int $tenantid
     * @return bool
     */
    public static function has_unique_custom_wwwroot(int $tenantid = 0): bool {
        global $CFG;
        if (empty($CFG->tool_tenant_wwwroot) || !self::is_site_multi_tenant()) {
            return false;
        }
        $cnt = 0;
        $url = self::get_tenant_wwwroot($tenantid);
        foreach (self::get_tenants() as $tenant) {
            $tenantwwroot = $CFG->tool_tenant_wwwroot[$tenant->id] ?? $CFG->tool_tenant_wwwroot[0] ?? $CFG->wwwroot;
            $cnt += (int)($tenantwwroot === $url);
            if ($cnt > 1) {
                // Early exit, no need to loop through the rest of tenants.
                return false;
            }
        }
        return $cnt == 1;
    ...
    /**
     * Returns true if current user can restore user enrolment
     *
     * @param int $userid user to restore enrolment for
     * @param string $enrolmethod enrolment method (not used yet)
     * @param int $roleid role id (not used yet)
     * @param int $courseid course id (not used yet)
     * @return bool
     */
    public static function can_restore_user_enrolment(int $userid, string $enrolmethod, int $roleid, int $courseid): bool {
        return !self::is_user_hidden_by_tenancy($userid);
    }

    /**
     * Returns the cache used in the web services to return the tenant id
     *
     * @return \cache
     */
    private static function get_cache_instance(): \cache {
        return \cache::make_from_params(\cache_store::MODE_REQUEST, 'tool_tenant', 'wstenantid');
    }
}
