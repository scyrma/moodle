...
class tenancy {
    ...
    public static function get_tenants() : array {
        ...
        if (!($tenants = $cache->get('list'))) {
            if (get_config('tool_tenant', 'version') >= 2020090202) {
                ...
                $tenants = $DB->get_records('tool_tenant', ['archived' => 0],
                    ...
                    'siteshortname, useloginurlid, useloginurlidnumber, timemodified, parentid, path');
            } else {
                ...
                $tenants = $DB->get_records('tool_tenant', ['archived' => 0],
                    'isdefault DESC, sortorder, id', 'id, name, idnumber, isdefault, sitename, categoryid, '.
                    'siteshortname, useloginurlid, useloginurlidnumber, timemodified, null AS parentid');
            }
            $first = reset($tenants);
    ...
    public static function get_users_subquery(bool $canseeall = true, bool $andpostfix = true,
        ...
        if (!$withsubtenants && $defaulttenantid != $tenantid) {
            ...
            if ($includedefault) {
                $query = "( $query OR {$useridfield} NOT IN (SELECT userid FROM {tool_tenant_user}))";
            }
        }
    ...
    public static function tenant_menu($renderer): string {
        ...
        if (self::is_site_multi_tenant()) {
            ...
            } else if (sharedspace::show_shared_space_in_switch_operations()) {
            ...
            // Only load JS for shared space.
            if (is_null(sharedspace::get_shared_space_id())) {
            ...
            if (count($tenants) > self::MAX_TENANTS) {
                $sharedspaceenabled = ($sharedid > 0) ? true : false;
                $PAGE->requires->js_call_amd('tool_tenant/switch', 'init', [$url->out_omit_querystring(false),
                    $sharedspaceenabled]);
