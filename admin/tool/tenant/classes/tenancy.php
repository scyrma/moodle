...
class tenancy {
    ...
    public static function get_tenants() : array {
        ...
        if (!($tenants = $cache->get('list'))) {
            if (get_config('tool_tenant', 'version') >= 2021030402) {
                ...
                $tenants = $DB->get_records('tool_tenant', ['archived' => 0],
                    ...
                    'siteshortname, useloginurlid, useloginurlidnumber, showinloginselector, timemodified, parentid, path');
            } else {
                ...
                $tenants = $DB->get_records('tool_tenant', ['archived' => 0],
                    'isdefault DESC, sortorder, id', 'id, name, idnumber, isdefault, sitename, categoryid, '.
                    'siteshortname, useloginurlid, useloginurlidnumber, 0 as showinloginselector, timemodified, null AS parentid');
            }
            $first = reset($tenants);
    ...
    public static function tenant_menu($renderer): string {
        ...
        if (self::is_site_multi_tenant()) {
            ...
            // If shared space isn't enabled (NULL), and user can create a new tenant load JS module to enable shared space.
            if ($sharedid === null && $cancreatetenant) {
