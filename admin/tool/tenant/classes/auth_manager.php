...
class auth_manager {
     ...
     *
     * @param string $plugin Name of the authentication plugin
     * @param string $settingname Setting to be used
     * @return bool
     */
    public static function can_override_plugin_setting($plugin, string $settingname): bool {
        return ($plugin === 'auth_email' || $plugin === 'auth_manual' || $plugin === 'auth_oauth2')
            && preg_match('/^field_lock_/', $settingname);
    }
     ...
     */
    public static function auth_oauth2_complete_login_hook(\core\oauth2\client $client, array $userinfo, $linkedlogin): void {
        global $DB, $CFG, $FULLME, $SESSION;
        local\auth\oauth2\manager::require_issuer_available($client->get_issuer()->get('id'));
        ...
        if ($existingusertenantid != $tenantid) {
            ...
            if (!$wp) {
                manager::set_tenant_cookie($existingusertenantid);
    ...
    public static function detect_tenant_on_auth_page(): int {
        ...
        if ($CFG->wwwroot.'/auth/oauth2/login.php' === strip_querystring($FULLME)) {
            ...
            $issuerid = optional_param('id', 0, PARAM_INT);
            if (!$issuerid || !\tool_tenant\local\auth\oauth2\manager::issuer_available($issuerid)) {
