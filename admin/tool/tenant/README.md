...
## Plugins that are part of Workplace package or plugins defining dependency on tool_tenant

### Get current user's or another user's tenant:

    \tool_tenant\tenancy::get_tenant_id($userid = 0)

### Check if current user can access information across all tenants:

    \tool_tenant\permission::can_switch_tenant()

This method should be used in combination with other relevant capabilities check.
