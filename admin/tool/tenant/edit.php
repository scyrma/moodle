...
if (!\tool_tenant\permission::can_browse_users($id) && !\tool_tenant\permission::can_edit_tenant_theme($id)) {
    // User is allowed to view users or edit themes somewhere, just not in this tenant. Try to redirect to tenants page (for admin)
    // or throw an exception.
    if ($canviewalltenants) {
        redirect(\tool_tenant\manager::get_base_url());
    }
...
echo $OUTPUT->render_from_template('tool_wp/tabs',
    (new \tool_tenant\output\users(['isdefault' => $tenant->get('isdefault'), 'tenantid' => $id]))->export_for_template($OUTPUT));
