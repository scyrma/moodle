...
function tool_tenant_inplace_editable($itemtype, $itemid, $newvalue) {
    ...
    if ($itemtype === 'tenant_name') {
        ...
        permission::require_can_edit_tenant($itemid);
...
function tool_tenant_after_config() {
    ...
    if ($CFG->wwwroot.'/'.$CFG->admin.'/settings.php' === strip_querystring($FULLME) ||
            ...
            $CFG->wwwroot.'/message/output/airnotifier/requestaccesskey.php' === strip_querystring($FULLME)) {
    ...
    } else if ($tenantid != manager::get_tenant_cookie()) {
        manager::set_tenant_cookie($tenantid);
    }
    ...
    if ($CFG->wwwroot.'/user/editadvanced.php' === strip_querystring($FULLME)) {
        ...
        if ($id == -1 && !permission::check_quotas_to_add_users($tenantid, 1)) {
...
function tool_tenant_post_signup_requests(stdClass $user) {
    manager::preallocate_new_user($user, \tool_tenant\tenancy::get_tenant_id(), 'tool_tenant', 'Sign up');
}
...
function tool_tenant_get_fontawesome_icon_map() {
    return [
        ...
        'tool_tenant:arrow-right-circle' => 'fa-arrow-circle-o-right',
    ];
}
 ...
 * @return string
 */
function tool_tenant_before_standard_top_of_body_html() {
    global $OUTPUT, $PAGE;
    if ((!isloggedin() || isguestuser()) && in_array($PAGE->pagetype, ['login-index', 'login-signup'])) {
        $tenantmanager = new manager();
 ...
 */
function tool_tenant_theme_workplace_menu_items(): array {
    global $OUTPUT;
    ...
    if (permission::can_switch_tenant()) {
        // All users.
        $menuitems[] = [
            'url' => manager::get_allusers_url(),
            'name' => get_string('allusers', 'tool_tenant'),
            'imageurl' => $OUTPUT->image_url('users', 'tool_tenant')->out(false),
            'isglobal' => true,
        ];
        // All tenants.
        $menuitems[] = [
            'url' => new moodle_url("/admin/tool/tenant/index.php"),
            'name' => get_string('alltenants', 'tool_tenant'),
            'imageurl' => $OUTPUT->image_url('icon', 'tool_tenant')->out(false),
            'isglobal' => true,
        ];
    }
    // Users.
    if (permission::can_browse_users($tenantid)) {
        $menuitems[] = [
            'url' => manager::get_users_url(),
            'name' => get_string('users'),
            'imageurl' => $OUTPUT->image_url('usermanagement', 'tool_tenant')->out(false),
        ];
    }
    // Appearance.
    if (permission::can_access_tenant_settings_page($tenantid)) {
        $menuitems[] = [
            'url' => manager::get_details_url(),
            'name' => get_string('appearance'),
            'imageurl' => $OUTPUT->image_url('tenantappearance', 'tool_tenant')->out(false),
        ];
    }
...
/**
 * Add html attribute with tenant data.
 *
 * This attributes will be added to the <html> tag when htmlattributes() method of core_renderer is called.
 *
 * @return array
 */
function tool_tenant_add_htmlattributes(): array {
    // Get current tenantid.
    $tenantid = tenancy::get_tenant_id();

    // Create array of elements to be returned as HTML attributes.
    $tenantdata = ['data-tenantid' => $tenantid];

    // If idnumber is defined, then return it too.
    if (tenancy::get_tenants()[$tenantid]->idnumber !== '') {
        $tenantdata['data-tenantidnumber'] = tenancy::get_tenants()[$tenantid]->idnumber;
    }
