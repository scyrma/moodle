 ...
 * @license    Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class tool_tenant_privacy_provider_testcase extends advanced_testcase {
    ...
    public function test_get_contexts_for_userid() {
        ...
        $manager->allocate_user($user2->id, $tenant1->get('id'), 'tool_tenant', 'manual');

        // Create another tenant with user 1.
        $tenant2 = $manager->create_tenant_quick();
        $tenant2 = $manager->update_tenant($tenant2->get('id'), (object) ['categoryid' => $category2->id]);
    ...
    public function test_export_user_data() {
        ...
        $manager->allocate_user($user2->id, $tenant1->get('id'), 'tool_tenant', 'manual');
        $manager->allocate_user($user3->id, $tenant1->get('id'), 'tool_tenant', 'manual');
        $manager->allocate_user($user4->id, $tenant2->get('id'), 'tool_tenant', 'manual');
        ...
        $data = writer::with_context($context)->get_data([
            ...
            $tenant1->get('name') . $tenant1->get('id')
        ]);

        // User 2 has been assigned to a tenant and only has one entry.
        $this->assertEquals($tenant1->get('name'), $data->tenant_name);
        ...
        $data = writer::with_context($context)->get_data([
            get_string('tenants', 'tool_tenant'),
            $tenant2->get('name') . $tenant2->get('id')
        ]);

        // And assigned one user to this tenant.
        $this->assertEquals($tenant2->get('name'), $data->tenant_name);
        $this->assertCount(1, $data->tenant_users);
    }
