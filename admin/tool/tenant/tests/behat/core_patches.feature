...
Feature: Test multitenancy core patches
  ...
  Scenario: User inside a tenant can only award badges to users from the same tenant
    ...
    And I open the action menu in "Site Badge 1" "table_row"
  ...
  Scenario: Course other users list hides users from other tenants and some multi-tenancy roles
    ...
    And I set the field "Participants tertiary navigation" to "Other users"
    Then I should see "Tenantadmin 1"
    And I should not see "Tenantadmin 2"
    And I should not see "Tenant user"
  ...
  Scenario Outline: Ensure custom report audience observes current tenant
    Given "2" tenants exist with "2" users and "0" courses in each
    And the following "tool_tenant > reports" exist:
      | tenant   | name      | source                                   |
      | <tenant> | My report | core_user\reportbuilder\datasource\users |
    And the following "core_reportbuilder > Audiences" exist:
      | report    | configdata |
      | My report |            |
    And I am on the "My report" "reportbuilder > Editor" page logged in as "<user>"
    When I click on the "Access" dynamic tab
    Then I should see "<tenantuser>" in the "reportbuilder-table" "table"
    And I should not see "<nontenantuser>" in the "reportbuilder-table" "table"
    Examples:
      | tenant  | user         | tenantuser | nontenantuser |
      | Tenant1 | tenantadmin1 | User 11    | User 21       |
      | Tenant2 | tenantadmin2 | User 21    | User 11       |
