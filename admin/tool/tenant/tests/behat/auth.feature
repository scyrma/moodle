...
Feature: Test authentication in mutli-tenancy
  ...
  Scenario: When new user signs up and the account is created in the correct tenant
    Given the following "tool_tenant > tenants" exist:
      ...
      | Tenant1 | SITENAME1    | SITENAME1     |
      | Tenant2 | Wrong tenant | Wrong tenant  |
    ...
    Then I should see "It contains easy instructions to complete your registration."
    And I should see "SITENAME1"
  ...
  Scenario: When admin edits another user the auth settings are set for the user's tenant
    ...
    And I navigate to "Authentication" in current page administration
  ...
  Scenario: Capability to config authentication can be removed from Tenant administrator role
    ...
    And I navigate to "Users" in workplace launcher
    And "Users" "link" should exist in current page administration
    And "Authentication" "link" should not exist in current page administration
    And I log out
  ...
  Scenario: Oauth2 sign in from a wrong tenant
    ...
    And I set the field "Last name" to "Jones"
...
#    And I follow "Home"
  ...
  Scenario: Tenant administrator can change settings in the multi-tenant manual auth
    ...
    And I press "Edit user account" action in the "User 11" report row
    # Tenant admin can ALWAYS edit locked fields.
    Then "input[name=firstname][disabled]" "css_element" should not exist in the "Edit user 'User 11'" "dialogue"
    And "input[name=email][disabled]" "css_element" should not exist in the "Edit user 'User 11'" "dialogue"
    And "input[name=lastname][disabled]" "css_element" should not exist in the "Edit user 'User 11'" "dialogue"
  ...
  Scenario: Create new account with user limit
    ...
    And "Create new account" "link" should not exist
  ...
  Scenario Outline: Users with same email created in different tenants can sign in
    ...
    And I follow "Log in"
    And I set the field "Username or email" to "<useremail>"
  ...
  @_file_upload
  Scenario: Create user account with appearance images setted
    ...
    And I upload "lib/tests/fixtures/gd-logo.png" file to "Header logo" filemanager
    And I press "Save changes"
    # Reload the page first, because xpath node is not visible
    # TODO MDL-75067 remove the reload workaround when the issue is fixed
    And I reload the page
    And I upload "lib/tests/fixtures/gd-logo.png" file to "Login logo" filemanager
    And I press "Save changes"
    # Reload the page first, because xpath node is not visible
    # TODO MDL-75067 remove the reload workaround when the issue is fixed
    And I reload the page
    And I upload "lib/tests/fixtures/gd-logo.png" file to "Tenant selector logo" filemanager
    And I press "Save changes"
    # Reload the page first, because xpath node is not visible
    # TODO MDL-75067 remove the reload workaround when the issue is fixed
    And I reload the page
    And I upload "lib/tests/fixtures/gd-logo.png" file to "Login background image" filemanager
    And I press "Save changes"
    # Reload the page first, because xpath node is not visible
    # TODO MDL-75067 remove the reload workaround when the issue is fixed
    And I reload the page
    And I upload "lib/tests/fixtures/gd-logo.png" file to "Favicon" filemanager
    And I press "Save changes"
    And I log out
    And I am on homepage for tenant "Tenant1"
    And I follow "Log in"
