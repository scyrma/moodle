...
Feature: Testing functionality of auth_oauth2
  ...
  Scenario: OAuth2 issuer without email verification
    ...
    And I press "Update profile"
    And I should see "Mary Jones"
    And I should see "Preferences" in the "[role='main'] h2" "css_element"
  ...
  Scenario: OAuth2 login with a linked account without email confirmation
    ...
    # We need to log in as user1 since the automatic login in OAuth linked account confirmation was removed in order to prevent security issues.
    And I follow "Log in"
    And I follow "Moodle test OAuth2"
    Then I should see "You are logged in as Student 1"
