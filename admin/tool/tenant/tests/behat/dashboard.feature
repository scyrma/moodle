...
Feature: Tenant dashboard management
  ...
  Scenario: Manage tenant dashboards as admin
    ...
    And I click on "Appearance" "link" in the ".secondary-navigation .moremenu.navigation" "css_element"
    ...
    And I should see "You have no badges to display" in the "Latest badges" "block"
  ...
  Scenario: Manage own dashboard as tenantadmin
    ...
    And I select "Site administration" from primary navigation
    And I click on "Appearance" "link" in the "region-main" "region"
    And I should see "Category: Appearance"
  ...
  Scenario: Use a linked tenant dashboard
    ...
    And I should see "You have no badges to display" in the "Latest badges" "block"
    And "Tags" "block" should not exist
    # Customize the user dashboard with "Tags" block again.
    And I switch editing mode on
    And I add the "Tags" block
    And "Tags" "block" should exist
    And I log out
  ...
  Scenario: Reset dashboard for all users to un-linked tenant dashboard
    ...
    And I navigate to "Appearance" in workplace launcher
    And I navigate to "Dashboard" in current page administration
    Then I should see "Status: Linked to 'Default site dashboard page'"
    And I press "Create personalised dashboard..."
    And I click on "Proceed" "button" in the "Confirmation" "dialogue"
    And I should see "Status: Un-linked"
  ...
  Scenario: Users without permission cannot see dashboard tab
    Given the following "permission overrides" exist:
      | capability                   | permission | role                | contextlevel | reference |
      | tool/tenant:managedashboard  | Prevent    | tool_tenant_admin   | System       |           |
    # Check user without managedashboard capability cannot see dashboard tab.
    When I log in as "tenantadmin1"
    And I navigate to "Appearance" in workplace launcher
    Then I should not see "Dashboard" in the "[role=tablist]" "css_element"
