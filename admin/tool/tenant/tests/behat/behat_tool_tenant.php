...
class behat_tool_tenant extends behat_base {
    ...
    public function i_switch_to_tehant($tenantname) {
        ...
        if ($this->getSession()->getPage()->find('css', '.tenantswitch .dropdown-toggle')) {
            ...
            $this->execute('behat_tool_wp::i_open_the_autocomplete_suggestions_list_in_the_dialog');
            $this->execute('behat_general::i_click_on_in_the',
                [$tenantname, 'text', '.modal-dialog .form-autocomplete-suggestions', 'css_element']);
            $this->execute('behat_forms::i_press_key_in_the_field', ['27', 'Select tenant']);
            $this->execute('behat_tool_wp::i_press_in_the_modal_form_dialogue', 'Switch tenant');
        }
    }
