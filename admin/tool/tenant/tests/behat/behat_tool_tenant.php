...
class behat_tool_tenant extends behat_base {
    ...
    public function tenants_exist_with_users_and_courses_in_each($tenants, $users, $courses) {
        ...
        $this->execute("behat_data_generators::the_following_exist", ["categories", new TableNode($categories)]);
        ...
        $this->execute("behat_data_generators::the_following_exist", ["users", new TableNode($usersdata)]);
    ...
    public function i_am_on_homepage_for_tenant($tenantname) {
        ...
        $url = new moodle_url('/', ['redirect' => 0, 'tenantid' => $tenantid]);
