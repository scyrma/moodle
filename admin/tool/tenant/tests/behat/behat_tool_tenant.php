...
class behat_tool_tenant extends behat_base {
    ...
    protected function create_course($categoryid, $fullname, $shortname) {
        ...
        $data = ['backupfile' => $CFG->dirroot . '/admin/tool/tenant/tests/fixtures/backup.mbz',
    ...
    public function i_switch_to_tehant($tenantname) {
        // Use the drawer tenant switcher for mobile.
        $drawer = $this->getSession()->getPage()->find('css', '.drawer-primary');
        $containerclass = ($drawer && $drawer->isVisible()) ? '.drawer-primary ' : '';

        $switchdropdown = $this->getSession()->getPage()->find('css', $containerclass . '.tenantswitch .dropdown-toggle');
        if ($switchdropdown && $switchdropdown->isVisible()) {
            $this->execute('behat_general::i_click_on',
                [$containerclass . '.tenantswitch .dropdown-toggle', 'css_element']);
            $this->execute('behat_general::i_click_on_in_the',
                [$tenantname, 'link', $containerclass . '.tenantswitch .dropdown-menu.show', 'css_element']);
        } else {
            $this->execute('behat_general::i_click_on',
                [$containerclass . '.tenantswitch .nav-link', 'css_element']);
...
,
    ...
    /**
     * Checks for specific text in the <head>
     *
     * @Then /^html head should contain "(?P<text>(?:[^"]|\\")*)"$/
     * @param string $text
     * @throws ExpectationException
     */
    public function html_head_should_cointain($text) {
        $head = $this->find('css', 'head');
        $html = $head->getHtml();
        if (strpos($html, $text) === false) {
            throw new ExpectationException('Text not found in the html head', $this->getSession());
        }
    }
     ...
     * Checks for specific text in the <head>
     *
     * @Then /^mobile manifest should( not)? contain "(?P<text>(?:[^"]|\\")*)"$/
     * @param string $not
     * @param string $text
     * @throws ExpectationException
     */
    public function mobile_manifest_should_cointain($not, $text) {
        $link = $this->find('css', "head link[rel='manifest'][href]");
        if (!$link) {
            throw new ExpectationException('Manifest tag was not found in html head', $this->getSession());
        }
        $manifesturl = $link->getAttribute('href');

        // Download the manifest.
        $session = $this->getSession()->getCookie('MoodleSession');
        $content = download_file_content($manifesturl, ['Cookie' => 'MoodleSession=' . $session]);

        // Compare with $text.
        $textmatches = (strpos($content, $text) !== false);
        $textshouldmatch = !strlen($not);
        if (!$textmatches && $textshouldmatch) {
            throw new ExpectationException('Text not found in the html head: '.$content, $this->getSession());
        }
        if ($textmatches && !$textshouldmatch) {
            throw new ExpectationException('Text found in the html head: '.$content, $this->getSession());
        }
    }

    /**
     * I open the autocomplete suggestions list for legacy custom reports in the rbreport block
     *
     * @Given /^I open the autocomplete suggestions list for legacy custom reports in the rbreport block$/
     */
    public function i_open_the_autocomplete_suggestions_list_for_legacy_custom_reports() {
        if (!core_component::get_component_directory('block_rbreport')) {
            throw new ExpectationException('The rbreport block is not installed', $this->getSession());
        }
