...
Feature: Manage all users
  ...
  Scenario: Viewing all users as a user who can edit users but can not add new ones
    ...
    And I press "Edit user account" action in the "User 22" report row
  ...
  Scenario Outline: As a site admin I can filter users by tenant
    ...
    And I set the following fields in the "Tenant name" "core_reportbuilder > Filter" to these values:
      ...
      | Tenant name value    | <tenant>    |
    And I click on "Apply" "button" in the "[data-region='report-filters']" "css_element"
    Then "<tenant>" "text" should exist in the "<user>" "table_row"
    And I should not see "<userothertenant>" in the "reportbuilder-table" "table"
    And I log out
    Examples:
  ...
  Scenario: As a site admin I can filter users by custom profile fields
    Given the following "custom profile fields" exist:
      ...
      | text     | salutation | Salutation | 1      | 2       |
    # User profile fields need to be set a user identity to be shown in the users system report.
    And the following config values are set as admin:
      | showuseridentity | profile_field_salutation |
    # More space is needed to test the field after adding a new filter.
    And I change window size to "large"
    When I log in as "admin"
    And I navigate to "Users > Accounts > All users" in site administration
    And I press "Edit user account" action in the "User 11" report row
    ...
    And I press "Save"
    And I click on "Filters" "button"
    And I set the following fields in the "Salutation" "core_reportbuilder > Filter" to these values:
      | Salutation operator | Contains |
      | Salutation value    | Mister    |
    And I click on "Apply" "button" in the "[data-region='report-filters']" "css_element"
  ...
  Scenario Outline: Perform bulk actions from the all users list in shared space
    ...
    And I click on "Select user 'User 11'" "checkbox" in the "reportbuilder-table" "table"
    And I click on "Select user 'User 21'" "checkbox" in the "reportbuilder-table" "table"
