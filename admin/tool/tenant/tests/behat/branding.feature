...
Feature: Test tenant branding settings
  ...
  Scenario: Admin can force contact support settings for individual tenants
    Given the following config values are set as admin:
      | supportname         | Alcyone             |
      | supportemail        | alcyone@example.com |
      | supportpage         | http://example.com  |
      | supportavailability | 1                   |
    When I log in as "admin"
    And I navigate to "Server > Support contact" in site administration
    And I click on "Force for all tenants" "text" in the "Support name" "tool_wp > Row"
    And I click on "Force for all tenants" "text" in the "Support email" "tool_wp > Row"
    And I press "Save changes"
    And I navigate to "All tenants" in workplace launcher
    And I click on "Tenant1" "link" in the "Tenant1" "tool_wp > Table tree node"
    And I navigate to "Branding" in current page administration
    # Assert not locked fields are visible.
    And I should see "Link to 'Contact site support'"
    And I should see "Support availability"
    # Assert locked fields are not visible.
    And I should not see "Support name"
    And I should not see "Support email"
    # Assert there are no errors during form validation and submission.
    And I click on "Override" "radio" in the "[data-groupname=\"supportavailabilityformgroup\"]" "css_element"
    And I set the field "supportavailabilityvalue" to "Disabled"
    And I press "Save changes"
  ...
  Scenario: Invalid Custom SCSS value
    When I log in as "admin"
    And I navigate to "Appearance" in workplace launcher
    And I click on "Show more..." "link" in the "region-main" "region"
    And I set the following fields to these values:
      | Custom SCSS | h3 { color: black; |
    And I press "Save changes"
    Then I should see "SCSS code is not valid, fails with: unclosed block: failed at `` (stdin) on line 2, at column 0"
