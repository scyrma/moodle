 ...
 *
 * @package    tool_tenant
 * @covers     \tool_tenant\hierarchy
 * @copyright  2020 Moodle Pty Ltd <support@moodle.com>
 * @author     2020 Marina Glancy
 * @license    Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class hierarchy_test extends advanced_testcase {
    ...
    public function test_get_users_subquery() {
        ...
        $user71 = $this->generator->create_user(['tenantid' => $tenant7->id]);

        // Archive tenant 7, user 71 should be considered in the default tenant from now on.
        (new \tool_tenant\manager())->archive_tenant($tenant7->id);
        ...
        // The user that belongs to tenant 7 still has a record that shows that they are in the tenant7.
        $this->assertTrue($DB->record_exists('tool_tenant_user', ['userid' => $user71->id, 'tenantid' => $tenant7->id]));

        // Tests.
        // Users in tenant2 with subtenants.
        ...
        // Users in tenant2 without subtenants.
        $ualias = database::generate_alias();
        ...
        // Users in default tenant without subtenants.
        $ualias = database::generate_alias();
        $where = \tool_tenant\tenancy::get_users_subquery(false, false, "{$ualias}.id", $defaulttenantid, false);
        $sql = "SELECT {$ualias}.id FROM {user} {$ualias} WHERE $where ORDER BY id";
        $users = $DB->get_fieldset_sql($sql, []);
        $this->assertEquals([get_admin()->id, $user01->id, $user02->id, $user71->id], $users);

        // Users in default tenant with subtenants.
        $ualias = database::generate_alias();
        $where = \tool_tenant\tenancy::get_users_subquery(false, false, "{$ualias}.id", $defaulttenantid);
        $sql = "SELECT {$ualias}.id FROM {user} {$ualias} WHERE $where ORDER BY id";
        $users = $DB->get_fieldset_sql($sql, []);
        $this->assertEquals([get_admin()->id, $user01->id, $user02->id, $user61->id, $user71->id], $users);
        ...
        // Users in tenant1 with subtenants (also includes all users in default tenant).
        $ualias = database::generate_alias();
        ...
        $this->assertEquals([get_admin()->id, $user01->id, $user02->id, $user11->id, $user21->id, $user31->id,
            $user41->id, $user51->id, $user61->id, $user71->id], $users);
