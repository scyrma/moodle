...
class manager_test extends advanced_testcase {
    ...
    /**
     * Archiving a tenant with tenant admins should remove tenant admin roles
     */
    public function test_archive_tenant_with_tenantadmins(): void {
        $manager = new \tool_tenant\manager();
        $defaulttenantid = tenancy::get_default_tenant_id();
        $tenantid = $this->generator->create_tenant()->id;

        // Create a user in the new tenant and assign tenant admin role.
        $user = $this->generator->create_user(['tenantid' => $tenantid]);
        $manager->assign_tenant_admin_role($tenantid, [$user->id]);

        $this->assertCount(0, $manager->get_tenant_admins($defaulttenantid));
        $this->assertCount(1, $manager->get_tenant_admins($tenantid));

        $manager->archive_tenant($tenantid);

        // Archiving a tenant should remove tenant admin role from the user.
        $this->assertCount(0, $manager->get_tenant_admins($defaulttenantid));
        $this->assertCount(0, $manager->get_tenant_admins($tenantid));
    }
    ...
    /**
     * Test for get_theme_scss.
     */
    public function test_get_theme_scss(): void {
        // Create tenantone with tenant colour and customcss.
        ...
        // Create tenanttwo with empty tenant colour and no customcss.
        $tenanttwo = $this->generator->create_tenant(
            ['cssconfig' => json_encode([
                'brand' => '',
            ]), ]
        );
        // Create tenantthree with invalid tenant colour and no customcss.
        $tenantthree = $this->generator->create_tenant(
            ['cssconfig' => json_encode([
                'brand' => '#bbbb',
            ]), ]
        );
        // Create tenantfour with tenant grayscale setting enabled.
        $tenantfour = $this->generator->create_tenant(
            ['cssconfig' => json_encode([
                'brand' => '#ffa14a',
                'brandgraytones' => 1,
            ]), ]
        ...
        // Check tenantone theme scss.
        $scss = $manager->get_theme_scss($tenantone->id);
        $this->assertStringContainsString('$somevarible: blue;', $scss);
        $this->assertStringContainsString('$brand: #2b2b2b;', $scss);

        // Check tenanttwo theme scss.
        $scss = $manager->get_theme_scss($tenanttwo->id);
        $this->assertStringNotContainsString('$somevarible', $scss);
        $this->assertStringNotContainsString('$brand', $scss);

        // Check tenantthree theme scss.
        $scss = $manager->get_theme_scss($tenantthree->id);
        $this->assertStringNotContainsString('$somevarible', $scss);
        $this->assertStringNotContainsString('$brand', $scss);

        // Check tenantfour theme scss.
        $scss = $manager->get_theme_scss($tenantfour->id);
        $this->assertStringContainsString('$brand: #ffa14a;', $scss);
        $this->assertStringContainsString('Generate our pure tone colour and it\'s greyscale.', $scss);
    }
