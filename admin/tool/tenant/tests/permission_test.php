...
class permission_test extends advanced_testcase {
    ...
    /**
     * Test for can_edit_tenant_theme
     */
    public function test_can_edit_tenant_theme(): void {
        [$tenant1, [$user11, $user12, $user13, $user14]] = $this->generator->create_tenant_and_users(4);
        $tenantadmin = $this->generator->create_user(['tenantid' => $tenant1->id, 'tenantadmin' => true]);

        $context = context_system::instance();
        $role1id = create_role('tenant manager role', 'tenantmanagerrole', 'tenant manager role');
        assign_capability('tool/tenant:manage', CAP_ALLOW, $role1id, $context->id);
        $role2id = create_role('tenant advanced branding role', 'tenantadvbranding', 'tenant advanced branding role');
        assign_capability('tool/tenant:managetheme', CAP_ALLOW, $role2id, $context->id);
        assign_capability('tool/tenant:managethemeadvanced', CAP_ALLOW, $role2id, $context->id);
        $role3id = create_role('tenant branding role', 'tenantbranding', 'tenant branding role');
        assign_capability('tool/tenant:managetheme', CAP_ALLOW, $role3id, $context->id);
        $role4id = create_role('tenant advanced branding only role', 'tenantadvbrandingonly', 'tenant advanced branding role');
        assign_capability('tool/tenant:managethemeadvanced', CAP_ALLOW, $role4id, $context->id);

        $this->getDataGenerator()->role_assign($role1id, $user11->id);
        $this->getDataGenerator()->role_assign($role2id, $user12->id);
        $this->getDataGenerator()->role_assign($role3id, $user13->id);
        $this->getDataGenerator()->role_assign($role4id, $user14->id);

        // Check default tenant admin capabilities.
        $this->setUser($tenantadmin);
        $this->assertTrue(permission::can_edit_tenant_theme());
        $this->assertFalse(permission::can_edit_tenant_theme($tenant1->id, true));

        // User12 has 'managetheme' and 'managethemeadvanced' capabilities.
        $this->setUser($user12);
        $this->assertTrue(permission::can_edit_tenant_theme());
        $this->assertTrue(permission::can_edit_tenant_theme($tenant1->id, true));

        // User13 has 'managetheme' capability but not 'managethemeadvanced'.
        $this->setUser($user13);
        $this->assertTrue(permission::can_edit_tenant_theme());
        $this->assertFalse(permission::can_edit_tenant_theme($tenant1->id, true));

        // User14 has 'managethemeadvanced' capability but not 'managetheme'.
        $this->setUser($user14);
        $this->assertTrue(permission::can_edit_tenant_theme());
        $this->assertTrue(permission::can_edit_tenant_theme($tenant1->id, true));

        // User11 has global 'manage' tenant capability.
        $this->setUser($user11);
        $this->assertTrue(permission::can_edit_tenant_theme());
        $this->assertTrue(permission::can_edit_tenant_theme($tenant1->id, true));
        ...
        $this->assertFalse(permission::can_edit_tenant_theme());
    }

    /**
     * Test for can_edit_tenant_theme_advanced
     */
    public function test_can_edit_tenant_theme_advanced(): void {
        [$tenant1, [$user11, $user12]] = $this->generator->create_tenant_and_users(2);

        $context = context_system::instance();
        $role1id = create_role('tenant advanced branding role', 'tenantadvbranding', 'tenant advanced branding role');
        assign_capability('tool/tenant:managetheme', CAP_ALLOW, $role1id, $context->id);
        $role2id = create_role('tenant branding role', 'tenantbranding', 'tenant branding role');
        assign_capability('tool/tenant:managethemeadvanced', CAP_ALLOW, $role2id, $context->id);

        $this->getDataGenerator()->role_assign($role1id, $user11->id);
        $this->getDataGenerator()->role_assign($role2id, $user12->id);

        // User11 has 'managetheme' capability but not 'managethemeadvanced'.
        $this->setUser($user11);
        $this->assertFalse(permission::can_edit_tenant_theme_advanced());

        // User12 has 'managethemeadvanced' capability.
        $this->setUser($user12);
        $this->assertTrue(permission::can_edit_tenant_theme_advanced());
    }
}
