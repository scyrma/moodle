...
class tenancy_test extends advanced_testcase {
    ...
    public function test_get_users_sql() {
        ...
        // For default tenant.
        $ualias = database::generate_alias();
        ...
        database::validate_params($params + $params2);
    ...
    public function test_get_users_subquery() {
        ...
        $defaulttenantid = \tool_tenant\tenancy::get_default_tenant_id();
        $othertenantid = $this->get_generator()->create_tenant()->id;
        $emptytenantid = $this->get_generator()->create_tenant()->id;
        $user1 = $this->getDataGenerator()->create_user();
        $user2 = $this->getDataGenerator()->create_user();
        $user3 = $this->getDataGenerator()->create_user();
        $user4 = $this->getDataGenerator()->create_user();
        // User1 is not specifically allocated.
        $DB->delete_records('tool_tenant_user', ['userid' => $user1->id]);
        // User2 is allocated to the default tenant.
     ...
     * Test for function get_users_sql()
     */
    public function test_get_users_subquery_deleted() {
        global $DB, $USER;
        $this->setAdminUser();
        $adminuserid = $USER->id;
        ...
        $defaulttenantid = \tool_tenant\tenancy::get_default_tenant_id();
        $othertenantid = $this->get_generator()->create_tenant()->id;
        $thirdtenantid = $this->get_generator()->create_tenant()->id; // To make sure site always remains multi-tenant.
        $user1 = $this->getDataGenerator()->create_user();
        $user2 = $this->getDataGenerator()->create_user();
        $user3 = $this->getDataGenerator()->create_user();
        $user4 = $this->getDataGenerator()->create_user();
        ...
        // User2 is allocated to the default tenant.
        $this->get_generator()->allocate_user($user2->id, $defaulttenantid);
        // User3 is allocated to the other tenant. Add a user to the third tenant as well.
        $this->get_generator()->allocate_user($user3->id, $othertenantid);
        $this->get_generator()->allocate_user($user4->id, $thirdtenantid);
        ...
        // For other tenant.
        $where = \tool_tenant\tenancy::get_users_subquery(false, false, "u.id", $othertenantid);
        $sql = "SELECT u.id FROM {user} u WHERE $where ORDER BY id";
        $users = $DB->get_fieldset_sql($sql, []);
        $this->assertEquals([$user3->id], $users);

        // Archive and delete other tenant.
        (new manager())->archive_tenant($othertenantid);
        (new manager())->delete_tenant($othertenantid);

        // All users are moved to the default tenant.
        $where = \tool_tenant\tenancy::get_users_subquery(false, false, "u.id");
        $sql = "SELECT u.id FROM {user} u WHERE " . $where . ' ORDER BY id';
        $users = $DB->get_fieldset_sql($sql, []);
        $this->assertEquals([$adminuserid, $user1->id, $user2->id, $user3->id], $users);

        $this->assertEquals($defaulttenantid, tenancy::get_tenant_id($user3->id));
        $this->assertEmpty($DB->get_record('tool_tenant_user', ['userid' => $user3->id]));
    }
    ...
    /**
     * Data provider for {@see test_report_limit_tenant_reached}
     *
     * @return array
     */
    public static function report_limit_tenant_reached_provider(): array {
        return [
            [0, 1, false],
            [1, 1, true],
            [2, 1, false],
    ...
    /**
     * Test test_report_limit_tenant_reached method to check tenant custom reports limit
     *
     * @param int $tooltenantcustomreportslimit
     * @param int $existingtenantreports
     * @param bool $expected
     * @dataProvider report_limit_tenant_reached_provider
     */
    public function test_report_limit_tenant_reached(int $tooltenantcustomreportslimit, int $existingtenantreports,
                                                     bool $expected): void {
        global $CFG;

        $this->resetAfterTest();

        for ($i = 1; $i <= $existingtenantreports; $i++) {
            $this->generator->create_report([
                'name' => 'Limited report '.$i,
                'source' => users::class,
            ]);
        }

        // Set current tenant custom report limit, and check whether the limit has been reached.
        $CFG->tool_tenant_customreportslimit = $tooltenantcustomreportslimit;
        $this->assertEquals($expected, (new \tool_tenant\manager())->report_tenant_limit_reached());
    }
}
