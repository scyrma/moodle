...
class tool_tenant_tenancy_testcase extends advanced_testcase {
    ...
    public function test_get_tenant_admins() {
        ...
        $this->assertEmpty($manager->get_tenant_admins($tenant->get('id')));
        // Assign admin roles to users 1 and 2.
        $manager->assign_tenant_admin_role($tenant->get('id'), [$user1->id, $user2->id]);
        $tenantadminids = $manager->get_tenant_admins($tenant->get('id'));
    ...
    /**
     * Test that category association is deleted when category is deleted
     */
    public function test_event_delete_category() {
        ...
        $category = $this->getDataGenerator()->create_category();
        $tenant = $manager->update_tenant($tenantid, (object) ['categoryid' => $category->id]);
        $manager->change_tenant_category($tenantid, $category->id);

        $this->assertEquals($category->id, $tenant->get('categoryid'));

        $category->delete_full(false);
        $tenant = $manager->get_tenant($tenantid);

        $this->assertEquals(0, $tenant->get('categoryid'));
    }
