...
/**
 * Tests for multi-tenancy support in block rbreport (tool_reportbuilder reports)
 *
 * @package     tool_tenant
 * @category    test
 ...
 */
class block_rbreport_tool_test extends \advanced_testcase {
    /**
     * Set up
     */
    protected function setUp(): void {
        if (!\core_component::get_component_directory('tool_reportbuilder')) {
            $this->markTestSkipped('tool_reportbuilder is not installed');
        }
    ...
    public function test_get_report_options_tool(): void {
        ...
        $options = block_rbreport::get_report_options_tool('my-index', $sitedefaultpage, new \moodle_url('/my/indexsys.php'));
        $expected = [$sharedreport->get_id() => $sharedreport->get_reportname()];
        $this->assertEquals($expected, $options);

        // Tenant dashboard.
        $options = block_rbreport::get_report_options_tool('my-index',
        ...
        $this->setUser($user1);
        // User1 dashboard.
        $options = block_rbreport::get_report_options_tool('my-index', $user1page, new \moodle_url('/my/index.php'));
        $this->assertEmpty($options);

        $this->setUser($user2);
        // User2 dashboard.
        $options = block_rbreport::get_report_options_tool('my-index', $user2page, new \moodle_url('/my/index.php'));
