...
use core_course\reportbuilder\datasource\categories;
...
class datasources_test extends core_reportbuilder_testcase {
    ...
    public function test_multitenancy_user_badges_datasource(): void {
        ...
        $report = $this->rbgenerator->create_report(['name' => 'Badges', 'source' => users::class, 'default' => 1]);
    ...
    public function test_multitenancy_comments_datasource(): void {
        ...
        [$userfullname, $contextname, $contenttext, $timecreated] = array_values($content[0]);
        ...
        [$userfullname, $contextname, $contenttext, $timecreated] = array_values($content[1]);
        $this->assertEquals($coursecontext->get_context_name(), $contextname);
    ...
    /**
     * Test for multitenancy callbacks in the course categories datasource
     */
    public function test_multitenancy_categories_datasource(): void {
        global $DB;

        $this->resetAfterTest();

        // Create our categories/tenants.
        $tenantonecategory = $this->getDataGenerator()->create_category();
        [$tenantone, [$tenantoneuser]] = $this->generator->create_tenant_and_users(1, ['categoryid' => $tenantonecategory->id]);

        $tenanttwocategory = $this->getDataGenerator()->create_category();
        [$tenanttwo, [$tenanttwouser]] = $this->generator->create_tenant_and_users(1, ['categoryid' => $tenanttwocategory->id]);

        // Assign roles in each category to both users.
        $roleid = $DB->get_field('role', 'id', ['shortname' => 'manager']);

        role_assign($roleid, $tenantoneuser->id, $tenantonecategory->get_context());
        role_assign($roleid, $tenantoneuser->id, $tenanttwocategory->get_context());

        role_assign($roleid, $tenanttwouser->id, $tenantonecategory->get_context());
        role_assign($roleid, $tenanttwouser->id, $tenanttwocategory->get_context());

        // Create our report.
        $report = $this->generator->create_report(['name' => 'Categories', 'source' => categories::class, 'default' => false]);
        ...
        $this->rbgenerator->create_column([
            'reportid' => $report->get('id'),
            'uniqueidentifier' => 'role:shortname',
            'sortenabled' => true,
        ]);
        $this->rbgenerator->create_column(['reportid' => $report->get('id'), 'uniqueidentifier' => 'user:fullname']);

        // Tenant one user.
        $this->setUser($tenantoneuser);
        $content = $this->get_custom_report_content($report->get('id'));
        $this->assertEquals([
            [$tenantonecategory->name, 'manager', fullname($tenantoneuser)],
            [$tenantonecategory->name, 'tool_tenant_user', fullname($tenantoneuser)],
        ], array_map('array_values', $content));

        // Tenant two user.
        $this->setUser($tenanttwouser);
        $content = $this->get_custom_report_content($report->get('id'));
        $this->assertEquals([
            [$tenanttwocategory->name, 'manager', fullname($tenanttwouser)],
            [$tenanttwocategory->name, 'tool_tenant_user', fullname($tenanttwouser)],
        ], array_map('array_values', $content));
    }
    ...
    public function test_multitenancy_participants_datasource(): void {
        ...
        $this->rbgenerator->create_filter(['reportid' => $report->get('id'), 'uniqueidentifier' => 'enrol:plugin']);

        // User1 should only see tenant1's courses.
        $content = $this->get_custom_report_content($report->get('id'), 30, [
            'enrol:plugin_operator' => \core_reportbuilder\local\filters\select::EQUAL_TO,
    ...
    public function test_multitenancy_files_datasource(): void {
        ...
        $this->rbgenerator->create_column(['reportid' => $report->get('id'), 'uniqueidentifier' => 'context:name']);
