...
class outcome_allocation_test extends advanced_testcase {
    ...
    public function test_apply_to_user_in_other_tenant(): void {
        ...
        $generator->allocate_user($user1->id, $this->tenant1->id);
        $generator->allocate_user($user2->id, $this->tenant1->id);
        $generator->allocate_user($user3->id, $this->tenant1->id);
    ...
    public function test_apply_to_user_in_shared_tenant(): void {
        ...
        $tenantgenerator->allocate_user($user1->id, $this->tenant1->id);
        $tenantgenerator->allocate_user($user2->id, $this->tenant1->id);
        $tenantgenerator->allocate_user($user3->id, $this->tenant1->id);
        ...
        // We expect users to be moved from Tenant1 to Tenant2.
        $users = $DB->get_records('tool_tenant_user', ['tenantid' => $this->tenant2->id], '', 'userid');
        $this->assertEqualsCanonicalizing([get_admin()->id, $user1->id, $user2->id, $user3->id], array_column($users, 'userid'));

        // Check users should not be in the tenant1.
        $this->assertEquals(0, $DB->count_records('tool_tenant_user', ['tenantid' => $this->tenant1->id]));
    }
     ...
     *
     * @uses \tool_tenant\tool_dynamicrule\outcome\allocation::get_description
     * @uses \tool_tenant\tool_dynamicrule\outcome\allocation::get_uneditable_description
     */
    public function test_get_displayed_description(): void {
        ...
        $manager = new manager();
        $tenant1admin = $this->getDataGenerator()->create_user();
        $manager->allocate_user($tenant1admin->id, $this->tenant1->id, 'tool_tenant', 'testing');
        $manager->assign_tenant_admin_roles([$tenant1admin->id], $this->tenant1->id);
        $tenant2admin = $this->getDataGenerator()->create_user();
        $manager->allocate_user($tenant2admin->id, $this->tenant2->id, 'tool_tenant', 'testing');
        $manager->assign_tenant_admin_roles([$tenant2admin->id], $this->tenant2->id);
        ...
        // Admin user has required permission to view tenant2.
        self::setAdminUser();
        $this->assertEquals($outcome->get_description(), $outcome->get_displayed_description());

        // Tenant 1 admin does not have required permission to view tenant2.
        self::setUser($tenant1admin);
        ...
        // Tenant 2 admin has required permission to view tenant2.
        self::setUser($tenant2admin);
        $this->assertEquals($outcome->get_description(), $outcome->get_displayed_description());
    }
