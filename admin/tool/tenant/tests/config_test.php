...
class config_test extends advanced_testcase {
    ...
    /**
     * Test overridden core config settings (such as supportname) for tenants are retrieved for mobile.
     */
    public function test_overrides_core_mobile(): void {
        global $CFG;

        $this->resetAfterTest();

        [$tenant, [$user]] = $this->generator->create_tenant_and_users(1);
        set_config('supportname', 'Global support');
        config::set_config_tenant_override($tenant->id, 'supportname', 'Tenant support');

        // When we don't have any overrides (in default tenant) the default values are returned.
        $result = external_api::clean_returnvalue(
            \tool_mobile\external::get_config_returns(),
            \tool_mobile\external::get_config()
        );
        $settings = array_column($result['settings'], 'value', 'name');
        $this->assertEquals('Global support', $settings['supportname']);

        // Change to user in a tenant.
        $this->set_user_with_config($user);
        ...
        $settings = array_column($result['settings'], 'value', 'name');
        $this->assertEquals('Tenant support', $settings['supportname']);

        // Remove override.
        config::set_config_tenant_override($tenant->id, 'supportname', null);
        $result = external_api::clean_returnvalue(
            \tool_mobile\external::get_config_returns(),
            \tool_mobile\external::get_config()
        );
