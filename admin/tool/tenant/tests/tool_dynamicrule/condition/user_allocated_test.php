 ...
 * @package    tool_tenant
 * @group      tool_tenant
 * @covers     \tool_tenant\tool_dynamicrule\condition\user_allocated
...
class user_allocated_test extends advanced_testcase {
    ...
    public function test_get_matching_users(): void {
        ...
        // Create condition for users allocated in tenant1.
        $configdata = ['tenantid' => $tenant1->id];
        user_allocated::create($rule1->id, $configdata);

        // Check only users in tenant1 are recovered.
        $usersexpected = array_column($users1, 'id');
    ...
    public function test_get_description(): void {
        ...
        $this->assertEquals(get_string('conditionuserallocateddescription', 'tool_tenant', $tenant1->name),
    ...
    public function test_tenant_user_created_trigger_rule_processing(): void {
        ...
        // Create users into tenant 1, they should match immediately.
        $newuser1 = $this->generator->create_user(['tenantid' => $tenant1->id]);
        $this->get_generator()->assert_user_matched_rule($this, (int)$newuser1->id, $rule1->id);
        $newuser2 = $this->generator->create_user(['tenantid' => $tenant1->id]);
    ...
    public function test_tenant_user_updated_rule_processing(): void {
        ...
        // Create a user in tenant1, they should not match.
        $newuser1 = $this->generator->create_user(['tenantid' => $tenant1->id]);
