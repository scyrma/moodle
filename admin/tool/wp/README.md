...
Display tabs:
    ...
    $tabsoutput = new \tool_wp\output\secondary_tabs($attributes);
    $tabsoutput->add_tab(new TABCLASSNAME($attributes)); // Repeat for all tabs.
    // You have to finish adding all the tabs before calling $OUTPUT->header().
    echo $OUTPUT->header();
...
Example of the template with a heading that sets an action for the "Add" button and loads a tab:
    ...
    {{#js}}
        require(['tool_wp/secondary_tabs'], function(Tabs) {
...
Relevant behat step:
    ...
    And I should see "Rule1" in the "Archived" "tool_wp > Secondary tab"
