...
function xmldb_tool_wp_upgrade(int $oldversion) {
    ...
    if ($oldversion < 2022070500) {
        ...
        // In Workplace 4.0 we removed plugins block_mylearning and format_wplist. If they were not
        // added during upgrade, clean up database from everything related to these plugins.

        // If block_mylearning is no longer present, remove all instances and config.
        if (!file_exists($CFG->dirroot . '/blocks/mylearning/version.php')) {
            // First delete instances and related contexts.
            $instances = $DB->get_records('block_instances', ['blockname' => 'mylearning']);
            foreach ($instances as $instance) {
                blocks_delete_instance($instance);
            }
            // Delete block.
            $DB->delete_records('block', ['name' => 'mylearning']);
            // Cleanup config.
            unset_all_config_for_plugin('block_mylearning');
        }

        // If format_wplist is no longer present, update all courses and remove config.
        if (!file_exists($CFG->dirroot . '/course/format/wplist/version.php')) {
            // If wplist was a default course format, change it to topics.
            if (get_config('moodlecourse', 'format') === 'wplist') {
                set_config('format', 'topics', 'moodlecourse');
            }
            // Regardless of the default course format, change all courses in wplist format to topics
            // since it's the most similar looking course format.
            $DB->execute('UPDATE {course} SET format = ? WHERE format = ?', ['topics', 'wplist']);
            // Cleanup course format options and config.
            $DB->delete_records('course_format_options', ['format' => 'wplist']);
            unset_all_config_for_plugin('format_wplist');
        }

        upgrade_plugin_savepoint(true, 2022070500, 'tool', 'wp');
    }

    if ($oldversion < 2022091400) {
        // Remove 'myoverviewdisplayonmobileonly' setting.
        unset_config('myoverviewdisplayonmobileonly', 'tool_wp');
        // Wp savepoint reached.
        upgrade_plugin_savepoint(true, 2022091400, 'tool', 'wp');
    }

    if ($oldversion < 2022101100) {

        // If default home page is not equal to 0 (Home) then remove that option from mobile.
        if ($CFG->defaulthomepage != 0) {
            $disabledmobilefeatures = get_config('tool_mobile', 'disabledfeatures');
            $disabledmobilefeatures = preg_split('/,/', $disabledmobilefeatures, -1, PREG_SPLIT_NO_EMPTY);

            $disablemobilesitehome = '$mmSideMenuDelegate_mmaFrontpage';
            if (array_search($disablemobilesitehome, $disabledmobilefeatures, true) === false) {
                $disabledmobilefeatures[] = $disablemobilesitehome;
                set_config('disabledfeatures', implode(',', $disabledmobilefeatures), 'tool_mobile');
            }
        }
    ...
    if ($oldversion < 2023013001) {
        ...
        // If format_wplist is no longer present, update block positions defined.
        if (!file_exists($CFG->dirroot . '/course/format/wplist/version.php')) {
            tool_wp_upgrade_update_wplist_block_positions();
        }

        // Wp savepoint reached.
        upgrade_plugin_savepoint(true, 2023013001, 'tool', 'wp');
    }
    ...
    if ($oldversion < 2023013002) {
        ...
        if (!file_exists($CFG->dirroot . '/course/format/wplist/version.php')) {
            core_external\util::delete_service_descriptions('format_wplist');
        }

        // Wp savepoint reached.
        upgrade_plugin_savepoint(true, 2023013002, 'tool', 'wp');
    }
