...
Feature: Export and import custom core reports
  ...
  Background:
    ...
    And the following "tool_tenant > reports" exist:
      | name    | tenant  | source                   |
      | Report1 | Tenant1 | tool_wp\test\mock_report |
      | Report2 | Tenant2 | tool_wp\test\mock_report |
  ...
  Scenario: Import a custom report into current tenant
    ...
    And I press "Import"
    And I upload "admin/tool/wp/tests/fixtures/custom-reports-export.zip" file to "Upload a file" filemanager
    And I press "Next"
    And I should see "Custom reports (2)" in the "Content" "table_row"
    And I should see "Audiences (4)" in the "Content" "table_row"
    ...
    And I should see "Created new report 'Course completion' with 4 columns, 1 conditions and 2 filters"
    ...
    And the following should exist in the "reportbuilder-table" table:
      | Report name       | Time created        | Time modified       | Modified by   |
      | Course completion | ##today##%d %B %Y## | ##today##%d %B %Y## | Tenantadmin 1 |

  @_file_upload
  Scenario: Import a custom report into different tenant
    When I log in as "admin"
    And I perform a new import with these options:
      | Step 1 | Upload a file                     | admin/tool/wp/tests/fixtures/custom-reports-export.zip |
    ...
    And the following should exist in the "reportbuilder-table" table:
      | Report name       | Time created        | Time modified       | Modified by |
      | Course completion | ##today##%d %B %Y## | ##today##%d %B %Y## | Admin User  |
