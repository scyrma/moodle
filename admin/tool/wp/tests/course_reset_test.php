...
class course_reset_test extends advanced_testcase {
    ...
    /**
     * Create an appointment session, sign our test user up for it
     *
     * @param stdClass $appointment
     * @return void
     */
    public function complete_appointment(stdClass $appointment): void {
        /** @var \mod_appointment_generator $generator */
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_appointment');
        ...
        appointment_user_signup($session, $appointment, $this->course,
    ...
    public function complete_data(stdClass $data, stdClass $group): void {
        global $DB;
        /** @var \mod_data_generator $moddatagenerator */
        $moddatagenerator = $this->getDataGenerator()->get_plugin_generator('mod_data');
        ...
        foreach ($fieldtypes as $fieldtype) {
            ...
            ${$fieldname} = $moddatagenerator->create_field($record, $data);
        ...
        $this->setUser($this->user);
        $moddatagenerator->create_entry($data,
    ...
    public function complete_lesson(stdClass $lesson): void {
        ...
        $data = [
            [
                'name' => 'answerid',
                'value' => $DB->get_field('lesson_answers', 'id', ['pageid' => $tfrecord->id, 'jumpto' => -1]),
            ],
            [
    ...
    public function complete_workshop(stdClass $workshop): void {
        ...
        $submission = $workshopgenerator->create_submission($workshop->id, $this->user->id);
        $assessment = $workshopgenerator->create_assessment($submission, $this->user->id, [
            'grade' => 92,
        ]);
    }
    ...
    public function complete_feedback(stdClass $feedback): void {
        /** @var \mod_feedback_generator $fgenerator */
    ...
    public function complete_quiz(stdClass $quiz): void {
        ...
        $quizobj = \mod_quiz\quiz_settings::create($quiz->id, $this->user->id);
     ...
     * Test reset_course().
     *
     * TODO: This huge test should be re-factored/broken up into smaller methods that test each module
     */
    public function test_reset_course(): void {
        ...
        // Add an appointment module to the course.
        $appointment = $this->getDataGenerator()->create_module('appointment',
            ['course' => $this->course->id, 'usercalentry' => true]);
        $this->complete_appointment($appointment);

        // Basic test to confirm we have populated appointment signups/calendar events.
        $this->assertEquals(1, $DB->count_records('appointment_signups', ['userid' => $this->user->id]));
        $this->assertEquals(1, $DB->count_records('event', ['eventtype' => 'appointmentbooking', 'userid' => $this->user->id]));
        ...
        [$course, $cm] = get_course_and_cm_from_cmid($quiz->cmid, 'quiz', 0, $this->user->id);
        $completion = new \mod_quiz\completion\custom_completion($cm, $this->user->id);
        ...
        // Grade history is not empty.
        $this->assertNotEmpty($DB->get_records('grade_grades_history', ['userid' => $this->user->id]));

        // Test setup is now complete. Set teacher as the current user, and reset the course for our test user.
        $this->setUser($this->teacher);
        ...
        $result = $creset->reset_course($resetparams);
        $this->assertTrue($result);

        $events = $sink->get_events();
        $lastevent = end($events);
        $this->assertEquals("Course with id '{$this->course->id}' was reset for user with id '{$this->user->id}'.",
            $lastevent->get_description());
        $sink->close();
        ...
        // Check that at least there are 17 events (TODO: why?).
        $this->assertGreaterThan(17, $events);

        // Basic test to confirm we have cleared out appointment signups/calendar events.
        $this->assertEquals(0, $DB->count_records('appointment_signups', ['userid' => $this->user->id]));
        $this->assertEquals(0, $DB->count_records('event', ['eventtype' => 'appointmentbooking', 'userid' => $this->user->id]));
        ...
        // Ensure that grade history has been deleted.
        $this->assertEmpty($DB->get_records('grade_grades_history', ['userid' => $this->user->id]));
    }

    /**
     * Test reset course with tool_wp_deletegradeshistory setting disabled and enabled.
     */
    public function test_reset_course_setting_delete_grades_history(): void {
        global $DB, $CFG;

        // Disable tool_wp_deletegradeshistory setting to avoid deleting course grades history.
        $CFG->tool_wp_deletegradeshistory = false;
        ...
        // At this point we should have 1 submission and 1 grade.
        $params = ['assignment' => $assign->get_instance()->id, 'userid' => $this->user->id];
        $this->assertCount(1, $DB->get_records('assign_submission', $params));
        $this->assertCount(1, $DB->get_records('assign_grades', $params));
        ...
        // Assign module.
        $this->assertCount(0, $DB->get_records('assign_submission', $params));
        $this->assertCount(0, $DB->get_records('assign_grades', $params));

        // Ensure that grade history has NOT been deleted.
        $this->assertNotEmpty($DB->get_records('grade_grades_history', ['userid' => $this->user->id]));

        // Enable tool_wp_deletegradeshistory setting to delete course grades history.
        $CFG->tool_wp_deletegradeshistory = true;
        $this->complete_assign($assign);
        ...
        $resetparams = [
            'programid' => 33,
            'certificationid' => 44,
            'reason' => 'Testing course reset with grades history deletion :)',
        ];
     ...
     * @param int $userid
     */
    public function complete_course(int $courseid, int $userid): void {
        $ccompletion = new completion_completion(['course' => $courseid, 'userid' => $userid]);
        $ccompletion->mark_complete();
    }

    /**
     * Test that course reset adhoc task sets the correct user.
     */
    public function test_reset_course_with_assignments(): void {
        global $DB;
        ...
        $this->setUser($this->user);
        $this->complete_assign($assign);
        $this->complete_course($course->id, $this->user->id);

        // Assign module.
        $assignid = $assign->get_instance()->id;
        $params = ['assignment' => $assignid, 'userid' => $this->user->id];
        $this->assertEquals(1, $DB->count_records('assign_submission', $params));
        $this->assertEquals(1, $DB->count_records('assign_grades', $params));

        // There is no reset program task pending.
        $params = ['component' => 'tool_program', 'classname' => '\tool_program\task\reset_program'];
        $this->assertEmpty($DB->get_records('task_adhoc', $params));

        $this->setAdminUser();
        $coursereset = new course_reset_api($course->id, $this->user->id);
        $coursereset->reset_course(['programid' => 0, 'reason' => 'test']);

        $params = ['assignment' => $assignid, 'userid' => $this->user->id];
        $this->assertEquals(0, $DB->count_records('assign_submission', $params));
        $this->assertEquals(0, $DB->count_records('assign_grades', $params));

        // Create new user that can reset a program progress for a user but does not have capability to grade.
        $user2 = self::getDataGenerator()->create_user();
        // We assign capability to user.
        $roleid = create_role('Dummy role', 'dummyrole', 'dummy role description');
        assign_capability('tool/program:coursereset', CAP_ALLOW, $roleid, context_system::instance()->id);
        assign_capability('tool/program:allocateuser', CAP_ALLOW, $roleid, context_system::instance()->id);
        assign_capability('mod/assign:grade', CAP_PROHIBIT, $roleid, $context->id);
        ...
        $this->assertFalse(course_reset_api::can_reset_courses([$course->id]));
        $str = 'Sorry, but you do not currently have permissions to do that (Grade assignment).';
    ...
    /**
     * Test archiving certificates on course reset
     */
    public function test_reset_course_mod_coursecertificate(): void {
        global $DB;
        if (!\core_component::get_component_directory('mod_coursecertificate')) {
            $this->markTestSkipped('Plugin mod_coursecertificate not found');
            return;
        }

        // Create a course with a certificate module, two students and a teacher.
        $course = $this->getDataGenerator()->create_course(['enablecompletion' => 1]);
        $user1 = $this->getDataGenerator()->create_and_enrol($course);
        $user2 = $this->getDataGenerator()->create_and_enrol($course);
        $teacher = $this->getDataGenerator()->create_and_enrol($course, 'editingteacher');

        /** @var \tool_certificate_generator $certgen */
        $certgen = $this->getDataGenerator()->get_plugin_generator('tool_certificate');
        $certificate1 = $certgen->create_template((object)['name' => 'Certificate 1']);
        $record = ['course' => $course->id, 'template' => $certificate1->get_id()];
        $mod = $this->getDataGenerator()->create_module('coursecertificate', $record);

        // Both students have certificates issued.
        \mod_coursecertificate\helper::issue_certificate($user1, $mod);
        \mod_coursecertificate\helper::issue_certificate($user2, $mod);

        // Set teacher as the current user, and reset the course for one user.
        $this->setUser($teacher);
        $creset = new \tool_wp\course_reset_api($course->id, $user1->id);
        $resetparams = [
            'programid' => 33,
            'certificationid' => 44,
            'reason' => 'Testing course reset :)',
        ...
        // Now user1 has an archived certificate and user2 has a normal certificate.
        $sql = "SELECT userid, archived FROM {tool_certificate_issues}
        ...
        $this->assertEquals(['userid' => $user1->id, 'archived' => 1], (array)$records[0]);
        $this->assertEquals(['userid' => $user2->id, 'archived' => 0], (array)$records[1]);
        $this->assertCount(2, $records);
    }

    /**
     * Test reseting viewed modules.
     */
    public function test_reset_module_viewed(): void {
        global $DB;
        $course = $this->getDataGenerator()->create_course(['enablecompletion' => 1]);
        ...
        $cm = $DB->get_record('course_modules', ['course' => $course->id, 'instance' => $url->id]);
        ...
        // Get completion status of the module before resetting the course.
        $cmcompletion = \core_completion\cm_completion_details::get_instance($cminfo, $user->id);
        $cmcompletiondetails = $cmcompletion->get_details()['completionview'];

        // Confirm that the module is marked as completed.
        $this->assertTrue((bool) $cmcompletiondetails->status);

        $this->setAdminUser();
        $coursereset = new course_reset_api($course->id, $user->id);
        $coursereset->reset_course(['programid' => 0, 'reason' => 'test']);

        // Get completion status of the module after resetting the course.
        $cmcompletionreset = \core_completion\cm_completion_details::get_instance($cminfo, $user->id);
        $cmcompletiondetailsreset = $cmcompletionreset->get_details()['completionview'];

        // Confirm that the module is not marked as completed.
        $this->assertFalse((bool) $cmcompletiondetailsreset->status);
    }
}
