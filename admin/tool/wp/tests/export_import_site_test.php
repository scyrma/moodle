...
class tool_wp_export_import_site_testcase extends advanced_testcase {
    ...
    public function test_import_site(): void {
        ...
        $this->assertRegExp('/Imported tenant \'<a href="[^"]+">Tenant one<\/a>\'/', $details[1]);
        $this->assertEquals('Created user \'Admin User\'', $details[2]);
        $this->assertEquals('Created user \'Tenant One User\'', $details[3]);
        $this->assertRegExp('/Created new course category \'<a href="[^"]+">Tenant one category<\/a>\'/', $details[4]);
        $this->assertRegExp('/Created new certificate template \'<a href="[^"]+">Tenant one certificate<\/a>\'/', $details[5]);

        // Importing second tenant.
        $this->assertRegExp('/Imported tenant \'<a href="[^"]+">Tenant two<\/a>\'/', $details[6]);
        $this->assertEquals('Created user \'Tenant Two User\'', $details[7]);
        $this->assertRegExp('/Created new course category \'<a href="[^"]+">Tenant two category<\/a>\'/', $details[8]);
        $this->assertRegExp('/Created new cohort \'<a href="[^"]+">Tenant two cohort<\/a>\'/', $details[9]);
        $this->assertEquals('Allocated user \'Tenant Two User\' into cohort \'Tenant two cohort\'', $details[10]);
        $this->assertRegExp('/Created new course category \'<a href="[^"]+">Tenant two sub-category<\/a>\'/', $details[11]);
        $this->assertRegExp('/Created new course \'<a href="[^"]+">Tenant two sub-course<\/a>\'/', $details[12]);

        // Importing third tenant.
        $this->assertRegExp('/Imported tenant \'<a href="[^"]+">Tenant three<\/a>\'/', $details[13]);

        // Importing top-level category not associated with a tenant.
        $this->assertRegExp('/Created new course category \'<a href="[^"]+">Non-tenant category<\/a>\'/', $details[14]);
        $this->assertRegExp('/Created new certificate template \'<a href="[^"]+">Non-tenant category certificate<\/a>\'/',
            $details[15]);
        $this->assertRegExp('/Created new cohort \'<a href="[^"]+">Non-tenant category cohort<\/a>\'/', $details[16]);
        ...
        $this->assertRegExp('/Created new cohort \'<a href="[^"]+">System cohort<\/a>\'/', $details[18]);
        ...
        $this->assertRegExp('/Created new certificate template \'<a href="[^"]+">System certificate<\/a>\'/', $details[21]);
