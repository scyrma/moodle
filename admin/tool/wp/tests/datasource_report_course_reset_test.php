...
use tool_wp\tool_reportbuilder\datasources\report_course_reset;
...
class datasource_report_course_reset_test extends advanced_testcase {
    ...
    protected function set_up_for_report(): array {
        ...
        $choice = $this->getDataGenerator()->create_module('choice', ['course' => $course1->id],
            ['completion' => 1]);
        $choicewithoptions = choice_get_choice($choice->id);
        $optionids = array_keys($choicewithoptions->option);
        $cm = get_coursemodule_from_instance('choice', $choice->id);
        choice_user_submit_response($optionids[2], $choice, $user1, $course1, $cm);
        $completion->update_state($cm, COMPLETION_COMPLETE, $user1);
    ...
    public function test_convert_to_core_reportbuilder(): void {
        ...
        $this->get_reportbuilder_generator()->datasource_test_convert_to_core_reportbuilder(report_course_reset::class, $this);
    }
}
