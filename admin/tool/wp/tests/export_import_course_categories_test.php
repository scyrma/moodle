...
class tool_wp_export_import_course_categories_testcase extends advanced_testcase {
    ...
    public function test_export_import_chaining_entities(): void {
        ...
        $cohort2 = $this->generate_cohort_with_member($category1a->id);

        $template = $this->generate_certificate_template(context_coursecat::instance($category1->id)->id, 'Certificate 1');
        $template2 = $this->generate_certificate_template(context_coursecat::instance($category1a->id)->id, 'Certificate 1a');

        // Sanity check.
        $this->assertCount(5, $DB->get_records('course_categories'));
        $this->assertCount(4, $DB->get_records('course'));
        $this->assertCount(2, $DB->get_records('cohort'));
        $this->assertCount(2, $DB->get_records('cohort_members'));
        ...
        $this->assertEqualsCanonicalizing([$cohort1->name, $cohort2->name], $cohortnames);
        ...
        $this->assertEqualsCanonicalizing([$template->get_name(), $template2->get_name()], $templatenames);
        ...
        $this->assertCount(4, $DB->get_records('cohort_members'));
        $this->assertCount(4, $DB->get_records(\tool_certificate\persistent\template::TABLE));
        $this->assertCount(8, $DB->get_records(\tool_certificate\persistent\element::TABLE));
        $this->assertCount(8, $DB->get_records(\tool_certificate\persistent\page::TABLE));
