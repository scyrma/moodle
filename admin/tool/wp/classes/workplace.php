...
class workplace {
    ...
    public static function copyright() {
        ...
        $copyrighttext = 'This installation contains ' .
            ...
            'Workplace components are exclusively owned and licensed by Moodle under the terms of its proprietary '.
            'Moodle Workplace License. Distribution is not permitted without authority.';
    ...
    /**
     * Get all workplace components role shortnames.
     *
     * @return array
     */
    public static function get_workplace_roles(): array {
        $roleshortnames = [];
        $callbacks = get_plugins_with_function('workplace_roles');
        foreach ($callbacks as $plugintype => $plugins) {
            foreach ($plugins as $plugin => $callback) {
                $roleshortnames = array_merge($roleshortnames, $callback());
            }
        }
     ...
     *
     * @param \core_admin_renderer $renderer
     * @param moodle_url|string|null $pageurl
    ...
    public static function copyright_notice_text(\core_admin_renderer $renderer, $pageurl = null, bool $withbutton = false) {
        ...
        $copyrightnotice = text_to_html(get_string('workplacelicensegeneric', 'tool_wp'));
        ...
        if ($withbutton) {
            $continue = new \single_button(new \moodle_url($pageurl, [
                'lang' => $CFG->lang, 'agreelicense' => 1, ]), get_string('continue'), 'get');
            $output .= $renderer->confirm(get_string('doyouagreelicense', 'tool_wp'), $continue, "https://moodle.com/workplace");
        }
    ...
    public static function print_cli_copyright_notice() {
        ...
        echo wordwrap(trim(get_string('workplacelicensegeneric', 'tool_wp')), 80) . PHP_EOL . PHP_EOL;
        return true;
    }
