...
class course_reset_api {
    ...
    public function reset_course(array $resetdata = []): bool {
        ...
        // Some languages format the grade with a comma instead of a dot.
        $resetdata['grade'] = $grd->str_grade ? unformat_float($grd->str_grade) : 0;
        ...
        foreach ($modules as $module) {
            ...
            if (method_exists($this, $method)) {
                ...
                try {
                    ...
                    $provider = '\mod_' . $modname . '\privacy\provider';
    ...
    private function reset_mod_using_privacy_api(stdClass $cm, string $modname, string $provider): void {
        ...
        // We need to check that mod provider class exists, and if it has been implemented the
        // core_user_data_provider interface.
        if (class_exists($provider) && (new $provider()) instanceof core_user_data_provider) {
    ...
    private function reset_mod_scorm(stdClass $cm): void {
        ...
        $status = self::RESET_NOTHING;
        if ($DB->count_records('scorm_attempt', ['scormid' => $cm->instance, 'userid' => $this->userid]) > 0) {
    ...
    private function reset_mod_feedback(stdClass $cm): void {
        ...
        if ($tmps = $DB->get_records('feedback_completedtmp', $params)) {
            foreach ($tmps as $tmp) {
                ...
                $DB->delete_records('feedback_completedtmp', ['id' => $tmp->id]);
                $DB->delete_records('feedback_valuetmp', ['completed' => $tmp->id]);
            }
            $status = self::RESET_ALL_DATA;
        }
    ...
    /**
     * Resets course certificate module - archive issued certificates
     *
     * @param stdClass $cm
     */
    private function reset_mod_coursecertificate(stdClass $cm): void {
        global $DB;
        $certificate = $DB->get_record('coursecertificate', ['id' => $cm->instance, 'course' => $cm->course]);
        if ($certificate && $certificate->template) {
            $DB->set_field_select('tool_certificate_issues', 'archived', 1,
                'courseid = ? AND templateid = ? AND component = ? AND archived = 0 AND userid = ?',
                [$cm->course, $certificate->template, 'mod_coursecertificate', $this->userid]);
        }
    }
     ...
     * @return stdClass|false The grade record
     */
    ...
    protected function get_user_submission(stdClass $cm, int $attemptnumber=-1) {
        ...
        $params = ['assignment' => $cm->instance, 'userid' => $this->userid, 'groupid' => 0];
        if ($attemptnumber >= 0) {
            $params['attemptnumber'] = $attemptnumber;
        }
    ...
    protected function get_group_submission(int $groupid, stdClass $cm, int $attemptnumber=-1) {
        ...
        $params = ['assignment' => $cm->instance, 'groupid' => $groupid, 'userid' => 0];
        if ($attemptnumber >= 0) {
            $params['attemptnumber'] = $attemptnumber;
        }
    ...
    private function reset_mod_lesson(stdClass $cm): void {
        ...
        if ($lessonbranch > 0 || $lessonattempts > 0) {
            ...
            $lesson = new lesson($DB->get_record('lesson', ['id' => $cm->instance]));
    ...
    private function delete_grades(): int {
        global $DB, $CFG;
        $rv = 0;
        /** @var grade_item[] $items */
        $items = grade_item::fetch_all(['courseid' => $this->courseid]);
        if ($items) {
            $itemids = [];
            foreach ($items as $item) {
                /** @var grade_grade[] $grades */
                $grades = grade_grade::fetch_all(['userid' => $this->userid, 'itemid' => $item->id]);
                if ($grades) {
            ...
            // Check if 'Delete grades history during course reset' setting is enabled.
            if ($itemids && $CFG->tool_wp_deletegradeshistory) {
                // Remove grade history, so when the user is enrolled with recovering grades, they are not recovered.
                [$insql, $params] = $DB->get_in_or_equal($itemids, SQL_PARAMS_NAMED);
                $DB->delete_records_select('grade_grades_history', "itemid $insql AND userid = :userid",
    ...
    private function reset_mod_completion(stdClass $cm): void {
        ...
        // Reset viewed.
        $modinfo = cm_info::create((object)['id' => $cm->id, 'course' => $course->id], $this->userid);
    ...
    public function reset_module_viewed(\completion_info $completion, \cm_info $cm): void {
        ...
        $completion->update_state($cm, COMPLETION_INCOMPLETE, $this->userid, true);
    }
    ...
    private function reset_course_completions(): int {
        ...
        $reset = 0;
        $params = ['userid' => $this->userid, 'course' => $this->courseid];
    ...
    /**
     * Checks if user has all necessary capabilities to reset the list of courses
     *
     * Required capabilities will depend on course contents, at the moment only assignments
     * have additional checks.
     *
     * @param array $coursesids
     * @return bool
     */
    public static function can_reset_courses(array $coursesids) {
        return self::can_reset_courses_assign_modules($coursesids);
    }

    /**
     * Check if the current user has mod/assign:grade capability on all assign modules from all courses passed to the function
     *
     * @param array $coursesids
     * @return bool
     */
    protected static function can_reset_courses_assign_modules(array $coursesids) {
        global $DB;

        // Check capabilities needed when reseting a course for assign modules.
        $canresetcourses = true;
        if (!empty($coursesids)) {
            [$sqlcourseids, $params] = $DB->get_in_or_equal($coursesids, SQL_PARAMS_NAMED, 'id');
            $sql = "
                SELECT cm.*
                FROM {course_modules} cm
                LEFT JOIN {modules} m
                ON m.id = cm.module
                WHERE cm.deletioninprogress = :deleteinprogress AND cm.course $sqlcourseids AND m.name = :moduletype
            ";
            $params['moduletype'] = 'assign';
            $params['deleteinprogress'] = 0;
            $modules = $DB->get_records_sql($sql, $params);
            foreach ($modules as $module) {
                $context = \context_module::instance($module->id);
                if (!has_capability('mod/assign:grade', $context)) {
                    $canresetcourses = false;
                    break;
                }
            }
        }

        return $canresetcourses;
    }
}
