...
class helper {
    ...
    public static function get_export_import_status_formatted(string $statusstr): string {
        debugging('get_export_import_status_formatted() is deprecated. Please use get_export_import_status_badge() instead.',
            DEBUG_DEVELOPER);
    ...
    /**
     * Returns html badge for export/import status.
     *
     * @param int $status
     * @param string $statusstr
     * @return string
     */
    public static function get_export_import_status_badge(int $status, string $statusstr): string {
        $statusclasses = [
            self::STATUS_DONE => 'badge badge-success',
            self::STATUS_IN_PROGRESS => 'badge badge-warning',
            self::STATUS_ERROR => 'badge badge-danger',
            self::STATUS_SCHEDULED => 'badge badge-info',
            self::STATUS_CREATED => 'badge badge-info',
        ];
        return \html_writer::span($statusstr, $statusclasses[$status] ?? 'badge badge-secondary');
    }
     ...
     *
     * @param int|string|null $importid id of the import, usually a number but can be a template such as ":id",
     ...
     *
     * @param int|string|null $exportid id of the export, usually a number but can be a template such as ":id",
     *     if empty the entrypoint will be used in the URL
     * @param string $entrypoint
     * @param int $entrypointid
     * @return moodle_url
    ...
    /**
     * Formatter for the exporter type
     *
     * @param int $type
     * @return string
     */
    public static function format_exporter_type(int $type): string {
        if ($type === exporter_base::FORMAT_WORKPLACE) {
            return get_string('csvwpcolumn', 'tool_wp');
        }
        if ($type === exporter_base::FORMAT_CSV) {
            return get_string('exportformatcsv', 'tool_wp');
        }
        return '';
    }
}
