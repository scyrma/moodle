...
class imports_list_report extends system_report {
    ...
    public function initialise() {
        ...
        $this->add_column((new report_column('tenantid', new \lang_string('tenant', 'tool_wp'), 'tool_wp_import'))
        ...
        $this->add_actions();
        $this->set_filters();
        // TODO: consider re-enabling once WP-1620 is fixed.
    ...
    public function format_tenantid($value) {
        ...
        if (array_key_exists($value, $tenants)) {
            return format_string($tenants[$value]->name);
        }
        return $value;
    }
    ...
    public function format_status($value): string {
        return helper::format_export_import_status((int)$value);
    }
    ...
    private function add_actions() {
        ...
        $action->add_callback(function(\stdClass $row) {
            $row->url = \moodle_url::make_pluginfile_url(\context_system::instance()->id, 'tool_wp',
                'import', $row->id, '/', 'import'.$row->id.'.zip')->out(false);
            return true;
        });
    ...
    private function set_filters(): void {
        ...
        $this->add_filter((new report_filter(
            ...
            ->set_options_callback(function() {
                ...
                return array_column($tenants, 'name', 'id');
            })
            ->set_is_default(true)
