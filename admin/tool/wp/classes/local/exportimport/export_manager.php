...
class export_manager {
    ...
    public function perform_export() {
        ...
        try {
            ...
            $status = helper::STATUS_ERROR;

            if ($exporter->is_format(exporter_base::FORMAT_ZIP)) {
                if ($this->create_archive(new \zip_packer(), $persistent, $filename) instanceof \stored_file) {
                    $status = helper::STATUS_DONE;
                }
            } else if ($exporter->is_format(exporter_base::FORMAT_CSV)) {
                if ($this->save_csv_file($persistent, $filename) instanceof \stored_file) {
                    $status = helper::STATUS_DONE;
                }
            } else {
                $status = helper::STATUS_ERROR;
