...
class export_manager {
    ...
    /** @var array */
    protected $files = [];
    /** @var exporter_base[][] caches all importers for given entry point, used in {@link find_exporter_for()} */
    ...
    public static function schedule_export(array $data, bool $createadhoctask = true) {
        global $USER, $CFG;
        $defaults = ['exporter' => '', 'entrypoint' => '', 'entrypointid' => 0];
        $data += $defaults;
        $manager = new self(0);
    ...
    public static function create_exporter(string $exporterclass, string $entrypoint, int $entrypointid = 0,
                                           array $configdata = []): exporter_base {
        global $USER, $CFG;
        $defaults = ['exporter' => '', 'entrypoint' => '', 'entrypointid' => 0];
        ...
        $manager->exportpersistent->set('tenantid',
            ($exporter->is_tenant_required() || !\tool_tenant\permission::can_switch_tenant()) ?
            tenancy::get_tenant_id() : null);
        return $exporter;
    }
    ...
    public function get_export_tenant_id(): int {
        return (int)$this->exportpersistent->get('tenantid');
    }
    ...
    public function perform_export() {
        ...
        try {
            ...
            // TODO more details are available from $e->getMessage(). They need to be stored/shown/included in notification.
            return;
        }
        ...
        if ($exporter->get_format() & exporter_base::FORMAT_ZIP) {
            ...
            $filename = clean_filename(\core_text::strtolower($exportername . '-export-' . $date . '.zip'));
    ...
    /**
     * Create an archive from the export directory
     *
     ...
     */
    protected function create_archive(\file_packer $zipper, export_persistent $export, string $filename) {
        global $USER;
