...
class import_general_form extends import_base_form {
    ...
    public function definition() {
        ...
        $this->get_import_manager()->save_general_settings([]);
        ...
        $mform->addElement('static', 'aboutbody', '', $this->get_import_manager()->get_export_file_summary_as_html());
        ...
        if (count($importers) == 1) {
            ...
            foreach ($importers as $importer) {
                $mform->addElement('radio', 'importer', '', $importer->get_name(), get_class($importer));
            }
            if (!$importers) {
        ...
        if (!\tool_tenant\permission::can_switch_tenant()) {
            ...
            foreach (tenancy::get_tenants() as $tenant) {
                $options[$tenant->id] = format_string($tenant->name);
            }
