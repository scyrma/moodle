...
abstract class export_import_base_form extends modal_form {
     ...
     *
     * @param callable $callback function(array $data, array $files) similar to {@link moodleform::validation()}
     */
    public function add_validation_callback(callable $callback) {
        $this->_form->addFormRule(function($data, $files) use ($callback) {
            return $callback($this->_form->exportValues(), $files);
        });
    }
     ...
     */
    protected function find_element(string $elementname) {
     ...
     *
     * See also {@link is_form_element_ignored_for_cli()}
     *
