...
class import_manager {
    ...
    /** @var importer_base[] see {@link get_importers()} */
    protected $importers;
    ...
    /** @var importer_base[] caches all importers defined in the system, used in {@link get_all_system_importers()} */
    protected $allimporters = null;
    /** @var importer_base[][] caches all importers for given entry point, used in {@link find_importer_for()} */
        ...
        if ($this->importers === null) {
            ...
            if ($importerclass = $this->importpersistent->get('importer')) {
                ...
                foreach ($allimporters as $importer) {
                    if ($importer->is_relevant($this->get_exporter())) {
                        $this->importers[] = $importer;
                    }
                }
            }
    ...
    public function get_import_tenant_id(): ?int {
        ...
        if (!$tenantid) {
            if (($importer = $this->get_importer()) && $importer->is_tenant_required()) {
                return tenancy::get_tenant_id();
            }
        }
     ...
     */
    public function import_file(array $filerecord) {
        $filepath = $this->get_file_path($filerecord);
        $filerecord['contextid'] = $this->get_mapping('context', $filerecord['contextid']);
        unset($filerecord['id'], $filerecord['contenthash']);
        get_file_storage()->create_file_from_pathname($filerecord, $filepath);
    }
    ...
    public function log_exception(\Throwable $exception) {
        $persistent = new import_detail_persistent(0, (object)[
            ...
            'type' => self::DETAILTYPE_EXCEPTION,
        ]);
        $persistent->set('data', [
     ...
     * @return string
     */
    protected function format_logged_exception(array $data): string {
        $message = get_string('importlogexception', 'tool_wp', $data['message']);

        // Include stacktrace for developers.
        if (debugging('', DEBUG_DEVELOPER)) {
            $message .= \html_writer::tag('pre', $data['trace']);
        }
    ...
    public function get_import_logs() {
        ...
        foreach ($details as $detail) {
            ...
            if (($importerclass = $detail->get('importer'))) {
                ...
                $importerdetail = $this->format_logged_exception($detail->get('data'));
            } else {
                $importerdetail = '';
            }
    ...
    public function is_zip() {
        $file = $this->get_file();
        return strtolower(pathinfo($file->get_filename(), PATHINFO_EXTENSION)) === 'zip';
    }
    ...
    public function get_export_file_summary(): array {
        ...
        $xmlfilepath = $this->get_unzip_dir() . '/workplace.xml';
        $info = helper::xml_file_to_array($xmlfilepath);
        ...
        if ($info['siteidentifier'] === md5(get_site_identifier())) {
