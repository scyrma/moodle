...
class course_reset_entity extends entity_base {
    ...
    public function convert_get_entity_class(): string {
        return \tool_wp\reportbuilder\local\entities\course_reset::class;
    }
    ...
    protected function get_all_columns(): array {
        ...
        // TODO: WP-3146 - re-factor this column, use 'user:fullname' or allow use of entire 'user' entity for requesting user.
        $viewfullnames = has_capability('moodle/site:viewfullnames', \context_system::instance());
        $ureqtable = database::generate_alias();
        $userrequestedjoin = "LEFT JOIN {user} {$ureqtable} " .
            "ON {$tablealias}.userid = {$ureqtable}.id AND {$ureqtable}.deleted = 0";

        // Generate required SQL snippets ourselves.
        [$userfullnamesql, $userfullnameparams] = fields::get_sql_fullname($ureqtable, $viewfullnames);
        $userfullnamegroupby = user::get_name_fields_select($ureqtable);
        ...
        $newcolumn = (new report_column(
            ...
            ->set_type(constants::DB_TYPE_TEXT)
            ->add_field($userfullnamesql, 'fullname', $userfullnameparams)
            ->set_is_sortable(true, false, null, null, ['fullname'])
            ->add_aggregation_fields('count', $tablealias . '.userrequested')
            ->set_groupby_sql($userfullnamegroupby);
        $columns[] = $newcolumn;
