...
class install_hook {
     ...
     *
     * @param bool $fullsetup Execute if this is the fresh install or it is an upgrade from LMS with $CFG->forcewpsetup.
     */
    public static function execute(bool $fullsetup = false) {
        global $DB, $CFG;
        ...
        if (!defined('BEHAT_SITE_RUNNING') && !(defined('PHPUNIT_TEST') && PHPUNIT_TEST)) {
            ...
            if ($fullsetup) {
                ...
                // Users can't edit their own dashboard by default.
                $userroleid = $DB->get_field('role', 'id', ['shortname' => 'user']);
                unassign_capability('moodle/my:manageblocks', $userroleid);

                // Remove all blocks from all dashboards (and default dashboard).
                $instances = $DB->get_records_select('block_instances',
                    'pagetypepattern=:myindex',
                    ['myindex' => 'my-index']);
            ...
            // Ensure the leading zeroes are not removed from dates.
            ...
            $settings = [
                ...
                ['plugin' => 'tool_mobile', 'name' => 'disabledfeatures', 'value' => '$mmSideMenuDelegate_mmaFrontpage',
                    'default' => '', ],
                ['plugin' => 'tool_mobile', 'name' => 'minimumversion', 'value' => '4.0.0', 'default' => ''],
