...
class course_reset extends datasource {
    ...
    protected function initialise(): void {
        ...
        $this->add_entity(($courseentity)
            ->add_join("LEFT JOIN {course} {$course} ON {$course}.id = {$coursereset}.courseid"));
        ...
        if ($canshowtenantcolumn) {
            ...
            $tenantentity->add_joins($tenantentity->get_user_tenant_joins("{$coursereset}.userid"));
    ...
    public function get_default_columns(): array {
        $columns = ['course:fullname', 'user:fullname', 'course_reset:timereseted'];

        // Add tenant name as default column if current tenant has subtenants.
