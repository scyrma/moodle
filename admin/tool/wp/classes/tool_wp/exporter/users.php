...
class users extends exporter_base {
    ...
    public function is_available(): bool {
        ...
        return (!strlen($this->entrypoint) || $this->is_chained_entrypoint())
            && has_capability('moodle/backup:userinfo', $tenantcontext);
    }
    ...
    public function add_to_options_form(export_settings_form $form): void {
        ...
        // TODO: WP-1847 admin needs to be able to select export tenant/all users.

        $tenantname = tenancy::get_tenant_name_from_id($this->get_export_tenant_id());
        ...
        $options = array(
            ...
            'data-area' => 'users',
            'data-itemid' => $this->get_export_tenant_id(),
            'multiple' => true,
    ...
    private function process_user_for_export(stdClass $user): array {
        ...
        $removeidentityfields = array_diff([
            ...
            'city',
            'country',
        ], get_extra_user_fields(context_system::instance()));
    ...
    private function get_users_for_export(): array {
        ...
        list($sort, $sortparams) = users_order_by_sql('u', null, context_system::instance());
