...
class users extends exporter_base {
    ...
    private function get_users_for_export(): array {
        ...
        if ($this->get_export_setting(self::EXPORT_INSTANCES) == self::EXPORT_INSTANCES_ALL &&
                has_capability('moodle/backup:userinfo', $systemcontext)) {

            $join = '';
            $where = 'id != :guest AND deleted = 0';
            $params = ['guest' => $CFG->siteguest];
        } else {
            [$join, $where, $params] = tenancy::get_users_sql('u', (int) $this->get_export_tenant_id());
        }
