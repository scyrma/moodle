...
class cohorts extends exporter_base {
    ...
    public function add_to_options_form(export_settings_form $form): void {
        ...
        $selectcategory = get_string('cohortsselectedcategory', 'tool_wp');
        ...
        $mform->addElement('autocomplete', self::EXPORT_SELECT_CATEGORIES, get_string('categories'),
            $categories, ['multiple' => false])->setHiddenLabel(true);
        ...
        $mform->addElement('autocomplete', self::EXPORT_SELECT_COHORTS, get_string('cohorts', 'cohorts'),
    ...
    private function get_cohorts_list(): array {
        ...
        // Check if user has capability to export these cohorts.
        $listofcohorts = [];
        foreach ($cohorts as $cohort) {
            $context = \context::instance_by_id($cohort->contextid);
            if (has_capability('moodle/cohort:manage', $context)) {
                $listofcohorts[$cohort->id] = $cohort->name;
            }
        }
    ...
    public function get_cohorts_to_export(array $data): array {
        ...
        switch ($data[self::EXPORT_INSTANCES]) {
            ...
            case self::EXPORT_INSTANCES_CATEGORY:
                // Cohorts in selected category.
                $category = \core_course_category::get($data[self::EXPORT_SELECT_CATEGORIES]);
                $cohorts = $DB->get_records('cohort', ['contextid' => $category->get_context()->id]);
                break;
        ...
        // Check if user has permission to export these cohorts.
        $listofcohorts = [];
        foreach ($cohorts as $cohort) {
            $context = \context::instance_by_id($cohort->contextid);
            if (has_capability('moodle/cohort:manage', $context)) {
