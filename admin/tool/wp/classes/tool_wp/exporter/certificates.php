...
class certificates extends exporter_base {
    ...
    /** @var string Element for selecting which contexts to export templates from (system/category). */
    ...
    public function get_templates_to_export(): array {
        ...
        switch ($this->get_export_setting(self::EXPORT_TYPE)) {
            ...
            case self::EXPORT_TYPE_CAT_MANUALLY:
                // Contexts will be an array of context_system|context_coursecat instances.
                $contexts = array_map(static function(int $contextid): \context {
                    return \context::instance_by_id($contextid);
                }, $this->get_export_setting(self::EXPORT_SELECTED_CATEGORIES, []));

                // If one of the selected contexts is the system, then export all certificates.
                $syscontext = array_filter($contexts, static function(\context $context): bool {
     ...
     *
     * TODO: should it just show full category tree, like the course/category exporters?
     *
     * @return string[] List of system/category paths indexed by context ID
     */
    private function get_template_categories_list(): array {
        ...
        foreach ($contexts as $ctx) {
            ...
            // Context will be an instance of system or category context.
            $templatecontexts[$context->id] = $context instanceof \context_system
                ? $context->get_context_name()
                : \core_course_category::get($context->instanceid)->get_nested_name(false);
        }
