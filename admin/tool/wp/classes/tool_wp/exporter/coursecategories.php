...
class coursecategories extends exporter_base {
    ...
    public function add_to_options_form(export_settings_form $form): void {
        ...
        $mform->disabledIf(self::EXPORT_COURSES_CONTENT, self::EXPORT_COURSES, 'eq', 0);
        ...
        if ($tenantcategory = tenancy::get_tenants()[tenancy::get_tenant_id()]->categoryid) {
    ...
    public function perform_export(): void {
        ...
        if (!empty($data[self::EXPORT_SELECT_CATEGORIES])) {
            ...
            if ($this->get_export_setting(self::EXPORT_COURSES) && $this->can_export_chained_entity('course')) {
                ...
                foreach ($categories as $category) {
                    ...
                    foreach ($courses as $course) {
                        $coursestoexport[] = $course->id;
                    }
                }
                if (!empty($coursestoexport)) {
                    $this->process_chained_entities('course', $coursestoexport, [
                        self::EXPORT_COURSES => 1,
                        self::EXPORT_COURSES_CONTENT => $this->get_export_setting(self::EXPORT_COURSES_CONTENT),
                    ]);
                }
            }
            ...
            if ($this->get_export_setting(self::EXPORT_CERTIFICATE_TEMPLATES) &&
                ...
                // Transform list of selected categories to their contexts.
                $categorycontexts = array_map(static function(int $categoryid): int {
                    return \context_coursecat::instance($categoryid)->id;
                }, $data[self::EXPORT_SELECT_CATEGORIES]);

                $this->process_chained_entities('tool_certificate_templates', [], [
                    certificates::EXPORT_TYPE => certificates::EXPORT_TYPE_CAT_MANUALLY,
                    certificates::EXPORT_SELECTED_CATEGORIES => $categorycontexts,
            ...
            if ($this->get_export_setting(self::EXPORT_COHORTS) && $this->can_export_chained_entity('cohort')) {
                $cohortstoexport = [];
                foreach ($categories as $category) {
                    $records = $DB->get_records('cohort', ['contextid' => $category->get_context()->id]);
                    foreach ($records as $record) {
                        $cohortstoexport[] = $record->id;
                    }
                }
                if (!empty($cohortstoexport)) {
                    $this->process_chained_entities('cohort', $cohortstoexport, [
