...
class courses extends importer_base {
    ...
    public function restore_course(array $data, $file, wp_imported_entity $importedentity): int {
        ...
        try {
            ...
            $this->add_notice_to_log('restorewarning', ['warning' => $e->getMessage()]);
            // TODO: Consider deleting restored course here, as it is not sucessfully restored.

            // Destroy controller, on unsuccessful restore it will remain in non-complete status.
            $controller->destroy();
        }
        // Notice, we are not destrying controller here, we will do it later (see WP-2782).
        fulldelete($fulltempdir);
