...
class courses extends importer_base {
    ...
    protected function initialise() {
        $this->register_entity('course', [
            ...
            self::ENTITY_INDIVIDUALIMPORT => function(array $ids, array $settings) {
                ...
                if (!$settings[self::IMPORT_SELECT_CATEGORY]) {
    ...
    public function add_to_options_form(import_settings_form $form): void {
        ...
        $mform->addElement('advcheckbox', self::IMPORT_CONTENT,
            get_string('exportcoursecontent', 'tool_wp'));
        ...
        $mform->addElement('header', 'destination', get_string('destination', 'tool_wp'));
        ...
        $str = get_string('selectcoursecategory', 'tool_wp');
        $mform->addElement('select', self::IMPORT_SELECT_CATEGORY, $str, $categories);
        ...
        foreach ($courses as $course) {
            $course
                ...
                ->set_import_callback(function($data, wp_imported_entity $entity) {
                    ...
                    return $this->restore_course($data, $filepath);
                })
                ->import($this);
        }
    }
     ...
     * @return int $courseid
     */
    public function restore_course(array $data, $file): int {
        global $CFG, $USER, $DB;
        ...
        // We want to make sure user information is not restored.
