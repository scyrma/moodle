...
/**
 * Class to allow mapping of user profile fields during export/import
 *
 * @package     tool_wp
 ...
 * @license     Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class userfield extends export_import_mapper_base {
    ...
    protected function initialise(): void {
        $this->register_potential_error(self::NOTFOUND, [
            self::ERROR_CONFLICTHEADER => get_string('mappingerroruserfieldheader', 'tool_wp'),
            self::ERROR_LOG => function(array $identifier): string {
                return get_string('mappingerroruserfieldlog', 'tool_wp', $this->get_identifier_for_display($identifier, true));
            },
            self::ERROR_IDENTIFIER => static function(array $identifier, bool $usequotes = false) {
                return self::display_identifier_idnumber_name($identifier, 'shortname', 'name', $usequotes);
            },
        ]);
    }

    /**
     * Returns an array of entity properties that can be used to find it during import
     *
    ...
    public function get_mapping_data_for_workplace_export(int $id): ?array {
        ...
        $userfield = $DB->get_record('user_info_field', ['id' => $id], 'id, shortname, name', IGNORE_MISSING);

        return $userfield ? (array) $userfield : null;
    }
    ...
    public function locate_mapping_default(string $identifier, ?int $tenantid = null): ?int {
        if ($id = $this->locate_mapping(['shortname' => $identifier])) {
            return $id;
        }
    ...
    public function locate_mapping(array $identifier): ?int {
        ...
        if (!empty($identifier['shortname']) &&
                $userfield = $DB->get_record('user_info_field', ['shortname' => $identifier['shortname']])) {

            // User must have access to user profile fields.
