 ...
 */
class learningplan extends export_import_mapper_base {
    ...
    protected function initialise(): void {
        $this->register_potential_error(self::NOTFOUND, [
            self::ERROR_CONFLICTHEADER => get_string('mappingerrorlpheader', 'tool_wp'),
            self::ERROR_LOG => function(array $identifier): string {
                return get_string('mappingerrorlplog', 'tool_wp', $this->get_identifier_for_display($identifier, true));
            },
    ...
    /**
     * Returns an array of learning plan templates properties that can be used to find it during import
     *
    ...
    public function get_mapping_data_for_workplace_export(int $id): ?array {
        ...
        $lp = $DB->get_record(core_competency\template::TABLE, ['id' => $id], 'id, shortname');

        return $lp ? (array) $lp : null;
    }

    /**
     * Locate an existing entity, that is available to the current user, by default identifier
     *
     * @param string $identifier the default identifier used by the entity (normally shortname)
    ...
    public function locate_mapping_default(string $identifier, ?int $tenantid = null): ?int {
        if ($lpid = $this->locate_mapping(['shortname' => $identifier])) {
            return $lpid;
        }
     ...
     *
     * @param array $identifier array or known entity's attributes
     * @return int|null
     */
    public function locate_mapping(array $identifier): ?int {
        if (!empty($identifier['shortname']) &&
            has_capability('moodle/competency:planmanage', \context_system::instance())) {
            // Loop over the matching learning plan templates, find one the user has access to.
            if ($learningplans = core_competency\template::get_records(['shortname' => $identifier['shortname']])) {
