...
use tool_wp\local\exportimport\csv\csv_exported_entity;
...
abstract class exporter_base {
     ...
     */
    final public function prepare_data_for_csv_export(string $entityname, array $data): csv_exported_entity {
        $this->require_csv_format();
        return new csv_exported_entity($this->exportmanager, $entityname, $data);
    }

    /**
     * Makes sure that the current format is CSV format, otherwise throws an exception
     *
     * @throws \coding_exception
     * @return void
     */
    final public function require_csv_format(): void {
        if (!$this->is_format(self::FORMAT_CSV)) {
            throw new \coding_exception('The exporter should be in CSV format in order to use this method');
        }
    }
}
