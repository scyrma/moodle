...
class tool_uploaduser {
    ...
    public static function validate_column($columname) {
        ...
        } else if ($columname == 'istenantadmin') {
            // This property can be found just once in each row. We verify access in allocate_user_on_creation()
            // and process_updated_user() methods using the correct tenantid.
            return true;
        } else {
            $f = 'coursecompleted|coursecompleteddate|' .
                 'jobposition|jobdepartment|jobstartdate|jobenddate|jobdelete|' .
