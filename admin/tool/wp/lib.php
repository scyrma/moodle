...
function tool_wp_override_webservice_execution(stdClass $function, array $params) {
    ...
    if ($function->name === 'tool_mobile_get_config') {
        ...
        // When WP-3277 have done, following changes would probably be reverted or refactored.
        // Get current theme name selected.
        $currenttheme = 'theme_'.$PAGE->theme->name;

        // Create array of custom settings related to current theme config.
        $customthemesettings = ['dashboardlearning', 'dashboardteams', 'hideprogramcourses'];

        foreach ($customthemesettings as $setting) {

            // Default value assigned in case that current theme isn't related to workplace.
            $settingvalue = false;

            // Check if current theme is 'workplace' or is used as a parent of it.
            if (in_array('workplace', $PAGE->theme->parents) || $PAGE->theme->name === 'workplace') {
                // If child theme doesn't have value in current setting then we get it from parent.
                $settingvalue = get_config($currenttheme, $setting) === false ?
                    get_config('theme_workplace', $setting) :
                    get_config($currenttheme, $setting);
            }

            $result['settings'][] = [
                'name' => 'theme_workplace_'.$setting,
                'value' => $settingvalue,
            ];
        }
