...
/**
 * Perform additional webservice validation/overrides
 *
...
function tool_wp_override_webservice_execution(stdClass $function, array $params) {
    ...
    if (WS_SERVER) {
        // Ensure the mobile app User-Agent doesn't match the standard Moodle app.
        $useragent = core_useragent::get_user_agent_string();
        if (preg_match('/MoodleMobile($| \d+)/', $useragent)) {
    ...
    if ($function->name === 'tool_mobile_get_config') {
        ...
        // Add My courses settings to tool_mobile_get_config call.
        $result['settings'][] = [
            'name' => 'wp_tool_catalogue_mycourses_defaultsortorder',
            'value' => get_config('tool_catalogue', 'defaultsortorder'),
        ];
    }
...
function tool_wp_after_config() {
    ...
    if ((defined('AJAX_SCRIPT') && AJAX_SCRIPT) || during_initial_install() || isset($CFG->upgraderunning)) {
        return;
    }
    ...
    if (!empty($CFG->tool_wp_installed)) {
        $fullsetup = $CFG->tool_wp_installed >= 2;
        unset_config('tool_wp_installed');
        \tool_wp\install_hook::execute($fullsetup);
    }
}
 ...
 */
function tool_wp_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=[]) {
    global $CFG, $PAGE;
 ...
 * @return string
 */
function tool_wp_before_footer(): string {
    global $OUTPUT;
 ...
 */
function tool_wp_theme_workplace_menu_items(): array {
    global $OUTPUT;

    $menuitems = [];
    if (has_capability('tool/wp:useexportimport', context_system::instance())) {
        $menuitems[] = [
            'url' => new moodle_url("/admin/tool/wp/exportimport.php"),
            'name' => get_string('exportimport', 'tool_wp'),
            'imageurl' => $OUTPUT->image_url('exportimport', 'tool_wp')->out(false),
            'isglobal' => class_exists(\tool_tenant\permission::class) && \tool_tenant\permission::can_switch_tenant(),
        ];
    }
    return $menuitems;
}
