...
function tool_wp_potential_users_selector(string $area, int $itemid): array {
    ...
    // Make sure current user can browse users in specified tenant.
    $tenantid = (new \tool_tenant\tenant($itemid))->get('id');
    if (!\tool_tenant\permission::can_browse_users($tenantid)) {
        return [];
    }

    return \tool_tenant\tenancy::get_users_sql('u', $tenantid);
}
