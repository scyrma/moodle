...
function tool_wp_override_webservice_execution(stdClass $function, array $params) {
    ...
    if ($function->name === 'tool_mobile_get_config') {
        ...
        // TODO: WP-2698 Revert forcing these settings for Mobile app.
        $dashboardblocks = array_map(function(array $block) {
            return $block['name'];
        }, core_block_external::get_dashboard_blocks($USER->id)['blocks']);
        $forcelearningtab = in_array('mylearning', $dashboardblocks) && empty(get_config('theme_workplace', 'dashboardlearning'));
        $forceteamstab = in_array('myteams', $dashboardblocks) && empty(get_config('theme_workplace', 'dashboardteams'));

        $result['settings'][] = [
            'name' => 'theme_workplace_dashboardlearning',
            'value' => $forcelearningtab ? "1" : get_config('theme_workplace', 'dashboardlearning'),
        ];
        $result['settings'][] = [
            'name' => 'theme_workplace_dashboardteams',
            'value' => $forceteamstab ? "1" : get_config('theme_workplace', 'dashboardteams'),
        ];
        $result['settings'][] = [
