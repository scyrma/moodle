...
class enrol_program_plugin extends enrol_plugin {
    ...
    public function can_self_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if (!\tool_program\permission::can_self_enrol_to_course($instance->courseid, $program)) {
            if (program_user::record_exists_select(
                ...
                if ($programcourseitem !== null && !$programcourseitem->isunlocked) {
                    // Get restrictions for course in the program.
                    // TODO WP-3578 Refactor All these unlock+restrictions.
                    $related = [
                        'context' => context_course::instance($programcourseitem->get_courseid()),
                        'course' => get_course($programcourseitem->get_courseid()),
                    ];
                    $exporter = new course_exporter(null, $related);
                    $data = $exporter->export($PAGE->get_renderer('tool_catalogue'));
                    return $data->restrictions[$program->get('id')] ?? get_string('canntenrollocked', 'enrol_program');
                }
            }
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        // If program doesn't contain this course return false for safety reasons (Enrolment method should not exist in any case).
        $program = new program($programid);
        if (!\tool_program\api::is_course_in_program($program, $instance->courseid)) {
            return false;
        }
     ...
     *
     * @deprecated since 4.3 No longer used
     *
     * @param stdClass $instance enrolment instance
     * @return void
     */
