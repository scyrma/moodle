...
class enrol_program_plugin extends enrol_plugin {
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        if (true === $enrolstatus) {
            ...
            $groups = \tool_program\api::get_user_groups_in_course($programid, $course, $userid);
            foreach ($groups as $groupid) {
                if (!groups_is_member($groupid, $userid)) {
                    groups_add_member($groupid, $userid, 'enrol_program', $instance->id);
                }
            }
            // Redirect.
            if (!empty($SESSION->wantsurl)) {
                ...
                $destination = course_get_url($course->id);
            }
