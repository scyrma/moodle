...
class enrol_dynamicrule_outcome_course_unenrol_testcase extends advanced_testcase {
    ...
    public function test_get_description() {
        ...
        $configdata = ['coursetounenrol' => $course1->id, 'action' => 0];
        $outcome = course_unenrol::create($rule0->id, $configdata);

        $str1 = get_string('actiondisableenrolment', 'enrol_dynamicrule');
        $strparams = ['coursename' => $course1->fullname, 'action' => $str1];
        $expected = get_string('outcomecourseunenroldescription', 'enrol_dynamicrule', $strparams);
