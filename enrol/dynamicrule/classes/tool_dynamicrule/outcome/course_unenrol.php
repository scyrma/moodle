 ...
 * @license    Moodle Workplace License, distribution is restricted, contact support@moodle.com
 */
class course_unenrol extends outcome_base {
    /** @var \enrol_plugin Enrol plugin instance */
    private $enrolplugin;

    /**
     * @var int possible action
     */
    ...
    public function get_uneditable_description(): string {
        $coursecontext = \context_course::instance($this->get_configdata()['coursetounenrol']);
        if (has_any_capability(['moodle/course:viewparticipants', 'moodle/course:enrolreview'], $coursecontext)) {
            // User can view this course participants.
    ...
    public function get_config_form(\MoodleQuickForm $mform) {
        ...
        $enrol = $mform->addElement('autocomplete', 'enrolmentmethod', get_string('enrolmentmethod', 'enrol'),
            $this->get_allowunenrol_enrolment_methods(), ['multiple' => true]);
    ...
    public function validate_config_form(array $data): array {
        global $DB;
        $errors = [];
        $course = $DB->get_record('course', ['id' => $data['coursetounenrol']]);
        if (!$course) {
            $errors['coursetounenrol'] = get_string('errorinvalidcoursetounenrol', 'enrol_dynamicrule');
        } else if (!api::is_course_category_allowed_in_rule($this->get_rule(), $course->category)) {
            $errors['coursetounenrol'] = get_string('errorinvalidcoursetenant', 'tool_dynamicrule');
        }
