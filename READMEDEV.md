...
```
  git clone --single-branch --branch WORKPLACE_37 --recurse-submodules git@git.in.moodle.com:workplace/workplacedev.git
  cd workplacedev
...
```
git submodule foreach 'git checkout WORKPLACE_37 && git pull'
```
...
```
git checkout WORKPLACE_37 && git pull
```
