...
function theme_workplace_get_main_scss_content($theme) {
    ...
    // Load workplace specific scss variables (loaded first to override some LMS variables).
    $scss .= file_get_contents($CFG->dirroot . '/theme/workplace/scss/workplace/variables.scss');
 ...
 */
function theme_workplace_get_fontawesome_icon_map() {
    return [
        'theme:a/hourglass' => 'fa-hourglass-half',
        'theme:a/arrow-right-circle' => 'fa-arrow-circle-o-right',
    ];
}

/**
 * Callback for theme_workplace, return list of workplace menu items to be added to the the launcher.
 *
 ...
 */
function theme_workplace_theme_workplace_menu_items(): array {
    global $OUTPUT;
    ...
    if (core_course_category::has_capability_on_any(['moodle/category:manage', 'moodle/course:create'])) {
        $menuitems[] = [
            ...
            'imageurl' => $OUTPUT->image_url('menu/courses', 'theme_workplace')->out(false),
            'isglobal' => class_exists(\tool_tenant\permission::class) ? \tool_tenant\permission::can_switch_tenant() : false
        ];
    }
    ...
    if (\core_reportbuilder\permission::can_create_report()) {
        $menuitems[] = [
            'url' => new moodle_url("/reportbuilder/index.php"),
            'name' => get_string('customreports', 'core_reportbuilder'),
            'imageurl' => $OUTPUT->image_url('menu/report_builder', 'theme_workplace')->out(false),
        ];
    }
