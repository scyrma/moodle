...
class behat_theme_workplace_behat_navigation extends behat_navigation {
    ...
    public function i_navigate_to_in_site_administration($nodetext) {
        ...
        if ($nodelist[$nodecount - 1] === get_string('userlist', 'admin')) {
            // We rename the "Browse list of users" administration node to differentiate it from our own "All users" node.
            $nodelist[$nodecount - 1] = get_string('alluserslegacy', 'tool_tenant');
        }
