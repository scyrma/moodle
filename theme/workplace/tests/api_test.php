...
class theme_workplace_api_testcase extends advanced_testcase {
    ...
    /**
     * Test for get_enrolled_courses_for_current_user_by_lowest_enddate for returned data.
     */
    public function test_get_enrolled_courses_for_current_user_by_lowest_enddate_data(): void {
        global $PAGE;
        ...
        $this->assertEquals($course1->id, $exported->id);
    }
    /**
     * Test for get_enrolled_courses_for_current_user_by_lowest_enddate for specified enrolment enddate.
     */
    public function test_get_enrolled_courses_for_current_user_by_lowest_enddate(): void {
        ...
        $this->enrol_user($user->id, $course2, 'manual', 0, $threeweeks);

        // Assert that we get 2 courses.
        $courses = api::get_enrolled_courses_for_current_user_by_lowest_enddate();
        $this->assertCount(2, $courses);
        $this->assertEquals($threeweeks, $courses[$course2->id]->enddate);
    ...
    /**
     * Test for get_enrolled_courses_for_current_user_by_lowest_enddate for hidden courses.
     */
    public function test_get_enrolled_courses_for_current_user_by_lowest_enddate_hidden(): void {
        global $DB;
        $oneweek = strtotime('+1 week');
        $twoweeks = strtotime('+2 week');
        ...
        // Sort courses by name (otherwise they are in reverse sortorder, so same enddate courses will be flipped over).
        $this->setAdminUser();
        $category = core_course_category::get($course1->category);
        ...
        // Now we get 2 courses.
        $courses = api::get_enrolled_courses_for_current_user_by_lowest_enddate();
        $this->assertCount(2, $courses);
        $this->assertEquals([$course3->id, $course1->id], array_keys($courses));

        // Grant editingteacher role to user to see hidden courses.
        $roleid = $DB->get_field('role', 'id', array('shortname' => 'editingteacher'), MUST_EXIST);
        $context = \context_course::instance($course2->id);
        role_assign($roleid, $user->id, $context->id);

        // We get 3 courses again.
        $courses = api::get_enrolled_courses_for_current_user_by_lowest_enddate();
        $this->assertCount(3, $courses);
        $this->assertEquals([$course2->id, $course3->id, $course1->id], array_keys($courses));

        // Remove capability.
        unassign_capability('moodle/course:viewhiddencourses', $roleid);
    ...
    /**
     * Test for get_enrolled_courses_for_current_user_by_lowest_enddate for specified course enddate
     */
    public function test_get_enrolled_courses_for_current_user_by_lowest_enddate_course_enddate(): void {
        $oneweek = strtotime('+1 week');
        $twoweeks = strtotime('+2 week');
        $threeweeks = strtotime('+3 week');
