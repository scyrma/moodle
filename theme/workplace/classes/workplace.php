...
class workplace {
    ...
    public function dashboard() {
        ...
        if (class_exists('\tool_organisation\output\managed_users_view')) {
        ...
        if (class_exists('\tool_program\output\programs_overview_view')) {
            ...
            if (count(\tool_program\api::get_user_accessible_programs($USER->id))) {
                $tab = new stdClass();
                $tab->name = 'programs';
                $tab->active = $template->hastabs ? false : true;
                $tab->classes = 'wp-programs p-3';
                $tab->title = get_string('programsoverview', 'tool_program');
