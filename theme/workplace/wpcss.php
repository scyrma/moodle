...
if ($type === 'editor') {
    ...
    $chunk = null;
} else if (preg_match('/^all-\d+-\d+(-rtl)?$/', $type)) {
    // We're fine.
    null;
} else {
     css_send_css_not_found();
...
if (preg_match('/^all-(\d+)-(\d+)(-rtl)?$/', $type, $matches)) {
    ...
    $tenant = \tool_tenant\tenancy::get_tenants()[$tenantid] ?? null;
    if (empty($tenant)) {
        css_send_css_not_found();
    }
    if ($tenant->timemodified != (int)$matches[2]) {
        $type = 'all-' . $matches[1] . '-' . $tenant->timemodified . ($matches[3] ?? '');
    }
...
// Now after we fixed the revision, type, etc., try to look for existing file again before generating it.
$candidatesheet = "{$candidatedir}/" . theme_workplace_styles_get_filename($type, $themesubrev, $usesvg);
$etag = theme_styles_get_etag($themename, $rev, $type, $themesubrev, $usesvg);
