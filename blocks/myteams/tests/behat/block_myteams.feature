...
Feature: The team overview block allows users to view their teams information
  ...
  Background:
    ...
    And the following "tool_tenant > users" exist:
      ...
      | user17    | User      | 17       | user17@example.com   | ##2022-07-14 09:28:00## | Tenant1 |
  ...
  Scenario: Message team members from team overview block
    ...
    And I select "My teams" from primary navigation
    # Ensure we have a single user, avoiding any random ordering problems.
    And I set the field "Search" in the "[data-region=myteams-search]" "css_element" to "User 12"
  ...
  Scenario: Show users course progress
    ...
    And I click on "//span[text()='Courses']/following::span/a[text()='Full report']" "xpath_element"
    And I should see "Course 1"
    And I should see "Manual enrolments" in the "Course 1" "table_row"
    And I should see "Active" in the "Course 1" "table_row"
    And I should see "1/1" in the "Course 1" "table_row"
  ...
  Scenario: View last access tooltip
    ...
    And I select "My teams" from primary navigation
    And I hover "button[data-region='block_myteams-lastaccessdate']" "css_element"
  ...
  Scenario Outline: Teams members report should not show suspended or not confirmed users to user without permissions
    ...
    And the following "permission overrides" exist:
      | capability              | permission | role             | contextlevel | reference | organisation |
  ...
  Scenario: View the team overview block as a manually assigned manager or subordinates
    Given user "user11" is a manually assigned manager over users "user15,user17" with permissions "7"
    And I run all adhoc tasks
    And I log in as "user11"
    And I select "My teams" from primary navigation
    Then I should see "User 12" in the "Team overview" "block"
    And I should see "User 13" in the "Team overview" "block"
    And I should see "User 15" in the "Team overview" "block"
    And I should see "User 17" in the "Team overview" "block"
    And I should not see "User 14" in the "Team overview" "block"
