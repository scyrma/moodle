...
function block_myteams_block_myteams_global_report_link(): ?global_report_link {
    if (!permission::can_view_course_progress()) {
        return null;
    }
 ...
 */
function block_myteams_block_myteams_global_notification(array $manageduserids): ?global_notification {
    global $DB;

    // Managed users condition.
    [$insql, $inparams] = $DB->get_in_or_equal($manageduserids, SQL_PARAMS_NAMED, 'mu');

    // Perform the query to get the count of managed users that have any overdue course.
    $sql = "SELECT COUNT(u.id)
         ...
         LEFT JOIN {course_completions} cc
             ...
             WHERE u.deleted = 0
               ...
               AND c.enddate > 0 AND c.enddate < :now
               AND u.id $insql";
    $params = $inparams + ['siteid' => SITEID, 'now' => time()];
    $shownotification = $DB->count_records_sql($sql, $params) > 0;

    $url = new moodle_url('/blocks/myteams/courseprogress.php', ['type' => course_progress::COURSE_STATUS_OVERDUE]);
    $message = get_string('managedusersoverduecourses', 'block_myteams');
