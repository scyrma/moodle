...
class course_progress extends system_report {
    ...
    /** @var int Active course status. */
    public const COURSE_STATUS_ACTIVE = 0;
    /** @var int Active course status. */
    public const COURSE_STATUS_COMPLETED = 1;
    /** @var int Active course status. */
    public const COURSE_STATUS_FUTURESTART = 2;
    /** @var int Active course status. */
    public const COURSE_STATUS_OVERDUE = 3;
     ...
     * @return string
     */
    private function get_course_entity_name(): string {
    ...
    protected function initialise(): void {
        ...
        // Define entity.
        $this->annotate_entity($this->get_course_entity_name(), new lang_string('courseprogress', 'block_myteams'));
        ...
        $enrolentity->set_table_alias('enrol', $enrol);
    ...
    protected function add_columns(string $course, string $userenrolment): void {
        ...
        // Simplify enrolment method column title.
        $this->get_column('enrol:name')->set_title(new lang_string('method', 'enrol'));
        ...
        $this->get_column('completion:progresspercent')
            ->set_title(new lang_string('activitycompletion', 'tool_wp'))
    ...
    protected function add_filters(): void {
        ...
        $this->add_filter((new filter(
            select::class,
            'status',
            new lang_string('coursestatus', 'block_myteams'),
            $this->get_course_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_field_sql($this->get_course_status_sql())
            ->set_options([
                self::COURSE_STATUS_ACTIVE => new lang_string('active'),
                self::COURSE_STATUS_COMPLETED => new lang_string('completed'),
                self::COURSE_STATUS_FUTURESTART => new lang_string('futurestart',  'block_myteams'),
                self::COURSE_STATUS_OVERDUE => new lang_string('overdue',  'block_myteams'),
            ])
        );
    ...
    /**
     * Get course status SQL.
     *
     * @return string
     */
    private function get_course_status_sql(): string {
        $course = $this->get_entity('course')->get_table_alias('course');
        $completion = $this->get_entity('completion')->get_table_alias('course_completion');
        $userenrolment = $this->get_entity('enrolment')->get_table_alias('user_enrolments');
        $now = time();
        return "CASE WHEN
                {$completion}.timecompleted IS NOT NULL
                THEN " . self::COURSE_STATUS_COMPLETED . "
                WHEN
                ({$course}.startdate > " . $now . " OR {$userenrolment}.timestart > " . $now . ")
                THEN " . self::COURSE_STATUS_FUTURESTART . "
                WHEN
                ({$course}.enddate > 0 AND {$course}.enddate < " . $now . ")
                THEN " . self::COURSE_STATUS_OVERDUE . "
                ELSE " . self::COURSE_STATUS_ACTIVE . "
            END";
    }
}
