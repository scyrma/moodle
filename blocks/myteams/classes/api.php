...
class api {
     ...
     * @return array
     */
    public static function get_all_global_report_links(): array {
        global $PAGE;
    ...
    /**
     * Look for plugins adding notifications and return them
     *
     * @param int[] $managedusersids list of managed users for whom the current user can view reports
     * @return string[]
     */
    public static function get_all_global_notifications(array $managedusersids): array {
        global $OUTPUT;

        if (empty($managedusersids)) {
            return [];
        }

        $notifications = [];
        $pluginswithcallback = get_plugins_with_function('block_myteams_global_notification');
        foreach ($pluginswithcallback as $plugincallbacks) {
            foreach ($plugincallbacks as $pluginname => $callback) {
                $notification = $callback($managedusersids);
                if (is_null($notification)) {
                    continue;
                }
                if (!($notification instanceof global_notification)) {
                    debugging('Method '.$callback.' must return a \blocks_myteams\global_notification',
                        DEBUG_DEVELOPER);
                    continue;
                }
                // Add notifications returned by the plugin callback.
                $link = html_writer::link($notification->get_url(), get_string('seereport', 'block_myteams'));
                $alert = $OUTPUT->notification($notification->get_message() . ' ' . $link, $notification->get_type());
                $notifications[] = html_writer::div($alert, '', ['data-region' => 'myteams-notification-' . $pluginname]);
            }
        }
