...
const saveHiddenCardsPreference = (blockRegion) => {
    ...
    const value = JSON.stringify(hiddenCards.map((card) => card.dataset.courseid));

    return setUserPreference('block_myinprogress_hidden_courses', value, carousel.dataset.userid);
};
...
const saveShowHiddenCardsPreference = (userId, value) => {
    return setUserPreference('block_myinprogress_show_hidden_cards', value, userId);
};
