...
class mod_appointment_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $mform->addElement('text', 'thirdparty', get_string('thirdpartyemailaddress', 'appointment'), ['size' => '64']);
    ...
    public function data_postprocessing($data) {
        ...
        if (!empty($data->completionunlocked)) {
            if (empty($data->completionbooked)) {
                $data->completionbooked = 0;
            }
            if (empty($data->completionattended)) {
    ...
    /**
     * Adds 'completionbooked' completion rule.
     *
     * @return string[]
     */
    public function add_completion_rules() {
        ...
        $mform->addElement('checkbox', 'completionbooked', '', get_string('completiondetail:booked', 'appointment'));
        $mform->addHelpButton('completionbooked', 'completionbooked', 'appointment');
        $mform->addElement('checkbox', 'completionattended', '', get_string('completionattended', 'appointment'));
        $mform->addHelpButton('completionattended', 'completionattended', 'appointment');
        // Disable these completion rules by default.
        $mform->setDefault('completionbooked', 0);
        $mform->setDefault('completionattended', 0);
        return ['completionbooked', 'completionattended'];
    }
    ...
    public function completion_rule_enabled($data) {
        return !empty($data['completionbooked'])
            || !empty($data['completionattended']);
    }
}
