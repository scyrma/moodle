...
$PAGE->set_url('/mod/appointment/index.php', ['id' => $id]);
...
if ($usesections) {
    ...
    $table->head  = [$strsectionname, $strappointmentname, get_string('sign-ups', 'appointment')];
    $table->align = ['left', 'left', 'center'];
} else {
    $table->head  = [$strappointmentname, get_string('sign-ups', 'appointment')];
...
foreach ($appointments as $appointment) {
    ...
    if (!$appointment->visible) {
        ...
        $link = html_writer::link("view.php?f=$appointment->id", $appointment->name, ['class' => 'dimmed']);
    } else {
    ...
    if ($usesections) {
        $table->data[] = [$printsection, $link, $totalsignupcount];
    } else {
        $table->data[] = [$link, $totalsignupcount];
    }
}
