...
class restore_appointment_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('appointment_session_data',
            '/activity/appointment/sessions/session/session_data/session_data_element');
        $paths[] = new restore_path_element('appointment_session_field',
            '/activity/appointment/sessions/session/session_field/session_field_element');
        if ($userinfo) {
    ...
    /**
     * Process appointment session data
     *
     * @param stdClass $data The data in object form
     * @return void
     */
    protected function process_appointment_session_data($data) {
        global $DB;
        ...
        $data->sessionid = $this->get_new_parentid('appointment_session');
        $data->fieldid = $this->get_mappingid('appointment_session_field');

        // Insert the entry record.
        $newitemid = $DB->insert_record('appointment_session_data', $data);
        $this->set_mapping('appointment_session_data', $oldid, $newitemid, true); // Childs and files by itemname.
    }

    /**
     * Process appointment session field
     *
     * @param stdClass $data The data in object form
     * @return void
     */
    protected function process_appointment_session_field($data) {
        global $DB;
        ...
        // Insert the entry record.
        $newitemid = $DB->insert_record('appointment_session_field', $data);
    }
    ...
    protected function after_execute() {
        // Appointment doesn't have any related files.
        // Add appointment related files, no need to match by itemname (just internally handled context).
    }
}
