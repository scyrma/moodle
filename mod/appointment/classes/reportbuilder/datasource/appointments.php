...
class appointments extends datasource {
     ...
     * Initialise report
     *
     * @uses \tool_tenant\sharedspace::is_shared_space()
     * @uses \tool_tenant\tenancy::get_tenant_id()
     * @uses \tool_tenant\tenancy::get_tenants()
     */
    protected function initialise(): void {
        ...
        // Default value is true (making it !true), so if the method is not present,
        // it will not enter the condition and it will not continue getting any current tenant.
        if (!component_class_callback('tool_tenant\\sharedspace', 'is_shared_space', [], true)) {
            // Only filter courses in the category of the current tenant.
            $tenantid = component_class_callback('tool_tenant\\tenancy', 'get_tenant_id', [], 0);
            $tenants = component_class_callback('tool_tenant\\tenancy', 'get_tenants', [], []);
            $categoryid = $tenants[$tenantid]->categoryid;
        ...
        if (class_exists('\tool_organisation\reportbuilder\local\entities\job')) {
            // Add Job entity.
            $jobentity = new \tool_organisation\reportbuilder\local\entities\job();
            $job = $jobentity->get_table_alias('tool_organisation_job');
            $jobjoin = "LEFT JOIN {tool_organisation_job} {$job} ON {$job}.userid = {$user}.id AND " .
                \tool_organisation\reportbuilder\local\entities\job::get_job_tenant_join("{$job}.tenantid");
