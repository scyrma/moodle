...
class session extends modal_form {
    ...
    public function definition() {
        ...
        $mform->addElement('hidden', 'instanceid', $cm->instance);
    ...
    protected function save(\stdClass $data, array $sessiondates) {
        ...
        $cm = $DB->get_record('course_modules', ['id' => $cmid], '*', MUST_EXIST);
        $course = $DB->get_record('course', ['id' => $cm->course], '*', MUST_EXIST);
        $appointment = $DB->get_record('appointment', ['id' => $data->instanceid], '*', MUST_EXIST);

        $context = \context_module::instance($cmid);

        $todb = new \stdClass();
        $todb->appointment = $data->instanceid;
        $todb->capacity = $data->capacity;
        $todb->allowwaitlist = $data->allowwaitlist;
        $todb->duration = $data->duration;
        if (has_capability('mod/appointment:configurecancellation', $context)) {
            $todb->allowcancellations = $data->allowcancellations;
        }
        ...
        if ($session != null) {
            ...
            $todb->id = $session->id;
            if (!appointment_update_session($todb, $sessiondates)) {
                $transaction->force_transaction_rollback();
                print_error('error:couldnotupdatesession', 'appointment');
            }

            // Remove old site-wide calendar entry.
            if (!appointment_remove_session_from_calendar($session, SITEID)) {
                $transaction->force_transaction_rollback();
                print_error('error:couldnotupdatecalendar', 'appointment');
            }
        } else {
            if (!$sessionid = appointment_add_session($todb, $sessiondates)) {
                $transaction->force_transaction_rollback();
                print_error('error:couldnotaddsession', 'appointment');
            }
        }
        ...
        if (!$session = appointment_get_session($sessionid)) {
            $transaction->force_transaction_rollback();
            print_error('error:couldnotfindsession', 'appointment');
        }
        ...
        $savedata = file_postupdate_standard_editor($data, 'details', $this->get_editor_options($context),
            $context, 'mod_appointment', 'session', $session->id);
        $DB->update_record('appointment_sessions',
            ['id' => $session->id, 'details' => $savedata->details, 'detailsformat' => $savedata->detailsformat]);
    }
