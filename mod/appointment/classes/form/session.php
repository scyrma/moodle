...
class session extends dynamic_form {
    ...
    public function definition() {
        ...
        $appointment = $DB->get_record('appointment', ['id' => $this->get_cm()->instance], '*', MUST_EXIST);
        ...
        $repeatarray = [];
        $repeatarray[] = $mform->createElement('html', html_writer::start_div('border rounded p-4 mb-3 br-1 bg-gray-200'));
        ...
        $mform->addHelpButton('capacity', 'capacity', 'appointment');
        $mform->setDefault('capacity', 10);

        $mform->addElement('checkbox', 'allowwaitlist', get_string('allowwaitlist', 'appointment'));
        $mform->addHelpButton('allowwaitlist', 'allowwaitlist', 'appointment');
        ...
        if (has_capability('mod/appointment:configurecancellation', $context)) {
            ...
            $mform->addHelpButton('allowcancellations', 'allowcancellations', 'appointment');
        }
    ...
    /**
     * Current course module where session is added or updated
     *
     * @return \cm_info
     */
    protected function get_cm(): \cm_info {
        if (!$this->cm) {
            if ($session = $this->get_session()) {
                [$course, $cm] = get_course_and_cm_from_instance($session->appointment, 'appointment');
            } else {
                $cmid = $this->optional_param('cmid', 0, PARAM_INT);
                [$course, $cm] = get_course_and_cm_from_cmid($cmid, 'appointment');
            }
            $this->cm = $cm;
        }
        return $this->cm;
    ...
    protected function save(\stdClass $data, array $sessiondates) {
        ...
        if (!empty($data->sessionid)) {
            ...
            appointment_update_session($data, $sessiondates);
        } else {
            // Create session.
