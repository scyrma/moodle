...
Feature: Use custom fields with appointments
  ...
  Scenario: Custom field should be shown when the value is set to 0
    When I log in as "teacher1"
    And I am on "Course 1" course homepage
    And I follow "Test appointment"
    And I follow "Add"
    ...
    And I set the following fields in the "Adding appointment" "dialogue" to these values:
      ...
      | endtime[0][hour]     | 02                |
      | endtime[0][minute]   | 00                |
      | Example number field | 0                 |
    And I click on "Save" "button" in the "Adding appointment" "dialogue"
    And I click on "[data-toggle='collapse']" "css_element" in the "1:00" "table_row"
    And I should see "Example number field"
  ...
  Scenario: Custom fields default values are shown in appointment forms
    Given the following "custom fields" exist:
      | name                  | category     | type     | shortname | configdata                                                        |
      | Default checkbox      | Other fields | checkbox | checkbox  | {"checkbydefault":1}                                              |
      | Default dropdown menu | Other fields | select   | select    | {"options":"AAA\nBBB\nCCC\nDDD","defaultvalue":"BBB"}             |
      | Default short text    | Other fields | text     | text      | {"defaultvalue":"Default short text value"}                       |
      | Default text area     | Other fields | textarea | textarea  | {"defaultvalue":"Default text area value","defaultvalueformat":1} |
    When I log in as "teacher1"
    And I am on "Course 1" course homepage
    And I follow "Test appointment"
    And I follow "Add"
    ...
    And I expand all fieldsets
    Then the following fields match these values:
      | Default short text    | Default short text value |
      | Default text area     | Default text area value  |
      | Default dropdown menu | BBB                      |
      | Default checkbox      | 1                        |
