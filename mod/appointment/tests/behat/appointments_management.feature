...
Feature: Manage appointments
  ...
  Scenario: Duplicate appointment
    ...
    And I click on "[data-toggle='collapse']" "css_element" in the "##tomorrow##%A, %d %B %Y##" "table_row"
    And I should see "Some example text" in the "##tomorrow##%A, %d %B %Y##" "table_row"
    And I should see "Test field value c" in the "##tomorrow##%A, %d %B %Y##" "table_row"
    And I click on "[data-toggle='collapse']" "css_element" in the "##tomorrow##%A, %d %B %Y##" "table_row"
  ...
  Scenario: Add attendees manually shows only users enroled to the course
    ...
    Then the "Potential Attendees" select box should contain "Rudolph Ritchie"
    And the "Potential Attendees" select box should contain "Din Djarin"
    And the "Potential Attendees" select box should not contain "Lionel Reindeer"
    And I set the field "addselect_searchtext" to "Rudolph"
    And I wait "1" seconds
    And the "Potential Attendees" select box should contain "Rudolph Ritchie"
    And the "Potential Attendees" select box should not contain "Din Djarin"
    And I log out
