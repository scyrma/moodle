...
if (!$session = appointment_get_session($s)) {
    print_error('error:incorrectcoursemodulesession', 'appointment');
}
...
if (optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstoassign)) {
        foreach ($userstoassign as $adduser) {
            ...
            if (!has_capability('moodle/course:view', $context, $adduser)) {
                ...
                if (!is_enrolled($context, $user)) {
                    if (!enrol_try_internal_enrol($course->id, $user->id)) {
                        $errors[] = get_string('error:enrolmentfailed', 'appointment', fullname($user));
                        $errors[] = get_string('error:addattendee', 'appointment', fullname($user));
            ...
            if (appointment_get_user_submissions($appointment->id, $adduser)) {
                $erruser = $DB->get_record('user', array('id' => $adduser), "id, {$usernamefields}");
                $errors[] = get_string('error:addalreadysignedupattendee', 'appointment', fullname($erruser));
            } else {
...
if (optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstoremove)) {
        foreach ($userstoremove as $removeuser) {
            ...
            if (appointment_user_cancel($session, $removeuser, true, $cancelerr)) {

                // Notify the user of the cancellation if the session hasn't started yet.
                $timenow = time();
                if (!$suppressemail and !appointment_has_session_started($session, $timenow)) {
                    appointment_send_cancellation_notice($appointment, $session, $removeuser);
                }
            } else {
                $errors[] = $cancelerr;
                $usernamefields = get_all_user_name_fields(true);
                $erruser = $DB->get_record('user', array('id' => $removeuser), "id, {$usernamefields}");
                $errors[] = get_string('error:removeattendee', 'appointment', fullname($erruser));
            }
        }
        $potentialuserselector->invalidate_selected_users();
        $existinguserselector->invalidate_selected_users();
...
echo $OUTPUT->heading(get_string('addremoveattendees', 'appointment'));
