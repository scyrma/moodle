...
$potentialuserselector = new appointment_candidate_selector('addselect', array('sessionid' => $session->id));
$existinguserselector = new appointment_existing_selector('removeselect', array('sessionid' => $session->id));
...
$PAGE->set_cm($cm);
$PAGE->set_url('/mod/appointment/editattendees.php', array('s' => $s, 'backtoallsessions' => $backtoallsessions));

$PAGE->set_title($pagetitle);
$PAGE->set_heading($course->fullname);
$PAGE->activityheader->set_attrs(['hidecompletion' => true, 'description' => '',
    'title' => get_string('addremoveattendees', 'appointment')]);
$url = new moodle_url('/mod/appointment/attendees.php', array('s' => $session->id, 'backtoallsessions' => $backtoallsessions));
...
$content = html_writer::start_tag('p') . html_writer::tag('label',
        get_string('potentialattendees', 'appointment'), array('for' => 'addselect')) . html_writer::end_tag('p');
...
$nonattendeesrs = $DB->get_recordset_sql(
     "SELECT
        ...
        AND ss.statuscode = ?
        ORDER BY
            u.lastname, u.firstname", array($session->id, MOD_APPOINTMENT_STATUS_REQUESTED)
);
