...
define('MOD_APPOINTMENT_MANAGERSEMAIL_FIELD', 'managersemail');
 ...
 *
 * @return array mapping between status code and corresponding string identifier
 */
 ...
 * @return string String identifier for this status code
 */
...
function appointment_add_instance($appointment) {
    ...
    $appointment->id = $DB->insert_record('appointment', $appointment);
...
/**
 * Checks all customfield_category for appointment module and checks
 * its custom fields. Adds the given ones when they don't exist.
 *
 * @param array $fields
 */
function appointment_create_customfields(array $fields) {
    $handler = \core_customfield\handler::get_handler('mod_appointment', 'appointment', 0);
    ...
    foreach ($categories as $category) {
        $existingfields[] = array_map(function($field): string {
            return $field->get('shortname');
        }, $category->get_fields());
    }
    $existingfields = array_merge(...$existingfields);
    $fieldstocreate = array_diff($fields, $existingfields);
    ...
    foreach ($fieldstocreate as $fieldname) {
        $record = (object)[
                ...
                'name' => get_string_manager()->string_exists($fieldname, 'mod_appointment') ?
                    get_string($fieldname, 'mod_appointment') : ucfirst($fieldname),
                'shortname' => $fieldname,
                'configdata' => [
                    'ispassword' => false,
                    'displaysize' => 50,
...
function appointment_reset_course_form_definition(&$mform) {
    $mform->addElement('header', 'appointmentheader', get_string('modulenameplural', 'appointment'));
    ...
    foreach ($appointments as $appointment) {
            ...
            foreach ($sessions as $session) {
                ...
                foreach ($attendees as $attendee) {
                    ...
                    if ($completion->is_enabled($cm) == COMPLETION_TRACKING_AUTOMATIC &&
                        ($appointment->completionbooked || $appointment->completionattended)) {
        ...
        if (!empty($data->timeshift)) {
            ...
            foreach ($sessions as $session) {
                ...
                appointment_update_session($session, $shiftedsessiondates);
            }
...
function appointment_delete_instance($id) {
    ...
    if (!$appointment = $DB->get_record('appointment', ['id' => $id])) {
        return false;
    }

    if (!$cm = get_coursemodule_from_instance('appointment', $appointment->id)) {
        return false;
    }
    ...
    $DB->delete_records('appointment', ['id' => $appointment->id]);
 ...
 */
function appointment_cleanup_session_data($session) {
...
function appointment_add_session(\stdClass $session, array $sessiondates, $context): int {
    ...
    $session->appointment = $cminstance;
    ...
    $session = appointment_cleanup_session_data($session);
 ...
 * @return int session id
 */
function appointment_update_session($session, $sessiondates) {
    global $DB;

    $oldsession = appointment_get_session($session->id, MUST_EXIST);
    [$course, $cm] = get_course_and_cm_from_instance($oldsession->appointment, 'appointment');
    $appointment = $DB->get_record('appointment', ['id' => $oldsession->appointment], '*', MUST_EXIST);
    $session = appointment_cleanup_session_data($session);
    $session->appointment = $oldsession->appointment; // Just in case.
    $context = context_module::instance($cm->id);
    ...
    $DB->delete_records('appointment_sessions_dates', ['sessionid' => $session->id]);
...
/**
 * Update session calendar entries for a given appointment
 *
 * @param stdClass $appointment
 * @return void
 */
function appointment_update_site_calendar_events_visibility(stdClass $appointment): void {
    global $DB;
    $sessions = appointment_get_sessions($appointment->id);
    if (empty($sessions)) {
        return;
    }

    $coursevisible = $DB->get_field('course', 'visible', ['id' => $appointment->course]);
    $visible = $coursevisible && instance_is_visible('appointment', $appointment);
    foreach ($sessions as $session) {
        // Determine event.
        $event = $DB->get_record('event',
            ['modulename' => 0, 'courseid' => 1, 'instance' => $session->appointment, 'uuid' => $session->id]);
        $event = new calendar_event($event);
        $event->toggle_visibility($visible);
    }
}
...
function appointment_update_attendees($session) {
    ...
    // Get course.
    $course = $DB->get_record('course', ['id' => $appointment->course]);
        ...
        if (empty($session->sessiondates)) {
            ...
            foreach ($users as $user) {
                if ($user->statuscode == MOD_APPOINTMENT_STATUS_BOOKED) {
                    appointment_user_signup($session, $appointment, $course, MOD_APPOINTMENT_STATUS_WAITLISTED, $user->id);
                }
            }
        } else {
            ...
            if ($booked < $capacity) {
                foreach ($users as $user) {
                    ...
                    if ($user->statuscode == MOD_APPOINTMENT_STATUS_WAITLISTED) {
                        appointment_user_signup($session, $appointment, $course, MOD_APPOINTMENT_STATUS_BOOKED, $user->id);
                        $booked++;
                    }
                }
            }
...
function appointment_delete_session($session) {
    ...
    $signedupusers = $DB->get_records_sql(
        "
            ...
            AND ss.superceded = 0
            AND ss.statuscode >= ?
        ", [$session->id, MOD_APPOINTMENT_STATUS_REQUESTED]);

    // Remove entries from user calendar.
    if ($signedupusers && count($signedupusers) > 0) {
        foreach ($signedupusers as $user) {
            appointment_user_cancel($session, $user->userid, true);
    ...
    $DB->delete_records('appointment_sessions', ['id' => $session->id]);
    $DB->delete_records('appointment_sessions_dates', ['sessionid' => $session->id]);
    $DB->delete_records_select(
        ...
        ");
    $DB->delete_records('appointment_signups', ['sessionid' => $session->id]);
    $transaction->allow_commit();
...
function appointment_email_substitutions($msg, $appointmentname, $reminderperiod, $user, $data, $sessionid) {
    ...
    $placeholders = [
        ...
        'details' => empty($data->details) ? '' : html_to_text($data->details),
        'reminderperiod' => $reminderperiod,
        'attendeeslink' => (new \moodle_url('/mod/appointment/attendees.php', ['s' => $sessionid]))->out(),
    ];
    ...
    // Process localised placeholders.
    // TODO WP-4085: This will not be required once localised placeholders are removed.
    foreach ($placeholders as $placeholder => $value) {
        $placeholder = get_string("placeholder:{$placeholder}", 'appointment');
        $msg = str_replace($placeholder, $value, $msg);
    }
...
function appointment_cron() {
    ...
    foreach ($signupsdata as $signupdata) {
        if (appointment_has_session_started($signupdata, $timenow)) {
            ...
            $DB->update_record('appointment_signups', $newsubmission);
            continue;
        }
        ...
        if (!$appointment = $DB->get_record('appointment', ['id' => $signupdata->appointmentid])) {
            continue;
        }
        ...
        email_to_user($manager, $from, $postsubject, $managertext, $posthtml);
    }
    return true;
}
 ...
 * @return bool
 */
function appointment_has_not_started_session(stdClass $session, int $timenow): bool {
...
function appointment_get_session_dates($sessionid) {
    ...
    $ret = [];
    if ($dates = $DB->get_records('appointment_sessions_dates', ['sessionid' => $sessionid], 'timestart')) {
        $i = 0;
 ...
 */
function appointment_get_session($sessionid, int $strictness = IGNORE_MISSING) {
    global $DB;

    $session = $DB->get_record('appointment_sessions', ['id' => $sessionid], '*', $strictness);
    if ($session) {
...
function appointment_get_attendees($sessionid) {
    ...
    $records = $DB->get_records_sql("
        ...
        ORDER BY
            ...
            ss.timecreated ASC
    ", [$sessionid, MOD_APPOINTMENT_STATUS_BOOKED, MOD_APPOINTMENT_STATUS_WAITLISTED, $sessionid,
 ...
 */
function appointment_user_signup($session, $appointment, $course, int $statuscode,
                                 ?int $userid = null, $notifyuser = true): bool {
    global $DB, $USER;
    ...
    if ($existingsignup = $DB->get_record('appointment_signups', ['sessionid' => $session->id, 'userid' => $userid])) {
    ...
    if (!empty($usersignup->id)) {
        $DB->update_record('appointment_signups', $usersignup);
    } else {
    ...
    // Update status.
    appointment_update_signup_status($usersignup->id, $newstatus, $userid);

    // Add to user calendar -- if appointment usercalentry is set to true.
    if ($appointment->usercalentry) {
        if (in_array($newstatus, [MOD_APPOINTMENT_STATUS_BOOKED, MOD_APPOINTMENT_STATUS_WAITLISTED])) {
    ...
    if ($notifyuser) {
        ...
        switch ($newstatus) {
            case MOD_APPOINTMENT_STATUS_BOOKED:
                $error = appointment_send_confirmation_notice($appointment, $session, $userid, false);
                break;

            case MOD_APPOINTMENT_STATUS_WAITLISTED:
                $error = appointment_send_confirmation_notice($appointment, $session, $userid, true);
                break;
...
function appointment_send_request_notice($appointment, $session, $userid) {
    ...
    email_to_user($user, $from, $postsubject, $posttextmgrheading . $posttext);
...
function appointment_update_signup_status($signupid, $statuscode, $createdby, $note = '', $grade = null) {
    ...
    $statusid = $DB->insert_record('appointment_signups_status', $signupstatus);
    ...
    // Recalculate activity completion.
    $appointment = $DB->get_record_sql('SELECT a.id, a.course, a.completionbooked, a.completionattended, su.userid
        FROM {appointment} a
        JOIN {appointment_sessions} s ON s.appointment = a.id
        JOIN {appointment_signups} su ON su.sessionid = s.id
        WHERE su.id = ?', [$signupid]);
    if ($appointment && ($appointment->completionbooked || $appointment->completionattended)) {
        $userid = $appointment->userid;
        [$course, $cm] = get_course_and_cm_from_instance($appointment->id, 'appointment', $appointment->course, $userid);
        $completion = new completion_info($course);
        if ($completion->is_enabled($cm) == COMPLETION_TRACKING_AUTOMATIC) {
 ...
 * @return bool returns true if user appointment was cancelled or the user did not have an appointment;
 *              returns false if the user has an appointment and it could not be cancelled, in this case
 *              the $errorstr will contain the error message
 */
function appointment_user_cancel($session, $userid = false, $forcecancel = false, &$errorstr = null, $cancelreason = '') {
    ...
    appointment_user_cancel_submission($session->id, $userid, $cancelreason);
 ...
 * @param string $appointmentnotification The type of notification to send
 * @param stdClass $appointment record from the appointment table
 * @param stdClass $session record from the appointment_sessions table
...
function appointment_send_notice($postsubject, $posttext, $posttextmgrheading,
                                 $appointmentnotification, $appointment, $session, $userid) {
    global $DB;
    ...
    if ($appointmentnotification & MOD_APPOINTMENT_INVITE) {
        $attachmentfilename = 'invite.ics';
    } else if ($appointmentnotification & MOD_APPOINTMENT_CANCEL && !appointment_was_user_on_waitlist($session, $userid)) {
        $attachmentfilename = 'cancel.ics';
    } else if ($appointmentnotification & MOD_APPOINTMENT_UPDATE && !appointment_is_user_on_waitlist($session, $userid)) {
    ...
    if (!empty($session->sessiondates)) {
        ...
        foreach ($sessiondatessplit as $sessiondatesplit) {
            ...
            $filename = $skipattachment ? '' : appointment_get_ical_attachment($appointmentnotification,
            ...
            $body = appointment_email_substitutions($posttext, $appointment->name, $appointment->reminderperiod,
                $user, $session, $session->id);
            $htmlbody = ''; // TODO.
            $icalattachments[] = ['filename' => $filename, 'subject' => $subject,
    ...
    foreach ($icalattachments as $attachment) {
        email_to_user($user, $from, $attachment['subject'], $attachment['body'],
    ...
    if (!empty($posttextmgrheading) && !empty($manageremail) && !empty($session->sessiondates)) {
        $managertext = $posttextmgrheading . $posttext;
        $manager = $user;
    ...
    if (!empty($appointment->thirdparty) &&
        ...
        foreach ($recipients as $recipient) {
            ...
            email_to_user($thirdparty, $from, $postsubject, $posttext, $posthtml);
        }
    }
 ...
 * @return string Error message (or empty string if successful)
 */
function appointment_send_confirmation_notice($appointment, $session, $userid, $iswaitlisted) {
    ...
    if (!$iswaitlisted) {
        ...
        // Set invite bit.
        $appointmentnotification = MOD_APPOINTMENT_INVITE;
    } else {
        $postsubject = $appointment->waitlistedsubject;
        $posttext = $appointment->waitlistedmessage;
        // Set invite bit.
        $appointmentnotification = MOD_APPOINTMENT_WAITLIST;
    }
...
function appointment_send_cancellation_notice($appointment, $session, $userid) {
    ...
    return appointment_send_notice($postsubject, $posttext, $posttextmgrheading,
        MOD_APPOINTMENT_CANCEL, $appointment, $session, $userid);
}
...
function appointment_send_update_notice($appointment, $session, $userid) {
    ...
    return appointment_send_notice($postsubject, $posttext, '',
        MOD_APPOINTMENT_UPDATE, $appointment, $session, $userid);
}
...
function appointment_get_manageremail($userid) {
    global $DB;
    $fieldid = $DB->get_field('user_info_field', 'id', ['shortname' => MOD_APPOINTMENT_MANAGERSEMAIL_FIELD]);
    if ($fieldid) {
...
function appointment_take_attendance($data) {
    ...
    // Load session.
    $session = appointment_get_session($sessionid, MUST_EXIST);

    // Check appointment has finished.
    if (!appointment_has_session_started($session, time())) {
        throw new moodle_exception('error:canttakeattendanceforunstartedsession', 'appointment');
    }
    ...
    foreach ($data as $key => $value) {
        ...
        if ($submissionidcheck == 'submissionid_') {
            ...
            appointment_take_individual_attendance($submissionid, $grade);
        }
    }
...
function appointment_approve_requests($data) {
    ...
    // Load appointment.
    if (!$appointment = $DB->get_record('appointment', ['id' => $session->appointment])) {
        debug('Appointment: Could not load appointment instance');
        return false;
    }

    // Load course.
    if (!$course = $DB->get_record('course', ['id' => $appointment->course])) {
    ...
    foreach ($data->requests as $key => $value) {
        ...
        switch ($value) {
            ...
            case 2:
                ...
                $cm = get_coursemodule_from_instance('appointment', $appointment->id, $course->id, 0, MUST_EXIST);
...
function appointment_cm_info_view(cm_info $coursemodule) {
    ...
    if (!($appointment = $DB->get_record('appointment', ['id' => $coursemodule->instance]))) {
        return null;
    }
    ...
    if ($submissions = appointment_get_user_submissions($appointment->id, $USER->id)) {
        // User has signedup for the instance, pick the current record.
        $submission = array_shift($submissions);
        $session = appointment_get_session($submission->sessionid);
        if (empty($session->sessiondates)) {
            $afterlink = get_string('status_'.appointment_get_status($submission->statuscode), 'appointment');
        } else {
            foreach ($session->sessiondates as $date) {
                ...
                if ($sessionobj->startdate == $sessionobj->enddate) {
                    ...
                    $bookedat = get_string($sessiondatelangkey, 'appointment', $sessionobj);
                } else {
            ...
            $afterlink = get_string('bookedat', 'appointment', $bookedat);
        }
    }
    ...
    if (!empty($afterlink)) {
        $coursemodule->set_content($afterlink);
    }
}
 ...
 *
 * @param int $appointmentnotification The method, see {{MOD_APPOINTMENT_INVITE}}
 ...
 */
function appointment_get_ical_attachment($appointmentnotification, $appointment, $session, $user) {
    global $CFG, $DB;
    ...
    $ical = new iCalendar();
    $icalmethod = ($appointmentnotification & MOD_APPOINTMENT_CANCEL) ? 'CANCEL' : 'PUBLISH';
    ...
    $sequence = ($appointmentnotification & MOD_APPOINTMENT_CANCEL) ? (int) $session->countmodified + 1 : $session->countmodified;
    ...
    foreach ($session->sessiondates as $date) {
        ...
        if ($appointmentnotification & MOD_APPOINTMENT_CANCEL) {
...
function appointment_is_user_on_waitlist($session, $userid = null) {
    ...
    return $DB->record_exists_sql($sql, [$session->id, $userid, MOD_APPOINTMENT_STATUS_WAITLISTED]);
}
...
function appointment_grade_item_update($appointment, $grades = null) {
    ...
    if (!isset($appointment->cmidnumber)) {
        ...
        $appointment->cmidnumber = $DB->get_field_sql($sql, [$appointment->id]);
    }

    $params = ['itemname' => $appointment->name,
...
function appointment_grade_item_delete($appointment) {
    $retcode = grade_update('mod/appointment', $appointment->course, 'mod', 'appointment',
        $appointment->id, 0, null, ['deleted' => 1]);
    return ($retcode === GRADE_UPDATE_OK);
}
...
function appointment_get_num_attendees($sessionid, $status = MOD_APPOINTMENT_STATUS_BOOKED) {
    ...
    // TODO WP-3232: this function is sometimes called with $status=MOD_APPOINTMENT_STATUS_BOOKED
    // and sometimes with $status=MOD_APPOINTMENT_STATUS_APPROVED (which will include wait-listed attendees).
    // It should be changed to return number of attendees "with wait-listed" or "without wait-listed".
    // The status "approved" is always considered as booked, however we don't have approval functionality
    // yet and this bug is not visible.
    ...
    $sql = 'SELECT count(ss.id)
        ...
        AND
            ss.superceded=0
        AND
        ss.statuscode >= ? AND ss.statuscode != ?';

    // For the session, pick signups that haven't been superceded, or cancelled.
    return (int)$DB->count_records_sql($sql, [$sessionid, $status, MOD_APPOINTMENT_STATUS_NO_SHOW]);
}
...
function appointment_get_user_submissions($appointmentid, $userid, $includecancellations = false) {
    ...
    $whereparams = [$appointmentid, $userid];
    ...
    if (!$includecancellations) {
        ...
        $whereparams = array_merge($whereparams, [MOD_APPOINTMENT_STATUS_REQUESTED, MOD_APPOINTMENT_STATUS_NO_SHOW]);
    }
...
function appointment_user_cancel_submission($sessionid, $userid, $cancelreason = '') {
    ...
    $signup = $DB->get_record('appointment_signups', ['sessionid' => $sessionid, 'userid' => $userid]);
    if (!$signup) {
        return true; // Not signed up, nothing to do.
    }

    appointment_update_signup_status($signup->id, MOD_APPOINTMENT_STATUS_USER_CANCELLED, $userid, $cancelreason);
    return true;
}
...
function appointment_user_complete($course, $user, $mod, $appointment) {
    ...
    if ($submissions = appointment_get_user_submissions($appointment->id, $user->id, true)) {
        ...
        foreach ($submissions as $submission) {
            ...
            if ($submission->statuscode == MOD_APPOINTMENT_STATUS_USER_CANCELLED) {
                print get_string('usercancelledon', 'appointment', $timesignedup) .
                    html_writer::empty_tag('br');
            } else if ($submission->statuscode == MOD_APPOINTMENT_STATUS_BOOKED) {
                print get_string('usersignedupon', 'appointment', $timesignedup) .
                    html_writer::empty_tag('br');
            } else {
                print get_string('status_'.appointment_get_status($submission->statuscode), 'appointment') .
                    ' '. $timesignedup;
            }
        }
    } else {
...
function appointment_add_session_to_calendar($session, $appointment, $calendartype = 'none', $userid = 0,
    ...
    foreach ($session->sessiondates as $date) {
        ...
        $newevent->visible = $course && $course->visible && instance_is_visible('appointment', $appointment);
        ...
        if ($calendartype == 'user' && $eventtype == 'booking') {
            ...
            $DB->delete_records_select('event', 'userid = ? AND instance = ? AND '
                ...
                [$userid, $session->appointment, 'appointmentsession', $shortname]);
        }
...
function appointment_remove_session_from_calendar($session, $courseid = 0, $userid = 0) {
    ...
    if (empty($userid)) { // Match any UserID.
        $params = [$modulename, $session->appointment, $courseid, $session->id];
        return $DB->delete_records_select('event', "modulename = ? AND
                                                    instance = ? AND
                                                    courseid = ? AND
                                                    uuid = ?", $params);
    } else {
        $params = [$modulename, $session->appointment, $userid, $courseid, $session->id];
        return $DB->delete_records_select('event', "modulename = ? AND
                                                    instance = ? AND
                                                    userid = ? AND
                                                    courseid = ? AND
...
function appointment_delete_user_calendar_events($session, $eventtype) {
    ...
    $whereparams = [$session->appointment, $session->id];
...
function appointment_get_available_seats_count(array $sessions, context_module $contextmodule): int {
    ...
    foreach ($sessions as $session) {
        if (\mod_appointment\permission::is_session_bookable($session, $contextmodule)) {
            $signupcount = appointment_get_num_attendees($session->id);
            $placesleft += max(0, $session->capacity - $signupcount);
        }
    }
...
function appointment_print_session($session, $showcapacity, $calendaroutput = false, $return = false, $hidesignup = false) {
    ...
    if (empty($session->sessiondates)) {
        $table->data[] = [$strdatetime, html_writer::tag('i', get_string('notset', 'appointment'))];
    } else {
        $html = '';
        foreach ($session->sessiondates as $date) {
            if (!empty($html)) {
    ...
    if ($showcapacity) {
        if ($session->allowwaitlist) {
            $table->data[] = [get_string('capacity', 'appointment'), $session->capacity .
                ' (' . strtolower(get_string('allowwaitlist', 'appointment')) . ')', ];
        } else {
            $table->data[] = [get_string('capacity', 'appointment'), $session->capacity];
        }
    } else if (!$calendaroutput) {
        $table->data[] = [get_string('seatsavailable', 'appointment'), max(0, $placesleft)];
    }
    ...
    if ($appointment->approvalreqd) {
        $table->data[] = ['', get_string('sessionrequiresmanagerapproval', 'appointment')];
    }

    // Display waitlist notification.
    if (!$hidesignup && $session->allowwaitlist && $placesleft < 1) {
        $table->data[] = ['', get_string('userwillbewaitlisted', 'appointment')];
    }
    ...
    if (!empty($session->details)) {
        ...
        $table->data[] = [get_string('details', 'appointment'), $details];
    }
...
function appointment_get_cancellations($sessionid) {
    ...
    $usernamefields = \core_user\fields::for_name()->get_sql('u', false, '', '', false)->selects;
    $instatus = [MOD_APPOINTMENT_STATUS_BOOKED, MOD_APPOINTMENT_STATUS_WAITLISTED];
    list($insql, $inparams) = $DB->get_in_or_equal($instatus);

    // Given we aren't searching, we don't need the returned params in order to sort users.
    [$userordersql] = users_order_by_sql('u');
    ...
    $sql = "
            ...
            ORDER BY
                ...
                c.timecreated
    ";
    $params = array_merge([MOD_APPOINTMENT_STATUS_USER_CANCELLED], $inparams);
    $params[] = $sessionid;
...
function appointment_get_requests($sessionid) {
    ...
    $params = [$sessionid, MOD_APPOINTMENT_STATUS_REQUESTED];
    ...
    $sql = "SELECT u.id, su.id AS signupid, {$usernamefields},
          ...
          ORDER BY {$userordersql}, ss.timecreated";
...
function appointment_get_declines($sessionid) {
    ...
    $params = [$sessionid, MOD_APPOINTMENT_STATUS_DECLINED];
...
class appointment_candidate_selector extends user_selector_base {
    ...
    public function find_users($search) {
        ...
        [$sort, $sortparams] = users_order_by_sql('u', $search);

        $fields = 'SELECT u.id AS userid, ' . $this->required_fields_sql('u');
                 ...
                 WHERE $wherecondition
                   AND u.id NOT IN
                       ...
                       SELECT s.userid
                         ...
                         JOIN {appointment_sessions} se ON s.sessionid = se.id
                        WHERE se.appointment = :appid
                          AND ss.statuscode >= :statuswaitlisted
                          AND ss.statuscode <> :statusnoshow
                          AND ss.superceded = 0
                       )
               ";
        $order = " ORDER BY $sort";
        $params = array_merge($params, $enrolparams,
            [
                'appid' => $session->appointment,
                'statuswaitlisted' => MOD_APPOINTMENT_STATUS_WAITLISTED,
                'statusnoshow' => MOD_APPOINTMENT_STATUS_NO_SHOW,
            ]);
...
class appointment_existing_selector extends user_selector_base {
    ...
    public function find_users($search) {
        ...
        $params = ['sessid1' => $this->sessionid, 'statusbooked' => MOD_APPOINTMENT_STATUS_BOOKED,
            'statuswaitlisted' => MOD_APPOINTMENT_STATUS_WAITLISTED, ];
...
function appointment_get_session_info(stdClass $session): array {
    ...
    if ($hassessiondates && appointment_has_session_started($session, $timenow)
            ...
            && $session->usersubmission->statuscode == MOD_APPOINTMENT_STATUS_BOOKED) {
        $signupstatus = appointment_get_status($session->usersubmission->statuscode);
        $statusparts[] = get_string('status_' . $signupstatus, 'appointment');
    ...
    } else if ($hassessiondates && !appointment_has_not_started_session($session, $timenow)
        && !appointment_is_session_in_progress($session, $timenow)) {
        ...
        $status = get_string('closed', 'appointment');
    }
...
function appointment_get_coursemodule_info($coursemodule) {
    ...
    if ($coursemodule->showdescription) {
        ...
        $result->content = format_module_intro('appointment', $appointment, $coursemodule->id, false);
    }

    // Populate the custom completion rules as key => value pairs, but only if the completion mode is 'automatic'.
    if ($coursemodule->completion == COMPLETION_TRACKING_AUTOMATIC) {
        foreach ($customcompletionfields as $completiontype) {
            $result->customdata['customcompletionrules'][$completiontype] = (int) $appointment->$completiontype;
        }
    }
    ...
    foreach ($cm->customdata['customcompletionrules'] as $key => $val) {
        switch ($key) {
            ...
            case 'completionattended':
                if (!empty($val)) {
                    $descriptions[] = get_string('completionattended', 'appointment');
                }
                break;
            default:
                break;
...
function user_has_booked_sessions(int $userid, int $appointmentid): bool {
    $bookedstatuses = [
        ...
        MOD_APPOINTMENT_STATUS_PARTIALLY_ATTENDED,
        MOD_APPOINTMENT_STATUS_FULLY_ATTENDED,
    ];

    return user_has_booked_or_attended_sessions($userid, $appointmentid, $bookedstatuses);
}

/**
 * Checks if a user has any attended sessions in an appointment.
 *
 * @param int $userid
 * @param int $appointmentid
 * @return bool
 */
function user_has_attended_sessions(int $userid, int $appointmentid): bool {
    $bookedstatuses = [
        MOD_APPOINTMENT_STATUS_PARTIALLY_ATTENDED,
        MOD_APPOINTMENT_STATUS_FULLY_ATTENDED,
    ];
...
/**
 * Checks if a user has any booked or attended sessions in an appointment.
 *
 * @param int $userid
 * @param int $appointmentid
 * @param array $bookedstatuses
 * @return bool
 */
function user_has_booked_or_attended_sessions(int $userid, int $appointmentid, array $bookedstatuses): bool {
    global $DB;

    [$sql, $params] = $DB->get_in_or_equal($bookedstatuses, SQL_PARAMS_NAMED);
    ...
    $sql = "SELECT COUNT(DISTINCT(ass.id)) FROM {appointment_signups_status} ass
                ...
                WHERE ass.statuscode $sql AND ass.superceded = 0";
    $params += ['userid' => $userid, 'appointmentid' => $appointmentid];
 ...
 */
function mod_appointment_get_fontawesome_icon_map() {
    return [
        'mod_appointment:collapsedchevron' => 'fa-chevron-up',
    ];
}
