...
function appointment_get_ical_attachment($method, $appointment, $session, $user) {
    ...
    foreach ($session->sessiondates as $date) {
        ...
        $organiseremail = core_user::get_noreply_user()->email;
 ...
 */
function appointment_get_completion_state($course, $cm, $userid, $type) {
    global $DB;
    ...
    if ($appointment->completionbooked) {
        return user_has_booked_sessions($userid, $appointment->id);
    } else {
