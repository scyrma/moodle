...
function appointment_get_status($statuscode) {
    ...
    if (!isset($statuses[$statuscode])) {
        print_error('Appointment status code does not exist: ' . $statuscode);
    }
    ...
    if (constant('MOD_APPOINTMENT_STATUS_' . strtoupper($string)) != $statuscode) {
        print_error('Appointment status code array does not appear to be up-to-date: ' . $statuscode);
    }
 ...
 * @return string
 */
function appointment_cost($userid, $sessionid, $sessiondata, $htmloutput = true) {
    global $CFG, $DB;

    $count = $DB->count_records_sql("SELECT COUNT(*)
                               FROM {appointment_signups} su,
                                    {appointment_sessions} se
                              WHERE su.sessionid = ?
                                AND su.userid = ?
...
function appointment_delete_instance($id) {
    ...
    $sessionids = $DB->get_records('appointment_sessions', ['appointment' => $appointment->id], '', 'id');
...
function cleanup_session_data($session) {
    ...
    $session->duration = appointment_hours_to_minutes($session->duration);
 ...
 */
function appointment_add_session(\stdClass $session, array $sessiondates = []): int {
    global $USER, $DB;
...
function appointment_update_session($session, $sessiondates) {
    ...
    return appointment_update_attendees($session);
}

/**
 * Update calendar entries for a given session
 *
 * @param stdClass $session ID of session to update event for
 * @param int $appointment ID of appointment activity (optional)
 */
function appointment_update_calendar_entries($session, $appointment = null) {
    global $USER, $DB;
    ...
    if ($appointment->usercalentry) {
        ...
        if (!in_array($USER->id, $users)) {
            appointment_add_session_to_calendar($session, $appointment, 'user', $USER->id, 'session');
        }
    ...
    if ($appointment->showoncalendar == MOD_APPOINTMENT_CAL_COURSE) {
        appointment_add_session_to_calendar($session, $appointment, 'course', $USER->id);
    } else if ($appointment->showoncalendar == MOD_APPOINTMENT_CAL_SITE) {
        appointment_add_session_to_calendar($session, $appointment, 'site', $USER->id);
    }
        ...
        if (empty($session->sessiondates)) {
            ...
            foreach ($users as $user) {
                if ($user->statuscode == MOD_APPOINTMENT_STATUS_BOOKED) {
                    ...
                    if (!appointment_user_signup($session, $appointment, $course, $user->discountcode,
...
function appointment_email_substitutions($msg, $appointmentname, $reminderperiod, $user, $data, $sessionid) {
    ...
    $msg = str_replace(get_string('placeholder:duration', 'appointment'), format_duration($data->duration), $msg);
    ...
    $customfields = appointment_get_session_customfields();
    $customdata = $DB->get_records('appointment_session_data', array('sessionid' => $sessionid), '', 'fieldid, data');
...
function appointment_cron() {
    ...
    foreach ($signupsdata as $signupdata) {
        ...
        $posthtml = ''; // FIXME.
        if ($fromaddress = get_config(null, 'appointment_fromaddress')) {
            $from = new stdClass();
...
function appointment_get_session($sessionid) {
    ...
    if ($session) {
        ...
        $session->duration = appointment_minutes_to_hours($session->duration);
    }
...
function appointment_get_sessions($appointmentid) {
    ...
    $fromclause = "FROM {appointment_sessions} s";
    $sessions = $DB->get_records_sql("SELECT s.*
                                   $fromclause
                        LEFT OUTER JOIN (SELECT sessionid, min(timestart) AS mintimestart
                                           FROM {appointment_sessions_dates} GROUP BY sessionid) m ON m.sessionid = s.id
                                  WHERE s.appointment = ?
                               ORDER BY m.mintimestart", [$appointmentid]);

    if ($sessions) {
        foreach ($sessions as $key => $value) {
            $sessions[$key]->duration = appointment_minutes_to_hours($sessions[$key]->duration);
...
function appointment_get_unmailed_reminders() {
    ...
    if ($submissions) {
        foreach ($submissions as $key => $value) {
            $submissions[$key]->duration = appointment_minutes_to_hours($submissions[$key]->duration);
 ...
 */
function appointment_user_signup($session, $appointment, $course, $discountcode,
    ...
    if (!$success) {
        print_error('error:couldnotupdateappointmentrecord', 'appointment');
        return false;
    }
    ...
    if (!appointment_update_signup_status($usersignup->id, $newstatus, $userid)) {
        print_error('error:appointmentfailedupdatestatus', 'appointment');
        return false;
    }
    ...
    if ($appointment->usercalentry) {
        if (in_array($newstatus, array(MOD_APPOINTMENT_STATUS_BOOKED, MOD_APPOINTMENT_STATUS_WAITLISTED))) {
            appointment_add_session_to_calendar($session, $appointment, 'user', $userid, 'booking');
        }
    }
 ...
 */
function appointment_get_manageremailformat() {
    $addressformat = get_config(null, 'appointment_manageraddressformat');
    if (!empty($addressformat)) {
        $readableformat = get_config(null, 'appointment_manageraddressformatreadable');
        return get_string('manageremailformat', 'appointment', $readableformat);
    }
 ...
 */
function appointment_check_manageremail($manageremail) {
    $addressformat = get_config(null, 'appointment_manageraddressformat');
...
function appointment_cm_info_view(cm_info $coursemodule) {
    ...
    if (!empty($afterlink)) {
        $afterlink = html_writer::tag('div', $afterlink , ['class' => 'badge badge-light font-weight-normal p-3 mr-4 rounded']);
...
function appointment_get_ical_attachment($method, $appointment, $session, $user) {
    ...
    foreach ($session->sessiondates as $date) {
         ...
         */
        $dtstamp = appointment_ical_generate_timestamp($session->timecreated);
        ...
        $sql = "SELECT COUNT(*)
            ...
            WHERE su.userid = ?
                ...
                AND sus.statuscode = ? ";
        $params = array($user->id, $session->id, MOD_APPOINTMENT_STATUS_USER_CANCELLED);
        ...
        $dtstart = appointment_ical_generate_timestamp($date->timestart);
        $dtend = appointment_ical_generate_timestamp($date->timefinish);

        // FIXME: currently we are not sending updates if the times of the session are changed. This is not ideal!
        $sequence = ($method & MOD_APPOINTMENT_CANCEL) ? 1 : 0;

        $summary = appointment_ical_escape($appointment->name);
        $description = appointment_ical_escape($session->details, true);

        $organiseremail = get_config(null, 'appointment_fromaddress');
        ...
        $icalmethod = ($method & MOD_APPOINTMENT_INVITE) ? 'REQUEST' : 'CANCEL';

        // FIXME: if the user has input their name in another language, we need to set the LANGUAGE property parameter here.
        $username = fullname($user);
        $mailto = $user->email;

        // The extra newline at the bottom is so multiple events start on their own lines.
        // The very last one is trimmed outside the loop.
        $vevents .= <<<EOF
BEGIN:VEVENT
UID:{$uid}
DTSTAMP:{$dtstamp}
...
BEGIN:VCALENDAR
CALSCALE:GREGORIAN
PRODID:-//Moodle//NONSGML Appointment//EN
VERSION:2.0
METHOD:{$icalmethod}
BEGIN:VTIMEZONE
...
/**
 * appointment_ical_generate_timestamp
 *
 * @param int $timestamp
 * @return string
 */
function appointment_ical_generate_timestamp($timestamp) {
 ...
 */
function appointment_ical_escape($text, $converthtml = false) {
    if (empty($text)) {
        return '';
    }
...
function appointment_add_session_to_calendar($session, $appointment, $calendartype = 'none', $userid = 0,
    ...
    if (!empty($appointment->description)) {
        $description .= html_writer::tag('p', clean_param($appointment->description, PARAM_CLEANHTML));
    }
    $description .= appointment_print_session($session, false, true, true);
    $linkurl = new moodle_url('/mod/appointment/signup.php', array('s' => $session->id));
    ...
    if ($calendartype == 'site' && $appointment->showoncalendar == MOD_APPOINTMENT_CAL_SITE) {
        ...
        $linkurl = $CFG->wwwroot . "/mod/appointment/" . $urlvar . ".php?s=$session->id";
        $description .= get_string("calendareventdescription{$eventtype}", 'appointment', $linkurl);
    } else {
        return true;
    }
    ...
    if (empty($shortname)) {
        $shortname = substr($appointment->name, 0, CALENDAR_MAX_NAME_LENGTH);
    }
...
function appointment_delete_user_calendar_events($session, $eventtype) {
    ...
    $whereparams = array($session->appointment);
...
function appointment_print_session($session, $showcapacity, $calendaroutput = false, $return = false, $hidesignup = false) {
    ...
    $customfields = appointment_get_session_customfields();
    $customdata = $DB->get_records('appointment_session_data', array('sessionid' => $session->id), '', 'fieldid, data');
    foreach ($customfields as $field) {
        $data = '';
        if (!empty($customdata[$field->id])) {
            if (CUSTOMFIELD_TYPE_MULTISELECT == $field->type) {
    ...
    if (!empty($session->duration)) {
        $table->data[] = array(get_string('duration', 'appointment'), format_duration($session->duration));
    }
 ...
 *
 * @param int $fieldid ID of a record from the appointment_session_field table
 * @param string $data Value for that custom field
 * @param int $otherid ID of a record from the appointment_sessions table
 * @return bool
 */
function appointment_save_customfield_value($fieldid, $data, $otherid) {
    global $DB;

    // TODO: This is going to be refactored, so quick table var fix for now (WP-1050).
    $table = 'session';
    $dbdata = null;
    if (is_array($data)) {
    ...
    $fieldname = "{$table}id";
    if ($record = $DB->get_record("appointment_{$table}_data", array('fieldid' => $fieldid, $fieldname => $otherid))) {
        if (empty($dbdata)) {

            // Clear out the existing value.
            return $DB->delete_records("appointment_{$table}_data", array('id' => $record->id));
        }

        $newrecord->id = $record->id;
        return $DB->update_record("appointment_{$table}_data", $newrecord);
    } else {
        if (empty($dbdata)) {
        ...
        return $DB->insert_record("appointment_{$table}_data", $newrecord);
    }
}
 ...
 *
 * @param stdClass $field A record from the appointment_session_field table
 * @param int $otherid ID of a record from the appointment_sessions table
 * @return array|string The data contained in this custom field (empty string if it doesn't exist)
 */
function appointment_get_customfield_value($field, $otherid) {
    global $DB;

    // TODO: This is going to be refactored, so quick table var fix for now (WP-1050).
    $table = 'session';
    if ($record = $DB->get_record("appointment_{$table}_data", array('fieldid' => $field->id, "{$table}id" => $otherid))) {
 ...
 *
 * @param int $sessionid ID of appointment_sessions record
 * @return array Indexed by field shortnames
 */
function appointment_get_customfielddata($sessionid) {
    global $CFG, $DB;

    $sql = "SELECT f.shortname, d.data
              FROM {appointment_session_field} f
              JOIN {appointment_session_data} d ON f.id = d.fieldid
              WHERE d.sessionid = ?";
...
/**
 * Return a cached copy of all records in appointment_session_field
 *
 * @return array
 */
function appointment_get_session_customfields() {
    global $DB;
    ...
    if (null == $customfields) {
        if (!$customfields = $DB->get_records('appointment_session_field')) {
            $customfields = array();
        }
    }
    return $customfields;
 ...
 * @return string
 */
function appointment_list_of_customfields() {
    global $CFG, $USER, $DB, $OUTPUT;

    if ($fields = $DB->get_records('appointment_session_field', array(), 'name', 'id, name')) {
        $table = new html_table();
        $table->attributes['class'] = 'halfwidthtable';
        foreach ($fields as $field) {
            $fieldname = format_string($field->name);
            $editurl = new moodle_url('/mod/appointment/customfield.php', array('id' => $field->id));
            $editlink = $OUTPUT->action_icon($editurl, new pix_icon('t/edit', get_string('edit')));
            $deleteurl = new moodle_url('/mod/appointment/customfield.php',
                array('id' => $field->id, 'd' => '1', 'sesskey' => $USER->sesskey));
    ...
    return get_string('nocustomfields', 'appointment');
}
        ...
        foreach ($trainers as $trainer) {
            ...
            if (!isset($oldtrainers[$roleid][$trainer])) {
                ...
                if (!$DB->insert_record('appointment_session_roles', $newtrainer)) {
                    print_error('error:couldnotaddtrainer', 'appointment');
        ...
        foreach ($oldtrainers as $roleid => $trainers) {
            ...
            foreach ($trainers as $trainer) {
                if (!$DB->delete_records('appointment_session_roles',
                    ...
                    print_error('error:couldnotdeletetrainer', 'appointment');
                    $transaction->force_transaction_rollback();
                    return false;
                }
            }
 ...
 * (used by the session add/edit page)
 *
 * @param MoodleQuickForm $mform
 * @param array $customfields
 * @param bool $alloptional
 */
function appointment_add_customfields_to_form(&$mform, $customfields, $alloptional = false) {
    foreach ($customfields as $field) {
        ...
        switch ($field->type) {
            ...
            case CUSTOMFIELD_TYPE_MULTISELECT:
                ...
                break;
            default:
                debug("appointment: invalid field type for custom field ID $field->id");
                continue 2;
        }
