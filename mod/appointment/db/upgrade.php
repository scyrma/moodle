...
function xmldb_appointment_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2022040102) {
        ...
        // Define table appointment_session_roles to be dropped.
        $table = new xmldb_table('appointment_session_roles');

        // Conditionally launch drop table for appointment_session_roles.
        if ($dbman->table_exists($table)) {
            $dbman->drop_table($table);
        }

        // Appointment savepoint reached.
        upgrade_mod_savepoint(true, 2022040102, 'appointment');
    }
    ...
    if ($oldversion < 2022110700) {
        ...
        upgrade_mod_savepoint(true, 2022110700, 'appointment');
    }

    if ($oldversion < 2022111501) {
        // Remove orphaned events that belong to appointments which no longer exists.
        mod_appointment_upgrade_remove_orphaned_user_events();

        upgrade_mod_savepoint(true, 2022111501, 'appointment');
    }

    if ($oldversion < 2023011800) {

        // Define field completionattended to be added to appointment.
        $table = new xmldb_table('appointment');
        $field = new xmldb_field('completionattended', XMLDB_TYPE_INTEGER, '1', null,
            XMLDB_NOTNULL, null, '0', 'completionbooked');
        ...
        // Appointment savepoint reached.
        upgrade_mod_savepoint(true, 2023011800, 'appointment');
    }
    return true;
}
