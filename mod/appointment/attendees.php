...
$PAGE->set_url('/mod/appointment/attendees.php', ['s' => $s]);
...
$PAGE->activityheader->set_attrs(['hidecompletion' => true, 'title' => '', 'description' => '']);
$PAGE->navbar->add(get_string('attendees', 'appointment'));
...
if ($canviewattendees || $cantakeattendance) {
    ...
    if (empty($attendees)) {
        ...
        if ($takeattendance) {
            ...
            echo html_writer::empty_tag('input', ['type' => 'hidden', ' name' => 'backtoallsessions',
        ...
        foreach ($attendees as $attendee) {
            ...
            if ($takeattendance) {
                ...
                $select = html_writer::tag('label', get_string('takeattendance', 'mod_appointment'),
                        ['for' => 'menu' . $optionid,  'class' => 'accesshide']).
                    html_writer::select($statusoptions, $optionid, $status);
                $data[] = $select;
            } else {
                $data[] = html_writer::span(
        ...
        if ($takeattendance) {
            ...
            echo '&nbsp;' . html_writer::empty_tag('input', ['type' => 'submit', 'name' => 'cancelform',
            ...
            if ($cantakeattendance && appointment_has_session_started($session, time())) {
                ...
                // Take attendance.
                $attendanceurl = new moodle_url('attendees.php', ['s' => $session->id, 'takeattendance' => '1',
    ...
    if (!$takeattendance) {
        if (has_capability('mod/appointment:addattendees', $context) ||
            ...
            $editattendeeslink = new moodle_url('editattendees.php', ['s' => $session->id,
                'backtoallsessions' => $backtoallsessions, ]);
...
if ($backtoallsessions) {
    $url = new moodle_url('/mod/appointment/view.php', ['f' => $appointment->id, 'backtoallsessions' => $backtoallsessions]);
}
