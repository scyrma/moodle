...
if (!$canviewattendees && !$cantakeattendance && !$canapproverequests && !$canviewcancellations) {
    print_error('nopermissions', '', "{$CFG->wwwroot}/mod/appointment/view.php?id={$cm->id}", get_string('view'));
}

// Check user has permissions to take attendance.
if ($takeattendance && !$cantakeattendance) {
    print_error('nopermissions', '', '', get_capability_string('mod/appointment:takeattendance'));
}
...
if ($takeattendance && !appointment_has_session_started($session, time())) {
    ...
    print_error('error:canttakeattendanceforunstartedsession', 'appointment', $link);
}
...
if ($canviewattendees || $cantakeattendance) {
    ...
    if (empty($attendees)) {
        ...
        if ($takeattendance) {
            ...
            if (!get_config(null, 'appointment_hidecost')) {
                $table->head[] = get_string('cost', 'appointment');
                $table->align[] = 'center';
                if (!get_config(null, 'appointment_hidediscount')) {
                    $table->head[] = get_string('discountcode', 'appointment');
                    $table->align[] = 'center';
                }
            }
        ...
        foreach ($attendees as $attendee) {
            ...
            if ($takeattendance) {
                ...
                if (!get_config(null, 'appointment_hidecost')) {
                    $data[] = appointment_cost($attendee->id, $session->id, $session);
                    if (!get_config(null, 'appointment_hidediscount')) {
